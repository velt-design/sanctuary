 ...
 {w.sites.strips.filter(s=>selecting||w.value.strips.includes(s.id)).map(s=>{
  ...
  return <g key={s.id}>
   ...
   {selecting&&<line role="button" tabIndex={0} aria-label={'LED strip: '+s.label} aria-pressed={selected} data-light-member={s.id}
    x1={s.start.x} y1={s.start.y} x2={s.end.x} y2={s.end.y} stroke="transparent" strokeWidth={26/scale} style={{cursor:'pointer'}}
    onClick={e=>{e.stopPropagation();w.toggle(s.id);}} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();w.toggle(s.id);}}}/>}
  </g>;
 })}
 {lights.map(s=><circle key={s.id} cx={s.point.x} cy={s.point.y} r={w.editing?Math.max(s.diameter/2,6/scale):s.diameter/2} fill="#fff1ce" stroke="#906832" strokeWidth={w.editing?1.5/scale:8} pointerEvents="none"/>)}
 </g>;
}

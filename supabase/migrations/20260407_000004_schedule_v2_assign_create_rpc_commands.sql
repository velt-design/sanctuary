...
begin
  ...
  if p_move is null then
    if v_assignment.job_id is null then
      raise exception 'p_assignment.job_id is required';
    end if;
    if v_assignment.forecast_duration_days is null or v_assignment.forecast_duration_days <= 0 then
    ...
    values (
      ...
      p_target_crew_id,
      'floating',
      v_assignment.forecast_duration_days,
      'not_started'
    )
  ...
  v_combined_forecast_payload := v_target_forecast_payload || v_source_forecast_payload;
...
create or replace function public.schedule_v2_create_downtime(
  p_crew_id uuid,
...
as $$
declare
  v_positions_payload jsonb := coalesce(p_positions, '[]'::jsonb);
...
begin
  ...
  if p_duration_days is null or p_duration_days <= 0 then
    raise exception 'p_duration_days must be a positive integer';
  end if;

  if p_insert_position is null or p_insert_position < 0 then
    raise exception 'p_insert_position must be a non-negative integer';
  end if;

  if jsonb_typeof(v_positions_payload) <> 'array' then
  ...
  select exists(select 1 from public.schedule_crews where id = p_crew_id)
  into v_crew_exists;
  ...
  from jsonb_to_recordset(v_positions_payload) as rows(id uuid, position integer);

...
- Moved the Projects index from a route-blocking server read to one staff-authenticated combined query, current-user cache bootstrap, explicit pending/cached/fresh/failure/access states, intent-only prefetch, and instant URL feedback without the global loading overlay. The fresh Projects Index bundle baseline is locked; five-run authenticated timing evidence remains pending.

...
- `GET /api/staff/v1/work-items/queue` is the staff-authenticated one-row-per-project Work Queue read model. It uses `requireStaffContext()` and the auth-bound client, composes durable work/recovery/state rows with canonical specialist candidates on the server, and returns bounded command metadata for normal staff actions. Dashboard consumes a limited preview of the same owner; personal reminders remain a separate route and table.

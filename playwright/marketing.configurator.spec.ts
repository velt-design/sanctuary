...
for (const width of [360, 390, 430]) {
  test(`empty dock appears after engagement without mobile overflow at ${width}px`, async ({ page }) => {
    ...
    expect(await readCumulativeLayoutShift(page)).toBeLessThanOrEqual(0.1);
    if (capture) {
      await page.screenshot({ path: `${evidenceDirectory}/eligible-configured-${width}.png` });
    }
  });
}
...
test('reset requires explicit confirmation', async ({ page }) => {
  ...
  expect(resetId).not.toBe(originalId);
  await expect(page.locator('[data-configurator-dock]')).toContainText(
    'Pitched · 4.0 × 3.0 m · Clear acrylic',
  );
  await assertNoHorizontalOverflowOrFixedCollision(page);
  if (capture) {
    await page.screenshot({ path: `${evidenceDirectory}/explicit-reset-430.png` });
  }
});

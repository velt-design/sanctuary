...
MobileOpeningPicker presents a quiet footprint of actual solved openings. MobileSideChoices and useMobileSides own single-page grouped editing: select openings with native checkboxes, then select one of five material-reference treatments to apply immediately. sideTreatment validates the complete group before one update, preserves existing same-kind customisations and unselected openings, and rejects incompatible blind groups without partial mutation. The opening map and individual screen/blind refinements remain disclosed on the same page. Sides and Lighting share page 3 as tabs, without Apply or confirmation subpages.
...
page 4 retains the daylight model stop immediately before Review.

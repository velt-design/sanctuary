...
- Global search performs bounded per-field reads, escapes literal `%`, `_`, and backslash characters before `ilike`, de-duplicates on canonical application IDs, and fails the whole response when a provider read fails; it must not silently present an incomplete group as a fresh result.

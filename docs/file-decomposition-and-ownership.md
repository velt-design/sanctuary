...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | Calculator UI, module drawing surfaces, estimate state, and custom editor markup are still concentrated in a few client files, though pure calculator input defaults, normalization, browser-draft persistence/session state, module navigation modelling and presentation, configuration-section definitions/presentation/field options, structure/site/workflow field-schema generation, issue labels/section ownership/model construction/focus navigation, infill draft/selection/modal/delete-undo state, save confirmation/persistence/outcome orchestration, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation and styles, save-readiness, quote-status/save-preflight helpers, the guided infill dialog, selected-infill header, Opening/Supports/Results stage presenters, visual infill shape-template picker/model, support-confirmation model, infill UI/payload plus summary view-model helpers, infill overview/list presenters, save dialog presenters, flashings view-model helpers, and blinds view-model helpers now live outside `CalculatorGridClient.tsx`. | Move the blinds and flashings custom-editor markup into focused presenters before decomposing the remaining calculator workspace JSX. | `npx vitest run apps/portal/app/staff/calculator`, `npm run test:portal:projects`, `npm run typecheck` |

...
export default function RailProvider({children}:{children:ReactNode}){
 ...
 return <Context.Provider value={{section:active,explored,choose:next=>{setExplored(previous=>previous.includes(next)?previous:[...previous,next]);if(next==='lighting'){if(!lighting.editing)lighting.open();}else if(lighting.editing)lighting.close();setSection(next);}}}>{children}</Context.Provider>;
}

...
As of 2026-07-20, `docker`, `psql`, and the Supabase CLI were unavailable on this workstation, so the local live command still stops at `spawnSync docker ENOENT` before starting a container. The dedicated `.github/workflows/background-jobs.yml` workflow is the executable database and artifact evidence: [run 29713940507](https://github.com/velt-design/sanctuary/actions/runs/29713940507) passed the six JOB-01/JOB-02 migrations on upstream PostgreSQL 18/PGMQ 1.10.0 and Supabase PostgreSQL 17/PGMQ 1.5.1, plus worker typecheck/tests/lint/build, built CLI, service-role strict guard, and non-root container build. JOB-03 seven-migration CI evidence: pending after the checkpoint commit is pushed. These checks prove the scoped background-job harness and artifact, not deployment to a shared environment or the repository's non-bootstrappable historical migration chain; no real provider send belongs in this gate.

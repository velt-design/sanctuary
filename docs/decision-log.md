...
Current guardrail: Validate each remaining route against an approved pilot archetype before adoption. Keep global discovery server-side through `requireStaffContext()` and the auth-bound RLS client; keep local filters local; search only canonical fields; and do not claim company or project-number support until those fields have a canonical schema owner.

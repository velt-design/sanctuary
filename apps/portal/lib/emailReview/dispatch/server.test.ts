...
describe('Outlook dispatch boundary',()=>{
 it('records an explicit actual mode and bounded missing-anchor fallback reason',()=>{const value={intentId:id,attemptId:id,outcome:'uncertain',actualDeliveryMode:'new',fallbackReason:'anchor_not_found_before_send'};expect(parseDispatchResult(value)).toEqual(value);expect(()=>parseDispatchResult({...value,fallbackReason:'timeout'})).toThrow();expect(()=>parseDispatchResult({...value,actualDeliveryMode:'reply'})).toThrow();});

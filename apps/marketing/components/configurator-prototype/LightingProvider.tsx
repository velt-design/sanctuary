...
type Workspace={editing:boolean;view:'Plan'|'3D';setView:(v:'Plan'|'3D')=>void;night:boolean;setNight:(n:boolean)=>void;open:()=>void;close:()=>void;value:PergolaLighting;change:(v:PergolaLighting)=>void;toggle:(id:string)=>void;sites:ReturnType<typeof pergolaLightSites>};
...
export default function LightingProvider({input,roof,onChange,children}:{input:SimpleCoverInput;roof:PreviewRoofChoices;onChange:(r:PreviewRoofChoices)=>void;children:ReactNode}){
 const [editing,setEditing]=useState(false),[night,setNight]=useState(false),[view,setView]=useState<'Plan'|'3D'>('Plan');
 ...
 return <Context.Provider value={{editing,view,setView,night:editing&&view==='3D'&&night,setNight,open:()=>{setEditing(true);setView('Plan');setNight(true);},close:()=>{setEditing(false);setNight(false);},value,change,sites,toggle:id=>change({...value,strips:value.strips.includes(id)?value.strips.filter(s=>s!==id):[...value.strips,id]})}}>{children}</Context.Provider>;
}

...
`enquiry_qualification_events` table and the security-definer
`enquiry_qualification_read(uuid,uuid)` / `enquiry_qualification_record(...)` RPCs.
The auth-bound staff GET/POST route at
`/api/staff/projects/[projectId]/enquiries/[enquiryId]/qualification` is the read/write
entry point; Original enquiry presents this separate assessment. Both RPCs require
an authenticated Portal member and the enquiry's current project. Anonymous
execution and direct authenticated table access are denied; RLS is enabled and an
append-only trigger protects historical rows. Staff identity comes from the session.

Assessments use the exact frozen residential configured-enquiry source, four fixed
criteria, a monotonic enquiry version, idempotent command ID and source-row lock.
They do not change pipeline, Project Work, outbox, invoices or marketing events.
`20260917000004_enquiry_qualification_reassignment.sql` makes history/version follow
the stable enquiry ID after project reassignment, while retaining current-project
...
Automatic payments (20260915000006): adds disabled-by-default `auto_record_payments_enabled` and an immutable `recording_method` on Xero matches. Automatic invoice-payment records have no human approver; ledger/audit attribution is `xero-automatic`. The service-only recorder retains the receipt/project locks, bound invoice/contact, current balance and replay checks, and refuses reversed or finance-flagged receipts. Manual wrappers still require a finance grant. History labels automatic records explicitly. Installed in staging only with its rollout flag false; production installation remains pending.
...
Migration `20260903000001_praxis_context_reporting_v1.sql` owns the server-only Praxis read boundary. It creates the `praxis_reporting` schema, a non-login `sanctuary_praxis_reader` group role, one environment-provisioned database identity row, 12 explicit versioned projections, and the bounded `context_page_v1` projection function used for a single terminal snapshot transaction. The resources are enquiry request, contact, project, estimate, quote, quote version, quote line item, invoice, invoice plan item, payment, payment allocation, and canonical project financial truth.
...
- `20260827000001`, its schema-cache reload `20260827000002`, and signing-boundary correction `20260827000003` were exact-file applied and ledgered in positively identified staging and production on 2026-08-27 after completed physical backups and rollback rehearsals. The final correction removed the authenticated enquiry-object Storage read policy in both environments; its canonical-LF SHA-256 is `a30875a29b713ccf7dd6f410dde41145f6fcc66ea26fc69a0921dfaa50932f70`. Production migration created no link rows. The separately approved backfill run `81401386-ed04-44be-8047-cd7008d4cb32` then inserted the exact 59-candidate subset and 59 link events without changing any of the 60 Storage objects. Post-apply dry-run evidence has zero remaining linkable files, 59 already linked files, zero missing/unmatched objects, and retains 22 ambiguous declarations plus nine changed-project entries for separate resolution.

...
- ✅ **Package-owned custom hipped topology**: fully hipped non-rectangular orthogonal house forms route through the `eave_graph_source_edge_envelope` solver, which commits one semantic roof facet per source eave edge and rejects duplicate lower-envelope fragments, internal eave-height seams, and fallback features before they can be committed as healthy geometry. Diagnostics expose the solver, failing edge, failure reason, semantic QA, and closed/expected face counts without adding Plan paint or active-module fallbacks.
...
- House-form Plan overlays and status resolve by `houseFormId`. Rail rows, inspector state, selection chrome, hit targets, dimensions, and visible body precedence must not borrow the first/legacy house form. House roof Plan bodies are package-owned eave-perimeter projections (`house_plan_roof:<formId>`); roof-material ribs/seams remain 3D/detail data and must not replace the semantic Plan roof body. Custom hipped roof repairs are render-only package diagnostics: saved wall footprint/eave settings remain unchanged, while Plan and 3D both consume the repaired roof eave topology and expose `roofEaveOffsetRepair*` metadata per form. Fully hipped custom orthogonal roofs must use the package eave-graph topology solver instead of the legacy rectilinear dissolve path; legacy candidates may commit only after semantic QA proves no unbacked internal boundary/chord, duplicate source faces, internal eave-height seams, fallback features, or unclassified feature lines. Any failure should be object-owned diagnostics, not a portal-only visual workaround.

...
export default function EditorialProductContent({ product, enquiryHref, gallery, evidence, supportingLinks }: Props) {
  ...
  return <>
    <Container width="wide">
      <section className={productStyles.hero} aria-labelledby="product-title" data-editorial-product-hero>
        <div className={productStyles.heroCopy}>
          ...
          <Link className={styles.jump} href={exploreTarget}>{gable ? 'Explore roof approaches' : 'Explore the details'} <span aria-hidden="true">↓</span></Link>
        </div>
    ...
    {gable && <section className={`${styles.section} ${styles.warm}`} id="roof-approaches">
      <Container width="wide">
        ...
        <RoofApproaches options={product.details.options?.slice(0, 3) ?? []} />
      </Container>
    </section>}

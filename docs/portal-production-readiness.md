...
| Durable background jobs | Yellow | JOB-01 defines `@sp/jobs`, one logged `portal_background_jobs` PGMQ queue, a durable ledger, private immutable payloads, lease-fenced service-role RPCs, explicit safe-summary/projection contracts, effect checkpoints, recovery/repair, and an isolated database harness. The hardened local `npm run test:jobs` suite passes 4 files and 71 tests. Background Jobs [run 29710584022](https://github.com/velt-design/sanctuary/actions/runs/29710584022) passed the contract job and both executable database legs: PostgreSQL 18/PGMQ 1.10.0 and Supabase PostgreSQL 17/PGMQ 1.5.1. | JOB-01 is repository-complete and executable on the supported matrix, but no migration was applied to a shared environment and no worker or producer was deployed. JOB-02 through JOB-08 remain pending, so the overall Wave 3 lane stays Yellow. |
...
- [x] `npm run test:jobs:db` passes in the isolated logged-PGMQ CI harness on both supported database images.
...
3. Preserve the green isolated JOB-01 database signal and require shared-environment deployment review before enabling any producer or worker rollout; continue JOB-02 through JOB-08 as repository work only.

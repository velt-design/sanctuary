...
Library tests cover input and HTTP boundaries. Passing isolated tests does not
...
or automatically retry an uncertain result. Keep private correspondence out of
repository fixtures, logs and published screenshots.


## Opening-brief source verification

`npx vitest run test/praxis-briefing-read.test.ts apps/portal/app/api/integrations/praxis/v1/routes.test.ts` tests complete compact populations, equal totals with changed records, stable business fingerprints despite provenance-only updates, exclusions, missing/invalid domains, 5,001-row overflow and the 2 MiB whole-response bound. The full-scale synthetic case has 1,310 projects, 1,198 contacts, 1,167 enquiries and 627 quote versions, including null, date-only and timestamp expiries; all project work-model coverage must reconcile. Invalid calendar dates still withhold the whole quote domain. Route tests preserve exact bearer/binding authorization and GET-only access.

`PRAXIS_DISPOSABLE_POSTGRES_BIN` selects local PostgreSQL 17 binaries for `node scripts/test-praxis-briefing-db.mjs`. This starts/removes an isolated local fixture cluster and reruns the same actual SQL with a SELECT-only role plus denied-write proof. Its prerequisites are synthetic safe-view-shaped tables, not a full migration replay or live database proof. No new migration or reporting privilege is required. Optional `PRAXIS_BRIEFING_WIRE_PATH` exports the small synthetic reader response; `PRAXIS_BRIEFING_SCALE_WIRE_PATH` exports the complete synthetic full-scale response unchanged to a caller-selected private evidence location for consumer interoperability.
...
Automatic payment Finance status: migration08 and `supabase/tests/xero_payment_finance_status.sql` extend the PostgreSQL jobs harness to verify clean → review → current queue transitions. Focused Finance tests cover approved-transfer labels, automatic exceptions and stale checks. Hosted rendered evidence is still required.
...
`npm run test:commercial:db` executes the commercial bootstrap, trust and commercial-truth migrations, stale-conflict correction, and SQL contracts in disposable in-process PGlite PostgreSQL 18 databases. It proves rollback, atomic apply, exact estimate/quote intents, current accepted base/add-on scope, acceptance and invoice caps, idempotent replay, whole-invoice payment/reversal rules, lifecycle tombstones, completion settlement, and service-role-only grants without reaching a shared database. The static migration test remains a complementary source-shape guard. Never use shared or production data as the disposable fixture.
...
Configurator contract changes should run `npm run test:configurator`, `npm run packages:guard` and `npm run typecheck`. The focused package suite covers the strict parser table, exact V1 invariants, canonical default and serialization, normalization, typed patches/seeds, customer-safe summary, migration/corrupt-data handling, unknown future-version preservation, the geometry adapter and one-solve artifact contract, customer-safe geometry failures, and Node/jsdom imports through both `@sp/configurator/core` and `@sp/configurator/geometry`.

Sanctuary AI contract changes should run `npm run test:ai`, `npm run packages:guard` and `npm run typecheck`. The focused suite covers strict V1 task/event, registry, approval, provenance, artifact, usage/evaluation and node parsing; exact approval and node invariants; unknown-field rejection; the no-runtime-dependency boundary; and static PR-AI-004 migration/security alignment. Engineering orchestration changes additionally validate representative task and completion JSON with `npm run ai:engineering:validate-task -- <task-file>` and `npm run ai:engineering:validate-completion -- <task-file> <completion-file>`. The engineering contract binds completion identity, evidence, changed paths, attempts and cost to the validated immutable manifest; enforces separate supervisor/worker/reviewer roles, human-only merge, no production effects, a clean pushed draft-PR handoff, and explicit blocked/failed outcomes. These checks perform no model call or production effect.

PR-AI-004/005 database changes additionally require `npm run test:ai:db`. `scripts/test-ai-task-ledger-db.mjs` starts and removes its own disposable database, applies the test-only bootstrap, runs each exact AI migration inside `BEGIN`/`ROLLBACK`, proves that its objects do not survive, reapplies the files in order, and executes the live task-ledger plus exact-approval contracts. Approval coverage includes cross-user reads, current admin role, request/decision replay, changed command identity, exact payload hash, rejection, expiry, task cancellation, explicit invalidation, duplicate consumption, decision preservation, append-only receipts, and frozen private envelopes. It reads no Supabase target credentials and must never run against a shared local, staging, or production database. The `AI Foundation` workflow runs the same harness on Supabase PostgreSQL 17 and upstream PostgreSQL 18. A missing Docker command or configured workflow is not a pass; wait for green executable evidence before deployment review.

Before that single-run harness, AI Foundation explicitly pulls the unchanged matrix image with `node scripts/ci/pull-ai-db-image.mjs`. Only the exact `toomanyrequests: Rate exceeded` registry message permits another pull: at most three attempts, with 30/60-second backoff and a two-minute bound per attempt. Every failed response stays visible. Other failures, process errors, and exhausted attempts stop the job before database startup. The workflow never retries SQL or the harness. `npx vitest run test/ai-db-image-pull.test.ts` verifies this policy using synthetic command responses and no Docker, registry, or database access.

PR-AI-007 additionally requires `npm run test:jobs`, `npm run test:worker`, `npm run typecheck:worker`, `npm run build:worker`, and `npm run test:jobs:db`. The package/worker suites prove the exact four-field synthetic payload, fixed fixture results, zero external-effect policy, worker-routed handler coverage, invalid-input quarantine, and legacy-kind fail-safe behavior. The live background-job harness applies the AI task and approval prerequisites, rehearses the exact synthetic-execution migration inside `BEGIN`/`ROLLBACK`, proves no execution objects remain, then applies it and executes real PGMQ claim/progress/completion, exact replay, linked-cancellation rejection, zero-cost usage, deterministic evaluation, RLS/grant, append-only, and atomic mismatch-rollback contracts on PostgreSQL 17 and 18. It must never target a shared environment.

PR-AI-006 Portal activity changes should run `node node_modules/vitest/vitest.mjs run apps/portal/lib/ai apps/portal/app/api/staff/v1/ai apps/portal/components/ai apps/portal/app/qa/ai-activity-fixture`, `node node_modules/@playwright/test/cli.js test playwright/portal.ai-activity-fixture.spec.ts --project=portal-fixture --workers=1`, `npm run packages:guard`, `npm run typecheck`, and the normal portal lint/build gates. The focused suite proves explicit public-column selection, safe row validation, request-client use, bounded filters, access ending, schema-not-ready handling, indistinguishable cross-project/missing detail, a disabled-by-default fixture, and a view with no mutation control. With `ENABLE_PORTAL_QA_FIXTURES=1`, `/qa/ai-activity-fixture` is the customer-data-free visual route; it does not authenticate, query Supabase, read private AI state, execute a task, or create an effect. The browser lane checks the 1440x1000 and 390x844 compositions, overflow, absence of mutation controls/private-boundary labels, and clean browser evidence.
...
- `npm run test:jobs` runs the `@sp/jobs` contract/state-machine/runtime-parser tests, the deterministic synthetic executor tests, the `@sp/email-provider` tests, static migration contract assertions, and repository security-boundary tests. The private-key guard searches tracked, non-binary Git content directly so checked-in visual evidence cannot make the security gate scale with binary asset size.
...
- `npm run test:jobs:db` is the live database contract. `scripts/test-background-jobs-db.mjs` starts a disposable PGMQ-capable container, reports the resolved image, PostgreSQL major version, and PGMQ extension version, applies `supabase/tests/background_jobs_bootstrap.sql`, discovers and applies the seven JOB-01/JOB-02/JOB-03 migrations in order with each migration protected by a transaction, runs real two-session enqueue and provider-message unique-index races, executes the rollback-wrapped `supabase/tests/background_jobs.sql`, then applies the AI prerequisites and runs the PR-AI-007 exact rollback plus executable synthetic contract before removing the container. The default image is `ghcr.io/pgmq/pg18-pgmq:v1.10.0`; intentional overrides can set `BACKGROUND_JOBS_DB_IMAGE` and the expected version variables.
- `npm run test:worker` runs the Node-only worker configuration, safe logger, health server, RPC adapter, CLI, concurrency, execution, retry, heartbeat, shutdown, modes, durable email coordinator, and deterministic synthetic handler tests. JOB-03 includes the persistent-world hard-crash matrix; PR-AI-007 proves exact fixed outputs, invalid-payload quarantine, zero effects, and startup coverage only for explicitly worker-routed kinds. `npm run typecheck:worker` and `npm run build:worker` prove the standalone TypeScript and bundled Node 22 entrypoint; `node apps/worker/dist/worker.mjs --help` is the built CLI smoke.
...
`npm run portal:scenarios:ensure` is the explicit service-role provisioning command for local/staging scenario data. It requires `PORTAL_TEST_SCENARIO_TARGET=local|staging`, `PORTAL_TEST_EMAIL`, `PORTAL_TEST_PASSWORD`, `NEXT_PUBLIC_SUPABASE_URL`, and `SUPABASE_SERVICE_ROLE_KEY`; staging has the same exact staging/production-ref and URL checks as test-user provisioning. It refuses missing or disguised targets, upserts deterministic `[Agent Scenario]` records, and writes non-secret route state to `playwright/.auth/portal-scenarios.json`. Optional env: `PORTAL_SCENARIOS=project-with-estimate,calculator-multi-module,quote-ready,job-pack-ready,workbench-multi-object` and `PORTAL_SCENARIO_PREFIX=agent`. The dedicated calculator scenario is reconciled to its current fixture revision on every provisioning run rather than sharing mutable route-smoke estimate state. `job-pack-ready` owns a deterministic sent quote and generated job-pack row so the authenticated performance gate never depends on customer data.

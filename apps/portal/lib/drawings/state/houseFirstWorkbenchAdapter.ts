...
/**
 * Legacy test-driven carrier. Production never sets `draft.houseFirst`
 * (the field was removed from `EstimateDrawingDraft` in Phase 1 and
 * defensively `delete`-d in `drawingEdits.ts`). The test suite, however,
 * still constructs drafts via `LegacyEstimateDrawingDraft = EstimateDrawingDraft
 * & { houseFirst?: ... }` and asserts that the carrier overrides drive
 * the resolved roof / decks / openings / pergola attachment fields. We
 * keep the carrier as a typed extension so the test path continues to
 * work without re-introducing the cross-file bridge synthesis that
 * PR-2A.1b removed. The conversion logic (objectFirst → houseFirst
 * shape) no longer exists — every consumer either reads canonical
 * `draft.objectFirst.*` shapes natively, or accepts the legacy carrier
 * override directly.
 */
...
function readAuthoredRoofDraft(
  draft: HouseFirstWorkbenchDraftCarrier | null | undefined,
): HouseFirstRoofDraft | null {
  // Legacy test path: carrier override wins (PR-2A.1b).
  if (draft?.houseFirst?.roof) return draft.houseFirst.roof;
  if (!draft?.objectFirst) return null;
  // Normalize first so the gable→hipped migration runs (slice 2 of M13 session C).
  // The bridge synthesis used to do this; we now do it inline so callers don't
  // have to remember to normalize.
  const normalized = normalizeObjectFirstWorkbenchDraftVNext(draft.objectFirst);
  const houseForm = normalized.houseAssembly?.houseForms[0];
  if (!houseForm?.roofIntentAuthored) return null;
  return {
...
function readDeckDrafts(
  ...
  if (legacy) {
    return legacy.map((deck) => ({
      ...
      attachment: deck.hostHouseFormId
        ? {
            host: {
              objectFamily: "house_forms",
              objectId: deck.hostHouseFormId,
              edgeKind: "wall",
              edgeId: "",
              myEdgeIndex: 0,
            },
  ...
  if (!draft?.objectFirst) return null;
  return normalizeObjectFirstWorkbenchDraftVNext(draft.objectFirst).decks;
}

function readOpeningDrafts(
  draft: HouseFirstWorkbenchDraftCarrier | null | undefined,
): ObjectFirstOpeningDraft[] | null {
  // Legacy test path: carrier override wins.
  const legacy = draft?.houseFirst?.openings;
  if (legacy) {
    return legacy.map((opening, index) => ({
      id: opening.id,
      ...
      sourceFormId: opening.hostHouseFormId ?? null,
  ...
  if (!draft?.objectFirst) return null;
  return normalizeObjectFirstWorkbenchDraftVNext(draft.objectFirst).openings;
}

function readPergolaDrafts(
  draft: HouseFirstWorkbenchDraftCarrier | null | undefined,
): HouseFirstPergolaDraft[] | null {
  // Legacy test path: carrier override wins.
  if (draft?.houseFirst?.pergolas) return draft.houseFirst.pergolas;
  if (!draft?.objectFirst) return null;
  const pergolas = normalizeObjectFirstWorkbenchDraftVNext(
    draft.objectFirst,
  ).pergolas;
...
function houseModelToObjectFirstHouseFormDraft(
  ...
  return {
    ...
    roofIntentAuthored:
      house.roof.source === "house_first_draft" ? true : undefined,
...
type SharedFieldResult<T> = {
  ...
  source: Extract<
    HouseRoofFieldSource,
    "legacy_shared_value" | "default_fallback"
  >;
};
...
type DerivedEnvelopeLookup = {
  ...
  byEdgeId: Map<
    string,
    NonNullable<HouseModel["derivedEnvelope"]>["edges"][number]
  >;
...
function buildDerivedEnvelopeLookup(input: {
  ...
  const envelopeAttachmentZones: NonNullable<
    HouseModel["derivedEnvelope"]
  >["attachmentZones"] = [];
  ...
  for (const resolvedWall of input.derivedWalls.byWallId.values()) {
    ...
    const hasAnyOpening = wallOpenings.some(
      (opening) => opening.validation.status === "valid",
    );
    ...
    for (const kind of candidateKinds) {
      let reason:
        | HouseModel["attachmentZoneDiagnostics"]["blocked"][number]["reason"]
        | null = null;
  ...
  const zonesBySideAndKind = new Map<
    string,
    DerivedAttachmentZoneResolution[]
  >();
  for (const zone of envelopeAttachmentZones) {
    const wall = zone.hostWallId
      ? (input.derivedWalls.byWallId.get(zone.hostWallId)?.wall ?? null)
      : null;
...
function normalizeStoreyMode(
  value: CalculatorModuleInputs["houseStoreyMode"],
): CalculatorHouseStoreyMode {
  if (value === "double_storey" || value === "custom") return value;
  return "single_storey";
}

function isBlankFootprintParams(
  value: CalculatorHouseFootprintParams | undefined,
): boolean {
  return (
    stableStringify(normalizeHouseFootprintParams(value)) ===
    stableStringify(makeDefaultHouseFootprintParams())
  );
}
...
function resolveSharedHouseField<T>(
  ...
  const normalizedEntries = modules.map((module, moduleIndex) => {
    ...
    return {
      ...
      blank: config.isBlank
        ? config.isBlank(value)
  ...
  const conflict = populated.find(
    (entry) => entry.comparable !== firstPopulated.comparable,
  );
  if (!conflict) {
    return {
...
/**
 * PR-2A.1a (2026-05-22): exported so the post-Phase-1 direct snapshot
 * adapter can call it without going through the full HouseFirst
 * round-trip. The function itself is unchanged — it still synthesizes
 * a legacy-shaped `HouseModel` (with `derivedEnvelope`, attachment zones,
 * etc.) from calculator-era module inputs. Callers that want object-first
 * output convert the result downstream.
 */
export function buildSharedHouse(
  modules: CalculatorModuleInputs[],
  roofDraft?: HouseFirstRoofDraft | null,
  deckDrafts?: ObjectFirstDeckDraft[] | null,
  openingDrafts?: ObjectFirstOpeningDraft[] | null,
  ...
  const normalizedFootprintMode = normalizeHouseFootprintMode(
    footprintMode,
  ) as CalculatorHouseFootprintMode;
  const normalizedFootprintPreset = normalizeHouseFootprintPreset(
    preset,
  ) as CalculatorHouseFootprintPreset;
  const normalizedFootprintParams =
    normalizeHouseFootprintParams(footprintParams);
  const normalizedFootprintPolygon =
    normalizeHouseFootprintPolygon(footprintPolygon);
  ...
  const normalizedRoofMaterial = normalizeHouseRoofMaterial(
    roofMaterial,
  ) as CalculatorHouseRoofMaterial;
  const normalizedStoreyMode = normalizeStoreyMode(
    storeyMode,
  ) as CalculatorHouseStoreyMode;
  const firstModuleLengthMm = Math.round(
    (Number(modules[0]!.lengthM) || 6) * 1000,
  );
  const firstModuleProjectionMm = Math.round(
    (Number(modules[0]!.projectionM) || 3) * 1000,
  );
...
/**
 * PR-2A.1a (2026-05-22): exported so the post-Phase-1 direct snapshot
 * adapter can resolve pergola attachment against a synthesized
 * `HouseModel` (with `derivedEnvelope`) without going through the full
 * HouseFirst round-trip. The function itself is unchanged.
 */
export function buildPergolas(input: {
  ...
  input.modules.forEach((module, moduleIndex) => {
    ...
    groups.set(pergolaId, {
      label,
      modules: [
        { moduleId: module.id, moduleIndex, moduleInput: module.input },
      ],
    });
  });
  ...
  const warnings: HouseFirstMigrationWarning[] = [];
  const pergolas: PergolaModel[] = Array.from(groups.entries()).map(
    ([pergolaId, group]) => {
...
export function buildHouseFirstWorkbenchProjectModel(input: {
  ...
  const effectiveSnapshot = mergeEstimateDrawingDraftIntoSnapshot(
    input.snapshot,
    input.draft,
  );
  const calculatorInputs =
    resolveCalculatorInputsFromSnapshot(effectiveSnapshot);
  const modules = buildEstimateDrawingModules(effectiveSnapshot, {
    ignoreModuleResults: input.ignoreModuleResults,
  });

  // PR-2A.1b (2026-05-22): read object-first draft fields directly. The
  // previous cross-file bridge synthesis (`deriveHouseFirstDraftViewFromObjectFirst`
  // + `buildObjectWorkbenchCompatibilityDraftFromObjectFirstDraft`) is gone —
  // downstream `buildSharedDecks`/`buildSharedOpenings` consume the
  // ObjectFirst draft shapes natively now. Legacy `draft.houseFirst.*`
  // overrides still win for the test suite (see carrier docstring above);
  // production never sets that field.
  const authoredRoofDraft = readAuthoredRoofDraft(input.draft);
  const allDeckDrafts = readDeckDrafts(input.draft);
  ...
  const legacyFormDraft =
    sharedHouse.house && !authoredHouseAssembly
  ...
  // "first form in list" — same routing semantics as the deleted
  // legacy `hostHouseFormId` field.
  const deckHostFormId = (deck: ObjectFirstDeckDraft): string | null =>
  ...
  const firstFormDeckDrafts = firstFormId
    ? (allDeckDrafts?.filter((deck) =>
        isHostedByFirstForm(deckHostFormId(deck)),
      ) ?? null)
    : null;
  const firstFormOpeningDrafts = firstFormId
    ? (allOpeningDrafts?.filter((opening) =>
        isHostedByFirstForm(openingHostFormId(opening)),
      ) ?? null)
    : null;
  ...
  for (let index = 0; index < unifiedFormDrafts.length; index += 1) {
    ...
    const formDeckDrafts = isFirstForm
      ? firstFormDeckDrafts
      : (allDeckDrafts?.filter(
          (deck) => deckHostFormId(deck) === formDraft.id,
        ) ?? null);
    const formOpeningDrafts = isFirstForm
      ? firstFormOpeningDrafts
      : (allOpeningDrafts?.filter(
          (opening) => openingHostFormId(opening) === formDraft.id,
        ) ?? null);

...
| JOB-02 | Yellow | Repository implementation and local gates pass: `apps/worker` is a standalone Node 22 runtime, dark by default and fail-closed with no commercial handlers; migration `20260720_000006_background_job_worker_runtime.sql` adds lease-fenced runtime context plus safe aggregate job/worker projections; `npm run test:jobs` passes 5 files/86 tests and `npm run test:worker` passes 9 files/76 tests. Docker is unavailable locally, so completion still requires the dedicated six-migration real-PGMQ matrix and non-root container build to pass in CI. No shared database, deployment, producer, handler, or rollout is implied. |
...
- [ ] JOB-02's six-migration matrix plus worker typecheck/tests/build/CLI, strict service-role guard, and non-root Docker build pass in the dedicated CI workflow.
...
3. Finish JOB-02's six-migration and worker-container CI checkpoint, then continue JOB-03 through JOB-08 as repository work only; require shared-environment deployment review before enabling any producer or active worker rollout.
...
- Implemented JOB-02 repository-local worker/runtime scope without enabling work: the Node 22 worker is dark by default, uses only strict service-role RPC contracts, appends a per-boot identity, fences handlers with lease/visibility heartbeats and abort settlement, bounds concurrency/retry/shutdown, exposes cached safe health and aggregate metrics, and supports one-shot, drain, reconciliation, and queue-health commands. The local 5-file/86-test job suite and 9-file/76-test worker suite pass with typecheck, lint, build/CLI, architecture, security, ownership, and dead-code gates. The sixth migration and non-root container still require dedicated CI evidence; no shared database, deployment, producer, handler, or real email was touched.

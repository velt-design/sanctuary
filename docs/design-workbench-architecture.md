...
- Deck dragging in projection-backed plans must invert the same screen-axis transform used to draw the top projection, so pointer movement is screen-native. Live snap candidates come from the committed top-projection house body; object/geometry frames are only commit targets. Release commits must map the rendered preview polygon through matching frame coordinates back into canonical object plan metres before persistence, then hold the preview until rebuilt geometry matches that canonical commit instead of silently snapping to a different source frame.

...
export default function ConfiguredEnquiryFields({onSubmit,errors,state,error}:{onSubmit:FormEventHandler<HTMLFormElement>;errors:ContactFieldErrors;state:string;error:string|null}){
 ...
 useEffect(()=>{if(state==='success'){try{sessionStorage.removeItem(key);}catch{} }if(state==='error'||state==='success'||Object.keys(errors).length)status.current?.focus();},[state,errors]);
 ...
 if(state==='success')return <div className={css.form} ref={status} tabIndex={-1} role="status"><h1>Your enquiry has been sent.</h1><p>We’ll review your design and contact you to discuss the next step.</p><p>Your design is shown here for reference.</p></div>;

...
This repo is optimized for coding agents. Read this file first, then follow links into `docs/README.md`.

## First Moves

1. Check `git status --short` before editing.
2. Read the smallest relevant current-state doc in `docs/`.
3. Prefer `rg` and `rg --files` for repo discovery.
4. Keep changes scoped to the requested surface.
5. Do not revert user changes or unrelated worktree changes.
...
- `apps/portal/lib/drawings` and `apps/portal/components/drawings`: object-first workbench, geometry compatibility, drawing persistence.

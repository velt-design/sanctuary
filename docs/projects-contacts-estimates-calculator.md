...
Successful calculator saves remain on the calculator long enough to show the actual local-first state (`syncing`, `synced`, `offline`, `error`, or `conflict`). The outcome dialog previews the exact quote lines and customer total mapped from the optimistic saved estimate, including the stored-cost basis after Preserve. Staff may stay, return to the exact saved estimate, or explicitly create a quote from that estimate. Quote handoff uses `?tab=quotes&createFromEstimateId=...`; navigation never creates a quote until the user selects that action. The action waits while state is still hydrating, permits queued/syncing/synced/offline local snapshots, and disables on sync error, conflict, or a quote-mapping blocker such as an invalid blind. Quote create and refresh enforce the same blocker at the shared mapping boundary, so the guard does not depend on calculator UI state.

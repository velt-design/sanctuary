...
### 2026-07-30 - Dashboard Project Work Truth - Retire Legacy Action Aggregates

Date: 2026-07-30
Area: Dashboard project-work presentation and legacy project disposition
Status: Promoted
Decision or mistake: The Dashboard labelled raw non-archived project `follow_up_date`/`next_action_date` counts as overdue or due-today project actions. Those compatibility dates span legacy stages, can also receive one-way V2 projections, and do not prove a real current obligation. The same card repeated the Pipeline's quoting inventory, while an old snapshot queue payload was mapped into the browser response without a UI consumer.
Why it mattered: Staff could interpret stale compatibility dates as operational work or as evidence that many projects were lost, then bulk-close valid quoting, commercial, Schedule, Running Jobs, or historical records.

...
export default function RoofBattenControls({roof,onChange}:{roof:PreviewRoofChoices;onChange:(roof:PreviewRoofChoices)=>void}){
  ...
  return <section className={ui.group} aria-label="Roof timber battens"><h3>Timber under your roof</h3>
    <label className={css.checkboxRow}><input type="checkbox" checked={!!p} onChange={e=>onChange({...roof,roofBattens:e.target.checked?{...DEFAULT_ROOF_BATTENS}:undefined})}/>Timber battens under rafters</label>
    {p&&<>
      ...
      <label className={css.select}>Batten size<select aria-label="Roof batten profile" value={p.profile} onChange={e=>change({profile:e.target.value as RepresentativeRoofBattens['profile']})}>{TIMBER_PROFILES.map(profile=><option key={profile} value={profile}>{profile.replace('x',' × ')} mm</option>)}</select></label>
      {p.profile!=='39x39'&&<fieldset className={`${styles.choices} ${ui.options}`}><legend>Batten face</legend>{[false,true].map(edge=><label key={String(edge)} data-selected={p.edge===edge}><input type="radio" name="roof-batten-edge" checked={p.edge===edge} onChange={()=>change({edge})}/>{edge?'Narrow edge facing out':'Wide face facing out'}</label>)}</fieldset>}

...
export default function EmailReviewWorkspace({ api = emailReviewApi, synthetic = false }: { api?: EmailReviewApi; synthetic?: boolean }) {
  ...
  return <PageLayout>
    <StaffPageHeader title="Email review" variant="index" description="Review and save each draft, then approve or skip it. Approval does not send an email." />
    ...
    <div className={styles.toolbar}>
      ...
      <Select label="Review status" disabled={saving} value={status} onChange={event => navigate(() => { setStatus(event.target.value as typeof status); setPage(1); })}><option value="draft">Pending</option><option value="approved">Approved</option><option value="skipped">Skipped</option><option value="all">All drafts</option></Select>
    ...
    <div className={styles.counts} aria-live="polite">{counts ? <><span>{counts.all} total</span><span>{counts.draft} pending</span><span>{counts.approved} approved</span><span>{counts.skipped} skipped</span></> : null}</div>

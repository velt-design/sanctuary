...
export default function SidePanelControls({panel,onChange}:{panel:SidePanel;onChange:(p:SidePanel)=>void}){
  ...
  return <div>
    {panel.kind==='acrylic'&&<>
      ...
      <label className={css.checkboxRow}><input type="checkbox" checked={panel.battens} onChange={e=>change({battens:e.target.checked})}/><span>Add horizontal timber battens</span></label>
    </>}
    {slats&&<>
      {panel.kind!=='aluminium'&&<label className={css.select}>Timber<select aria-label="Side batten timber" value={panel.species??'cedar'} onChange={e=>change({species:e.target.value as 'cedar'|'thermopine'})}><option value="thermopine">ThermoPine · value option</option><option value="cedar">Cedar · upgrade</option></select></label>}
      ...
      <label className={css.select}>Slat size<select aria-label="Side slat profile" value={panel.profile} onChange={e=>change({profile:e.target.value,edge:['50x10','39x39'].includes(e.target.value)?false:panel.edge})}>{(panel.kind==='aluminium'?ALUMINIUM_PROFILES:TIMBER_PROFILES).map(p=><option key={p} value={p}>{p.replace('x',' × ')} mm</option>)}</select></label>
      {!['39x39','50x10'].includes(panel.profile)&&<fieldset className={styles.choices}><legend>Slat face</legend>{[false,true].map(edge=><label key={String(edge)} data-selected={panel.edge===edge}><input type="radio" name="side-slat-edge" checked={panel.edge===edge} onChange={()=>change({edge})}/>{edge?'Narrow edge facing out':'Wide face facing out'}</label>)}</fieldset>}

...
- The Projects list renders its header, filters, and truthful list region before data settles. One authenticated React Query request to `GET /api/staff/v1/projects/index` loads the selected active/archived/all project scope and complete contact list concurrently through the existing paginated server loader; the page route does not repeat that read.

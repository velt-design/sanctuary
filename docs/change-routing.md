...
| Mac mini private-node setup, access, secret, backup, OpenClaw security, rebuild, revocation, or AI incident procedure changes | `docs/ai/operations/README.md` and the owning operations runbook | `docs/ai/00-vision.md`, accepted AI ADRs, and `docs/security-privacy-quality.md` when a cross-system security rule changes | Rehearse the affected control with secret-free evidence. Keep production and business effects dark. |
...
| `packages/ai/**`, future AI task/approval/node adapters | `docs/ai/README.md`, `docs/ai/00-vision.md`, `docs/ai/09-decisions/README.md`, `docs/target-architecture.md` | Keep shared contracts provider-neutral and consult the relevant accepted ADR from the decision index. Keep the package free of secrets, persistence, UI, raw private payloads, and business-domain implementation. Production effects stay dark until their own approved rollout. |
| `docs/ai/operations/**`, future private-node deployment configuration | `docs/ai/operations/README.md`, `docs/ai/00-vision.md`, `docs/ai/09-decisions/AI-ADR-003-mac-mini-private-node.md` | Keep the Mac replaceable, hosted state canonical, credentials narrow, private networking non-public, and OpenClaw dark until every named control has evidence. Do not commit machine identifiers or secrets. |

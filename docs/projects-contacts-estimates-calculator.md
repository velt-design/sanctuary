...
- `POST /api/staff/v1/projects` owns project creation. The browser submits one stable project command ID plus either an existing contact ID or one stable new-contact command. The server checks normalized exact email/phone duplicate candidates, writes the contact/project through the auth-bound client, and returns a durable record receipt with a separate initial-setup-automation state. Matching stable-ID replays return the committed project without repeating automation and conservatively mark setup as not rechecked; a stable ID found with different details returns `PROJECT_CREATION_COMMAND_CONFLICT` and requires a form reload rather than claiming retry is useful. A setup-automation failure preserves the confirmed records and returns `202` with `needs_attention`; the UI opens the saved project and names the exception instead of claiming that nothing was saved. Service-role cleanup is limited to a newly inserted, unused contact after a definitively failed project write. If that cleanup or an indeterminate write cannot be verified, the route returns `PROJECT_CREATION_REVIEW_REQUIRED` and staff are told not to retry. The create UI never claims that a separate contact was saved first.
...
- The Overview Tasks panel is a UI-foundation consumer: canonical badges show To do/Pending/Saving/Done, and shared task feedback announces pending and per-task Retry states. This is presentation-only; the task-key mutation owner, field-owned rollback, authenticated API, snapshot refresh, and server-confirmed pipeline/automation behavior remain unchanged.

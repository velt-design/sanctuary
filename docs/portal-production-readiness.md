...
| Geometry/costing migration | Yellow | The workbench has a solved geometry artifact, a package-owned `GeometryQuantityTakeoff` derived from `Assembly3D`, and shadow `workbench_solved` commercial payload coverage that maps that takeoff. Baked fixture parity now compares `calculator_compat` and `workbench_solved` for mono, gable, box, mono-join, and screenshot-style hipped fixture shapes; live estimate and quote pricing still use the calculator costing path. | Keep the commercial path shadow-only, expand parity to representative saved estimate snapshots, extend geometry takeoff buckets as solver support grows, and retire compatibility only after manual workbench QA and parity are stable. |

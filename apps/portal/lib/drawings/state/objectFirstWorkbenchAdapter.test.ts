...
describe("addHouseFormToObjectFirstDraft", () => {
  ...
  it("clones the source forms footprint and resolves unauthored roof intent to the house default", () => {
    const result = addHouseFormToObjectFirstDraft({
      draft: makeSingleFormDraft(),
    });
    ...
    expect(added!.roofIntent.form).toBe("hipped");
    expect(added!.roofIntentAuthored).toBeUndefined();
  });

  it("preserves authored mono roof intent when cloning from an explicit mono source", () => {
    const result = addHouseFormToObjectFirstDraft({
      draft: makeSingleFormDraft({ roofIntentAuthored: true }),
    });
    const [, added] = result.houseAssembly!.houseForms;
    expect(added!.roofIntent.form).toBe("mono");
    expect(added!.roofIntentAuthored).toBe(true);
  });
  ...
  it("clones from a non-primary source when sourceHouseFormId is set", () => {
    ...
    expect(result.houseAssembly!.houseForms.at(-1)!.transform.offsetXM).toBe(
      30,
    );
  });
  ...
  it("creates a deterministic first form when the assembly is empty", () => {
    const result = addHouseFormToObjectFirstDraft({
      draft: makeEmptyHouseAssemblyDraft(),
    });
    ...
    expect(result.houseAssembly!.houseForms[0]).toMatchObject({
      ...
      roofIntent: { form: "hipped", material: "corrugated_iron" },
    });
  });
  ...
  it("creates a house assembly when no houseAssembly is present", () => {
    ...
    expect(result.houseAssembly!.houseForms.map((form) => form.id)).toEqual([
      "house-form-1",
    ]);
  });
});
...
describe("resolveNextHouseFormIdAfterRemoval", () => {
  ...
  it("clears selection when removing the only form and reports missing ids distinctly", () => {
    expect(
      resolveNextHouseFormIdAfterRemoval([{ id: "house-main" }], "house-main"),
    ).toBeNull();
    expect(
      resolveNextHouseFormIdAfterRemoval(
        [{ id: "house-main" }],
...
describe("add -> remove -> add round-trip", () => {
  it("reuses the freed id slot after a remove (no orphaned ids)", () => {
    // Add house-form-2.
    const afterAdd1 = addHouseFormToObjectFirstDraft({
      draft: makeSingleFormDraft(),
    });
  ...
  it("starts over at house-form-1 after removing every house form", () => {
    ...
    expect(afterAdd.houseAssembly!.houseForms.map((form) => form.id)).toEqual([
      "house-form-1",
    ]);
  });
});

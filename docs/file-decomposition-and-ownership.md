...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | Calculator UI, module drawing surfaces, estimate state, and presentation helpers are still concentrated in a few client files. | Continue reducing shared drawing atoms and any remaining oversized PlanSvg/model-space presenters now that plan layout is split into shared geometry, sheet layout, and model-space layout owners. | `npm run test:portal:projects`, targeted calculator/component tests, `npm run typecheck` |

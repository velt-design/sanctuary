...
- The local, default-disabled `GET/POST /api/staff/v1/projects/[projectId]/correspondence` requires a staff session and auth-bound project visibility. GET checks configuration only; an empty-body, same-origin POST requests the server-derived project/actor through a dedicated signed Velt request. Current staff membership and project access are rechecked before delivering private/no-store evidence. No email, actor, provider input or owner cookie is accepted from the browser. The Velt receiver and replay/audit/stop controls are implemented and locally tested in the paired Velt branch; deployment and real staff acceptance remain unverified. This route is not a live staff email connection. The exact paired protocol and server-only configuration are owned by `project-command-centre-architecture.md`.
...
- `GET /api/staff/v1/ai/tasks` and `GET /api/staff/v1/ai/tasks/[taskId]` are bounded, read-only Sanctuary AI activity models. They require `requireStaffContext()`, use only its auth-bound Supabase client, select explicit safe columns from `public.ai_tasks`, `public.ai_task_events`, and `public.ai_approvals`, validate closed contract values, and return `private, no-store`. List status and limit are bounded; an RLS-hidden cross-project task and a missing task both return `404`. Session-ending database responses become `401`, missing AI schema becomes `503 AI_ACTIVITY_SCHEMA_NOT_READY`, and raw provider/database detail is never returned. Private AI tables, user IDs, service role, mutation controls, and execution are outside these routes.
- `GET /api/staff/v1/projects/index` is the staff-authenticated Projects-list read model. It accepts bounded `archive`, `q`, `status` (with `stage` retained as a transport-compatible alias), `journey`, `state`, `owner`, `page`, `pageSize`, and `sort` parameters; calls `staff_projects_index_v3()` through the request's auth-bound client; returns one exact-count project page plus linked contact display rows, Project Owner, Waiting/Closed context, and the current server-ranked action for only that page; and is always `private, no-store`. The V3 function fails closed when portfolio rollout completeness is not provable. Missing schema returns `503 PROJECTS_INDEX_SCHEMA_NOT_READY`; `20260731000002_project_work_portfolio_rollout.sql` owns the portfolio/state prerequisite and `20260731000003_project_pipeline_accountability_reads.sql` owns the current index read contract.
...
- `POST /api/staff/v1/projects/[projectId]/delete` remains the existing admin-only full-project hard-delete path. It requires stage-sensitive typed confirmation, removes quotes before the project to satisfy commercial references, and appends a non-project tombstone audit. Project Work governed/append-only guards permit only the resulting real quote/project cascades, including repair-signal cleanup; they do not permit direct V2 child-row deletion.
...
- `GET /api/staff/v1/projects/[projectId]/command-centre` is the staff-authenticated Project Overview read model. It uses `requireStaffContext()` and the auth-bound client, resolves current design, owner, and V2 Project Work truth on the server, and returns only bounded normalized facts. An unexpectedly unmarked project receives a non-actionable retired-model integrity response, never a legacy command surface. Every response is `private, no-store`.
- `GET /api/staff/v1/work-items/queue` is the staff-authenticated one-row-per-project Work Queue read model. It uses `requireStaffContext()` and the auth-bound client, pages `project_work_queue_v3()` in 1,000-row hosted-safe ranges, proves whole-portfolio marker/state completeness with `staff_project_state_counts_v1()`, composes durable work/recovery/state rows with canonical specialist candidates on the server, and returns bounded command metadata for normal staff actions. The full route filters that authoritative result in memory by search, effective owner, detailed stage, and urgency before pagination; filters do not rerank or invent work. Dashboard consumes a limited preview of the same owner; personal reminders remain separate. Missing marker/state or queue schema (`PGRST200`, `PGRST202`, or `PGRST205`), incomplete/malformed portfolio counts, or reaching the explicit 5,000-row safety ceiling returns `503 WORK_ITEMS_UNAVAILABLE`; clients render the named not-ready state and must not expose stale actions.
- The legacy Contacted classifier and per-project migration routes are retired after the whole-portfolio rollout. Historical legacy rows remain database evidence only and have no staff/admin action route.
...
- `GET /api/admin/project-work/inactive-enquiries` is the admin-authenticated, `private, no-store` 30-day stale-enquiry review. Its `POST` accepts one stable command ID, the exact report timestamp/threshold, and a bounded exact candidate list from that review. The database revalidates every original fingerprint plus current activity/source and future-Waiting protection before atomically delegating each unchanged row to the normal state command. Any drift returns `409` and closes nothing; the browser never infers eligibility.
- `GET /api/staff/v1/staff-directory`, `PATCH .../command-centre/owners`, and `GET /api/staff/v1/dashboard/project-exceptions` own the staff-directory, project-owner, and V2 project-exception contracts. Project-owner changes are admin-only, accept only the approved Jordan/JP/Joe/Bruce keys, and require UUID command IDs plus optimistic versions. Every success and error is `private, no-store`. The legacy primary-action command route is retired.
...
- The Praxis integration is a separate least-privilege database boundary, not a service-role exception. `apps/portal/lib/praxis/server.ts` may use only `PRAXIS_SANCTUARY_DATABASE_URL`; runtime identity and privilege checks fail closed before projection reads.
...
- Estimate create accepts a stable `clientIntentId` and the exact `calculator_snapshot`. A matching committed intent may replay without resending the snapshot; a new intent without it returns `409` and must not substitute the latest saved estimate. Add-on creation also requires `commercialScopeKind=add_on` plus a valid stable `commercialScopeId`; base creation omits both, and duplicate/revision flows preserve their source scope.
...
- `GET /api/staff/v1/projects/[projectId]/invoice-schedule` requires `requireStaffSession` and exposes aggregate base/add-on job totals plus quote-scoped stages to all staff; payment-entry detail is added only when the resolved role is admin. `POST /api/admin/projects/[projectId]/invoices`, `POST /api/admin/projects/[projectId]/payments`, `POST /api/admin/payments/[paymentId]/allocations`, `POST /api/admin/payments/[paymentId]/reverse`, `POST /api/admin/invoices/[invoiceId]/paid`, `POST /api/admin/invoices/[invoiceId]/void`, `POST /api/admin/quotes/[quoteVersionId]/supersede`, `POST /api/staff/v1/quotes/[quoteId]/invoice` (legacy compatibility adapter), and `DELETE /api/quotes/[quoteVersionId]` require `requireAdminSession`. They create one whole invoice against the exact accepted quote version, reconcile actual job payments, mark or void a whole invoice, retire a quote, or delete only an unsent current draft. Non-admin callers receive `401`/`403`; ordinary staff invoice list/preview/send and quote read/edit/delivery routes remain staff-authenticated.

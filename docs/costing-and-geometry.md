...
The staff calculator's `roof_material: none` mode is an open pergola frame, not a new roof form. `@sp/costing` normalizes it to the standard pitched structure at `0°`, fixes rafters, the front beam, and any attached ledger to `150x50`, and leaves the post profile unchanged. `rafter_spacing_mm` is a positive target maximum: it defaults to `500`, has no upper cap, and the engine evenly distributes rafters so the resolved spacing does not exceed it. Roofing, roof foam, gutters, downpipes, automatic or entered flashings, and their covering/drainage labour are zero. The mode always resolves through Bespoke pricing and is not part of the public Simple cover calculator.

-- Materialize normalized search values so RLS planning never falls back to
-- recomputing document bigrams for every row during typeahead.
...
alter table public.projects
  add column if not exists portal_search_bigrams text[]
  generated always as (
...
alter table public.contacts
  add column if not exists portal_search_name_bigrams text[]
  generated always as (
...
begin
  ...
  execute format(
    'create index if not exists projects_portal_search_document_materialized_trgm_idx on public.projects using gin (portal_search_document %I.gin_trgm_ops)',
    trgm_schema
  );
  execute format(
    'create index if not exists contacts_portal_search_document_materialized_trgm_idx on public.contacts using gin (portal_search_document %I.gin_trgm_ops)',
    trgm_schema
  );
end
$indexes$;

create index if not exists projects_portal_search_bigrams_materialized_idx
  on public.projects using gin (portal_search_bigrams);

create index if not exists contacts_portal_search_bigrams_materialized_idx
  on public.contacts using gin (portal_search_bigrams);

create index if not exists contacts_portal_search_name_bigrams_materialized_idx
  on public.contacts using gin (portal_search_name_bigrams);

drop index if exists public.projects_portal_search_document_trgm_idx;
drop index if exists public.contacts_portal_search_document_trgm_idx;
drop index if exists public.projects_portal_search_document_bigram_idx;
drop index if exists public.contacts_portal_search_document_bigram_idx;
drop index if exists public.contacts_name_portal_search_bigram_idx;
...
as $function$
  ...
  project_candidate_ids as (
    ...
    where length(input.normalized_query) = 2
      ...
      and project.portal_search_bigrams @> array[input.normalized_query]
    ...
    where length(input.normalized_query) >= 3
      and (select allowed from access_state)
      and project.portal_search_document ilike input.contains_pattern escape E'\\'
    ...
    where length(input.normalized_query) = 2
      and (select allowed from access_state)
      and contact.portal_search_name_bigrams @> array[input.normalized_query]
  ...
  contact_candidate_ids as (
    ...
    where length(input.normalized_query) = 2
      and (select allowed from access_state)
      and contact.portal_search_bigrams @> array[input.normalized_query]
    ...
    where length(input.normalized_query) >= 3
      and (select allowed from access_state)
      and contact.portal_search_document ilike input.contains_pattern escape E'\\'
  ),
  contact_ranked as (
    select
      contact.id,
...
comment on function public.portal_search_v1(text, integer) is
  'Bounded Projects and Contacts header search using materialized bigram and trigram documents. SECURITY INVOKER preserves caller RLS and reports portal access in-band.';

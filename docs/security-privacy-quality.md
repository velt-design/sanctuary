...
- The separate enquiry-identity projection exposes only canonical intake/dispatch IDs and times, fixed receipt outcome/code, provider API identifier, and null/unverified RFC identity. It contains no address, message, subject/body, attachment, raw payload, provider idempotency key or frozen email hash. Its authenticated UTC window is at most 31 days, candidate list at most 100 before deduplication, canonical union at most 100 rows, and complete UTF-8 response at most 128 KiB. Overflow returns an error without records or terminal-success coverage. Terminal means all rows/candidates processed in the current database snapshot, not permanent mailbox completeness or confirmed delivery; unmatched references and absent intents do not establish that no email was sent.

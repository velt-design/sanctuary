-- Forward-compatible reporting installation for a current portal database.
-- Historical migration 20260903000001 remains immutable. This reporting DDL
-- snapshot intentionally repeats that boundary so an uninstalled production
-- database need not replay obsolete finance logic or backfill business rows.
-- Apply this reviewed migration directly when the historical reporting version
-- is absent; do not mark that older version applied or replay it afterwards.
begin;
...
-- Supabase's postgres operator is not a superuser. Do not attempt a redundant
-- superuser-only ALTER ROLE; refuse an existing role with broader authority.
do $role_posture$
begin
  if exists (select 1 from pg_roles where rolname = 'sanctuary_praxis_reader'
    and (rolcanlogin or rolsuper or rolcreatedb or rolcreaterole or rolinherit
      or rolreplication or rolbypassrls))
    or exists (select 1 from pg_auth_members membership join pg_roles member
      on member.oid=membership.member where member.rolname='sanctuary_praxis_reader') then
    raise exception 'Existing reporting group has unexpected authority';
  end if;
end
$role_posture$;
...
-- No historical business row is updated. Existing rows retain unknown update
-- time; projection freshness falls back to the source creation/cancellation date.
...
alter table public.project_invoice_plan_items alter column updated_at set default now();
...
create trigger project_invoice_plan_items_set_updated_at before update on public.project_invoice_plan_items
for each row execute function public.set_updated_at();

-- Preserve the current canonical finance implementation byte-for-byte except
-- its narrow reporting-role authorization. Never replay the old calculation.
do $reporting_auth$
declare
  definition text := pg_get_functiondef('public.commercial_project_financial_truth(uuid)'::regprocedure);
  anchor text := 'if auth.role() <> ''service_role'' and not public.has_portal_access() then';
  replacement text := 'if auth.role() <> ''service_role'' and not public.has_portal_access() and not pg_has_role(session_user, ''sanctuary_praxis_reader'', ''member'') then';
begin
  if strpos(definition, replacement) > 0 then return; end if;
  if strpos(definition, 'and not pg_has_role(session_user, ''sanctuary_praxis_reader'', ''member'') then') > 0 then return; end if;
  if (length(definition)-length(replace(definition,anchor,'')))/length(anchor) <> 1 then
    raise exception 'Finance authorization changed; review current definition before reporting installation';
  end if;
...
cross join lateral (values (jsonb_build_object(
  ...
  -- Some deployed project schemas have no optimistic-lock version column.
  -- Record hashing below still identifies this exact source snapshot.
...
select
  ...
  coalesce(quote.updated_at, quote.created_at) as recorded_at,
  praxis_reporting.version_v1(payload.payload) as record_version,
  payload.payload,
  payload.policy_version,
  payload.redaction_count,
...
cross join lateral (values (jsonb_build_object(
  ...
  'updatedAt', coalesce(quote.updated_at, quote.created_at)
))) assembled(value)
cross join lateral praxis_reporting.safe_payload_v1(assembled.value) payload;
...
select
  ...
  item.project_id,
  item.quote_version_id as parent_id,
  coalesce(item.updated_at, item.cancelled_at, item.created_at) as recorded_at,
  praxis_reporting.version_v1(payload.payload) as record_version,
  payload.payload,
  payload.policy_version,
  payload.redaction_count,
...
cross join lateral (values (jsonb_build_object(
  ...
  'createdAt', item.created_at,
  'updatedAt', coalesce(item.updated_at, item.cancelled_at, item.created_at)
))) assembled(value)
cross join lateral praxis_reporting.safe_payload_v1(assembled.value) payload;
...
cross join lateral (
  select greatest(
    ...
    coalesce((select max(coalesce(item.updated_at, item.cancelled_at, item.created_at)) from public.project_invoice_plan_items item where item.project_id = project.id), project.updated_at),

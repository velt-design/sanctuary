...
The desktop preview rail is a tabbed Result Inspector. Its header keeps the current or last-valid customer price, freshness, input-issue count, and quote readiness visible while Pricing, Materials, Labour, Workings, and Issues route the existing output owners. Pricing contains pricing context, exact-cent price-by-item, permitted actual-cost review, and admin cost-impact surfaces without repeating the rounded lead total. Materials consumes the whole-job `@sp/costing` trusted breakdown: all BOM rows are grouped by user-facing purpose and show quantity, unit, scope ownership, profile, assumptions, and supported stock/sheet `Why this quantity?` disclosures. Staff see quantities and explanations but not internal dollar costs; admins retain cost visibility and the separate Advanced debug trace. Labour retains its existing admin and Advanced gates, then groups every install action by work stage with quantity, unit, minutes, crew hours, non-neutral loadings, ownership, internal cost, and package-owned explanation. Both breakdowns explicitly label retained last-valid results and fail closed for old outputs without the contract. Workings follows the selected module and presents its rafter answer and written calculation before Plan/Section drawings and Advanced structure outputs. Issues contains Quote Status and warnings. Jump closes the Issues dialog, completes any module switch, reveals an Advanced-only section when required, then uses the nearest real vertical scroll owner to reveal the complete field and inline error below visible chrome before moving focus to the invalid control; queued routing is cancellable and does not move the Inspector rail. The Workspace controls the active tab. A genuine tab change resets the independent desktop result rail to its start, while same-tab activation does nothing; ordinary tab changes in the stacked layout do not move the page. Tabs use the standard keyboard arrow, Home, and End behavior; inactive panels remain mounted so switching tabs does not discard local disclosure, drawing, debug, or actual-cost state. The tab strip becomes horizontally scrollable in a narrow preview rail, while the established stacked-layout page scrollbar remains authoritative below `1080px`. Calculations, permissions, save behavior, quote mapping, and source-of-truth boundaries are unchanged.

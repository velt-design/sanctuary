...
export default function RailProvider({children}:{children:ReactNode}){
 ...
 useEffect(()=>{if(new URLSearchParams(window.location.search).get("resume")!=="1")return;const saved=readDesignContinuation().section;setSection(saved);if(saved==='lighting')lighting.open();},[]);

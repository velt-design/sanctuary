...
import { refreshDraftQuoteVersionFromEstimate } from '@/lib/quotes/server';
...
export async function POST(req: Request, ctx: { params: Promise<{ quoteVersionId: string }> }) {
  ...
  try {
    const quoteVersion = await refreshDraftQuoteVersionFromEstimate(id, estimateVersionId, actor);

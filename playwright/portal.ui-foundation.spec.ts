...
const evidenceDir = path.resolve(
  process.cwd(),
  "artifacts/ui-foundation-hardening-final",
);
...
async function expectNoDocumentOverflow(page: Page) {
  ...
  expect(
    ...
    `document overflowed by ${dimensions.scrollWidth - dimensions.clientWidth}px: ${JSON.stringify(dimensions.offenders)}`,
...
async function expectMajorSectionsDoNotOverlap(page: Page) {
  const overlap = await page
    .locator(
      "main[data-ui-foundation]:visible > section, main[data-ui-foundation]:visible > footer",
    )
...
async function expectCoreControlsUncropped(page: Page) {
  const cropped = await page
    .locator(
      "main[data-ui-foundation]:visible button:visible, main[data-ui-foundation]:visible input:visible, main[data-ui-foundation]:visible select:visible",
    )
    .evaluateAll((elements) => {
...
async function expectNoLegacyRoundedSurfaces(root: Locator) {
  const offenders = await root.locator("*:visible").evaluateAll((elements) =>
    elements
      .flatMap((element) => {
        ...
        const rect = element.getBoundingClientRect();
        const isSmallCircle =
          Math.abs(rect.width - rect.height) < 1 && rect.width <= 24;
        if (
...
async function firstProjectDetailRoute(page: Page): Promise<string | null> {
  ...
  await expect(
    page.locator("main[data-projects-index-state]:visible"),
  ).toHaveAttribute(
    "data-projects-index-state",
    /^(fresh|refresh-failed|unavailable)$/,
  );
  const href = await page
    .locator("a", { hasText: "Open" })
    .first()
    .getAttribute("href")
    .catch(() => null);
...
test("foundation is responsive, semantic, keyboard-operable, and reduced-motion safe", async ({
  page,
}) => {
  ...
  for (const viewport of viewports) {
    ...
    await expect(page.locator("main[data-ui-foundation]:visible")).toHaveCount(
      1,
    );
  ...
  const mobileBar = page
    .locator("header")
    .filter({
      has: page.getByRole("button", { name: "Open portal navigation" }),
    });
  ...
  await openFresh(page, "/staff/ui-foundation");
  const actionStates = page.locator(
    '[class*="interactionTable"] button[data-visual-state]',
  );
  for (let index = 0; index < (await actionStates.count()); index += 1) {
    expect(
      await computedContrastRatio(actionStates.nth(index)),
    ).toBeGreaterThanOrEqual(4.5);
  }
  expect(
    await computedContrastRatio(
      page.locator('[class*="stageBadge"][data-stage="quoting"]').first(),
    ),
  ).toBeGreaterThanOrEqual(4.5);
  expect(
    await page.locator('a[aria-label="Projects"]').count(),
  ).toBeLessThanOrEqual(1);
  const destructiveTrigger = page.getByRole("button", {
    name: "Open destructive confirmation",
  });
...
test("Projects Index and Project Detail prove the foundation on real workflows", async ({
  page,
}) => {
  const detailRoute = await firstProjectDetailRoute(page);
  await expect(
    page.locator('[data-page-header-variant="index"]:visible'),
  ).toHaveCount(1);
  await expect(
    page.getByRole("search", { name: "Search and filter" }),
  ).toBeVisible();
  await expect(page.locator("[data-portal-sidebar-panel] > div")).toHaveCSS(
    "background-color",
    "rgb(11, 11, 10)",
  );
  await expect(
    page
      .locator('[data-portal-sidebar-panel] a[aria-current="page"]')
      .first()
      .locator(".."),
  ).toHaveCSS("border-left-color", "rgb(240, 90, 0)");
  await expect(
    page.locator(
      'main[data-projects-index-state] section[aria-label="Filters"]',
    ),
  ).toHaveCSS("border-radius", "0px");
  await expect(
    page.locator(
      'main[data-projects-index-state] section[aria-label="Projects list"]',
    ),
  ).toHaveCSS("border-radius", "0px");
  await expectNoDocumentOverflow(page);
  ...
  test.skip(
    !detailRoute,
    "Authenticated test account has no representative project.",
  );
  for (const viewport of viewports) {
    await page.setViewportSize({
      width: viewport.width,
    ...
    await expect(
      page.locator('[data-page-header-variant="detail"]:visible'),
    ).toHaveCount(1);
    await expect(
      page.locator("[data-project-snapshot-state]:visible"),
    ).toHaveAttribute(
      "data-project-snapshot-state",
      /^(fresh|refresh-failed|unavailable)$/,
    );
    await expect(
      page.locator("[data-command-centre-state]:visible"),
    ).not.toHaveAttribute("data-command-centre-state", "pending");
    await expect(
      page.locator('[data-project-orientation="true"]:visible'),
    ).toHaveCount(1);
    await expect(
      page.locator('[data-project-work-section="true"]:visible'),
    ).toHaveCount(1);
    await expect(page.locator("[data-project-owner]:visible")).toHaveCount(1);
    await expect(
      page.locator('ol[aria-label="Project stage"]:visible li'),
    ).toHaveCount(0);
    await expectNoDocumentOverflow(page);
  ...
  await expect(
    page.locator('[data-project-active-tab="quotes"]:visible'),
  ).toHaveCount(1);
  await expect(
    page.locator('[data-project-commercial-view="quotes"]:visible'),
  ).toHaveCount(1);
  await expect(page.getByText("Loading quotes…", { exact: true })).toHaveCount(
    0,
  );
  await expectNoDocumentOverflow(page);
  await expectNoLegacyRoundedSurfaces(
  ...
  await expect(
    page.locator('[data-project-active-tab="invoices"]:visible'),
  ).toHaveCount(1);
  await expect(
    page.locator('[data-project-commercial-view="invoices"]:visible'),
  ).toHaveCount(1);
  await expectNoDocumentOverflow(page);
  await expectNoLegacyRoundedSurfaces(
    page.locator('[data-project-commercial-view="invoices"]:visible'),

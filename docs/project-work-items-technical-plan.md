...
- the specialist primary-action adapter covers quote-delivery recovery, stage-aware Site Visit routing, draft-quote send, and Quoting-only estimate-to-quote handoff;
...
`project_work_queue_v3()` returns durable repair, work-item, blocked, Waiting-review, and triage candidates plus optimistic row versions and effective responsibility. Repair rows include the exact repair-signal ID and row version; confirmation reconciliation must use both and may update only that signal. `teamQueue.ts` reads the RPC in 1,000-row hosted-safe ranges, fails closed when the explicit 5,000-row ceiling is reached, proves marker/state completeness with `staff_project_state_counts_v1()`, and composes the same stage-aware Site Visit and quote/estimate specialist selectors used by project-specific reads without assembling lifecycle or commercial truth in the browser. The full route groups Overdue, Today, Next seven business days, Blocked, and Needs triage; the Dashboard receives only a bounded preview. Far-future-only projects remain out of the operational surface.
...
- Keep the approved quote adapter gated to `Quoting`; a reasoned stage correction to `Quoting` is the explicit no-visit path.
...
The Schedule and work-items integration is deployed, while Schedule V2 remains authoritative for install assignment, readiness, actual start, and actual finish. Running Jobs owns materials- and roofing-ordered facts for V2. Site Visits is hidden from normal navigation and remains directly reachable as dormant Schedule-owned code; the separately approved active-V2 Site Visit-stage deep link opens that retained workflow without creating a work item or automatic stage/Schedule side effect. The optional V2 completion confirmation remains a separate manual fact.
...
2026-07-30 window after a completed physical backup was confirmed. Cutover record
`c8471e92` records the postflight, and snapshot-cache hotfix merge `809f2c5e`
followed. `db push`, `migration up`, and migration repair

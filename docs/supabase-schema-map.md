...
The allowlist keeps customer identity, contact, project, commercial, and financial facts while excluding raw enquiry payloads, attachments and storage paths, raw commercial design input, token hashes, PDF/email bodies and recipients, provider identifiers/errors, credentials, private execution data, and unrestricted audit JSON. The approved nested enquiry, estimate, and quote-version JSON containers pass through the versioned recursive sanitizer; safe business shape such as `inputs.modules[].attachmentSide` remains, while forbidden keys and recognisable credential strings are removed or redacted before the view returns them. The reader receives only schema/function/view access needed for these projections. It receives no base-table, private/auth/storage, sequence, write-RPC, or unexpected security-definer access. A separate per-environment LOGIN and exact identity row are deliberately not created by the migration and remain deployment work.

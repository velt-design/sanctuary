...
`projects.version` is a legacy compatibility field from `supabase/contacts_projects.sql`, not a maintained Project Work revision. `projects.deposit_received_at` belongs to `20260730_000001_marketing_conversion_delivery.sql`, whose broader outbox effects must not be replayed just to prepare a reader. The core project projection extracts these two optional keys from the source row, preserving genuine values and emitting the required JSON null keys when unavailable. All other fields remain explicitly allowlisted; `recordVersion` remains the canonical payload hash. The source-only correction of the demonstrably uninstalled core and its original hash are recorded in `docs/environment-auth-supabase.md`.

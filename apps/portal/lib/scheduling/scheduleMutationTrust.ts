...
export function isValidScheduleCrewSchedule(value: unknown): value is ScheduleCrewSchedule {
  ...
  for (const rawConflict of schedule.conflicts) {
    ...
    const pair = `${conflict.job_id}:${conflict.conflicting_job_id ?? ''}`;
    if (
      !isUuid(conflict.job_id) ||
      !scheduledJobIds.has(conflict.job_id) ||
      conflictPairs.has(pair) ||
      (conflict.overlap_key !== undefined && (typeof conflict.overlap_key !== 'string' || conflict.overlap_key.length === 0 || conflict.overlap_key.length > 300)) ||
      (conflict.conflicting_job_id !== undefined && (!isUuid(conflict.conflicting_job_id) || !scheduledJobIds.has(conflict.conflicting_job_id) || conflict.conflicting_job_id === conflict.job_id)) ||

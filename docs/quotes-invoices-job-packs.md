...
- Admin invoice creation is atomic and serialised against job payment changes. When multiple accepted families exist, the modal first selects the base or add-on quote; next stage, full remaining, custom fixed or percentage-of-remaining, and a two-to-ten-way split are resolved only against that selected quote's balance. Due date, label/reference, save-only or create-and-send, and the resolved before/after balance are explicit. Amounts above the remaining selected quote balance fail unless the administrator supplies an override reason, which is stored on the invoice and in the transaction audit event.
- Admin-only routes record payments/adjustments, replace allocations, reverse ledger entries, mark a whole open invoice paid, or void a whole open invoice with a required reason. Ledger and allocation audit triggers preserve evidence even if application-level audit insertion is unavailable. Voiding preserves the invoice number and audit evidence, clears public-token access, leaves accepted-job state unchanged, and makes that scheduled term eligible to invoice again. Paid invoices cannot be voided or partially paid. Incorrect issued invoices are voided and recreated, never silently edited. Staff retain read/preview/send access.

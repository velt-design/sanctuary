...
- ~~The deeper fix is to migrate `form: 'gable'` -> `form: 'hipped' + openGableEndIds: <all terminals>` at the workbench draft normalization boundary so every consumer reads coherent state.~~ **PARTIAL 2026-05-14 (Slice 2):** `normalizeHouseFormRoofIntent` now performs this migration when an explicit footprint polygon is available (custom-mode houses and any preset-mode house whose polygon has already been resolved). Inspector `terminalEnds[].isOpen` derivation in `objectWorkbenchStatusModel.ts` falls back to `intent.form === 'gable' ? true : intent.openGableEndIds.includes(end.id)` so the rail labels reflect the rendered topology even for preset-mode houses still carrying `form: 'gable'`. The geometry-side compat migration at `normalize.ts:691-720` REMAINS in place as a safety net for the preset-mode/empty-polygon case. **Slice 2B (retiring the geometry compat):** still open; requires moving the migration to a later boundary that always has the resolved polygon (e.g., `buildObjectFirstWorkbenchProjectModelFromLegacyEstimateSnapshot` or the workbench solver). Slice 2B unblocks Milestone 13 session C (drop `'gable'` from `HouseRoofForm`).

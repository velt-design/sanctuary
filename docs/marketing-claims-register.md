...
indexable `/simple-pergolas-auckland` product and conversion route. That route uses the
approved workmanship and acrylic UV wording, qualifies roof and blind weather
behaviour, publishes only the separately approved input-specific live Simple
estimate described below, makes no generic price-band or timing promise and uses approved product-fit
limits rather than structural or consent guarantees. The retired acrylic v2 route permanently
...
| --- | --- | --- | --- | --- | --- | --- |
| Pergola price bands and examples | Pending approval. Do not publish a generic number or representative band. | Dated set of representative projects and current sell pricing, with scope and outliers identified. | State GST treatment, inclusions, exclusions, engineering, consent, site access and accessories beside every band. This pending status does not prohibit the separately approved input-specific Simple estimate below. | Sanctuary commercial lead and finance | Pending | Cost guide and any generic page-level price example |
| Live input-specific Simple cover estimate | Approved wording family: `Initial installed estimate`, `From $X`, `GST and standard installation included.` and `Subject to site measure, structural review and confirmed scope.` A priced continuation may use `Request a site measure`. | Explicit Sanctuary leadership instruction to preserve the existing published-costing calculator and use site measure as the post-estimate request on 2026-08-06. The marketing server resolves the immutable published configuration and calculates through the package-owned costing and customer-pricing boundary. | Applies only to the visitor's current validated Simple inputs and the exact published costing version. It is not a generic price band, fixed quote, final scope, structural/consent outcome or promise of a visit or quote. Missing, invalid, Custom, unavailable or unverifiable calculations show no fallback price. | Sanctuary leadership | 2026-08-06 | `/simple-pergolas-auckland` and `/simple-cover-calculator` only |
...
| Simple cover product-fit limits | Approved wording: a Sanctuary Simple cover may be up to 30 m2 on a ground-level deck or patio, up to 20 m2 on an elevated or first-floor deck, project up to 6 m from the house and generally use post spacing up to 4 m when the house connection and site access are straightforward. | Explicit Sanctuary leadership approval on 2026-08-04 for the Simple cover sales pathway. | These are product qualification limits, not structural capacity, span, consent, exemption, price or site-suitability promises. Preserve `generally` for post spacing and state that the actual site, connection, exposure and approval pathway still need confirmation. | Sanctuary leadership | 2026-08-04 | Noindex Simple cover conversion route and its enquiry qualification only |

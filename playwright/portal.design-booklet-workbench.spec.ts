...
test.describe("design booklet workbench fixture", () => {
  test("composes mixed pages and downloads the matching dynamic PDF", async ({
    ...
    const railButtons = pageRail.getByRole("button");
  ...
  test("uses a compact desktop control panel beside a wide preview workspace", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto(FIXTURE_PATH);

    const controlPanel = page.locator("[data-booklet-control-panel]");
    const previewWorkspace = page.locator("[data-booklet-preview-workspace]");
    const [controlBounds, previewBounds] = await Promise.all([
      controlPanel.boundingBox(),
      previewWorkspace.boundingBox(),
    ]);

    expect(controlBounds).not.toBeNull();
    expect(previewBounds).not.toBeNull();
    if (!controlBounds || !previewBounds) return;

    expect(controlBounds.x).toBeLessThan(previewBounds.x);
    expect(previewBounds.width / controlBounds.width).toBeGreaterThan(2);
    expect(Math.abs(controlBounds.y - previewBounds.y)).toBeLessThan(2);
    await expect(page.locator('[data-page-kind="cover"]')).toBeVisible();
  });
  ...
  test("keeps the composer and scaled A4 drawing preview usable on a narrow screen", async ({
    ...
    await page
      ...
      .getByRole("button", { name: /Drawings 1/ })
      .click();
    await expect(page.locator('[data-page-kind="drawings"]')).toBeVisible();

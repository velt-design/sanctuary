...
try {
  ...
  const projectionDefinition = () => sql("select pg_get_functiondef('praxis_reporting.safe_payload_v1(jsonb)'::regprocedure);");
  const originalProjection = projectionDefinition();
  const projectionGrants = () => sql("select proacl::text from pg_proc where oid='praxis_reporting.safe_payload_v1(jsonb)'::regprocedure;");
  const originalGrants = projectionGrants();
  const boundsMigration = read('supabase/migrations/20260917000001_praxis_projection_aggregate_bounds.sql');
  sql(boundsMigration.replace(/commit;\s*$/, 'rollback;'));
  assert.equal(projectionDefinition(), originalProjection);
  sql(boundsMigration);
  const boundedProjection = projectionDefinition();
  sql(boundsMigration);
  assert.equal(projectionDefinition(), boundedProjection, 'projection replay changed definition');
  assert.equal(projectionGrants(), originalGrants, 'projection migration changed grants');
  assert.equal(businessSnapshot(), before, 'projection migration changed business rows');
  sql(read('supabase/tests/praxis_projection_aggregate_bounds.sql'));

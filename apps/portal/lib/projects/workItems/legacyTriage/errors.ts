...
type LegacyTriageError = {
  status: number;
  code: string;
  message: string;
};

export function legacyTriageDatabaseError(error: unknown): LegacyTriageError {
  const raw = error as { code?: unknown; message?: unknown };
  const databaseCode = typeof raw?.code === 'string' ? raw.code : '';
  const message =
    typeof raw?.message === 'string' ? raw.message : 'Project work review failed';
  ...
  if (
    ...
    || /already retracted|stale_project|already uses a governed work model/i.test(message)
  ) {

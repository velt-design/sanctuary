...
`npm run test:portal:performance:fixture` runs credential-free interaction gates. The workbench journey measures object selection and Plan-to-3D feedback against `/qa/design-workbench-fixture`. The project-mutation route mounts both the production Projects-index controller and Project Details local-first controller at `/qa/projects-index-mutation-fixture`. It intercepts sample-project requests and proves the visible update/Done feedback completes within 100 ms while a deliberately 750 ms persistence response continues in the background. Paired rejection checks prove index rollback/error visibility and Project Details rollback with a retained reviewable draft. The route binds only a synthetic fixture owner, clears its local-first state, and uses no durable/customer record IDs. The gates produce separate schema-v2 artifacts at `artifacts/portal-workbench-performance.json` and `artifacts/portal-project-mutation-performance.json`.

...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, project picker, browser-draft persistence/session state, module navigation, configuration-section presentation, issue labels/section ownership/model construction, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation/styles, and the infill production Results presenter now live outside `CalculatorGridClient.tsx`. The client no longer owns draft hydration, local persistence effects, module action policy, section ordering, responsive field layout, issue model construction, preview-resize effects, pricing-summary markup, price-impact ranking/styles, or Results-stage structure, but it still owns field construction, deeper save orchestration, issue focus timing, and most infill draft/modal state. | Move infill draft/state orchestration into a controller before unrelated calculator expansion; run `npm run files:changed` before handoff. |

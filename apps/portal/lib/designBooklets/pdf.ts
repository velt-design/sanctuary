...
async function createShadeOverlay(
  kind: "cover" | "image-page" | "review-edge",
): Promise<Uint8Array> {
  ...
  for (let y = 0; y < OVERLAY_HEIGHT; y += 1) {
    ...
    for (let x = 0; x < OVERLAY_WIDTH; x += 1) {
      ...
      if (kind === "cover") {
        ...
        const bottom =
          bottomDistance <= 0.45 ? 0.28 * (1 - bottomDistance / 0.45) : 0;
        opacity = bottom;
      } else if (kind === "image-page") {
...
async function embedShadeOverlays(pdf: PDFDocument) {
  const [coverBytes, imagePageBytes, reviewEdgeBytes] = await Promise.all([
    createShadeOverlay("cover"),
    createShadeOverlay("image-page"),
    createShadeOverlay("review-edge"),
  ]);
  return {
    cover: await pdf.embedPng(coverBytes),
...
function renderCover(
  ...
  page.drawImage(context.overlays.cover, {
    x: 0,
    y: 0,
    width: PAGE_WIDTH,
    height: PAGE_HEIGHT,

...
| ---------- | -------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ---------------------------------------------------------- |
| 2026-07-19 | Portal Workflow Bundles          | Promoted | Keep the default Activity workflow initial, isolate responsive Details and other project workflows, and load the Three-based workbench viewport only from exact 3D Review intent while accounting for it in the workbench bundle gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 2026-07-19 | Portal Project Opening           | Promoted | Open from the authenticated user's cached project/contact summary, refresh through the canonical snapshot query, hide stale data on access-ending responses, and intent-preload tab code/data without automatic row fan-out.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 2026-07-18 | Calculator Commercial UI         | Active   | Keep internal true cost distinct, derive the non-persisted pergola customer-price preview through the shared quote-pricing helper, keep blinds separate, and expose Preserve versus Reprice before quote handoff.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 2026-07-18 | Infill Configurator UX           | Promoted | Keep requested automatic choices separate from resolved costing inputs, treat planned supports as results rather than warnings, and route actionable blockers to the guided stage that owns the affected detail.                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 2026-07-19 | Infill Support Confirmation      | Promoted | New structural-edge answers default to conservative Unsure, legacy saved booleans infer Yes/No unchanged, and only resolved booleans cross into the costing input contract.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
Status: Promoted
Decision or mistake: The default Activity tab was deferred while the responsive Details sidebar and the Three-based workbench viewport were imported before they were necessarily visible. Activity is now initial, Details is shared through one responsive lazy boundary, and the 3D viewport loads only for exact `3D Review` intent. Plan and Sheet keep their existing immediate paths.
Why it mattered: Deferring the default workflow added a needless request, while eagerly carrying Three/React Three Fiber made ordinary Project Detail users download a large 3D renderer even when they never opened the workbench. Moving the renderer without accounting for it would only hide bytes from one route's report.
Current guardrail: Keep default lightweight content initial and preload only the exact workflow the user signals. A deferred workflow must show a truthful local loading state. The Project Detail and Design Workbench gates must be evaluated together: deferred 3D code belongs to the workbench budget, and neither route's preserved combined allowance may increase automatically. This is a loading/import boundary only; it does not change workbench props, solved geometry, or costing authority.
...
Status: Promoted
Decision or mistake: Opening a project repeated the full snapshot read at the server route boundary even when the browser had already prefetched the authenticated snapshot. Project routes now render without that duplicate read, show a current-user list/contact-cache summary immediately, and replace it through the existing snapshot query in the background. Project tabs are intent-preloaded workflow code/data boundaries instead of one initial bundle. The Projects list also used a legacy compatibility URL and loaded independent project/contact datasets serially.
Why it mattered: The duplicate read made a routine project click feel like a 6-8 second page load, while eager first-three-project loading spent bandwidth without user intent. Placeholder activity/email arrays could also look like truthful empty states.
Current guardrail: Keep the summary derived only from the authenticated user's QueryClient and canonical project/contact query keys. A `401`, `403`, or `404` must hide cached project data; network/server failure may retain it with Retry. Activity and Emails must say they are updating until the fresh snapshot arrives. Internal project navigation uses `/staff/projects`; `/projects` remains compatibility-only. Independent list reads start together. Browser Back clears the instant view and relies on Next's retained route state; never cache a server-rendered React node. The lightweight shell is part of the Projects list's normal bundle; only workflow tabs are lazy boundaries, because making the shell itself intent-lazy makes useful content depend on hover duration. Do not restore server-side snapshot blocking or automatic row fan-out.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/testing-and-qa.md`

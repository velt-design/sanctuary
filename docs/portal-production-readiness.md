...
| Browser smoke | Yellow | The last executed calculator signal remains the 8/8 authenticated pass from 2026-07-18. The suite now also targets a valid three-module/two-pergola V2 scenario, local edit retention across module switches, explicit browser-draft status, and reload restoration at the established 1600px, 1366px, 1024px, and 768px widths. The post-change run is blocked in this checkout because `PORTAL_TEST_EMAIL` and `PORTAL_TEST_PASSWORD` are unavailable; no scenario or user was provisioned implicitly. | Supply the explicit local/staging fixture credentials, run `npm run portal:calculator-ui:provision`, and retain the shared browser evidence before treating the new coverage as executed. |
...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, project picker, browser-draft persistence, and draft/session state now live outside `CalculatorGridClient.tsx`. The client no longer owns draft hydration or persistence effects, but it still owns the main form schema, deeper save orchestration, and most infill modal state. | Continue with the module navigator and configuration-section definitions before adding more inline calculator workflow; run `npm run files:changed` before handoff. |
...
| Local-first flows | Yellow | Existing store/queue alias, retry, conflict, estimate-lock, and portal-mutation coverage remains the durable server-sync boundary. Calculator-specific draft persistence now has focused coverage for working-copy/session precedence, every local status, active-module restoration, failed writes, write races, and draft-key isolation; it does not enqueue estimate mutations. | Keep manual calculator reload restoration plus pending, failed, retry, conflict, and lock-state browser QA open until valid staff fixture credentials are available. |

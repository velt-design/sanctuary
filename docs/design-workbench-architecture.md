...
- House-form Plan overlays and status resolve by `houseFormId`. Rail rows, inspector state, selection chrome, hit targets, dimensions, and visible body precedence must not borrow the first/legacy house form; when a house form has roof-material projection bodies, those are the preferred visible roof bodies over same-form raw roof solids.

...
export type RepresentativeRoofBattens={species?:'cedar'|'thermopine';profile:'39x39'|'65x39'|'90x39';edge:boolean;gap:number;customGap:boolean};
...
export function addRepresentativeRoofBattens(assembly:Assembly3D,battens:RepresentativeRoofBattens,covering?:RoofFinishGeometry):RoofFinishGeometry{
  ...
  for(const [index,roof] of assembly.roofPlanes.entries()){
    ...
    const mesh=roofMesh('roof-battens-'+index,'cedar');mesh.timberSpecies=battens.species??'cedar';mesh.grainAlong=f.u;mesh.grainAcross=f.v;

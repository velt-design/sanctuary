...
select
  ...
  item.project_id,
  item.quote_version_id as parent_id,
  greatest(item.created_at, coalesce(item.cancelled_at, item.created_at)) as recorded_at,
  praxis_reporting.version_v1(payload.value) as record_version,
  payload.value as payload
...
cross join lateral (
  select greatest(
    ...
    coalesce((select max(greatest(item.created_at, coalesce(item.cancelled_at, item.created_at))) from public.project_invoice_plan_items item where item.project_id = project.id), project.updated_at),

...
export type RepresentativeBoxOptions = {freestanding?:boolean;widthMm:number;projectionMm:number;connection:'facade'|'soffit'};

/** Isolated level-box concept; no authored solve, quantity takeoff or price claim. */
export function buildRepresentativeBox({widthMm:width,projectionMm:projection,connection,freestanding=false}:RepresentativeBoxOptions) {
  ...
  const assembly:Assembly3D={family:'box',outline,attachmentEdge:freestanding?null:{start:outline[0],end:outline[1]},house:{},
    ...
    semantics:{connectionType:freestanding?'freestanding':connection==='facade'?'wall':'soffit',roofType:'box',structuralZones:['level_box','inset_roof']}};
  assembly.members.push(
    boxMember('box-rear',freestanding?'beam':'ledger',{x:0,y:25,z},{x:width,y:25,z},'300x50'),
  ...
  if(freestanding)for(let i=0;i<rules.postCount;i++) {
    const x=rules.postSize/2+(width-rules.postSize)*i/(rules.postCount-1),y=rules.postSize/2;
    assembly.members.push(boxMember(`box-rear-post-${i}`,'post',{x,y,z:0},{x,y,z:rules.bottomZ},`${rules.postSize}x${rules.postSize}`,{x:0,y:1,z:0}));
  }

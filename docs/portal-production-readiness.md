...
| Browser smoke | Yellow | The fixture-safe workbench performance journey passed, and drawing smoke passed with 5 tests plus 1 intentional scenario skip after aligning its diagnostic assertion with the established `geometry-canvas` contract. On 2026-07-19 the authenticated calculator trust suite passed all 10 checks against the dedicated revisioned three-module/two-pergola V2 scenario at 1600px, 1366px, 1024px, and 768px, including untouched 480px/440px preview defaults, customer-price parity, preview order with Module views before True cost change, quiet helper-free configuration sheets with retained errors, aligned controls, narrow Save access, stale-price labelling, sticky-header issue-focus clearance, module switching, draft restoration, and quiet empty add-ons. The 5-test infill lane also covers exact piece/purchase/CSV checks, production-first tablet ordering, unmanufacturable stock routing, and invalid partial-edge rafter routing using browser-draft-only mutations. | Keep the canonical calculator, authenticated smoke, and authenticated performance gates on compatible provisioned scenarios. |

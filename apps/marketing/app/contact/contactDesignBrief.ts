...
export function buildContactDesignBrief({ input, roof, result }: PreviewSelection): ContactDesignBrief {
  const label = roof.family === 'mono' ? 'Pitched acrylic pergola' : roof.family === 'gable' ? 'Gable acrylic pergola' : 'Box perimeter acrylic pergola';
  ...
  return {
    ...
    estimate: roof.family !== 'mono' ? null : {

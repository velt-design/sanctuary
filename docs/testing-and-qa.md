...
`portal:doctor:quick` runs docs guard, mojibake check, typecheck, lint, and portal Vitest. `portal:doctor` adds portal build, general route bundle budgets, drawing and fixture-performance browser smoke, authenticated smoke, route performance, the production soft-offline shell gate, and production security audit.
...
npm run test:portal:work-queue:read-only-auth
npm run test:portal:performance
npm run test:portal:offline-shell
npm run test:portal:offline-shell:build
...
`npm run test:portal:browser` includes the gated, customer-data-free Project Command Centre fixture at `/qa/project-command-centre-fixture?scenario=...&work=...&state=...`. The fixture composes the production `ProjectOverviewLayout`, `ProjectOrientationBand`, `ProjectWorkSection`, `ProjectCurrentDesignCommercialCard`, and `ProjectRecentNotesEvents` owners with synthetic data; its shell variant additionally sends a deterministic command-centre response through the production query and `OverviewTab`. It performs no authenticated data reads. Its catalogue supplies current first-email/follow-up/close-review and stage-review work, stage-aware Contacted/Site Visit/visit-complete/Quoting specialist cases, normal through blocked due states, long customer/project/site/action content, no owner/action, Waiting/Closed/Archived/correction review, strict commercial-source cases, and pending/summary/refreshing/stale/mismatch/error/retry/access-ending states.

The current fixture Playwright specs directly assert the read-only commercial, Project Work, read-state, semantic email-command, project-shell, and stage-aware journey-action contracts. They check the standard composition at 1600x1000, 1440x1000, 1280x800, 1024x900, 768x1024, and 390x844 with attached rendered evidence; long-content resilience at each width; the shared 1440px content ceiling; one-column mobile-priority order at 768px and below; one visually dominant primary action and explicit outcome-recording controls; Contacted/Site Visit/Quoting labels and canonical destinations at every standard viewport plus 640 CSS-pixel effective zoom; commercial exception-before-metrics order; one Retry for a failed command-centre read; no document horizontal overflow, nested vertical scroll owner, or cropped control; semantic headings/regions; actual mobile Tab order; visible focus; reduced motion across descendants; prohibited lifecycle/commercial controls; and 44px coarse-pointer controls. The same synthetic Overview fixture opens the dedicated Close Project dialog at every required viewport and records screenshots while asserting unselected Lost/Cancelled/Complete paths, Lost optional-note behavior, exact final CTA, consequence copy, and containment without submitting a command. Access-ending fixture states remove the complete protected Overview, while page-level component coverage proves 401/403/404 cache clearing and unavailable presentation. Catalogue entries and acceptance properties not named by those assertions remain required verification rather than completed evidence. The route requires `ENABLE_PORTAL_QA_FIXTURES=1`; the standard fixture harness supplies that flag.
...
For a protected Vercel Preview, set `PORTAL_VERCEL_PROTECTION_BYPASS` only in the test process to the existing project automation-bypass secret. Playwright sends Vercel's documented bypass and cookie headers; the value is never checked in or written to browser evidence. Keep Preview protection enabled and do not place the secret in a URL.

`npm run portal:enquiries:inactive` calls only the service-granted `project_enquiry_inactivity_report_v1()` read model. It reports active/waiting New and Contacted projects whose latest recorded project, note, email, task, staff command/event, confirmation, commercial, Site Visit, Schedule, design-package, running-job, file, or enquiry activity is more than 30 days old. It identifies future Waiting projects separately and emits an evidence fingerprint. The command never closes, archives, reassigns, or changes stage. Work Queue's admin review uses the same read model, selects none by default, and submits only an exact twice-confirmed list. `project_enquiry_bulk_close_v1()` rechecks the original fingerprint plus current activity/source and future-Waiting protection for every selected project before invoking the standard state command atomically. A changed row returns `409`, closes nothing, and requires a fresh review. Never invoke the bulk command during fixture or read-only QA; production closure requires the exact final report list to be presented for explicit approval. For an auditable review artifact, pass `--output-json=<absolute path outside the repository>`; the command refuses to overwrite an existing file or write customer evidence inside the repository.
...
`npm run test:portal:performance` writes a schema-version-2 journey artifact. It measures cold Dashboard, Projects, Project Detail, Contacts, and Schedule; warm Dashboard to Projects/Contacts/Schedule/Work Queue/Drafting Queue/Running Jobs, Projects to project/Dashboard, browser back, and the current Overview, Calculator, Commercial Quotes/Invoices, and conditional Job Packs project tabs; and Schedule/Calculator interactions. Journeys with an explicit final data-ready marker also record `liveDataReadyMs` separately from useful shell and generic network settlement; the five-run summary reports its p50/p75/p95 and enforces 3,500 ms Projects Index and 2,500 ms Work Queue p75 ceilings. The cold Project Detail journey discovers a real project in a separate authenticated context, then opens the canonical detail URL in a new context with no project-list cache so PROJECT-01 has a truthful cold-read signal.

Useful content for a routine route is its route-specific final frame, not a heading, generic skeleton, progress overlay, or intermediate loading screen. Dashboard must expose its hero plus all six named regions; Projects must expose its actions, complete filter set, table, and real column structure; each other measured route has an equivalent owned-frame assertion. Pending values may remain inline inside that frame. A browser-side observer fails the journey if the accepted frame disappears while live values settle. The warm suite also advances the browser clock beyond two minutes before returning to Dashboard, so an idle revisit cannot pass only because it happened immediately after a warm read. Every journey continues to separate visible feedback, useful structure, and background-settled data, and records same-origin requests/transfer, long tasks, and blocking overlays. Each journey also records every same-origin API request's route template, query-key names, status, duration, encoded body bytes, and identifier-free `Server-Timing` names/durations. Customer IDs and query values must never enter the artifact. The Dashboard, Projects index, project summary/snapshot, and command-centre owners expose named server steps so browser, authentication, aggregate, and database-facing time can be distinguished. Project-opening background completion still requires both the fresh snapshot and active tab workflow.

Project-tab intent begins inside the measured journey, so hover/focus preloads are not invisible setup. Calculator background completion requires a current calculated result after its saved design values settle. Commercial Quotes and Invoices background completion requires their live-value pending state to clear. Feedback and useful-content budgets continue to measure the immediate stable shell separately from that data-ready time.

Portal Performance CI builds once and runs all five authenticated repetitions against `next start`; development compilation time must never be recorded as product latency. CI rejects missing journeys and publishes p50/p75/p95. Product targets stay visible separately from regression ceilings so noisy baselines cannot redefine the product goal.
...
Ordinary authenticated route changes must keep the persistent navigation visible and replace route content with the destination's real, data-free final structure immediately. Only value slots and route-owned specialist content may remain busy. Layered Blueprint, generic skeleton, and route-progress-only states are failures for the common-route performance gate. `npm run test:portal:shell` covers the shared transition owner and navigation controls; authenticated routing smoke verifies Schedule view changes never show the blocking overlay.

The offline contracts are covered by `PortalRouteTransition.test.tsx`, `portalCoreShellPreload.test.ts`, `portalStaticCache.test.ts`, `portalServiceWorkerContract.test.ts`, `PortalStaticCacheRuntime.test.tsx`, `PortalAuthProvider.test.tsx`, and `portalBrowserDataBoundary.test.ts`. Together they prove soft offline navigation while the already-open app is mounted, release-version cache validation, static-only allowlisting, and per-owner cleanup. `npm run test:portal:offline-shell` is the blocking production-build browser journey when `.next/playwright-fixture` already exists; `npm run test:portal:offline-shell:build` creates that isolated production build first. The portable runner uses the checked-in Playwright CLI on Windows and Linux. The gate requires real staff authentication and seeded project access, fails rather than skipping when those prerequisites are missing, opens a real project through the live Projects control before disconnecting, and accepts only a completed current-release CacheStorage entry containing real allowlisted assets. It does not prove an offline hard refresh or new tab; this slice deliberately has no cached HTML/document fallback, so that journey must remain out of acceptance until it receives a separate security review and browser gate.
...
The 2026-08-06 local production revalidation found a current-ref Projects-to-project useful-content p75 regression at 1,329 ms before the application change. Promoting the current combined Projects-index response into the existing project-shell cache lookup, while leaving the authenticated complete snapshot as background authority, reduced the exactly-five-run feedback/useful p50/p75/p95 to 14/14/16 ms and 18/19/20 ms. Cold Project Detail useful content was 2,651/2,663/2,682 ms, still inside the unchanged 2,699 ms guard; all five project runs had no blocking overlay or observed long task. The same pass disabled viewport-wide sidebar route prefetch and retained route/data preload on hover, focus, touch, or pointer-down. One additional attempted repetition stopped on an unrelated 10,426 ms cold Schedule shell sample against its 4,500 ms per-run guard; the replacement fifth run passed, and the failed attempt is not part of the aggregate.

The 2026-08-06 instant-shell completion repeated the five newly covered warm journeys exactly five times against one optimized local production build. Useful-content p75 moved from the pre-change Projects-to-Dashboard/Schedule/Work Queue/Drafting Queue/Running Jobs samples of 2,389/3,869/1,852/6,390/3,377 ms to 19/18/24/17/23 ms. Final feedback p75 was 17/16/21/15/20 ms. Every run met the locked 100 ms feedback and 500 ms useful-content targets with no blocking overlay or observed long task, so all five budgets are enforced rather than advisory.

The 2026-08-08 live-data baseline used the identifier-free per-request artifact against an optimized authenticated local production build. Dashboard was 10,321.5 ms server time, of which the portfolio Work Queue consumed 9,641.7 ms. Project snapshot was 5,606.1 ms with 3,442.6 ms in authoritative work projection; the parallel command-centre request was 4,601.7 ms with 2,718.0 ms in its aggregate owner. Projects Index was 1,434.2 ms with 740.6 ms in page action enrichment versus 268.4 ms in the index RPC. The first application slice moved Dashboard Work Queue to its own bounded, auth-bound memory query without changing server authority or cache policy. In the immediate production recheck, `/api/dashboard` completed in 1,103.2 ms while the queue continued inline in its existing card; the slow aggregate therefore no longer withheld pipeline, state counts, recent estimates/activity, or personal tasks. This is single-run local diagnostic evidence used to choose the slice, not a replacement for the five-run CI aggregate.

The second live-data slice made the complete project snapshot the one cold-detail bootstrap owner. It embeds the command-centre response, reuses that response for owner and Project Work, and runs selected-design detail, domain facts, and the base work projection concurrently. The authenticated production recheck completed the snapshot in 1,119.9 ms, down from 5,606.1 ms, with `command_base` 391.6 ms, `command_detail` 236.3 ms, `domain_facts` 204.9 ms, and `work_projection` 230.1 ms. The browser made no separate command-centre request; cold useful structure was visible in 1,466 ms. An immediately preceding attempt encountered an upstream connection reset before application APIs began and failed the outer shell budget, then the unchanged build passed on retry. Treat these numbers as single-run diagnostic evidence and retain the five-run production gate for regression decisions.

The corrected project-tab data-ready baseline kept Calculator/Commercial useful structure below 100 ms but exposed materially different live-data waits. Commercial Quotes settled in 964 ms and Invoices in 962 ms. Calculator took 4,021 ms: project estimate metadata took 1,102.1 ms, the selected estimate detail then took 1,523.7 ms, and the workspace issued two costing requests of 941.6 ms and 913.0 ms as default inputs were replaced by saved inputs. This is the evidence for collapsing the Calculator metadata/detail waterfall and suppressing the known-obsolete first calculation; Commercial requires no application change from this sample.

The matching Calculator slice returned active-draft detail with metadata, seeded the exact current-user detail query, delayed costing until saved inputs were hydrated, and joined the immutable published costing version to its publication pointer. The same optimized production journey then settled in 3,008 ms rather than 4,021 ms and made two live requests rather than four. The estimates response was 1,228.2 ms browser / 933.2 ms server; the one costing response was 569.9 ms browser / 566.6 ms server, with 367.8 ms auth, 188.6 ms configuration, 8.9 ms calculation, and 0.4 ms serialization. The joined configuration read was down from the preceding instrumented 374.1 ms sample to 188.6 ms. These are single-run diagnostic numbers used to validate the slice, not a replacement for the five-run production aggregate.

The Work Queue slice replaced twelve sequential 100-ID enrichment chunks with concurrent bounded portfolio inventories while retaining the strict completeness RPC and fail-closed 5,000-row ceilings. The authenticated endpoint fell from 5,853.6 ms browser / 5,851 ms server to 1,215 ms browser / 1,212.7 ms server; final server composition was 819.6 ms and the private response remained about 609 KiB. Explicit navigation intent now overlaps the current-user memory-only query with route loading, and hydration preserves the exact pending frame until the client can hand off without a blank-frame flash. In the optimized production journey, shell feedback/useful structure was 56/66 ms and real Work Queue rows were visible 1,947 ms after measured navigation intent, down from the earlier 6,957 ms generic settlement baseline. The remaining gap above the ~1.2-second endpoint is route commit/hydration plus rendering of the first 100 rows, not a database query waterfall. These are single-run diagnostic numbers; the five-run gate owns the new 2,500 ms p75 live-data ceiling.

The Projects Index slice corrected its journey so hover-triggered data preload is inside the measured window, then removed a forced on-mount refetch that could issue the same private request twice. The page-scoped action enrichment now reuses the active-state proof already returned by `staff_projects_index_v3`, validates that it is a subset of the requested page, retains the separate completeness proof, and starts project, confirmation, durable-queue, and count reads together. In the comparable production sample, `work_queue` fell from 728.1 ms to 329.9 ms and the removed `queue_inventory`/`queue_states` sequence no longer appeared. Shell feedback/useful structure remained 65/107 ms. Live rows settled at 2,999 ms in that sample because the request's live access check alone took 1,606 ms; a later sample was 3,012 ms with slower database-facing reads. These are variable single-run diagnostics, not a claim that remote authority is always sub-1.5 seconds; the five-run gate owns the conservative 3,500 ms p75 regression ceiling and will catch the former duplicate-request path.

The final combined 2026-08-08 optimized production audit passed all six authenticated journeys, including the two-minute idle Dashboard revisit, on one isolated build. Warm feedback/useful/live-data timing was Projects 58/97/1,971 ms, Work Queue 57/74/1,955 ms, Calculator 90/102/1,974 ms, Commercial Quotes 72/85/967 ms, Commercial Invoices 63/71/940 ms, and idle Dashboard 18/45/2,906 ms. The idle Dashboard's core API completed in 1,251.5 ms while its intentionally separate queue completed in 1,478.4 ms; neither blocked the stable page structure. Projects made one 931.6 ms API call, including 405.5 ms auth, 238.5 ms index RPC, and 284.8 ms concurrent action enrichment. The remaining common per-request floor is live access verification plus remote database variance. A production `/login` response entered Vercel at `syd1` but its `x-vercel-id` recorded function execution at the default `iad1`; local Supabase traffic terminated at the Melbourne Cloudflare edge, which is not proof of the database's primary region. Confirm the Supabase project region in its dashboard before changing Vercel geography, then benchmark the same authenticated suite before and after any alignment.
...
`npm run test:portal:project-work:production-read-only-auth` requires the exact production portal hostname and Supabase project ref, blocks every non-GET/HEAD/OPTIONS application request after authentication, suppresses portal web-vitals writes, and covers Projects/Dashboard state, bounded Work Queue pagination, and one real Overview. The manually dispatched `main` Portal Quality step runs only when separate `PORTAL_PRODUCTION_TEST_EMAIL` and `PORTAL_PRODUCTION_TEST_PASSWORD` secrets are provisioned; it never reuses the normal staging identity. The production release proof used a short-lived labelled QA login that was deleted immediately after the run. Routine PR commands remain staging-only and production-refusing.
...
At the Overview V2 handoff on 2026-07-30, a clean current build and an isolated clean `060bea19` build with the same analyser both exceeded only the Contacts and Calculator initial ceilings; Project Detail remained within its unchanged allowance, and those aggregate baseline failures were not authority to raise unrelated budgets. A later isolated `.next-route-optimization` production build closed the Contacts and Calculator debt without changing any ceiling. Contacts measured 550.8/154.6 KiB raw/gzip initial against 588/168 KiB limits, with 8.8/3.4 KiB lazy. Calculator measured 1158.5/302.4 KiB initial against 1182/312 KiB limits, with no lazy group. The independent Turbopack analysis confirmed the shared `PortalFloatingPanel` owner on both routes, no `@radix-ui/react-dropdown-menu`, and only the two material-catalogue JSON sources—not the full costing manifest, rules, overhead, hardware, or install-action inputs—in Calculator's route graph. The aggregate assertion then passed every monitored route.
...
### Design Booklet Fixture

The credential-free Design Booklet Workbench fixture is `/qa/design-booklet-workbench-fixture`. It requires `ENABLE_PORTAL_QA_FIXTURES=1` and mounts the production page composer and browser preview with Toni's bundled assets. Its gated QA POST route uses the production draft parser, image validation, shared render model, and PDF renderer. It does not authenticate, read or write the database, persist a draft, upload to shared storage, or trigger customer/workflow side effects.

Run the focused unit, component, route, and browser checks:
...
- Portal Performance Report runs five authenticated journey repetitions and the production soft-offline shell journey as separate blocking gates, rejects missing schema-v2 journeys, publishes p50/p75/p95, and uploads the `portal-performance-baseline` artifacts. It also writes the authenticated runtime prerequisites to the GitHub step summary before timing routes.

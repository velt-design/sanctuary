...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, project picker, outer workspace shell, preview/configuration presentation, browser-draft persistence/session state, module navigation, configuration-section presentation, structure/site/workflow field-schema generation, issue labels/section ownership/model construction/focus navigation, infill draft/selection/modal/delete-undo state, infill item mutations/stage policy, infill clipboard/hotkeys/warning focus/telemetry, infill baseline/variant cost coordination, infill dialog/stage/rail/empty-state presentation, save confirmation/persistence/outcome orchestration, blinds/flashings custom editor markup, materials-debug request/lifecycle/export behavior, warnings/BOM/debug/labour/structure preview presentation, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation/styles, and the infill production Results presenter now live outside `CalculatorGridClient.tsx`. The client no longer owns draft hydration, local persistence effects, module action policy, section ordering, responsive field layout, field option/schema construction, issue model/focus timing, infill draft cleanup, selection reconciliation, modal flags, delete-undo expiry, infill item mutation policy, infill stage-selection policy, infill clipboard/hotkeys/warning-focus/telemetry effects, infill option-cost effects, outer workspace rendering, save dialog presentation, blinds/flashings rendering, materials-debug effects, preview-detail cards, preview-resize effects, pricing-summary markup, price-impact ranking/styles, or Results-stage structure, but it still owns estimate/project coordination, house-footprint interactions, validation construction, and custom input mutation wiring. | Move house-footprint editor gesture/lifecycle state into a focused controller, then extract calculator validation construction and custom blinds/flashings mutation controllers; run `npm run files:changed` before handoff. |

...
export const INVOICE_PDF_FONT_FILES = {
  regular: "Inter-Regular.ttf",
  medium: "Inter-Medium.ttf",
  semibold: "Inter-SemiBold.ttf",
} as const;

export const INVOICE_PDF_LOGO_FILE = "sp_dark_icon.png";
...
function assetFilePath(assetUrl: URL): string {
  ...
  if (!pathname.startsWith("/_next/static/media/")) {
    throw new Error(`Unsupported invoice PDF asset URL ${href}`);
  }
...
export async function readInvoicePdfFont(
  filename: string,
): Promise<Uint8Array> {
...
export async function readInvoicePdfImage(
  filename: string,

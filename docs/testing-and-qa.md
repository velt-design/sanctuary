...
On 2026-07-30, a clean current build and an isolated clean `060bea19` build with the same analyser both exceeded only the Contacts and Calculator initial ceilings; Project Detail remained within its unchanged allowance. The user accepted those two pre-existing overruns as a narrow exception for the completed Overview V2 handoff only. Their limits remain unchanged, the aggregate gate remains truthful, and a separate route-optimization review owns the debt.

...
export default function LightingProvider({input,roof,onChange,children}:{input:SimpleCoverInput;roof:PreviewRoofChoices;onChange:(r:PreviewRoofChoices)=>void;children:ReactNode}){
 ...
 const sites=useMemo(()=>{const g=solvePergolaPreview(input,roof).geometry;if(!g)return {strips:[],rafters:[],cedar:[],allRafters:[]};const sites=pergolaLightSites(g.assembly,g.covering);return {...sites,allRafters:sites.rafters,rafters:availableRafterSpots(sites.rafters,roof.lighting?.strips??[])};},[input,roof]);
 const rafterUnavailableReason=roof.roofBattens
  ? 'No batten gaps are wide enough for the 40 mm rafter lights. Increase the clear gap to at least 41 mm in Roof & ceiling.'
  : roof.lighting?.strips.length

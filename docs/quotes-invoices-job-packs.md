...
The invoice expansion is gated by `INVOICE_DRAFTS_ENABLED=true` on the portal server. Deploy compatible portal and public readers first, apply the forward invoice migrations `20260911000002` through `20260911000008`, then enable creation after staging verification. Disabling the flag stops draft commands through the application; issued invoices remain readable, payable, previewable and sendable. It does not remove administrator database command privileges. Do not roll back the storage expansion or rewrite issued records.

Admins save, edit, delete and issue drafts from the project Invoices tab. Authenticated direct table writes are revoked; commands recheck the admin role, and a restrictive read policy hides drafts from staff while retaining issued reads. Drafts live in `deposit_invoices`, have no number/token/artifact and reserve no balance. Save uses an expected revision; issue uses a stable UUID command, assigns the number in the transaction and checks current quote/scope availability again. A send failure after issue returns an issued result with a separate delivery error: retry sending from history, never create a replacement for a transport failure.
...
Rollout evidence (2026-09-11): all eight migrations (20260911000001 through 20260911000008) were rollback-rehearsed and applied to staging and production under explicit owner approval. Each ledger stores the exact canonical-LF reviewed body and all eight MD5 values were independently matched after application. Production backup 1639271131 completed at 02:13 UTC. The guarded production transaction preserved project/quote/invoice/payment counts (1,282 / 219 / 51 / 18) and compared every pre-existing invoice field before commit; no historical invoice content or artifact was rewritten. Direct authenticated invoice writes are denied and the restrictive draft-read policy is present.

Authenticated staging browser verification created one labelled synthetic project, recorded unscheduled delivery completion, saved and reopened a standalone draft after refresh, issued INV-0006 for $10, verified separate standalone value, then voided it with an audit reason and zero remaining exposure. No email was sent. The rollback SQL also verified issue retries, payment, settled closure, reversal and repayment. The retained staging record is project 221504f5-f2ac-45e5-989f-da3449247ec5, with void invoice 974b35b9-3802-495d-9925-41d411055d89. Preview deployment CHj3RUfkF5YBmdBTHqU1k78jjxoD is ready with INVOICE_DRAFTS_ENABLED=true. Production deployment 8U7jxaoVFfBYSMnHstcFXPb3i8X8 is ready on merged main caf2d04 with INVOICE_DRAFTS_ENABLED=true for portal.sanctuarypergolas.co.nz. Production postflight found 51 unchanged legacy invoices, 18 payments, the issue command present, no direct staff invoice mutation grant and the restrictive draft-read policy. The existing owner browser required sign-in during the final production screen check; authenticated production UI verification remains pending that sign-in. No production QA invoice, payment or completion record was created.
...
The project header exposes one Commercial tab. Its accessible inner switch keeps Estimates, Quotes, and Invoices as separate lazy owners and preserves the existing `tab=estimates`, `tab=quotes`, and `tab=invoices` URLs. `QuotesTab.tsx` is the quote composition owner: query/selection, lifecycle actions, PDF-preview effects, and presentation now live behind the named owners above, while draft editing plus local-first create/persist and send-form orchestration remain in the tab. `CommercialTab.tsx` owns only composition, Edit/Preview URL state, and navigation. Opening a commercial subview is list-first: switching Estimates/Quotes/Invoices clears record detail/preview query state, while an explicit `quoteId` or estimate intent deep link may open a record. Quote selection uses a pending URL-intent barrier: one Back-to-list action remains visible while the old query string is still committing, then the genuinely new URL becomes authoritative for Back/Forward. Create-from-estimate and unrelated query context remain intact. `quotes.internal_name` is optional staff-only metadata owned by the quote family, so every version inherits one searchable name without rewriting version rows; the quote reference/version remains visible as the stable secondary identifier. It may seed from the source estimate on first creation and can be renamed independently from draft locks. It is excluded from PDFs, emails, public token responses, invoices, and job packs. Email audit data and quote/invoice delivery side effects remain available through their domain records and APIs even though the standalone project Emails tab is retired.

The base contract and each add-on are independent commercial families within one project. `quotes.commercial_scope_id = null` identifies the original family; an estimate-led add-on uses the stable UUID from its source estimate family, while a standalone manual add-on owns a deterministic UUID derived from its create intent. Each non-null scope owns a separate quote reference/version chain. Creating or revising an add-on quote never supersedes the base quote, and manual supersession remains scoped to the selected version. A manual quote may establish the base family before that contract is accepted; once the base family has entered the accepted lifecycle, each newly created manual quote receives its own add-on scope so a variation cannot replace the accepted contract value. Accepted project value is the sum of the current accepted version in each family. Current means the newest version that entered the accepted lifecycle is still `ACCEPTED`; declining or superseding that accepted version is a terminal tombstone and never revives an older acceptance. This is aggregation for job reporting, not a merge of immutable quote artifacts. The full figure/source matrix is in `docs/commercial-truth-audit.md`.
...
- `apps/portal/lib/quotes/pricing.ts` re-exports the shared `@sp/costing` customer-price sequence used by quote mapping and calculator pergola/site lines: `1.25x`, frozen policy uplift, discount, rounded ex GST, then GST. Version 5 Simple estimates freeze a 21% uplift in `pricing_policy`, exactly 10% above Version 4's result; Version 6 retains that uplift while longer-rafter labour changes the frozen true-cost basis. Historical outputs keep their frozen costing and uplift or default to zero uplift, so quote handoff never infers either from current settings. The calculator preview sums those exact line cents with authoritative approval, blind, and structured per-pergola lighting lines, then derives ex GST using the quote totals helper. `quoteDiscountPct` applies to pergola and shared-site sell lines only; approval, blind, and lighting lines remain at their direct customer price. Structured lighting is repriced through the shared package owner and creates one line per pergola; invalid or non-acrylic lighting blocks handoff rather than creating a zero-dollar line. Historical snapshots without structured lighting retain their frozen opaque lighting total as one compatibility line, and structured inputs always take precedence to prevent double charging. Engineering and full building consent come from the saved estimate's package-owned `customer_add_ons.approval`, receive GST only, and are never recalculated during quote handoff. Module infills stay included in their pergola line rather than becoming additive quote items. Existing-pergola infills on a pergola-free add-on estimate use their saved standalone costing output and create a separate quote line without inventing a pergola. The Calculator may explain a module-infill line as the exact no-infill base customer price plus reconciled incremental infill additions; this does not alter saved costing, quote mapping, quote line count, or totals.
- Automatically generated customer descriptions lead with verified included scope, then express saved selections in customer language. Pergola lines state that the custom design is supplied and installed, translate structural fields into roof form, overall size, roof covering, frame finish, support posts, home connection, and post foundations/fixings, and keep configured infills visible within their owning priced line. Shared site lines name installation setup, project travel, and project-specific allowances only when those saved components exist; when shared costs are folded into a single pergola price, the same inclusion is stated inside that pergola description. Blind, lighting, existing-pergola infill, supply-only aluminium, engineering, and consent lines state their verified inclusion, exact saved selections where available, and any material scope boundary; compatibility fallbacks remain customer-safe and never expose legacy implementation notes or internal discount rules. The structured pergola editor accepts both the legacy labels and this current wording, preserves included infills and discount copy, and rebuilds the current customer format. These editorial changes never alter line count, quantities, cents, discount scope, source metadata, or historical sent artifacts; only new or explicitly refreshed editable drafts receive regenerated wording.
...
- Administrators may create a manual draft quote without an estimate by entering at least one priced line item. Its quote version has `pricing_source = 'manual'`, a null source estimate, explicit staff/audit metadata, and normal draft/send/accept/payment-schedule/invoice behavior. It cannot refresh from an estimate or generate an estimate-backed job pack; revisions preserve its manual provenance. Non-admin staff remain estimate-backed.
...
- Declining a sent quote marks it declined. Declining an accepted quote also voids only that exact version's open invoices; paid invoices remain historical payment evidence. Decline and supersede share the project invoice lock and commit status, voiding and audit evidence atomically.
...
Estimate-backed quote versions copy only compact pricing source metadata from the source estimate when line items are created, refreshed, or revised. The copied record identifies the source (`calculator_live` or `workbench_solved`), compact metadata/hash values, source estimate ID, copy time, actor, and copy reason. Admin-created manual quotes instead carry `manual` provenance and no estimate ID. Existing historical quote versions are not backfilled. Raw `commercial_design_input` stays out of quote versions, public-token responses, PDFs, invoice payloads, emails, and job-pack outputs unless a later explicit output task changes that boundary.
...
- All staff can read the aggregate accepted job total, actual paid, whole open-invoice value and remaining amount. Only administrators receive payment-entry detail and controls. Current schedule rows retain quote family/reference identity and show allocated paid value; the complete invoice history labels only versions outside every current accepted family as historical.
- Admin invoice creation is atomic and serialised against job payment and quote terminal-state changes. When multiple accepted families exist, the modal first selects the base or add-on quote; next stage, full remaining, custom fixed or percentage-of-remaining, and a two-to-ten-way split are capped at the lower of that selected scope's balance and the project-wide remaining balance. Due date, label/reference, save-only or create-and-send, and the resolved before/after balance are explicit. Amounts above the authoritative remaining job balance fail unless the administrator supplies an override reason, which is stored on the invoice and in the transaction audit event.
- Admin-only routes record payments/adjustments, replace allocations, reverse ledger entries, mark a whole open invoice paid, or void a whole open invoice with a required reason. Ledger and allocation audit triggers preserve evidence even if application-level audit insertion is unavailable. Voiding preserves the invoice number and audit evidence, clears public-token access, leaves accepted-job state unchanged, and makes that scheduled term eligible to invoice again. Paid invoices cannot be voided or partially paid; reversing the ledger entry created by Mark paid reopens that whole invoice transactionally. Incorrect issued invoices are voided and recreated, never silently edited. Staff retain read/preview/send access.
...
npx vitest run apps/portal/lib/invoices/paymentScheduleProjection.test.ts apps/portal/app/api/admin/projects/[projectId]/payments apps/portal/app/api/staff/v1/projects/[projectId]/invoice-schedule apps/portal/components/projects/ProjectPage/tabs/CreateInvoiceDialog.test.tsx apps/portal/components/projects/ProjectPage/tabs/InvoicesTab.test.tsx test/admin-payment-reconciliation-migration.test.ts
npm run test:portal -- apps/portal/lib/jobPacks
npm run test:portal -- apps/portal/app/api/quotes
npm run test:marketing
```

`apps/portal/lib/quotes/quoteArtifactVisualFixtures.test.ts` owns deterministic, non-persistent simple, discount, multi-page, long-description, long-terms, value-led multi-item scope, HTML-email, and plain-text-email fixtures. Set `QUOTE_ARTIFACT_OUTPUT_DIR` to write review artifacts, render every generated PDF page for inspection, and preview the email at desktop and narrow widths. The marketing public-quote page test can write a static presentation fixture with `QUOTE_PUBLIC_FIXTURE_PATH`. These fixture paths do not send email, write quotes, or require production data.

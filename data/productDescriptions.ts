export const productDescriptions: Record<string, string> = {
  // Pergolas
  pitched: `A pitched pergola extends your roofline with a clean, single‑slope canopy that sheds water reliably and keeps headroom where you need it. Built in powder‑coated aluminium, the structure stays light, straight and corrosion‑resistant with spans engineered to your wind zone. Rafters run from a higher wall plate to a lower front beam and roof sheets lock into aluminium joiners that sit on long‑run butterfly rubber running the length of each rafter for a tidy underside.

How it works: posts set on footings or a deck subframe support beams that bolt to precision brackets. A head flashing seals to the house while allowing movement. Services, lighting and blinds can be added later without re‑framing.

Why it’s good: the simple geometry suits modern homes, fits neatly under narrow eaves and moves rain off seating. With no exposed timber there’s nothing to rot or twist—rinse it down and enjoy low‑maintenance shade and shelter.

Structure & materials
• Powder‑coated aluminium beams, rafters and posts
• Roof sheets held in aluminium joiners sitting on long‑run butterfly rubber along each rafter for a clean soffit
• Engineered brackets and fixings suited to wind zone
• Integrated head/house flashing with movement allowance

Options
• Clear, opal or soft‑tint acrylic/polycarbonate roofing
• Post layouts to suit doors, sliders and paths
• LED downlights or perimeter strip lighting
• Side screens or drop‑down blinds for wind and privacy

Performance
• Reliable single‑direction fall for rain control
• High daylight levels; add tint for glare reduction
• Strong, straight spans with low visual bulk
• Quiet, rattle‑free roof fastening via aluminium joiners on butterfly rubber

Choose this if
• You want clean lines and efficient drainage under low eaves
• You prefer simple forms that keep views open

Consider instead if
• You want an architectural peak and ridge ventilation (see Gable)
• You need wind managed at corners and all‑round fall (see Hip)

Programme
• Lead time ~6–8 weeks • Install 2–5 days

Maintenance
• Hose down frame/roof periodically • Check gutters/flashings annually

Upgrade path
• Add blinds, screens and lighting now or later

FAQs
• Will it darken the room inside? Clear/opal sheets keep high daylight; add tint to cut glare.
• Can posts move off corners? Yes—offset layouts can preserve paths and doors.`,

  gable: `A gable pergola creates a peaked roof that feels tall, bright and airy. Two roof planes meet at a ridge, drawing warm air up and out while larger spans cover dining settings and sliders with ease. The aluminium frame uses deep beams for stiffness with discrete ridge/hip components that carry loads without visual clutter.

How it works: columns are set to centre the ridge; rafters and purlins lock into engineered brackets; roof sheets lock into aluminium joiners sitting on long‑run butterfly rubber along each rafter to manage drainage and movement. Translucent options maximise daylight; integrated lighting supports evening use.

Why it’s good: the symmetrical form adds street presence, handles gusty sites well and pairs beautifully with blinds and screens. You get real headroom, excellent airflow and a refined, long‑life finish.

Structure & materials
• Twin roof planes meeting at a ridge
• Powder‑coated aluminium frame with deep beams
• Engineered ridge and hip components
• Roof sheets held in aluminium joiners sitting on long‑run butterfly rubber along each rafter for drainage and movement

Options
• Acrylic roofing—clear, light grey, dark grey or opal across both roof planes; 99% UV block with tints to tune heat and glare
• Timber sarking roof panels for a warm, premium ceiling and strong heat‑reduction over dining and lounge zones
• Combination roof using insulated timber sarking panels with acrylic skylight strips so the ridge stays bright while seating areas stay cooler
• Gable infills at the ends—open, slatted or acrylic—to temper wind, frame views and help manage weather

Performance
• Excellent headroom and perceived volume
• Natural convection via ridge for warm‑weather comfort
• Stable in gusty conditions with good structural depth
• Daylight‑rich; glare managed with tint or blinds
• Quiet, rattle‑free fastening via aluminium joiners on butterfly rubber

Recommended for
• Larger decks and sliders needing coverage
• Homes that benefit from architectural symmetry

Not ideal for
• Tight eaves where a single slope reads cleaner
• Sites with severe height limits at the house junction

Choose this if
• You want a centrepiece roofline and maximum headroom
• You value natural airflow via the ridge on warm days

Consider instead if
• You need all‑round fall and corner wind control (see Hip)
• Height at the house is very constrained (see Pitched)

Programme
• Lead time ~6–8 weeks • Install 2–5 days

Maintenance
• Wash down frame/roof • Clear gutters and confirm downpipe flow

Upgrade path
• Add blinds/screens; consider heaters for evening comfort

Best paired with
• Drop‑down blinds at long faces • Downlights along ridge
• Strip lighting in beams for ambient wash

Indicative performance (TBC)
• Typical ridge spans ~3.6–5.4 m depending on bay spacing/wind zone
• Gable pitch and sheet type tuned to site

FAQs
• Is a gable too bright? Use opal/tint to balance light and glare.
• Can I keep views through the gable? Yes—open or clear infills maintain outlook.`,

  hip: `A hip pergola pitches down on all sides so the roof wraps the space neatly and manages wind at the corners. It integrates naturally with complex rooflines and reads like built‑in architecture. Frames are powder‑coated aluminium with concealed brackets for crisp hip lines and a clean soffit.

How it works: a central ridge and diagonal hips transfer loads to perimeter beams and posts; custom flashings step roof sheets around the hips while maintaining drainage. Posts can offset to suit doors and paths.

Why it’s good: multi‑direction fall sheds rain without splash zones, wind eddies are reduced and the silhouette looks premium without fuss. It’s strong, recyclable and made to withstand New Zealand sun and salt.

Structure & materials
• Four‑sided roof with ridge and hip beams
• Powder‑coated aluminium frame and posts
• Concealed brackets for clean lines
• Custom flashings around hips to manage water

Options
• Clear/opal/tint roof mix to balance light and heat
• Post locations to open pathways and doors
• Integrated lighting and perimeter strip
• Drop‑down blinds at prevailing wind faces

Performance
• All‑round fall controls splash and runoff
• Reduced wind eddies at corners
• Premium architectural silhouette with tidy soffit
• Strong weathering for coastal NZ environments

Recommended for
• Complex façades and corner decks
• Sites exposed to changeable wind directions

Not ideal for
• Narrow spaces where a single pitch saves height

Choose this if
• You want the most composed silhouette and balanced drainage
• You need better corner wind behaviour than a single pitch

Consider instead if
• You prefer a central ridge and taller feel (see Gable)
• You have very tight height constraints (see Pitched)

Programme
• Lead time ~6–8 weeks • Install 3–4 days

Maintenance
• Rinse frame/roof • Check gutters and hip junctions annually

Upgrade path
• Add blinds/screens and heaters for shoulder seasons

Best paired with
• Mesh blinds at windward faces • Downlights over seating
• Strip lighting around the perimeter for night definition

Indicative performance (TBC)
• Typical bay ~2.7–4.2 m depending on exposure and layout
• Hip angles and sheet selection tuned to site

FAQs
• Is a hip darker than other roofs? Daylight remains high; mix clear/opal sheets to tune brightness.
• Can it meet complex walls/flashings? Yes—custom flashings step sheets cleanly around hips.`,

  // Screens & walls
  'slat-screens': `Slat screens introduce privacy and wind tempering while keeping decks open and breathable. Panels are built on rigid aluminium frames and fixed with adjustable brackets between posts or pergola beams. Choose warm timber for natural texture or powder‑coated aluminium for a sharp, low‑maintenance look—the mounting method is the same.

How it works: slats run vertically or horizontally at a pitch that’s tuned to outlook and sun path. Mixed layouts can use denser lower portions with lighter uppers to stop draughts without darkening the space. Matching gates and service panels use the same profiles so utilities disappear into the design.

Why it’s good: you get human‑scale boundaries that soften views, hide bins and heat pumps and cut glare—without building heavy walls. Timber ages gracefully when maintained; aluminium keeps lines crisp with a quick washdown.

Structure & materials
• Aluminium perimeter frame with adjustable brackets
• Timber or aluminium slats in several profiles
• Durable exterior finishes and fixings

Options
• Vertical or horizontal orientation; mixed density layouts
• Integrated gate or removable service panel
• Colour‑matched powder coat; timber oil/stain systems

Performance
• Cuts wind and glare while retaining airflow
• Privacy control without heavy walls
• Durable, low‑maintenance aluminium option available

Recommended for
• Screening neighbours, bins and heat pumps
• Defining edges of an outdoor room

Not ideal for
• Full weather blocking—choose acrylic panels or blinds instead

Choose this if
• You want filtered privacy and softer wind without losing air movement
• You like timber texture or a fully colour‑matched aluminium solution

Consider instead if
• You need rain blocking and near‑clear views (see Acrylic panels)
• You want deployable shade/wind control (see Drop‑down blinds)

Programme
• Lead time ~2–4 weeks • Install 1 day

Maintenance
• Aluminium: periodic washdown • Timber: re‑oil/stain as required

Upgrade path
• Combine with blinds or acrylic panels for seasonal control

Best paired with
• Drop‑down blinds on the open face • Downlights for evening use

Indicative performance (TBC)
• Typical panel width ~0.9–1.8 m based on profile and exposure
• Fixing method and post spacing confirmed at design

FAQs
• Will slats rattle? Profiles and fixings are selected to avoid noise.
• Can I mix timber and aluminium? Yes—frames are compatible with both.`,

  'acrylic-infill-panels': `Acrylic infill panels add year‑round usability by blocking wind and rain while preserving views. Clear or softly tinted sheets slot into slim aluminium channels with gasket isolation so they stay quiet and allow for thermal movement. Panels can be fixed, removable or configured to slide depending on exposure.

How it works: impact‑resistant acrylic is cut to size and captured in sealed frames; discreet mullions control deflection; optional vent trims keep air moving where needed.

Why it’s good: you gain shoulder‑season comfort without the weight, cost or permanence of full glazing. Light transmission remains high, cleaning is simple and the result feels integrated with the pergola.

Structure & materials
• Clear or tinted acrylic sheets in aluminium frames
• Gasket isolation for quiet, stress‑free mounting
• Discreet mullions sized to exposure and spans

Options
• Fixed, removable or sliding panel configurations
• Tint choices to balance privacy and heat
• Trickle vents for air movement where required

Performance
• Significant wind and rain protection
• High light transmission; manageable glare with tint
• Lighter and more economical than full glazing

Recommended for
• Exposed faces of pergolas and decks
• View corridors where transparency matters

Not ideal for
• Full thermal insulation—consider glazing or insulated panels instead

Choose this if
• You want near‑clear views with real weather protection
• You prefer permanent, low‑maintenance enclosure over deployable blinds

Consider instead if
• You want flexible open/closed control (see Drop‑down blinds)
• You need full thermal performance (consider glazing/insulated panels)

Programme
• Lead time ~3–5 weeks • Install 1–2 days

Maintenance
• Wash with mild soap and soft cloth • Avoid harsh solvents

Upgrade path
• Pair with blinds for sun control; add heaters for winter use

Best paired with
• Heaters for winter comfort • Downlights for even light without glare

Indicative performance (TBC)
• Typical clear panel ~1.0–1.5 m wide × ~2.0–2.4 m high depending on exposure
• Mullion spacing engineered to wind zone; fixings hidden where possible

FAQs
• Will panels yellow? Quality acrylic resists UV; clean with neutral pH soap.
• Can I remove panels seasonally? Removable and sliding options are available.`,

  'drop-down-blinds': `Drop‑down blinds give on‑demand control of wind and low sun. Fabrics range from clear PVC for weather blocking to open‑weave meshes that cut glare while keeping the view. Blinds run in side channels or stainless guides to stay steady in gusts.

How it works: a manual crank or motorised tube rolls the fabric into a compact headbox; side channels capture the edges and a weighted bottom bar seals to the deck. Motors can integrate with remotes, wall switches and wind sensors to protect the blind automatically.

Why it’s good: tune comfort instantly without permanent walls. The components colour‑match the pergola for a built‑in look and fabrics are replaceable down the track.

Structure & materials
• Extruded aluminium headboxes, side channels or cable guides
• Clear PVC or mesh fabrics with welded hems
• Weighted bottom bars and sealed end caps

Options
• Manual crank or motorised operation (remote/wall switch)
• Wind sensors and group control for multiple blinds
• Fabric openness and colours to suit privacy and outlook

Performance
• Quick, adjustable protection from wind and low sun
• Stays steady in gusts with channels or guides
• Enhances warmth when paired with heaters

Recommended for
• West/North‑west faces and breezy edges
• Outdoor rooms needing flexible enclosure

Not ideal for
• Full weather sealing—use acrylic panels if permanence is required

Choose this if
• You want deployable comfort without permanent walls
• You value switching between open views and shelter

Consider instead if
• You want a fixed, glass‑like barrier (see Acrylic panels)
• You need a completely sealed envelope (consider glazing)

Programme
• Lead time ~2–4 weeks • Install ~1 day

Maintenance
• Hose down mesh • Keep PVC clean and fully dry before rolling

Upgrade path
• Integrate with lighting/heaters; add automation for convenience

Best paired with
• Heaters for shoulder‑season warmth • Downlights/strip for night use

Indicative performance (TBC)
• Max width ~3.0–5.0 m based on system and exposure
• Recommended wind limit ~X km/h with side channels (manufacturer TBC)

FAQs
• Will mesh keep rain out? Mesh reduces wind and spray; clear PVC blocks rain.
• Do motors need power on the pergola? Yes—supply by a licensed electrician.`,

  // Lighting & heating
  downlights: `Recessed outdoor downlights provide even, glare‑controlled illumination under your pergola. Low‑profile LED fittings sit flush with the frame so the ceiling reads clean; the beam is shaped to light tables and walkways without hot spots.

How it works: 24V or mains drivers mount in protected cavities; sealed luminaires and wiring are rated for exterior use; circuits can be dimmed independently and colour temperature chosen warm for relaxed dining.

Why it’s good: efficient LEDs sip power and run cool, giving years of service. Good downlighting makes the space usable every evening without lighting the neighbours and pairs well with strip lighting for layered ambience.

Structure & materials
• Low‑profile IP‑rated LED downlights
• 24V or mains drivers in protected locations
• Exterior‑rated cabling and junctions

Options
• Warm/neutral colour temperatures; high CRI where needed
• Separate dimmable circuits for zones
• Trim styles to match the frame finish

Performance
• Even, low‑glare illumination for tasks and dining
• Low power draw and long service life
• Complements strip lighting for layered scenes

Recommended for
• Pergolas used nightly and for dining
• Paths and steps needing safe, even light

Not ideal for
• Broad ambient wash alone—pair with strip for ambience

Choose this if
• You want bright, focused light on tables and prep areas
• You prefer a clean ceiling with recessed fittings

Consider instead if
• You want purely ambient glow (see LED strip)
• You prefer decorative pendants (not typical outdoors)

Programme
• Lead time ~1–2 weeks • Install with pergola or retrofit by electrician

Maintenance
• Wipe trims periodically • Expect long LED life with minimal upkeep

Upgrade path
• Add smart dimmers or scenes; pair with strip for accents

Best paired with
• LED strip for ambient layers • Sensors or scenes for convenience

Indicative performance (TBC)
• Per fitting ~300–900 lm depending on model/beam angle
• Typical spacing ~1.2–1.8 m; circuits dimmable

FAQs
• Will they attract bugs? Warm CCT and lower output reduces attraction.
• Can I add later? Yes—plan cable paths; retrofit by electrician.`,

  'led-strip-lighting': `LED strip lighting creates a soft perimeter glow that defines your outdoor room after dark. The tape recesses into slim aluminium channels or hides behind pelmets so the diodes are never in view—only a continuous wash of light.

How it works: 24V tape runs from weather‑protected drivers with smooth dimming; diffusers even out the output; colour temperature can be warm for ambience or neutral for task zones. Wiring is concealed within the frame.

Why it’s good: flattering, efficient and highly controllable. Strip lighting avoids tabletop glare, improves step safety and adds depth when layered with downlights.

Structure & materials
• Aluminium channels with diffusers; hidden pelmet options
• 24V LED tape with exterior‑rated wiring
• Remote drivers mounted in protected locations

Options
• Warm/neutral white; single‑ or multi‑zone dimming
• Corner, perimeter or step‑edge placement
• Smart control integration where desired

Performance
• Soft, uniform glow without diode spotting
• Excellent for ambience and edge definition
• Very low power consumption

Recommended for
• Evening entertaining and safe circulation
• Pairing with downlights to layer light

Not ideal for
• Task lighting on its own—combine with downlights

Choose this if
• You want soft, even ambience without visible fittings
• You prefer wide, comfortable light rather than spots

Consider instead if
• You need task light over tables (see Downlights)
• You want decorative pendants (indoor‑style)

Programme
• Lead time ~1–2 weeks • Install with pergola or retrofit

Maintenance
• Wipe diffusers occasionally • Expect long LED lifespan

Upgrade path
• Add scenes and sensors; expand to steps and paths

Best paired with
• A few downlights for tasks • Dimmers/scenes to set mood

Indicative performance (TBC)
• Output ~500–1200 lm/m depending on tape and diffuser
• IP rating and channel type matched to exposure

FAQs
• Will I see dots? Diffusers and placement hide diodes.
• Can I retrofit? Yes—drivers mount in protected cavities.`,

  'patio-heaters': `Electric patio heaters extend the season by warming people and surfaces directly rather than the air. Infrared elements mount to beams or walls and focus heat where you sit, so comfort arrives quickly on cool evenings.

How it works: quartz or ceramic emitters are angled with reflectors; zones are switched so you heat only what you use; clearances are set to manufacturer guidelines and controls can include timers or wall dimmers.

Why it’s good: no gas bottles or open flame—just clean, immediate radiant warmth. Combined with blinds or screens to tame breezes, heaters make alfresco dining practical year‑round while keeping the installation tidy and compliant.

Structure & materials
• Electric infrared heaters with adjustable brackets
• Powder‑coated housings to match frame colours
• Dedicated circuits and compliant wiring

Options
• Output levels matched to area and exposure
• Wall switch, dimmer or remote control
• Zoned layout so you heat only what you use

Performance
• Fast, targeted radiant warmth
• Efficient when paired with blinds/screens
• No open flame or LPG storage required

Recommended for
• Evening dining and lounge settings
• Shoulder‑season use under pergolas and covered decks

Not ideal for
• Fully open, windy sites without any wind management

Choose this if
• You want immediate comfort for seated areas
• You prefer clean, electric heat with no bottles

Consider instead if
• You have fully open/windy sites—add blinds first for efficacy
• You need full‑space heating (not typical outdoors)

Programme
• Lead time ~1–2 weeks • Install by licensed electrician

Maintenance
• Wipe lenses/grilles periodically • Check operation at season start

Upgrade path
• Add sensors/timers; integrate with lighting scenes

Best paired with
• Drop‑down blinds to hold warmth • Dimmers for comfort control

Indicative performance (TBC)
• Output ~1800–3200 W per unit; coverage ~6–12 m² in low wind
• Mounting height and angle tuned to seating layout

FAQs
• Will they scorch surfaces? Clearances and angles are set to spec for safety.
• Do they run on standard power? Some models require dedicated circuits—confirm at design.`,

  // Back‑compat (legacy keys that may still be linked)
  'timber-slat-screens': `See Slat screens: available in durable exterior‑finished timber for natural warmth and privacy with adjustable spacing.`,
  'aluminium-slat-screens': `See Slat screens: powder‑coated aluminium option for crisp lines and very low maintenance.`,
  'box-perimeter': `Box Perimeter frames your outdoor room with a defined edge. A 300×50 mm aluminium perimeter sets the line, while 150×150 mm posts anchor the corners so the form reads deliberate and architectural. The profile feels contemporary and pairs well with modern façades. Acrylic roofing sits within the frame to keep the space bright and dry—the louvre‑like look without motors or moving parts.

Structure & materials
• 300×50 mm aluminium perimeter frame
• 150×150 mm aluminium posts
• Acrylic roof set into the frame
• Integrated gutter and downpipe
• Durable powder‑coated finishes

Options
• Clear or softly tinted acrylic
• Concealed fixings where feasible
• Downlights within the perimeter
• Roller blinds for wind and privacy

Performance
• High daylight with better glare control than a basic canopy
• Strong street presence and tidy junctions
• Similar rain noise to standard acrylic roofs
• Moderate heat control—improved further with tint and blinds

Recommended for
• Street‑visible terraces and contemporary homes
• Clients who want a louvre aesthetic without motors or extra upkeep

Not ideal for
• Sites that need acoustic damping or insulated panels
• Heritage façades that call for minimal visual change

Choose this if
• You want a crisp, framed edge that reads architectural
• You prefer low‑maintenance looks over motorised systems

Consider instead if
• You need acoustic/thermal panels (consider insulated options)
• You prefer a traditional eave or ridge line (see Pitched/Gable)

Programme
• Lead time ~6 weeks • Install 2–4 days

Maintenance
• Rinse the frame/roof occasionally • Clear gutters and confirm downpipe flow

Upgrade path
• Add lighting and blinds now or later
• Step up to an Architectural spec for lined ceiling and insulated panels

Best paired with
• LED strip within the perimeter • Discrete downlights over tables
• Mesh blinds at exposed faces for comfort

Indicative performance (TBC)
• Typical bay ~2.7–4.0 m depending on exposure and sheet spec
• Post foundations and fixings engineered to wind zone

FAQs
• Will the frame look heavy? The 300×50 profile reads crisp; colours can reduce contrast.
• Can gutters hide in the frame? Yes—gutter and downpipe integrate within the perimeter.`,
};

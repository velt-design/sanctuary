...
| File decomposition | Yellow | Calculator input normalization and save readiness already lived outside `CalculatorGridClient.tsx`; the calculator trust slice adds dedicated result-freshness, command-bar, and project-picker owners instead of adding those responsibilities inline. Broader strict enforcement remains advisory while the client still owns draft/session orchestration and the main form schema. | Continue with draft/session orchestration as the next safe calculator extraction; run `npm run files:changed` before handoff. |

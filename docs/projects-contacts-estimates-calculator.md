...
- The shared staff-header pilot adds the same global Projects/Contacts search to Dashboard, Projects Index, and Project Detail only. Global search is an authenticated server read and remains separate from route-local filtering: `View all matching projects` hands `q` to the existing Projects filter, while `View all matching contacts` initializes the existing Contacts filter without migrating the Contacts header. Search results never bypass the normal Project/Contact route, permissions, cache, or local-first owners.

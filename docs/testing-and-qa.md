...
`npm run test:portal:project-work:production-read-only-auth` is reserved for the manually dispatched `main` Portal Quality workflow. It requires the exact production portal hostname and Supabase project ref, uses the protected staff test identity, blocks every non-GET/HEAD/OPTIONS application request after authentication, suppresses portal web-vitals writes, and covers Projects/Dashboard state, Work Queue, and one real Overview. Routine PR and local commands remain staging-only and production-refusing.

...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | `CalculatorGridClient.tsx` is now below the critical threshold and owns final configuration/workspace composition only. Estimate/project loading and duplicate-draft orchestration live in `useCalculatorWorkspaceSession.ts`; result-derived presentation lives in `useCalculatorResultPresentation.ts`; the outer workspace shell lives in `CalculatorWorkspaceView.tsx`; house-footprint editor gesture/lifecycle state lives in `useCalculatorHouseFootprintController.ts`; deterministic module validation lives in `calculatorValidation.ts`; and job/module plus blinds/flashings mutation state lives in focused controllers. Pure calculator input defaults, normalization, browser-draft persistence/session state, module navigation modelling/presentation, configuration-section definitions/presentation/field options, structure/site/workflow field-schema generation, issue model/focus navigation, infill lifecycle/actions/interactions/cost coordination/presentation, save orchestration/dialogs, blinds/flashings editor markup, materials-debug behavior, preview presentation, pricing presentation/splitting, and quote-status helpers also live outside the client. | Do not split solely for size; if future work expands the client, extract configuration-schema or workspace-prop assembly before adding inline behavior. | `npx vitest run apps/portal/app/staff/calculator`, `npm run test:portal:projects`, `npm run typecheck` |

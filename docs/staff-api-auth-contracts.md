...
- `GET /api/staff/v1/projects/index` is the staff-authenticated Projects-list read model. It accepts bounded `archive`, `q`, `status`, `page`, `pageSize`, and `sort` parameters; calls `staff_projects_index_v1()` through the request's auth-bound client; returns one exact-count project page plus its linked contact display rows and query identity; and is always `private, no-store`. Legacy next-action due filters and sorting are retired. Missing RPC schema returns `503 PROJECTS_INDEX_SCHEMA_NOT_READY`.
...
- `GET /api/staff/v1/projects/[projectId]/command-centre` is the staff-authenticated Project Overview read model. It uses `requireStaffContext()` and the auth-bound client, resolves quote/estimate truth plus the canonical owner and work-model boundary on the server, returns the authoritative V2 projection or bounded pre-V2 retirement metadata, and is always `private, no-store`. It does not read, rank, or return legacy task/action/conflict/history rows.
...
- `GET /api/staff/v1/staff-directory`, `PATCH .../command-centre/owners`, and `GET /api/staff/v1/dashboard/project-exceptions` own the retained staff-directory, owner, and bounded exception contracts. The legacy primary-action command route is deleted. Project-owner changes are admin-only and accept only the approved Jordan/JP/Joe/Bruce keys. Every success and error is `private, no-store`; owner mutations require UUID command IDs and optimistic versions.

...
- `PRAXIS_DISPOSABLE_POSTGRES_BIN=<absolute PostgreSQL 17 bin directory> node scripts/test-praxis-customer-read-db.mjs` creates its own local disposable cluster and checks the customer join, exact receipt amount/currency, wrong-project/invoice exclusion, match and ledger reversal exclusion, migration rollback, and reporting-only grants with the read-only default deliberately disabled for denial probes. It never accepts a remote database URL. Finance write-lifecycle tests remain separate; this synthetic fixture proves the reporting view, not hosted Xero behavior. Focused `server.test.ts` cases cover query bounds, source identity, positive/absent receipts, overflow and safe failures.

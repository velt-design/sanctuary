...
function Part({part,species}:{part:SidePanelMesh;species?:SidePanel['species']}){
  ...
  return <mesh geometry={geometry}><meshStandardMaterial side={DoubleSide} color={part.kind==='timber'?(species==='thermopine'?'#9a7952':'#aa7950'):part.kind==='acrylic'?'#b5d1cd':'#242824'} roughness={part.kind==='timber'?.88:.4} metalness={part.kind==='frame'?.25:0} transparent={part.kind==='acrylic'} opacity={part.kind==='acrylic'?.25:1} depthWrite={part.kind!=='acrylic'}/></mesh>;
}
export default function PreviewSidePanel({opening,panel,onSelect}:{opening:BlindOpening;panel:SidePanel;onSelect:()=>void}){
  ...
  return <group name={'side-panel-'+opening.id} onClick={e=>{if(e.delta<6){e.stopPropagation();onSelect();}}}>{parts.map((part,i)=><Part key={i} part={part} species={panel.species}/>)}</group>;
}

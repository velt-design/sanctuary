...
| Browser smoke | Yellow | The fixture-safe workbench performance journey passed, and drawing smoke passed with 5 tests plus 1 intentional scenario skip after aligning its diagnostic assertion with the established `geometry-canvas` contract. The staging calculator scenario has been refreshed to the canonical three-module/two-pergola V2 input. Authenticated calculator customer-pricing, module switching, local restoration, stale/live state, responsive configuration columns, and no-clipping checks pass at 1600px, 1366px, 1024px, and 768px using browser-draft-only mutations. | Keep the canonical calculator, authenticated smoke, and authenticated performance gates on compatible provisioned scenarios. |
...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, project picker, browser-draft persistence/session state, module navigation, configuration-section presentation, and pricing-summary presentation now live outside `CalculatorGridClient.tsx`. The client no longer owns draft hydration, local persistence effects, module action policy, section ordering, responsive field layout, or pricing-summary markup, but it still owns field construction, deeper save orchestration, issue navigation, and most infill modal state. | Extract issue-to-section mapping before adding more inline calculator workflow; run `npm run files:changed` before handoff. |

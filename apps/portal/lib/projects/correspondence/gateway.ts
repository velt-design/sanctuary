...
export async function readStaffCorrespondence(config: CorrespondenceGatewayConfig, input: { actorId: string; projectId: string }, signal: AbortSignal,
  ...
  const body = JSON.stringify({ schemaVersion: STAFF_CORRESPONDENCE_VERSION, actorId, projectId });

...
Marketing enquiry routes can create public lead/enquiry records and send or log autoresponder email behavior. `marketing_enquiry_intake` owns contact, project, and enquiry creation as one database transaction. A browser-generated `submission_id`, unique constraint, and transaction advisory lock make retries and concurrent duplicates return the original IDs. Keep public marketing writes narrow and server-owned; public responses expose stable validation/service messages, never raw Supabase errors.

...
export async function POST(req: Request) {
  const diagnostics = createRouteDiagnostics(req, '/api/staff/v1/schedule/job/mark-in-progress');
  ...
  const snappedToday = snapToday(ctx.today, crewCtx.crewRow.calendar_region || 'Auckland', ctx.calendar);
  ...
  const jobs = crewCtx.jobs.map((job) =>
    job.id === jobRow.id
      ? {
          ...
          actualStart: ensureActualStart(job, snappedToday),
        }
      : job,
  );
  ...
  const commitRes = await commitScheduleJobPatch({
    ...
    failureMessage: 'Failed to mark scheduled job in progress',
  });

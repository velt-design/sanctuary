...
- UI foundation proof currently covers Projects Index, Project Detail, and the full Contacts workflow. Projects Index composes the shared operational header, search/filter controls, stage badges, loading and truthful empty/error/stale states without changing `useProjectsIndexData`, prefetch, archive scope, or optimistic mutations. Project Detail composes the shared two-row detail header, inline stage badge, controls, alert, and navigation-loss guard without changing lazy tab boundaries or `useProjectDetailsDraft` queue semantics.

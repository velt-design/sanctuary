...
MobileSideLooks leads mobile page 3 with the actual model and previous/next chevrons, horizontal swipe and keyboard navigation. mobileSidePresets builds complete configurations from actual front/left/right openings, validates them through sideTreatment and omits incompatible blind layouts. Only a deliberate preset choice replaces side settings; exact settings comparison preserves custom configurations under Your combination, including after reload. Presets use established blind and timber defaults and never change roof, size or lighting. The geometry currently has no rear-screen opening, so copy names front and sides explicitly. The model remains at a steady overview while browsing. Under Customise sides, MobileSideChoices and useMobileSides retain native grouped checkboxes and all five treatments; same-kind customisations and unselected openings survive grouped edits. MobileOpeningPicker and individual screen/blind refinements remain available. Sides and Lighting share page 3 as tabs, without Apply or confirmation subpages. Starting a new design clears side selection/errors and returns to the Sides tab.

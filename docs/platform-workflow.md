...
### Original website enquiry receipt (2026-09-14)

Project Work now includes an Original enquiry tab alongside Work and Files. It loads on demand through the staff-authenticated enquiry-receipts route. The database projection returns the original customer brief, preferences, submitted selling-price ranges and confirmation-email status, scoped to one project. The durable intake receipt is used instead of mutable estimate output; legacy enquiries are labelled as lacking an immutable price record. No email bodies, provider payloads, internal cost lines or lease information are exposed. Migration 20260914000002 depends on the unactivated durable-delivery migration and must pass the real database gate before rollout. The existing Files tab continues to own attachment access. The durable producer stores enquiry, draft receipt, outbox and job atomically when WEBSITE_ENQUIRY_DURABLE_DELIVERY is enabled; the default legacy path remains unchanged until release activation.

...
2. **Project Page Command Centre**
   ...
   - The historical Stage 2 legacy-action implementation has been retired following staff confirmation that it was not used operationally. The project header still shows one canonical Project Owner from the approved Jordan/JP/Joe/Bruce roster, but pre-V2 Overview now shows one read-only retirement notice with no task rows, action ranking, history, or commands.
   ...
   - The Project Work V2 new-project foundation, Work Queue, Dashboard preview, guarded legacy review, and authenticated staging command smoke are complete. The legacy-task retirement keeps historical rows as review evidence and does not bulk-close, migrate, or otherwise mutate existing projects.
   - Implemented slice: the approved Overview V2 composition in `project-command-centre-architecture.md`. Five required top-level owners compose orientation, one Project Work region, current design/commercial, and bounded recent notes/events. V2 controls remain fully actionable through server-owned commands; pre-V2 projects use `LegacyProjectWorkRetiredCard` and expose no generic project-task controls.

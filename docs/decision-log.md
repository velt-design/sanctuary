...
| ---------- | -------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-27 | Project Enquiry Attachments      | Promoted | Treat portal visibility as durable project data independent of the 8 MB email delivery choice. Link verified paths atomically at enquiry intake, keep Storage private, authorize and audit each short-lived URL, and backfill only exact reviewed path evidence with an idempotent run ledger. Never infer historical merges or move existing objects during reconciliation. |
...
| 2026-08-13 | Commercial Truth                 | Promoted | Select one current accepted lifecycle version per quote family without reviving terminal history; derive Accepted/Paid/Open/Remaining from current accepted cents, the net job ledger, and whole open invoices; cap invoice creation at the lower job/scope balance; and gate completion from that record rather than typed paid dates. |
| 2026-08-13 | Portal Action And Recovery        | Promoted | Lock commands synchronously before awaiting, give non-idempotent writes a stable retry identity, apply returned results immediately, and treat follow-up refresh as reconciliation. If refresh fails after commit, report completion plus stale data rather than inviting a duplicate write. |
| 2026-08-13 | Portal State Synchronisation     | Promoted | Keep active interaction state above loading/remount boundaries; model URL changes as pending `{from,target}` intent until commit; let genuinely new Back/Forward state win; discard superseded async responses by identity; and scope optimistic rollback to the failed entity/field rather than replacing a whole stale snapshot. |
...
| 2026-08-11 | Calculator Additional Aluminium | Promoted | Store rare full-bar additions on their owning module, derive choices from the published pricebook, and price them through the package BOM and existing powdercoat policy. Include materials and module true cost without inventing geometry or labour; keep assumed rates supplier-confirmation visible and versioned. |
| 2026-08-11 | Costing Unified Commercial Policy | Promoted | Gate the `$500` site startup, `$500` per productive crew-day, shared `1.3x` multiplier, Bespoke design/additional-structure fees, 20% productive-time allowance, and expanded Simple gable/box eligibility behind manifest `v2.4`. Freeze multiplier and uplift in outputs; older manifests retain their original commercial semantics. |
| 2026-08-11 | Calculator Roof Mode Transitions | Promoted | Treat Open as a destructive roof mode only while it is selected. When staff return to Acrylic, Timber or Mixed, apply one shared set of fresh roofed defaults—including Auto pitch, primary flashing, gutter settings and removal of Open-injected frame overrides—while preserving dimensions and non-roof job context. Guard the standard Open → Acrylic round trip with exact state regression coverage. |
| 2026-08-10 | Project Commercial Navigation    | Promoted | Keep project navigation list-first: Commercial owns Estimates, Quotes, then Invoices. Show every saved estimate version before mounting the calculator; explicit create, open/edit, duplicate and deep-link intents enter a focused workspace with one compact return/context bar, while browser history preserves the list journey. |
| 2026-08-10 | Job Payment Reconciliation       | Promoted | Keep invoices whole and immutable, but derive Paid from an append-only job ledger across quote versions. Treat paid superseded invoices as allocatable credit; use audited adjustments/reversals and reversible stage allocations. Serialise payment and invoice creation, and require an atomic reason/audit record for over-invoicing. |
...
| 2026-08-10 | Quote Payment Schedules          | Promoted | Freeze ordered fixed-dollar or percentage-of-remainder defaults on the quote. Acceptance creates only the first whole invoice; later admin creation may follow, replace or split the remaining schedule under Job Payment Reconciliation. Never infer payment from quote acceptance or add partial-paid invoice amounts. |
| 2026-08-10 | Commercial Record Retirement     | Promoted | Keep older invoices historical, void open invoices with evidence instead of deleting them, permit permanent deletion only for an unsent current draft, and let admins manually retire sent or accepted versions as superseded without erasing their evidence. Reconcile paid value at job level under the newer Job Payment Reconciliation rule. |
...
Current guardrail: Freeze ordered fixed-dollar or percentage-of-remainder terms on the quote; require percentage terms to total 100% and resolved cents to reconcile. Acceptance creates only term one. Later admin invoices remain whole but may follow, replace or split the remaining default schedule under Job Payment Reconciliation. Never add partial-paid invoice amounts or infer payment from acceptance. Commercial navigation is list-first unless an explicit deep link or create action selects a record.
...
Current guardrail: Label older quote-version invoices historical, but reconcile paid value across the job under the Job Payment Reconciliation decision. Admins void an open invoice with a required reason, retained invoice identity and invalidated public token; paid invoices are not voidable. Only admins may permanently delete the authoritative unsent current draft quote after typed confirmation. A sent or accepted quote may instead be manually and immediately marked `SUPERSEDED`; never infer that transition from revision, send, or acceptance. Preserve its acceptance, PDF, invoice and payment evidence, revoke its acceptance token, and prevent new send, acceptance, invoice or job-pack side effects.
Promoted to: `docs/quotes-invoices-job-packs.md`; `docs/staff-api-auth-contracts.md`; `docs/ui-foundation.md`
Related docs/tests: `apps/portal/lib/invoices/paymentScheduleProjection.test.ts`; `apps/portal/app/api/admin/invoices/[invoiceId]/void/route.test.ts`; `apps/portal/app/api/quotes/[quoteVersionId]/route.test.ts`; `apps/portal/app/api/admin/quotes/[quoteVersionId]/supersede/route.test.ts`; `apps/portal/lib/quotes/adminLifecycle.test.ts`; `apps/portal/components/projects/ProjectPage/tabs/QuotesTab.test.tsx`; `test/quote-superseded-migration.test.ts`

### 2026-08-10 - Job Payment Reconciliation - Separate Money From Invoice State

Date: 2026-08-10
Area: Commercial invoices, job payments and administrator reconciliation
Status: Promoted
Decision or mistake: Scoping paid value to the latest accepted quote hid money already received against an older accepted version, while adding a part-paid amount to invoices would have made invoice status, delivery and correction ambiguous.
Why it mattered: Real jobs can be requoted after a deposit is paid, and administrators need to correct imported or mistakenly recorded money without deleting financial evidence. Invoice generation can also race payment recording unless both use one project-level transaction boundary.
Current guardrail: Keep invoices whole as `OPEN`, `PAID` or `VOID`. Derive actual Paid from append-only project payment entries across all quote versions; use equal/opposite reversals and reasoned adjustments, and keep stage allocations independently reversible. Backfill paid historical invoices as unallocated credit. Serialise job payment and invoice creation commands, compute remaining as accepted total minus net paid minus whole open invoices, block over-invoicing by default, and persist any override reason plus audit event inside the invoice transaction. Void and recreate incorrect issued invoices rather than editing them.
Promoted to: `docs/quotes-invoices-job-packs.md`; `docs/supabase-schema-map.md`; `docs/staff-api-auth-contracts.md`; `docs/ui-foundation.md`; `docs/platform-workflow.md`
Related docs/tests: `apps/portal/lib/invoices/paymentScheduleProjection.ts`; `apps/portal/lib/invoices/paymentLedger.ts`; `apps/portal/components/projects/ProjectPage/tabs/CreateInvoiceDialog.tsx`; `supabase/migrations/20260810000004_admin_payment_reconciliation.sql`; `test/admin-payment-reconciliation-migration.test.ts`

### 2026-08-10 - Project Commercial Navigation - Estimates Before The Calculator

Date: 2026-08-10
Area: Project navigation, estimates and embedded Calculator
Status: Promoted
Decision or mistake: Calculator was a primary project tab and opened an active or blank design immediately, while Quotes and Invoices were grouped as commercial records. That mixed a specialist editing tool with top-level project areas and made estimate history harder to discover.
Why it mattered: Estimates, quotes and invoices form one commercial record chain. Staff need a stable version list before choosing whether to edit, inspect or duplicate a design, and ordinary navigation should not silently enter an editor.
Current guardrail: Keep Overview, Commercial and conditional Job Packs as the primary project tabs. Commercial opens to Estimates, followed by Quotes and Invoices. `tab=estimates` without record intent is list-first; only explicit `estimateId`, `fromEstimateId` or `newDesign=1` intent mounts the Calculator. While mounted, replace the normal project header and Commercial subtabs with one compact Back to estimates, project-name, and estimate-version bar so the Calculator receives the remaining viewport. Preserve deep links and browser history, restore the normal shell on return, and never mutate a historical source during Duplicate or Start revision.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/platform-workflow.md`; `docs/ui-foundation.md`
Related docs/tests: `apps/portal/lib/projects/projectTabs.ts`; `apps/portal/components/projects/ProjectPage/tabs/CommercialTab.tsx`; `apps/portal/components/projects/ProjectPage/tabs/ProjectCalculatorTab.tsx`; `apps/portal/components/projects/ProjectPage/tabs/EstimatesListView.tsx`; `apps/portal/components/projects/ProjectPage/tabs/ProjectCalculatorTab.test.tsx`

### 2026-08-10 - Calculator Detail Disclosure And Module Price Allocation

Date: 2026-08-10
Area: Calculator configuration, workings, price explanation and labour visibility
Status: Promoted
Decision or mistake: A global Basic/Advanced mode hid ordinary structure and labour information, while calculator-era house-footprint controls kept legacy movement behavior in the Workings plan. Price by item also stopped at pergola totals even when a pergola contained multiple physical modules.
Why it mattered: Staff need one predictable editor, readable plans, module-level commercial context, and a whole-job time estimate without being shown internal hourly rates or labour dollars.
Current guardrail: Keep the Calculator free of house-footprint configuration and plan movement controls while preserving historical footprint reads for old estimates. Use scoped native disclosures for Flashings, Structural overrides and admin diagnostics; permission checks remain independent of disclosure state. Allocate the finalized pergola sell cents across modules by package-produced module true-cost weight with deterministic cent reconciliation, nest infill contributions under their owning module, and never treat either explanation as a separate quote line. All staff may see labour activities, minutes and crew hours; internal rates and dollar costs remain admin-only.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/costing-and-geometry.md`; `docs/ui-foundation.md`; `docs/calculator-trust-and-explainability-goal.md`
Related docs/tests: `apps/portal/app/staff/calculator/calculatorModulePricing.ts`; `apps/portal/app/staff/calculator/CalculatorConfigurationForm.test.tsx`; `apps/portal/app/staff/calculator/CalculatorPreviewDetails.test.tsx`; `apps/portal/app/staff/calculator/ModuleViewsCard.test.tsx`; `apps/portal/app/staff/calculator/calculatorPricingPreview.test.tsx`

### 2026-08-11 - Calculator Roof Mode Transitions - Covered Roof Defaults

Date: 2026-08-11
Area: Calculator Roof Mode Transitions
Status: Promoted
Decision or mistake: Selecting Open correctly rewrote roof fields to its fixed frame contract, but returning to a roofed material changed only `roofMaterial`. The Open pitch, cleared flashing, gutter state and injected `150x50` overrides therefore leaked into the covered-roof calculation.
Why it mattered: A visually identical standard 6m × 3m acrylic selection could return a materially different price. Reduced productive labour crossed the Simple site-day boundary from four days to three, amplifying the hidden input drift through overhead.
Current guardrail: When the current material is Open and the next material is Acrylic, Timber or Mixed, route through one roofed-default helper. Reset only roof-owned selections, rebuild the primary flashing from the retained dimensions, remove Open-injected ledger/rafter/front-beam overrides, and retain non-roof job context. The standard Open → Acrylic transition must reproduce the exact original default module state.
Promoted to: `docs/projects-contacts-estimates-calculator.md`
Related docs/tests: `apps/portal/app/staff/calculator/calculatorOpenPergola.ts`; `apps/portal/app/staff/calculator/calculatorStructureFields.ts`; `apps/portal/app/staff/calculator/calculatorStructureFields.test.ts`

### 2026-08-11 - Costing Unified Commercial Policy - One Site Basis

Date: 2026-08-11
Area: Costing commercial policy, overhead, classification and customer pricing
Status: Promoted
Decision or mistake: Simple and Bespoke had materially different overhead and sell-price mechanisms, while gable and box-perimeter acrylic structures were forced to Bespoke even when every other Simple condition passed. A global markup constant also could not change without repricing historical outputs.
Why it mattered: Staff need one explainable commercial basis, explicit design compensation for Bespoke work, realistic additional build time, and reproducible historical estimates across calculator, quote, dashboard and marketing consumers.
Current guardrail: From manifest `v2.4`, calculate operational overhead once per site as `$500 ex GST` plus `$500` per pro-rated productive eight-hour crew-day. Multiply Bespoke productive actions by `1.2`, excluding mobilisation/demobilisation; add `$1,200` design per site, `$800` per additional pergola and `$300` per module beyond the first module in each pergola. Allow acrylic gable and box-perimeter structures through the existing Simple checks without bypassing structural warnings. Freeze `customer_price_multiplier: 1.3` and zero uplift in the output, allocate site overhead deterministically across pergolas, and default historical outputs without a multiplier to `1.25`. Preserve all pre-`v2.4` policy behavior by effective manifest version.
Promoted to: `docs/costing-and-geometry.md`; `docs/projects-contacts-estimates-calculator.md`
Related docs/tests: `packages/costing/src/commercial/simpleRangePricing.ts`; `packages/costing/src/engine/simpleSiteDayPolicy.ts`; `packages/costing/src/engine/calculate.ts`; `packages/costing/src/commercial/simpleRangePricing.test.ts`; `apps/portal/app/staff/calculator/calculatorPricingPreview.test.ts`; `apps/portal/lib/quotes/mapping.test.ts`; `apps/marketing/lib/simpleCoverPricing.server.test.ts`

### 2026-08-11 - Calculator Commercial Defaults - Price The Job Basis

Date: 2026-08-11
Area: Calculator defaults and commercial policy
Status: Promoted
Decision or mistake: The staff calculator started with soffit and four posts while the marketing calculator started with fascia, so neither represented the intended standard facade job. Raising the shared multiplier to reach a single reference price would have increased every material, labour and overhead component.
Why it mattered: Starting inputs must describe the common build, remain editable for exceptions, and produce an explainable reference price without distorting the common Simple and Bespoke margin.
Current guardrail: Use the shared `@sp/costing` suggestion of posts at no more than four metres along each support beam: one front beam for attached jobs and both support beams for freestanding jobs. Start attached calculators on facade and update the suggestion only while the current count still matches the prior suggestion. Keep the shared `1.3x` multiplier; manifest `v2.5` uses a `$1,000 ex GST` once-per-site startup while published `v2.4` retains `$500`.
Promoted to: `docs/costing-and-geometry.md`; `docs/projects-contacts-estimates-calculator.md`
Related docs/tests: `packages/costing/src/engine/postLayout.ts`; `packages/costing/src/engine/postLayout.test.ts`; `packages/costing/src/commercial/simpleRangePricing.test.ts`; `apps/portal/app/staff/calculator/useCalculatorInputController.test.tsx`; `apps/marketing/lib/simpleCoverCalculator.test.ts`

### 2026-08-11 - Calculator Additional Aluminium - Explicit Material Only

Date: 2026-08-11
Area: Calculator module accessories, pricebook catalogue and costing BOM
Status: Promoted
Decision or mistake: Rare extra aluminium members did not justify new structural calculator rules, but allowance-only pricing would disconnect quantities from the governed material catalogue and powdercoat behavior.
Why it mattered: Staff need to capture odd full bars without pretending they affect geometry or installation time, while module pricing and material purchasing still need an auditable source.
Current guardrail: Store ordered profile, stock-length and whole-quantity rows on the active calculator module. Derive choices from the active published pricebook, price them through the package BOM in the module finish, and apply the existing custom powdercoat multiplier. Include them in module true cost and Materials, but never add geometry, structural calculations, or labour. Keep assumed powdercoat rows explicitly marked for supplier confirmation.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/costing-and-geometry.md`
Related docs/tests: `apps/portal/app/staff/calculator/CalculatorAdditionalAluminiumEditor.tsx`; `apps/portal/lib/costing/additionalAluminiumCatalogue.test.ts`; `apps/portal/lib/estimates/costingPayload.test.ts`; `packages/costing/src/engine/calculate.test.ts`

### 2026-08-11 - Commercial Internal Identity - Name The Lineage, Not The Artifact

Date: 2026-08-11
Area: Estimate and quote internal identity
Status: Promoted
Decision or mistake: Version labels and quote references were the only list identity, so multiple alternatives all appeared alike. Putting a mutable staff label on each quote version would duplicate family state and blur immutable customer artifacts.
Why it mattered: Staff need recognizable alternatives without renumbering estimates, rewriting quote history, or exposing internal wording to customers.
Current guardrail: Store an optional name on each estimate and one optional name on the quote family. Show the stable estimate version or quote reference/version underneath, preserve unnamed fallbacks, let historical records be renamed without changing pricing locks, seed estimate duplicates with `Copy of…`, and keep the names out of PDFs, emails, public tokens, invoices, and job packs.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/quotes-invoices-job-packs.md`; `docs/ui-foundation.md`; `docs/supabase-schema-map.md`
Related docs/tests: `apps/portal/lib/commercial/internalName.test.ts`; `apps/portal/app/api/quotes/[quoteVersionId]/internal-name/route.test.ts`; `test/commercial-internal-names-migration.test.ts`

### 2026-08-11 - Project Commercial Add-ons - Separate The Family, Aggregate The Job

Date: 2026-08-11
Area: Project estimates, quotes, invoices and payments
Status: Promoted
Decision or mistake: Later work such as blinds, infills, or an extra post belongs to the existing customer project, but treating it as another revision of the original quote would rewrite commercial meaning and make supersession, acceptance, invoicing, and payment allocation ambiguous.
Why it mattered: Staff need one operational project and accurate total job value without losing the immutable original contract or mixing balances between independently accepted work.
Current guardrail: Keep the original estimate/quote family on a null commercial scope and assign each add-on one stable UUID across estimate revisions and its own quote reference/version chain. Never supersede the base family automatically. Sum the newest accepted version from every family for job reporting, while invoice creation, scheduled terms, and payment allocations remain scoped to one exact accepted quote version. Manual quote supersession still acts only on the selected version.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/quotes-invoices-job-packs.md`; `docs/ui-foundation.md`; `docs/supabase-schema-map.md`; `docs/staff-api-auth-contracts.md`
Related docs/tests: `apps/portal/lib/commercial/scope.test.ts`; `apps/portal/lib/invoices/paymentScheduleProjection.test.ts`; `apps/portal/lib/projects/commandCentre/resolve.test.ts`; `test/project-commercial-add-on-scopes-migration.test.ts`

### 2026-08-12 - Add-on Estimates - Empty Until Scope Requires A Pergola
...
### 2026-08-12 - Existing-Pergola Infill Add-ons - Standalone Costing Scope

Date: 2026-08-12
Area: Existing-Pergola Infill Add-ons
Status: Promoted
Decision or mistake: An add-on estimate with no new pergola could not capture infills for the customer's existing pergola because every infill was nested inside a calculator module.
Why it mattered: Adding a synthetic module would misstate the sale, add unrelated structure and labour, and create a false pergola quote line.
Current guardrail: Store existing-pergola infills outside `pergolas[]`, with an independent aluminium finish. Reuse the canonical infill takeoff, BOM and productive labour owners internally, but expose only the standalone infill result and never persist or emit synthetic pergola geometry. Do not offer module presets, dimension inference, or rafter matching when no referenced pergola geometry exists. Map the saved standalone result to one separate customer quote line.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/costing-and-geometry.md`; `docs/quotes-invoices-job-packs.md`; `docs/ui-foundation.md`
Related docs/tests: `packages/costing/src/engine/standaloneInfills.ts`; `packages/costing/src/engine/calculate.test.ts`; `apps/portal/app/staff/calculator/useCalculatorInfillActions.test.tsx`; `apps/portal/lib/estimates/costingPayload.test.ts`; `apps/portal/lib/quotes/mapping.test.ts`

### 2026-08-13 - Infill Labour Allowances - Restrained Small-Work Pricing

Date: 2026-08-13
Area: Infill labour allowances
Status: Promoted
Decision or mistake: Small, unique infill work needed a restrained allowance without replacing the detailed existing fabrication and installation actions.
Why it mattered: A generic minimum or a setup charge per opening would overprice straightforward infills, while no small-work allowance understated the coordination and setup involved.
...
Current guardrail: Ordinary estimates still start with one pergola but may remove the final pergola after confirmation; add-ons start empty. Price additional aluminium once at estimate level, including with no geometry. Quote-source estimate projections must preserve every estimate-level priced output, including standalone infills and additional aluminium, rather than forwarding only pergola and shared-site results. Admin-created manual quotes must carry manual pricing provenance and null estimate provenance, retain the normal quote/invoice lifecycle, and remain ineligible for estimate refresh or estimate-backed job-pack generation.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/quotes-invoices-job-packs.md`
...
### 2026-08-13 - Portal State Synchronisation - Preserve Newer Intent

Date: 2026-08-13
Area: Portal state synchronisation
Status: Promoted
Decision or mistake: Route loading, URL reconciliation, overlapping requests, and optimistic rollback had several independent owners that could replay an older default or snapshot over newer staff input.
Why it mattered: Lost search text, two-click Back navigation, stale detail reopening, and whole-list rollback make correct data feel untrustworthy even when the server eventually holds the right value.
Current guardrail: Keep active interaction state above loading/remount boundaries. Represent URL navigation as pending `{from,target}` intent until commit, but let genuinely new Back/Forward state win. Discard superseded reads by request/entity identity, and rollback only the failed entity or field rather than restoring a whole captured collection. Lock exact editors when a safe field-level merge is unavailable.
Promoted to: `docs/portal-state-synchronisation-audit.md`; `docs/ui-foundation.md`; `docs/projects-contacts-estimates-calculator.md`; `docs/quotes-invoices-job-packs.md`; `docs/schedule.md`
Related docs/tests: `apps/portal/components/layout/GlobalPortalSearch.client.test.tsx`; `apps/portal/components/page-state/PortalRouteTransition.test.tsx`; `apps/portal/components/projects/ProjectPage/tabs/QuotesTab.test.tsx`; `apps/portal/components/spreadsheet/SpreadsheetPageTemplate.state.test.tsx`; `apps/portal/app/admin/access/AccessClient.test.tsx`

### 2026-08-13 - Portal Action And Recovery - A Committed Command Is Not A Failed Refresh

Date: 2026-08-13
Area: Portal action progress, idempotency and recovery
Status: Promoted
Decision or mistake: Several actions relied only on rerender-time disabled state, reused no caller identity on ambiguous retries, or placed the command and its follow-up refresh in one catch path.
Why it mattered: A fast double-click could create two records, while a committed write followed by a failed refresh looked like a failed action and encouraged staff to repeat it. Both outcomes make correct data feel untrustworthy.
Current guardrail: Acquire a ref-backed lock before the first await. Give creates, sends and append-only commands a stable client intent or caller record ID and reuse it for ambiguous retry. Apply returned command data immediately, then treat refresh as reconciliation: report committed success plus stale/recovery state if refresh fails, and block only the resource whose truth is unverifiable. Freeze destructive dialogs and exact save navigation while pending.
Promoted to: `docs/portal-action-recovery-audit.md`; `docs/projects-contacts-estimates-calculator.md`; `docs/quotes-invoices-job-packs.md`; `docs/schedule.md`; `docs/portal-production-readiness.md`
Related docs/tests: `apps/portal/components/projects/ProjectPage/tabs/InvoicesTab.test.tsx`; `apps/portal/components/projects/ProjectPage/tabs/QuotesTab.test.tsx`; `apps/portal/app/staff/contacts/ContactsImportDialog.test.tsx`; `apps/portal/app/staff/calculator/useCalculatorSaveController.test.tsx`; `apps/portal/app/staff/schedule`; `test/commercial-admin-action-idempotency-migration.test.ts`

### 2026-08-13 - Commercial Truth - One Version-Bound Financial Record

Date: 2026-08-13
Area: Commercial Truth
Status: Promoted
Decision or mistake: Quote acceptance, project totals, invoice availability, payment state, and completion eligibility were derived by different readers, and some lifecycle or Running Jobs actions could mutate date/status projections without proving the underlying commercial record.
Why it mattered: Historical accepted versions could be counted as current, an add-on invoice could exceed the remaining whole-job balance, a reversed invoice payment could leave the invoice marked paid, and typed dates could make an unsettled job appear complete.
Current guardrail: Select one current accepted lifecycle version per base/add-on quote family without reviving terminal accepted history. Derive accepted, paid, open, remaining, and over-committed totals from current version cents, the append-only net ledger, and whole open invoices. Cap ordinary invoice creation at the lower scope/job balance, serialize lifecycle and financial commands per project, reopen a whole invoice when its owned payment is reversed, and require ledger settlement with no open invoices before commercial completion. Treat `PAID` as a projection: later scope, invoice, reversal, or negative-adjustment changes that invalidate settlement atomically return the project to `COMPLETED` and clear its compatibility final-paid date.
Promoted to: `docs/commercial-truth-audit.md`; `docs/quotes-invoices-job-packs.md`; `docs/running-jobs.md`; `docs/supabase-schema-map.md`
Related docs/tests: `supabase/tests/commercial_truth_invariants.sql`; `apps/portal/lib/commercial/authoritativeAcceptedVersions.test.ts`; `apps/portal/lib/invoices/paymentScheduleProjection.test.ts`; `apps/portal/app/api/staff/v1/projects/[projectId]/action/mark_deposit_received/route.test.ts`; `apps/portal/app/api/staff/v1/projects/[projectId]/action/mark_paid/route.test.ts`

### 2026-08-14 - Customer Quote Descriptions - Explain The Value, Preserve The Contract

Date: 2026-08-14
Area: Automatically generated customer quote descriptions
Status: Promoted
Decision or mistake: Generated lines accurately named many selections but often presented raw fields, internal compatibility notes, and pricing-policy language instead of explaining the scope the customer receives.
Why it mattered: The quote is a primary sales artifact. Technical data without a clear inclusion or boundary understates value, while internal labels weaken confidence and can imply the wrong scope.
Current guardrail: Lead each generated line with verified included scope, translate saved specifications into customer language and practical consequences only when grounded, and state supply/install or allowance boundaries explicitly. Never expose legacy/regeneration notes, internal discount rules, or unverified performance claims. Preserve price, quantity, line count, source metadata, sent-version immutability, and the structured pergola editor's lossless round trip.
Promoted to: `docs/quotes-invoices-job-packs.md`
Related docs/tests: `apps/portal/lib/quotes/estimateQuoteDescriptions.ts`; `apps/portal/lib/quotes/estimateQuoteAddonDescriptions.ts`; `apps/portal/lib/quotes/estimateQuoteDescriptions.test.ts`; `apps/portal/lib/quotes/mapping.test.ts`; `apps/portal/lib/quotes/pergolaDraft.test.ts`; `packages/quote-format/src/index.test.ts`; `apps/portal/lib/quotes/quoteArtifactVisualFixtures.test.ts`; `apps/marketing/app/quote/[quoteId]/page.test.tsx`

### 2026-08-18 - Synthetic AI Execution - Reuse The Durable Spine And Commit Evidence Atomically

Date: 2026-08-18
Area: Sanctuary AI synthetic execution
Status: Promoted
Decision or mistake: PR-AI-007 uses the existing logged PGMQ ledger and worker runtime for `ai_synthetic_v1` instead of creating an AI-specific queue. Its private payload is an exact task/fixture snapshot, its handler is deterministic and effect-free, and the database accepts completion only when the fixed result matches the frozen input. Task completion, zero-cost usage, and deterministic evaluation are recorded atomically. Worker startup handler coverage is required only for registry kinds explicitly routed to the worker, so legacy commercial kinds remain dark.
Why it mattered: A second execution spine would duplicate leases, retries, privacy rules, observability, and recovery. Updating the task separately from job completion could leave contradictory evidence after a crash. Requiring handlers for every legacy registry kind would either block the safe synthetic pilot or accidentally encourage incomplete commercial handlers.
Current guardrail: Keep AI queue messages minimal and frozen input private. Enqueue through the service-only semantic RPC, preserve exact replay and one task-to-job link, and reject linked task-only cancellation. Validate the exact deterministic result inside the same transaction that records usage/evaluation and advances the task. Never treat a registered synthetic handler as authority for a model/provider, OpenClaw, external effect, business mutation, production migration, or worker rollout. An accidentally claimed unsupported kind must fail safe.
Promoted to: `docs/ai/README.md`; `docs/target-architecture.md`; `docs/supabase-schema-map.md`; `docs/security-privacy-quality.md`; `docs/testing-and-qa.md`; `apps/worker/README.md`
Related docs/tests: `packages/jobs/src/aiSynthetic.ts`; `apps/worker/src/handlers/aiSynthetic.ts`; `supabase/migrations/20260818000004_ai_synthetic_execution.sql`; `supabase/tests/ai_synthetic_execution.sql`; `test/ai-synthetic-execution-migration.test.ts`; `npm run test:ai`; `npm run test:jobs`; `npm run test:worker`; `npm run test:jobs:db`

### 2026-08-18 - Manual Quote Variations - Preserve The Accepted Base

Date: 2026-08-18
Area: Manual quote creation and commercial scope ownership
Status: Promoted
Decision or mistake: A manual variation created after base acceptance joined the base quote family. Its acceptance would have replaced the accepted contract value with the variation total, so the project-wide invoice guard correctly rejected the new deposit invoice.
Why it mattered: Weakening the invoice guard would have hidden a false job total and over-committed balance. The quote needed independent add-on identity instead.
Current guardrail: Before base acceptance, a manual quote may establish the base family. Once any base version has entered the accepted lifecycle, derive a deterministic add-on scope from the stable create intent and create a separate quote family. Repair historical mistakes only with exact-record preconditions, the project commercial advisory lock, and an audit event; never reinterpret or sum multiple accepted versions within one family.
Promoted to: `docs/quotes-invoices-job-packs.md`; `docs/supabase-schema-map.md`
Related docs/tests: `apps/portal/lib/quotes/manualQuoteScope.server.test.ts`; `apps/portal/lib/quotes/pricingSourcePreservation.test.ts`; `test/manual-quote-variation-scope-repair-migration.test.ts`; `supabase/migrations/20260818000001_rehome_sent_manual_variation.sql`

### 2026-08-26 - OpenClaw Engineering Isolation - State Is Part Of The Authority Boundary

Date: 2026-08-26
Area: OpenClaw autonomous engineering runtime
Status: Promoted
Decision or mistake: The first development activation used one Codex-backed agent and the default `~/.openclaw` state. An unrelated experiment later changed the effective Codex approval mode, added agents and enabled a channel in that same state, so the engineering proof began prompting and no longer matched the reviewed repository config.
Why it mattered: A checked-in config does not govern a running agent when another workflow can mutate the same state, approvals, plugins or gateway. Role instructions also cannot make one powerful agent both an independent supervisor and implementer.
Current guardrail: Give Sanctuary engineering unique OpenClaw state, config, gateway port, token, logs and workspace roots. Preseed the isolated approvals file before any OpenClaw command and fingerprint the default config and approval authority around every command because the CLI may otherwise migrate legacy default state. Pin the official Codex plugin exactly. Use explicit named Engineering Lead, Coding Worker and Code Reviewer agents; only the bounded lead is the deterministic default route required by the installed schema, only the leaf worker gets no-prompt coding authority, only the lead may target named children, and humans retain merge and production authority. Treat the shared macOS account as a trusted development boundary, not an adversarial sandbox, and keep production credentials absent.
Promoted to: `docs/ai/operations/autonomous-engineering.md`; `docs/ai/operations/openclaw-engineering-runtime.md`
Related docs/tests: `infra/openclaw/engineering/openclaw.json`; `infra/openclaw/engineering/exec-approvals.json`; `scripts/ai/mac-openclaw-engineering-activate.mjs`; `test/ai-operations-openclaw-engineering.test.ts`; `npm run test:ai:ops`

### 2026-08-26 - Autonomous Engineering Lanes - Bind Local Work And Hosted Authority

Date: 2026-08-26
Area: OpenClaw engineering branch, worktree and draft-PR authority
Status: Promoted
Decision or mistake: A repository-scoped GitHub App needs content-write permission to push feature branches, but that permission does not distinguish a feature branch from `main`. A general supervisor shell or raw `gh` wrapper would also make the reviewed role boundary depend only on prompt obedience.
Why it mattered: Deterministic local paths are not enough if an agent can adopt an unknown branch, write outside the declared lane, expose a short-lived token, push the default branch, or turn a draft into an unreviewed terminal change. Conversely, treating every Git command as a new human approval would defeat the autonomous engineering goal.
Current guardrail: Canonicalize the strict task manifest and bind its hash to an owner-only lane record, exact base SHA, safe feature ref and worktree under the isolated worker root. The lead receives only provision/status/cleanup tools; the worker receives status/publish. Publication requires a clean descendant commit set entirely inside `ownedPaths`, uses an explicit non-force feature refspec, and accepts only an exact open draft PR. Cleanup revalidates evidence and removes only the owned worktree without force or branch deletion. Keep raw token and merge modes absent. Before promotion, enforce hosted `main` protection with no GitHub App bypass; local wrappers are defense in depth, not the final hosted boundary.
Promoted to: `docs/ai/operations/openclaw-engineering-lanes.md`; `docs/ai/operations/openclaw-engineering-runtime.md`; `docs/ai/operations/autonomous-engineering.md`
Related docs/tests: `infra/openclaw/engineering/plugins/sanctuary-engineering-lanes/**`; `scripts/ai/engineering-lane.mjs`; `scripts/ai/github-app-token.mjs`; `test/ai-operations-engineering-lanes.test.ts`; `npm run test:ai:ops`

### 2026-08-26 - Autonomous Engineering Proof - Workers Propose, Independent Gates Prove

Date: 2026-08-26
Area: Autonomous engineering CI and independent review
Status: Promoted
Decision or mistake: A worker-authored draft PR and its self-reported tests are only candidate evidence. The supervisor now binds GitHub checks to the exact PR head, classifies failure evidence, permits one exact transient rerun, returns repairable failures to the same lane, and asks a separately identified read-only reviewer for a strict machine-readable verdict only after CI passes.
Why it mattered: A worker can be mistaken, stale checks can describe another revision, repeated reruns can hide defects, and a supervisor that both writes and approves is not an independent control. Declaring success before those gates would make autonomy faster but less trustworthy.
Current guardrail: Persist exact PR identity, head SHA, check evidence hash, rerun history, review prompt hash, reviewer identity and native session timing in durable supervision state. Complete only when required exact-head CI checks pass and the strict review verdict is approved with no blocking findings. In hosted CI, compare worktree ownership across the exact pull-request merge-base range so dependency-install churn cannot masquerade as authored changes; retain dirty-worktree inspection for local handoff. Keep repairs inside the original lane and budget, cap diff evidence, make merge and production human-only, and keep hosted `main` protection free of app bypasses.
Promoted to: `docs/ai/operations/openclaw-engineering-ci-review.md`; `docs/ai/operations/openclaw-engineering-supervision.md`; `docs/ai/operations/autonomous-engineering.md`
Related docs/tests: `infra/openclaw/engineering/plugins/sanctuary-engineering-lanes/supervision-ci-review.mjs`; `infra/openclaw/engineering/plugins/sanctuary-engineering-lanes/ci-runtime.mjs`; `infra/openclaw/engineering/plugins/sanctuary-engineering-lanes/review-runtime.mjs`; `scripts/ai/github-main-protection.mjs`; `test/ai-operations-engineering-ci-review.test.ts`; `test/ai-operations-engineering-review-loop.test.ts`; `npm run test:ai:ops`

### 2026-08-26 - Autonomous Engineering Kill Switch - Stop One Known Gateway

Date: 2026-08-26
Area: Autonomous engineering operations and recovery
Status: Promoted
Decision or mistake: The isolated gateway had a detached start command but no equally bounded stop owner. A generic process kill would risk stopping another OpenClaw instance or hiding an unhealthy, unowned process.
Why it mattered: Autonomous work needs a dependable kill switch, but the switch must narrow authority rather than add a broad host-control escape hatch.
...
### 2026-08-27 - Project Enquiry Attachments - Durable Project Links Independent Of Email Delivery

Date: 2026-08-27
Area: Project Enquiry Attachments
Status: Promoted
Decision or mistake: Website enquiry files were durable private Storage objects and email inputs, but the portal project had no canonical file link. Email delivery also had two valid branches at the 8 MB threshold, so treating one branch as the project record would omit files.
Why it mattered: Staff could receive a successful enquiry while being unable to find all of its evidence on the associated project. A filename/contact match during repair could silently attach customer material to the wrong project.
Current guardrail: Link every verified file from the existing `enquiry_requests.files` evidence in the same transaction that creates the enquiry. Keep Storage private, verify staff plus the exact project/file relationship before each short signed URL, and audit URL issuance without storing its token. Historical repair must be a dry-run-first, path-exact, run-ID-idempotent command that reports missing/unmatched/ambiguous evidence and known project changes. Never infer an undocumented merge or move, rename, or delete the existing objects during this rollout.
Promoted to: `docs/project-enquiry-attachments.md`; `docs/automation-email-audit.md`; `docs/supabase-schema-map.md`; `docs/staff-api-auth-contracts.md`
Related docs/tests: `supabase/migrations/20260827000001_project_enquiry_attachments.sql`; `scripts/project-enquiry-attachments-backfill.ts`; `test/project-enquiry-attachments-migration.test.ts`; `apps/portal/lib/projects/enquiryAttachments/server.test.ts`; `apps/marketing/app/api/enquiry/route.test.ts`

  ...
  return (
    <div className={styles.navStateGrid} aria-label="Marketing navigation states">
      ...
      <article className={`${styles.navState} ${styles.navStateTransparent}`}>
        <div className={styles.navStateImage} aria-hidden="true" />
        <div className={styles.navStateBar}><span>Sanctuary</span><span className={styles.navStateLinks}>Projects&nbsp;&nbsp; Pergola Styles&nbsp;&nbsp; Materials&nbsp;&nbsp; Our Approach&nbsp;&nbsp; Contact</span><strong>Start your project</strong></div>
  ...
  return (
    <section className={styles.heroPreview} aria-label={`${isProject ? 'Project' : 'Homepage'} hero pattern`}>
      <div className={styles.heroPreviewMedia}>
        <Figure
          image={isProject ? '/images/project-warkworth-outdoor-room-01.jpg' : '/images/riverhead-gable-01.jpg'}
...
export function EditorialSplit({ eyebrow, heading, copy, image, alt, reverse = false, action }: { eyebrow: string; heading: string; copy: string; image: string; alt: string; reverse?: boolean; action?: { label: string; href: string } }) {
  return <Container width="wide"><div className={cn(styles.split, reverse && styles.splitReverse)}><Figure image={image} alt={alt} className={styles.splitMedia} /><div className={styles.splitCopy}><Eyebrow>{eyebrow}</Eyebrow><Heading>{heading}</Heading><Text>{copy}</Text>{action && <TextLink href={action.href}>{action.label}</TextLink>}</div></div></Container>;
}
...
export function StaggeredGallery({ items }: { items: Array<{ image: string; alt: string; title: string; detail?: string; href?: string }> }) {
  return <div className={styles.gallery}>{items.map((item, index) => <article key={item.title}><Figure image={item.image} alt={item.alt} caption={item.title} detail={item.detail} ratio={index % 3 === 1 ? 'portrait' : 'landscape'} />{item.href && <TextLink href={item.href}>View project</TextLink>}</article>)}</div>;
}
...
export function ProjectStory({ image, alt, title, metadata, copy, href }: { image: string; alt: string; title: string; metadata: string[]; copy: string; href: string }) {
  return <article className={styles.projectStory}><Figure image={image} alt={alt} ratio="landscape" /><div className={styles.projectStoryCopy}><ProjectMeta items={metadata} /><Heading as="h3">{title}</Heading><Text>{copy}</Text><TextLink href={href}>View project</TextLink></div></article>;
}

...
| `project-command-centre-v1.md` | Historical V1 product baseline | You need the original Command Centre product rationale or a non-conflicting design/commercial rule. For current Overview implementation, start with the approved V2 handover in `project-command-centre-architecture.md`. |
| `project-command-centre-roadmap.md` | Active programme | You need Command Centre history, completion evidence, or the current approved Overview redesign stage. |
| `project-command-centre-architecture.md` | Current architecture and approved Overview handover | You are changing the Command Centre read model, resolver, API, Overview components, caching, fixtures, or implementing the approved V2 Overview redesign. Read its `Approved Overview V2 Implementation Handover (READ FIRST)` section first. |
| `project-work-items-and-follow-up.md` | Approved product contract; staging-verified foundation | You are changing the email-only lead/quote cadence, project-state/journey-phase model, V2 work items, Site Visit interim rule, Contacted backlog triage, or legacy-task retirement. |
| `project-work-items-technical-plan.md` | Current V2 technical and cutover contract | You are changing the work-item schema, commands, Auckland calendar, compatibility projection, legacy cutover, reconciliation, or verification for the staging-verified Project Work foundation. |
...
| `sanctuary-project-led-visual-finder-homepage-prototype.md` | Active experimental-homepage specification | You are implementing or validating the protected `/home-project-finder` visual finder, its brief builder, project evidence, enquiry continuation, or analytics contract. |

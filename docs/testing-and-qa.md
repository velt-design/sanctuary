...
As of 2026-07-20, `docker`, `psql`, and the Supabase CLI were unavailable on this workstation. The static/unit command passed. `npm run test:jobs:db` was attempted and failed at its Docker readiness check with `spawnSync docker ENOENT`; no container started and no SQL executed. The dedicated `.github/workflows/background-jobs.yml` workflow runs `npm run test:jobs` plus a real-database matrix against upstream PostgreSQL 18/PGMQ 1.10.0 and the Supabase PostgreSQL 17/PGMQ 1.5.1 image for relevant pull requests and `main` pushes. Manual dispatch is configured, but a newly added workflow cannot be manually dispatched from a feature branch until GitHub knows it on the default branch; the draft pull request is therefore the first live execution path for this branch. Only successful matrix runs count as live database evidence.

...
| Browser smoke | Yellow | The fixture-safe workbench performance journey passed, and drawing smoke passed with 5 tests plus 1 intentional scenario skip after aligning its diagnostic assertion with the established `geometry-canvas` contract. Authenticated calculator module-navigator verification passed locally at 1600px, 1366px, 1024px, and 768px using browser-draft-only mutations; the full provisioned scenario suite still requires a local/staging service-role scenario refresh. | Run the provisioned calculator, authenticated smoke, and authenticated performance gates in a compatible local/staging environment. |

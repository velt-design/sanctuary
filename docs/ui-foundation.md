...
Within Project Work, `ProjectWorkControls` owns V2 manual/state/correction controls and the existing manual hidden Site Visit completion fact; `useProjectWorkCommandController` owns V2 stable command/feedback orchestration; and `useLegacyProjectWorkCommandController` owns the equivalent legacy stable command and cache orchestration. `ProjectPrimaryActionCard` is only the nested legacy shell, with conflict/fallback in `LegacyProjectWorkConflict` and audit disclosure in `LegacyProjectWorkHistory`. `projectWorkVisibilityPolicy` is the shared fail-closed Call/Site Visit filter across both models and recent events. All cache writes remain in `projectWorkCache`: `patchProjectCommandCentreCache` is the sole complete command-centre response patch owner, while V2 projection fan-out and shared invalidation retain their named owners there. None of these owners adds shared tokens, restyles another route, infers project truth, or moves specialist workflow ownership.
...
| Project Work Queue and legacy review | Work Queue route/components plus `teamQueue.ts`; admin review uses the bounded legacy-triage contract | Keep one row per project, server-owned precedence, durable command feedback, customer-contact-data exclusion, and one-project migration. Personal reminders remain separate. |

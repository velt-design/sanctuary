...
The desktop preview rail is a tabbed Result Inspector. A compact overview keeps the current or last-valid customer price, freshness, input-issue count, and quote readiness visible while Pricing, Materials, Labour, Workings, and Issues route the existing output owners. Pricing contains the full price, price-by-item, permitted actual-cost review, and admin cost-impact surfaces. Materials contains whole-job BOM quantities plus the existing admin/Advanced debug trace. Labour retains its existing admin and Advanced gates. Workings follows the selected module through Plan/Section drawings and Advanced structure outputs. Issues contains Quote Status and warnings. Tabs use the standard keyboard arrow, Home, and End behavior; inactive panels remain mounted so switching tabs does not discard local panel state. The tab strip becomes horizontally scrollable in a narrow preview rail, while the established stacked-layout page scrollbar remains authoritative below `1120px`. This is presentation-only: calculations, permissions, save behavior, quote mapping, and source-of-truth boundaries are unchanged.

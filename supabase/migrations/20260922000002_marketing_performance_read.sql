...
create function public.marketing_performance_read(p_start date, p_end date)
...
comment on function public.marketing_performance_read(date,date) is
  'Staff-only minimal enquiry cohort and current business evidence; Auckland dates, no tracking writes or provider results.';

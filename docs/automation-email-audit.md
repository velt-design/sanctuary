...
Forward marketing attribution starts with project audit events. New enquiries store compact UTM attribution, consent-filtered Google click identifiers, and—only after analytics consent—the first-party GA client ID plus the submission consent snapshot in `enquiry_requests.raw_payload.attribution`. Lifecycle events are `marketing.lead_submitted`, confirmed-only `marketing.site_visit_booked`, `marketing.quote_accepted`, `marketing.deposit_received`, and structured `marketing.project_lost`.

The browser sends the initial `generate_lead` through GTM. The four downstream event types enter `marketing_conversion_deliveries` from an `audit_events` trigger and are delivered by the marketing cron as GA4 `qualify_lead`, `quote_accepted`, `close_convert_lead`, or `close_unconvert_lead`. The first three downstream events and `generate_lead` are GA4 key events; the loss event is diagnostic only. The claim/complete RPCs are service-role-only and use bounded leases, attempts, and retry delays. The sender uses the originating visitor's consented GA client ID and projects an explicit analytics-safe payload: source, value/currency where authoritative, confirmed visit status, or a closed loss code. It does not send portal IDs, contact data, notes, campaign names, or provider response bodies. Missing consent/identity and events outside the 72-hour GA4 window are terminally skipped. This is not a direct Google Ads API or enhanced-conversions integration.

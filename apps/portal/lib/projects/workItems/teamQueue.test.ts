...
describe('composeProjectWorkQueue', () => {
  ...
  it('keeps the bounded Site Visit specialist visible in the shared queue', () => {
    const siteVisitCandidate = candidate();
    siteVisitCandidate.stage = 'site_visit';
    siteVisitCandidate.actions.specialistAction = {
      kind: 'specialist',
    ...
    const result = composeProjectWorkQueue({
      ...
      domainCandidates: [siteVisitCandidate],
      limit: 200,
    });
    ...
    expect(result[0]).toMatchObject({
      actionKind: 'specialist',
      title: 'Complete the site visit',
      sourceKey: `journey-site-visit:complete:${PROJECT_ID}`,
      actionLabel: 'Book or confirm site visit',
      href: `/staff/schedule?view=site-visits&project=${PROJECT_ID}`,
    });
  });
  ...
  it('sorts by operational group and honours the result limit', () => {
    ...
    expect(result.map((row) => row.projectId)).toEqual([ids.overdue, ids.today]);
  });
});
...
describe('getAuthoritativeProjectWorkQueue', () => {
  ...
  it('scopes durable and commercial reads to the requested project page', async () => {
    ...
    expect(modelBoundaryMocks.getProjectWorkModelV2Ids).toHaveBeenCalledWith(supabase, [PROJECT_UUID]);
  ...
  it('fails closed when the bounded queue inventory reaches its safe ceiling', async () => {
    ...
    await expect(getAuthoritativeProjectWorkQueue(supabase)).rejects.toThrow(/exceeded the safe 5000-row fetch limit/i);
    expect(range).toHaveBeenCalledTimes(5);
  });

  it.each(['marker', 'state'])('fails closed when portfolio %s completeness is missing', async (missingKind) => {
    const { rpc } = pagedRpc({
      completenessError: {
        code: 'P0001',
        message: `PROJECT_WORK_ROLLOUT_INCOMPLETE: project ${missingKind} is missing`,
      },
    });
    const supabase = {
      rpc,
  ...
  it('fails closed when the portfolio completeness payload is malformed', async () => {
    ...
    const rpc = vi.fn((name: string) =>
      name === 'staff_project_state_counts_v1' ? Promise.resolve({ data: {}, error: null }) : baseRpc(name),
    );
    const supabase = {
      rpc,
      from: vi.fn(() => emptyProjectsQuery()),
  ...
  it('loads active commercial candidates through the complete portfolio state owner', async () => {
    ...
    const from = vi.fn((table: string) => {
      ...
      if (table === 'project_confirmation_events') {
        return { select: confirmationSelect };
      }
  ...
  it('rejects an actionable row without command concurrency metadata', async () => {
    ...
    await expect(getAuthoritativeProjectWorkQueue(supabase)).rejects.toThrow(/missing command metadata/i);
  });
  ...
  it('propagates the RPC schema error for route-level availability mapping', async () => {
    ...
    await expect(getAuthoritativeProjectWorkQueue(supabase)).rejects.toMatchObject(error);
  });
});

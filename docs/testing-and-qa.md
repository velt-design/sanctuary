...
`npm run portal:enquiries:inactive` calls only the service-granted `project_enquiry_inactivity_report_v1()` read model. It reports active/waiting New and Contacted projects whose latest recorded project, note, email, task, staff command/event, confirmation, commercial, Site Visit, Schedule, design-package, running-job, file, or enquiry activity is more than 30 days old. It identifies future Waiting projects separately and emits an evidence fingerprint. The command never closes, archives, reassigns, or changes stage; review its exact output before authorizing a separate bulk `Lost - No response` operation. For an auditable review artifact, pass `--output-json=<absolute path outside the repository>`; the command refuses to overwrite an existing file or write customer evidence inside the repository.

...
| 2026-08-06 | Design Booklet PDF Authority     | Promoted | Treat a drawing sheet as one generated-PDF artifact: render that PDF page in the workbench, cache its exact bytes for download, invalidate it on source changes, and serialize writes to the stable project export path. Project-linked drafts use neutral media states; Toni assets remain fixture-only. |
...
Current guardrail: Generate the current complete PDF when a drawing page is selected, render that exact page with PDF.js, and download the same cached Blob. Invalidate the artifact after any draft or media-source change, serialize generations before using the stable project export, and block preparation while media is loading or failed. Every project-linked draft uses explicit neutral media sources, including older saved drafts; Toni defaults remain available only to standalone QA.
Promoted to: `docs/design-booklets.md`
Related docs/tests: `apps/portal/app/staff/design-booklets/useDesignBookletPdfArtifact.ts`; `apps/portal/app/staff/design-booklets/DesignBookletDrawingPdfPreview.tsx`; `apps/portal/app/staff/design-booklets/DesignBookletPreviewImage.test.tsx`; `apps/portal/app/staff/design-booklets/DesignBookletProjectPersistence.test.tsx`; `playwright/portal.design-booklet-workbench.spec.ts`

...
| Single-rectangle composites silently change their wall output because the substitution gate (`primitives.length > 1`) flips for some edge case. | Low | The existing Phase 3.2 byte-equivalence test (`houseFormGeometryInputComposition.test.ts`) pins single-rectangle behaviour. The 4a gate is `> 1` (strict), so single-rectangle composites can never enter the new branch. Test stays green = invariant holds. |

...
The direct mobile configurator entry preloads its default roof reference in the server response, with a phone-only media condition, and marks the displayed choice image high priority. This shortens the opening image request chain without changing the selected design or loading the mobile reference on desktop.

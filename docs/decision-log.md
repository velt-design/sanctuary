...
Current guardrail: use `readWorkbenchCapturedReproPayload(page)` / `attachWorkbenchCapturedReproPayload(testInfo, page)` for workbench browser evidence. Keep `CAPTURED_SANCTUARY_GEOMETRY_WORKBENCH_FIXTURES` empty until an exact copied staff-workbench payload is intentionally pasted through `buildCapturedSanctuaryGeometryWorkbenchFixture`.

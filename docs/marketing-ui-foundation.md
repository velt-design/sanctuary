...
The current comparison presents Sanctuary as an Auckland designer, builder and installer of bespoke fixed-roof architectural pergolas. It keeps the Warkworth project as the primary evidence, combines the design-manifesto content into one three-part approach, and provides distinct residential, custom, commercial and professional routes before the detailed form, roof and integration guidance. Acrylic, solid and combination roofing are separate from the aluminium or project-specific steel structure; timber is described only as a lining or ceiling treatment. The five-step process, three curated reviews, project-specific assurance rows, guide gateway and qualified final enquiry avoid general timing, warranty and performance promises.

Homepage interaction links expose stable `data-homepage-event` attributes. The route-local tracker records only the event name, V2 variant, destination and optional card label, and only after analytics consent. Hero, pathway, pergola-form, roof, project, guide and final-enquiry interactions therefore remain distinguishable without collecting project or customer details.

The comparison route owns its content and scoped tokens under `apps/marketing/app/home-v2/`. It is `noindex, nofollow` in both rendered metadata and the `X-Robots-Tag` response header, canonicalises to `/`, and remains absent from the sitemap, manifest, header, footer and public homepage links. Its title, description, Open Graph identity and WebSite/WebPage schema are ready for the later root migration, but staging indexability remains closed. Do not redirect, relink or replace `/` with this route without explicit approval. The established owners `apps/marketing/app/page.tsx`, `apps/marketing/app/home.css` and `apps/marketing/components/home/**` remain unchanged.
...
The Playwright lane checks the standalone catalogue, shared header, homepage, homepage comparison, every SEO programme route, the project collection and case studies, and the guide directory at desktop, compact desktop, tablet and mobile widths. It covers metadata, canonical/index state, unique identities, green accent, project and FAQ rendering, all ten directory destinations, horizontal overflow, mobile navigation, form attribution, sitemap inclusion, resolving internal links and visible schema parity. The projects suite checks all canonical records, loaded hero media, intentional missing-data omission, the seven-width responsive matrix, desktop filtering and keyboard navigation, sticky behavior, focus-managed mobile scroll locking, progressive technical detail, circular navigation and reduced motion. The homepage comparison suite additionally proves `/home-v2` is unlisted and noindex at both metadata and response-header layers, retains its live review and enquiry paths, renders the approved visitor, product, project, process, trust and guide structure, records consented CTA events without customer data, loads all imagery, preserves the Warkworth exterior gable apex with visible breathing room in both placements, uses the correct responsive header states and leaves the established homepage composition intact. The hero-navigation matrix proves every programme route has true desktop image/header overlap, no canvas gap at the fold, a viewport-centred Projects/Products gap, the guide hero split on that same centreline, transparent top and solid scrolled states, and the solid collapsed treatment at tablet and mobile widths.

...
The contact form keeps the existing `/api/enquiry` payload, browser-generated submission UUID across retries, professional attachment policy and metadata fallback, attribution fields, consent-gated conversion events, privacy link and direct contact routes. Required fields match the intake contract: project type, name and phone. Every visible field has a persistent label, first-error and result focus are explicit, server failures retain entered values, and a synchronous ref lock closes the double-click window before any asynchronous upload or request begins. Query values `residential`, `commercial` and `professional` remain server rendered, while unknown values leave the chooser open. Visible and generated contact content must not use em dashes.

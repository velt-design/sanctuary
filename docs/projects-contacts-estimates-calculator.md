...
- UI foundation proof is deliberately limited to Projects Index and the Project Detail shell/Overview details editor. The index now composes the shared operational header, search/filter controls, stage badges, loading and truthful empty/error/stale states without changing `useProjectsIndexData`, prefetch, archive scope, or optimistic mutations. Detail composes the shared detail header, stage tracker, controls, alert, and navigation-loss guard without changing lazy tab boundaries or `useProjectDetailsDraft` queue semantics.

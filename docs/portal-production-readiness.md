...
| Project command centre | Green | Stage 1 replaces the staff-facing Activity label and module with Overview while retaining the internal `activity` key. A server-owned staff read model now applies strict accepted > sent > draft > estimate selection, exact quote-source and stored-price ownership, explicit freshness/delivery/warning states, and access-ending cache clearing. The unsafe client fallback resolver/bar is retired. The 9-scenario browser matrix plus 390px check passed, as did 322 project tests, 9 fixture performance checks, typecheck, lint, isolated build, and unchanged bundle assertions. | Review the working-tree Stage 1 implementation. Do not start Stage 2 ownership or primary-action work without explicit approval; rerun authenticated smoke/performance when credentials are available. |

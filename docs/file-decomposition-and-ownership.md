...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | Calculator UI, module drawing surfaces, estimate state, and presentation helpers are still concentrated in a few client files, though pure calculator input defaults, normalization, and save-readiness helpers now live outside `CalculatorGridClient.tsx`. | Continue extracting cohesive calculator owners: deeper save orchestration, draft/session state, and remaining infill/blind editor state should move before adding new calculator workflows. | `npx vitest run apps/portal/app/staff/calculator`, `npm run test:portal:projects`, `npm run typecheck` |

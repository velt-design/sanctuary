...
Sanctuary AI contract changes should run `npm run test:ai`, `npm run packages:guard` and `npm run typecheck`. The focused suite covers strict V1 task/event, registry, approval, provenance, artifact, usage/evaluation and node parsing; exact approval and node invariants; unknown-field rejection; the no-runtime-dependency boundary; and static PR-AI-004 migration/security alignment. It performs no model call or production effect.

...
`buildDesignBookletRenderModel()` is the shared owner of page order, stable page keys, labels, numbering, and count for the browser preview and PDF. `contentLayouts.ts` owns the ten base content-page frames, typography presets, and reusable slot visibility in landscape-A4 points. `contentPresentation.ts` resolves the three curated framing variants, bounded role-specific typography scaling, and geometry-aware overflow warnings. The browser and PDF renderers consume that shared resolved contract directly, so layout switching and responsive preview scaling do not alter the printed composition. The drawing-layout definitions, image focal positions, title resolution, and fixed review copy are also shared.
...
- `apps/portal/app/staff/design-booklets/**`: workbench, page composer, instant shared-geometry A4 preview, ordered background media persistence, media display states, on-demand PDF download artifact owner, and route-owned styling.

...
Calculator configuration section order, Basic/Advanced visibility, and field-width presentation live outside `CalculatorGridClient` in the configuration-section model and form presenter. Field dependencies, validation IDs, and focus targets are unchanged. Consecutive ordinary sections share quiet sheets with subtle dividers, while Blinds and Infills remain separate configuration cards. Labels and input-like toggles use a consistent control rhythm, and full helper copy remains visible with quieter presentation. The responsive field grid deliberately renders three columns at the 1600px and 1024px browser checks, and two columns at 1366px and 768px, with one column on narrower mobile widths. Specialist Flashings, Blinds, and Infills controls use the full section width. The presentation-only Orientation diagram is omitted because Roof Length and Roof Span retain their directional helper text. The narrow command bar keeps Save prominent while retaining project/module identity, browser-draft explanation, freshness, and Basic/Advanced controls.

...
| 2026-07-18 | Calculator Commercial UI         | Active   | Cost-engine totals must be labelled as internal true cost, quote-stage sell values must be identified separately, and estimate saves must expose Preserve versus Reprice before any quote handoff.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Current guardrail: Cost-engine `cost_ex_gst` and `cost_inc_gst` values must be labelled internal true cost. Quote-stage sell values such as blinds must say customer price and remain excluded from pergola true cost. Existing-estimate Save must compare stored and Live costing and make Preserve versus Reprice explicit. Quote creation must use the exact saved estimate through the quote domain and must not occur until the user selects it.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/quotes-invoices-job-packs.md`
Related docs/tests: `apps/portal/app/staff/calculator/calculatorPricingComparison.test.ts`; `apps/portal/app/staff/calculator/calculatorEstimateSave.test.ts`; `apps/portal/app/staff/calculator/CalculatorSaveDialogs.test.tsx`; `playwright/portal.calculator.spec.ts`

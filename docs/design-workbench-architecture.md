...
13. ⏳ **Drop pergola `houseContext` wrapping (closes audit row 9).** Today `buildRawGeometryModuleInput` is per-pergola: each pergola module input wraps the house as context. After (12) lands the house has its own world position, so the geometry pipeline can iterate per object instead of per pergola. **Scope**: add per-family raw input builders (`buildRawHouseInput`, `buildRawPergolaInput`, `buildRawDeckInput`); change solve orchestration so the house solves first into a stable `HouseModel3D`, then pergolas/decks read the house as input rather than wrapping it. **Verification**: house-only and pergola-only projects solve cleanly; existing multi-pergola tests still pass; the `houseContext` field on `RawGeometryModuleInput` either retires or shrinks to a back-compat alias. **Unblocks**: deck/opening attachment to multiple houses, future per-object solving optimizations.
14. 🟡 **Move tool + project-wide undo (partial -- moves shipped, edge-drag undo deferred).** *Move tool*: pergola and deck bodies can now be dragged in plan view to translate `position.origin`. The previously-built `MoveTool.ts` and `commandBus.ts` (both standalone-tested but not wired) are now plumbed through `PlanViewport` -> `WorkbenchViewportHost` -> `DrawingWorkbench` -> `DesignWorkbenchEstimateClient`. The tool dispatcher chains EdgeDrag -> Move -> Select via `onPointerDownFallthrough`: an edge click starts a resize, a body click starts a translate, anything else falls through to selection. `MoveTool` got a new `onPointerDownFallthrough` config (mirroring `EdgeDragTool`'s pattern) so non-primary buttons bubble to `usePanZoom` and rejected primary clicks chain to the next tool. *Undo*: one CommandBus instance per `PlanViewport` mount, with ctrl-Z / ctrl-shift-Z (or cmd-Z / cmd-shift-Z) keyboard handlers that skip when an input/textarea is focused. Each move is a single Command with apply/invert symmetry; undo flips the delta and re-runs `commitMove`. *Commit pipeline*: `onCommitMove` reads the target's persisted `position.origin`, adds `request.delta` (in plan-projection mm), and writes via the existing atomic patch actions (`commitSharedPergolaEdgeDragResult` for pergolas, `commitSharedHouseDeckPatch` for decks). Rotation is *deferred* (needs local-frame bbox math when `rotationDeg != 0`). House-form move is *deferred* (needs more orchestration; pergola + deck cover the immediate user need). Openings stay anchored to walls and don't move via this tool. *Verification*: 17 `MoveTool` tests pass including 2 new ones for `onPointerDownFallthrough` (no shape -> fallthrough fires; non-primary button -> fallthrough does NOT fire so pan can claim the event); 643/645 portal tests pass (the 2 pre-existing `getScreenCTM` JSDOM failures are unchanged); typecheck clean. **Remaining**: extend undo to cover edge-drag commits (currently they persist directly, bypassing the CommandBus -- so a resize can't be undone via Ctrl-Z). The `EdgeDragCommit` -> `Command` adapter is the next slice; once shipped, "project-wide undo" is fully shipped and milestone 14 closes.
...
16. ⏳ **3D occlusion + hover sync.** *Occlusion*: deck-under-house geometry needs to be hidden in the 3D viewport (deck floor inside the house bounds shouldn't poke through the walls/roof). Approach: rely on the depth buffer with correct z-ordering; avoid CSG. *Hover sync*: hovering an object in plan highlights it in 3D, and vice versa. `selectionRouter.ts` already shares typed selection between surfaces; add a parallel hover state. Plan -> 3D hover requires `Geometry3DViewport` to subscribe to hover state and render a highlight; 3D -> plan hover requires the read-only 3D viewport to emit hover via `selectionRouter`'s hover lane. **Verification**: hovering a pergola in plan highlights its 3D body; the deck under a house wall is invisible in 3D from any normal viewing angle.

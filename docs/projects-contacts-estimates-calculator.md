...
- Legacy task toggles retain their existing optimistic/retry behavior only for legacy projects. Legacy task/manual-action/follow-up writers reject V2 projects.
- The existing Overview conditionally renders the V2 work command card/sidebar from one command-centre projection without changing the approved portal visual system. Blocked work is an exception rather than an enabled primary action. A shared Project Work cache owner keeps accepted command results coherent across command-centre, snapshot, summary, Work Queue, and Dashboard reads; both legacy and V2 controls pause whenever the owning read is refreshing, stale, refresh-failed, or disagrees with the other authoritative read about the work model.
...
- `ProjectTasksSidebarClient` remains the legacy-project task card; V2 projects use `ProjectWorkCommandCard` and the V2 work sidebar in the same existing composition.

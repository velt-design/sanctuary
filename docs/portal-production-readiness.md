...
| --- | --- | --- | --- |
| Portal tests | Green | `npm run portal:doctor:quick:log` passed on 2026-07-18 after the intentional server-only marketing-attribution service-role owner was added to the explicit boundary allowlist. The focused shell and projects suites also passed with 19 files/47 tests and 51 files/250 tests. | Keep the service-role owner explicit and rerun quick doctor after shared auth, persistence, or API changes. |
...
| Portal route bundle budgets | Yellow | The reusable analyzer now covers Schedule, Project Detail, Calculator, and Design Workbench and the three new routes have fixed fresh-build +5% ceilings. The original Schedule limits remain exact, but the 2026-07-18 fresh build measures 359.3 KiB lazy raw and 83.4 KiB lazy gzip against 351.6/82.0 KiB limits, so `npm run portal:bundle-budget` correctly fails rather than silently raising the budget. | Reduce the Schedule lazy output in its owner lane; do not ratchet this ceiling upward. Rerun build and the general gate from the same checkout. |
...
| Browser smoke | Yellow | The fixture-safe workbench performance journey passed. The broader no-auth drawing smoke currently has 3 failures because its tests expect `data-plan-render-source="geometry"` while the existing live canvas reports `geometry-canvas`; Wave 0 did not change that guarded workbench renderer. Authenticated calculator module-navigator verification passed locally at 1600px, 1366px, 1024px, and 768px using browser-draft-only mutations; the full provisioned scenario suite still requires a local/staging service-role scenario refresh. | Resolve the stale Plan render-source contract in the workbench owner lane, rerun fixture smoke, and run the provisioned calculator/authenticated performance gates in a compatible local/staging environment. |
...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, project picker, browser-draft persistence/session state, and module navigation modelling/presentation now live outside `CalculatorGridClient.tsx`. The client no longer owns draft hydration, local persistence effects, or module action policy, but it still owns the main form schema, deeper save orchestration, and most infill modal state. | Extract configuration-section definitions and issue-to-section mapping before adding more inline calculator workflow; run `npm run files:changed` before handoff. |
...
- [ ] `npm run portal:bundle-budget` passes after the same fresh portal build; Schedule currently exceeds its preserved lazy limits.
...
- [ ] `npm run test:portal:browser` passes for drawing/workbench browser smoke without silent full-suite skips; the existing `geometry` versus `geometry-canvas` assertion is open.

...
export type SidePanel={opening:string;kind:'acrylic'|'timber'|'aluminium';profile:string;edge:boolean;gap:number;customGap:boolean;frame:50|100;battens:boolean};
...
export function parseSidePanels(value:unknown):SidePanel[]|null {
  ...
  for(const p of value){
    ...
    seen.add(p.opening);result.push({opening:p.opening,kind:p.kind,profile:p.profile,edge:p.edge,gap:p.customGap?p.gap:faceWidth(p.profile,p.edge),customGap:p.customGap,frame:p.frame,battens:p.battens});
  }return result;
}
export function describeSidePanels(panels:SidePanel[]=[]){return panels.map(p=>{
  ...
  return `${label}: ${p.kind==='acrylic'?`full-height acrylic, ${p.frame} × 50 mm framing${p.battens?', horizontal timber battens '+slats:''}`:`horizontal ${p.kind} slats, ${slats}, matching perimeter angle`}`;
}).join('; ');}

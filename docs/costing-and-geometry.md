...
`SiteInputsV2` is the scene-derived costing input used by the workbench save-reprice path; it carries pergola costing data derived from `WorkbenchProjectModel` scene adjacency and delegates to the current pricing logic through the V2 cost-engine entrypoint. Marketing and other non-workbench shells remain on V1 inputs.

`CommercialDesignInputV1` is the separate commercial parity and rollout-readiness contract. It is downstream of solved geometry and site/commercial options; it must not become a parallel geometry model or a competing live pricing path. Saved estimate source metadata still defaults to `calculator_live`; `workbench_solved` becomes a saved estimate source only when the server-owned rollout gate allows it.
...
Host house identity is a geometry contract. Object-first callers pass the host form id through `RawGeometryModuleInput.houseContext.houseId`; normalization preserves it on `GeometryConfig.houseContext.houseId`; solvers stamp it onto the resulting host `HouseModel3D`. The legacy fallback id remains `host-house` only for callers that have not supplied an object-first house id, and portal code must not repair this with scene-retag bridges.

Object-first pergolas without persisted calculator modules use a portal runtime adapter only. `objectFirstPergolaSolveSources` may synthesize temporary `CalculatorModuleInputs` so the current per-module solve/render path can run, but those rows must not be written to `inputs.modules[]`. Rail-created freestanding pergolas use this same path: the authored truth is `objectFirst.pergolas[]`, with solver-valid defaults such as four posts for freestanding mono. This is a named coexistence bridge until the per-object solve loop consumes pergola objects directly.

Workbench project solving now enters geometry through `@sp/geometry solveProject` for eligible object-first host-house groups. The portal builds explicit persisted + transient solve sources, groups them by resolved host `HouseFormModel`, passes one `RawHouseInput` plus that group's raw pergola inputs to the package boundary, and then rehydrates the existing solved-module view contract. Internally `solveProject` still normalizes and solves each pergola and the raw pergolas still carry compatibility `houseContext`; future cleanup should shrink/delete that field rather than adding new portal-owned normalize/solve branches.

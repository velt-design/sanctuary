...
Migration `20260909000002_praxis_enquiry_identity_v1.sql` adds `praxis_reporting.enquiry_identities_v1` (an owner-rights security-barrier view) and `enquiry_identity_snapshot_v1` (SECURITY INVOKER). They join canonical `enquiry_requests.id/submission_id/created_at` to the immutable email intent and receipt, expose no personal content or frozen email hash, and grant no base/private access. `created_at` is explicitly the canonical intake-record time, not a browser event, update time or dispatch time. The union of the submission window and candidate-reference point matches is unique by enquiry ID; a 101st row is an overflow sentinel, never a complete partial page. Window counts exclude outside-window point matches. Missing intent, missing receipt and recorded accepted/failed/unknown remain distinct; provider acceptance is not delivery and verified RFC Message-ID stays null. This migration depends on `20260909000001_marketing_enquiry_email_correlation.sql`; it does not change intake, dispatch, receipt writes, retention or the existing broad core projection.
...
- Enquiry email correlation: private immutable `marketing_enquiry_email_intents` and append-only `marketing_enquiry_email_receipts`, owned by migration `20260909000001_marketing_enquiry_email_correlation.sql`. Only service-role execution of `marketing_enquiry_email_begin`, `marketing_enquiry_email_record`, and `marketing_enquiry_email_read` is granted; all app roles lack direct table access. One new intent commissions one request-bound send; missing receipt means unknown, not permission to retry. Stored identity/hash and provider API acceptance do not authenticate mailbox evidence or establish an RFC Message-ID. Evidence has no automatic purge and restricts deletion of linked enquiries. See the correlation section in `docs/automation-email-audit.md` for rollout, uncertainty and recovery limits.

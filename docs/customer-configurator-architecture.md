...
Cedar uses complete 2/4/6/9-light grids centred in actual rafter bays, with regular slope rows. Six lights offers rows of two or three where they fit. Gable totals are paired across slopes (2/4/6); odd nine-light layouts are unavailable. Acrylic regions are excluded. Preview grids require at least 600mm between lights, or 1000mm for nine; unavailable grids are hidden. Existing cedar quantities migrate to the nearest available grid. Rafter lights use Off/Low/Medium/High: low is one centred light on mirrored alternate rafters, medium two at 25/75% on those rafters, high two on every eligible rafter. Gable patterns select paired rafter rows along the ridge, mirrored onto both slopes. Pitched and gable edge rafters are excluded from spots but remain available for LED strips. Counts are derived and update with dimensions. Old quantity selections migrate to the closest preset. Editing opens with no tool selected; only explicit LED strips mode enables member taps.

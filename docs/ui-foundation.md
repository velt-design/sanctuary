...
The Commercial Invoices surface uses Foundation cards, metric groups, tables, badges, overflow actions, and focus-managed action dialogs. Its current-schedule summary is visually distinct from the complete invoice history; historical quote-version invoices remain labelled in the history rather than entering the current accepted quote metrics. Orange is reserved for the row's current primary action, while preview, payment evidence, and admin void actions live in the overflow or owned dialogs.

-- Prospective stage timing; no historical inference or business-row backfill.
alter table public.projects add column if not exists stage_changed_at timestamptz;
comment on column public.projects.stage_changed_at is
  'Database-recorded entry to the current stage. NULL means historical evidence is unavailable. Corrections to a different stage start a new recorded interval.';

create or replace function public.projects_capture_stage_changed_at()
returns trigger language plpgsql set search_path = public, pg_temp as $body$
begin
  if TG_OP = 'INSERT' then
    new.stage_changed_at := statement_timestamp();
  elsif upper(btrim(new.pipeline_stage::text)) is distinct from upper(btrim(old.pipeline_stage::text)) then
    new.stage_changed_at := statement_timestamp();
  else
...
$body$;
revoke all on function public.projects_capture_stage_changed_at() from public, anon, authenticated;
drop trigger if exists projects_capture_stage_changed_at on public.projects;
create trigger projects_capture_stage_changed_at before insert or update on public.projects
for each row execute function public.projects_capture_stage_changed_at();

create or replace function public.staff_projects_index_v4(
  ...
  p_due_project_ids uuid[] default null
)
returns jsonb
language plpgsql
stable
...
declare
  v_result jsonb;
begin
  if cardinality(p_due_project_ids) > 5000 then
    raise exception 'Project work order exceeds the bounded reader limit';
  end if;
  if exists (
    select 1
    ...
    order by
      ...
      case
        when input.sort_key = 'next_action_asc'
          then array_position(p_due_project_ids, filtered.id)
      end asc nulls last,
      case
        when input.sort_key = 'next_action_desc'
          then array_position(p_due_project_ids, filtered.id)
      end desc nulls last,
      case when input.sort_key = 'stage_oldest' then filtered.stage_changed_at end asc nulls last,
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'stage_changed_at', ordered.stage_changed_at,
        ...
        order by
          ...
          case
            when input.sort_key = 'next_action_asc'
              then array_position(p_due_project_ids, ordered.id)
          end asc nulls last,
          case
            when input.sort_key = 'next_action_desc'
              then array_position(p_due_project_ids, ordered.id)
          end desc nulls last,
          case when input.sort_key = 'stage_oldest' then ordered.stage_changed_at end asc nulls last,
...
revoke all on function public.staff_projects_index_v4(
  text,text,text,text,date,integer,integer,text,text,text[],text,uuid[]
) from public, anon, authenticated, service_role;
grant execute on function public.staff_projects_index_v4(
  text,text,text,text,date,integer,integer,text,text,text[],text,uuid[]
) to authenticated, service_role;

comment on function public.staff_projects_index_v4(
  text,text,text,text,date,integer,integer,text,text,text[],text,uuid[]
) is

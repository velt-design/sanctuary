...
- Started the file-decomposition lane with the largest calculator hotspot by extracting pure calculator input/default/normalization helpers from `CalculatorGridClient.tsx` into a focused calculator helper module. `npx vitest run apps/portal/app/staff/calculator` passed with 11 files and 114 tests; broad typecheck is still blocked by unrelated dirty design-workbench type errors in `apps/portal/lib/drawings/state/workbenchSolvedModel.ts` and `apps/portal/lib/drawings/views/plan/buildPlanViewModel.test.ts`.

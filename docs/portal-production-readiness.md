...
| Typecheck | Green | `npm run typecheck` passed on 2026-05-03 after marketing public-token route test mocks were aligned to the current Buffer, Uint8Array, and quote-accept helper contracts; focused marketing and portal `tsc --noEmit --incremental false` checks also passed. | Keep typecheck in quick doctor and CI. |
...
- Continued calculator decomposition by extracting pure save-readiness helpers, estimate-target resolution, redirect builders, and design-request tier labelling into `calculatorSaveWorkflow.ts`. `CalculatorGridClient.tsx` is down to 7065 lines from 7067 at HEAD; `npx vitest run apps/portal/app/staff/calculator` passed with 12 files and 120 tests, and `npm run test:portal:projects` passed with 40 files and 205 tests. Broad `typecheck` and `test:portal:log` are currently blocked by unrelated dirty Design Workbench changes in `WorkbenchViewportHost.tsx` / import-guard coverage, not by this calculator slice.

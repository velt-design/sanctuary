...
Run the Overview retirement, snapshot, V2 projection/controller, shared cache, component, V2 command-route, and queue boundary:
...
The retirement contract is structural as well as visual: `ProjectPageSnapshot` and its cache placeholders contain no legacy task collection; snapshot reads do not query `project_task_checks` or task-only related rows; and the old project-task API, sidebar, mutation state, local-first patch helper, QA fixture block, and task-specific browser journeys stay absent. A legacy Overview renders one read-only retired Project Work card with no stage rows, task controls, Call actions, or Site Visit actions. V2 work remains actionable only through its server-owned commands. Use `npm run test:portal:command-centre:read-only-auth` as the authenticated integrated proof and do not substitute a task-toggle or customer-data mutation journey.
...
Despite its historical filename, the current spec covers Schedule only: Board at 1440/1280/1024/768/390, Gantt, the retained direct Site Visits compatibility route, Schedule and Site Visit dialogs, 720x500 at 200% zoom, document overflow, mobile targets, focus return, reduced motion, and browser/runtime evidence. It opens forms and dialogs but does not save, drag, delete, or unschedule anything; it does not open Project Overview or exercise retired project-task UI.

...
Provider-package, adapter, and webhook tests use injected/mocked transport and signed fixtures only. A real commercial email, shared database migration, or production webhook is never a repository test prerequisite. JOB-03 local provider, integration, worker, contract, typecheck, lint, security, and production-build gates pass; dedicated seven-migration CI evidence remains pending until the checkpoint is pushed.

...
- `GET /api/staff/v1/projects/[projectId]/invoice-schedule` requires `requireStaffSession` and exposes reconciled job totals to all staff; payment-entry detail is added only when the resolved role is admin. `POST /api/admin/projects/[projectId]/invoices`, `POST /api/admin/projects/[projectId]/payments`, `POST /api/admin/payments/[paymentId]/allocations`, `POST /api/admin/payments/[paymentId]/reverse`, `POST /api/admin/invoices/[invoiceId]/paid`, `POST /api/admin/invoices/[invoiceId]/void`, `POST /api/admin/quotes/[quoteVersionId]/supersede`, and `DELETE /api/quotes/[quoteVersionId]` require `requireAdminSession`. They create one whole invoice, reconcile actual job payments, mark or void a whole invoice, retire a quote, or delete only an unsent current draft. Non-admin callers receive `401`/`403`; ordinary staff invoice list/preview/send and quote read/edit/delivery routes remain staff-authenticated.

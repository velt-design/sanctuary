...
Current guardrail: Keep the summary derived only from the authenticated user's QueryClient and canonical project/contact query keys. A `401`, `403`, or `404` must hide cached project data; network/server failure may retain it with Retry. Activity and Emails must say they are updating until the fresh snapshot arrives. Do not restore server-side snapshot blocking or automatic row fan-out.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/testing-and-qa.md`

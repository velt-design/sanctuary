...
`npm run test:portal:browser` includes the gated, customer-data-free Project Command Centre fixture at `/qa/project-command-centre-fixture`. Its matrix covers a new lead with a draft estimate, no current design, standard estimate, multiple estimates, sent revision, accepted quote plus a newer unrelated estimate, declined quote, missing exact source, missing quote price, and a 390px no-horizontal-overflow check. The fixture renders the production current-design/commercial card and requires `ENABLE_PORTAL_QA_FIXTURES=1`; the standard fixture harness supplies that flag.

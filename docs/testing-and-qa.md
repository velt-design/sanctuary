...
- Website autoresponder rendering and staging-preview tests also use mocked provider transport. `npm run emails:preview -- enquiry-variants` writes the five customer HTML/plain-text fixtures under `tmp/email-previews`; it never sends email. A real inbox delivery is a deliberate manual staging action from the authenticated `/staff/email-previews` page after the preview-only Vercel variables are configured.

...
- `GET`/`POST /api/admin/projects/[projectId]/invoices`, `POST /api/admin/invoices/[invoiceId]/paid`, `POST /api/admin/invoices/[invoiceId]/void`, `POST /api/admin/quotes/[quoteVersionId]/supersede`, and `DELETE /api/quotes/[quoteVersionId]` require `requireAdminSession`. They read the current accepted quote schedule, create one whole term invoice, mark one whole open invoice paid, void one whole open invoice with a reason, immediately retire a sent or accepted quote as superseded, or permanently delete only an unsent current draft quote. Non-admin callers receive `401`/`403`; ordinary staff invoice list/preview/send and quote read/edit/delivery routes remain staff-authenticated.

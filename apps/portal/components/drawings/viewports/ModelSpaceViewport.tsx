...
export default function ModelSpaceViewport({
  ...
  const clearDrawPopoverPosition = useCallback(() => {
    setDrawPopoverPosition((current) => (current === null ? current : null));
  }, []);
  ...
  useEffect(() => {
    if (drawOutlineViewModel.isActive && drawOutlinePopoverAnchorPointCount > 0) return;
    clearDrawPopoverPosition();
  }, [clearDrawPopoverPosition, drawOutlinePopoverAnchorPointCount, drawOutlineViewModel.isActive]);

  useEffect(() => {
    if (!drawOutlineViewModel.isActive) return;

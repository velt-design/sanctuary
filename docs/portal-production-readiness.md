...
| --- | --- | --- | --- |
| Portal tests | Green | Current-head Portal Quality run `29682571695` passed repository typecheck/lint, the full portal Vitest suite, production build, all bundle budgets, security audit, fixture browser/performance, and authenticated smoke. Focused calculator coverage passed 50 files/310 tests plus 11 intentional skips; Projects passed 58 files/287 tests; workbench passed 55 files/475 tests. | Keep the service-role owner explicit and rerun quick doctor after shared auth, persistence, or API changes. |
...
| Portal route bundle budgets | Green | The reusable analyser passes Schedule, Projects Index, Contacts Index, Project Detail, Calculator, and Design Workbench from one fresh build and detects Turbopack lazy groups honestly. Project Detail is 690.4/197.9 KiB raw/gzip initial plus 1,694.8/353.5 KiB lazy, below its preserved combined cap. Design Workbench is 1,582.5/385.8 KiB initial plus 942.8/247.1 KiB lazy under a redistributed, not increased, cap. | Keep every per-boundary gate blocking and ratchet limits down from fresh evidence; never hide nested lazy bytes or increase the combined allowances automatically. |
| Portal performance evidence | Yellow | Current-head run `29682571695` completed exactly five authenticated production repetitions. Warm Projects/Contacts/project opening, calculator feedback, and fixture workbench interactions are green. Cold Project Detail useful-content p75 is 2,749 ms against the 2,699 ms 10%-regression guard derived from pre-Slice run `29681955081`; one cold run also observed a 55 ms long task. | Keep every ceiling locked. Remove the remaining repeated authenticated reads at the reviewed request boundary, then rerun exactly five repetitions; do not round away the 50 ms gap. |
...
| PROJECT-01 | Yellow | Current warm project opening is green at 32 ms feedback / 47 ms useful-content p75 with one canonical snapshot query and intent-only prefetch. The isolated pre-Slice cold p75 is 2,454 ms, so the 10% guard is 2,699 ms; current-head cold p75 is 2,749 ms, 50 ms over. Repeated access/user reads in the page/API/snapshot chain are the next reviewed target. |
| PROJECT-02 | Green | Activity is the initial lightweight workflow. Emails, Estimates, Quotes, Invoices, Job Packs, and responsive Details are isolated code boundaries with exact intent preloading and truthful snapshot-dependent states. Desktop Details loads only when its rail is rendered; narrow Details preloads on hover/focus/touch/pointer intent. Focused project and workbench coverage enforces the split. |
| BUNDLE-01 | Green | A fresh build passes all six route gates. Project Detail is 2,442,452 raw / 564,668 gzip combined, below the preserved 3,014,656 / 757,760 cap. Three/React Three Fiber is isolated behind exact `3D Review` intent and counted by Design Workbench, whose split allowance does not increase its previous 2,681,856 / 671,744 cap. |
| CALC-01 | Green | `useCalculatorCostingRequest` owns debounce, abort, newest-result protection, last-valid continuity, and errors without moving costing authority into the browser. Current-head five-run feedback is 35/36/36 ms p50/p75/p95, fresh result is 1,907/1,909/1,909 ms, and no overlay or long task was observed. Focused calculator coverage passed 310 tests. |
| WB-01 | Green | Workbench clients memoize one solved base by draft/project identity; selection, visibility, and viewport changes reuse that solved-model object while draft changes still solve. Focused coverage proves identity reuse. Current-head fixture evidence measured selection at 87/126 ms feedback/useful and Plan-to-3D at 93/97 ms, with no request, overlay, or long task. |
...
- Kept Activity initial, isolated responsive Details and the remaining project workflows, and moved the workbench's Three-based viewport behind exact `3D Review` intent. A fresh build restores Project Detail below its original combined cap while the Design Workbench gate accounts for the deferred 3D code without increasing its previous allowance.
- Extracted the calculator costing request lifecycle so obsolete responses cannot replace newer results; current-head p75 visible feedback is 36 ms and server costing remains authoritative.
- Memoized the workbench solved base by draft/project identity so UI-only selection, visibility, and viewport changes reuse geometry. Current-head fixture selection and Plan-to-3D useful feedback are 126 ms and 97 ms respectively.
- Locked the isolated cold Project Detail comparison: 2,454 ms pre-Slice p75 gives a 2,699 ms guard, while current head is 2,749 ms. PROJECT-01 remains Yellow rather than relaxing the guard.

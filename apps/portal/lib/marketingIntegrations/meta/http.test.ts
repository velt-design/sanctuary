...
vi.mock('./store', async original => ({...await original<typeof import('./store')>(),metaStore:vi.fn()}));
import { sanctuaryMetaResponse } from './http';
import { MetaStoreFailure } from './store';
...
it.each(['rpc_complete','parse_complete','delegation_complete','parse_report','rpc_delete'] as const)('logs only fixed %s diagnostics and keeps the public error unchanged',async stage=>{
 const f=fixture(),original=f.command.getMockImplementation()!;
 if(stage==='rpc_complete'||stage==='rpc_delete')f.command.mockImplementation(async action=>{
  if(action===(stage==='rpc_complete'?'complete':'delete'))throw new MetaStoreFailure('42501');return original(action);
 });
 if(stage==='parse_complete')f.command.mockImplementation(async action=>action==='complete'?{private:'private-secret-body'}:original(action));
 if(stage==='delegation_complete')f.dependencies.delegation=()=>vi.fn().mockResolvedValueOnce(undefined).mockRejectedValue(new Error('private-secret-body'));
 if(stage==='parse_report')f.dependencies.read.mockResolvedValue({...f.report,currency:'private-secret-body'});
 const r=await sanctuaryMetaResponse(stage==='rpc_delete'?f.request('action=delete',{},'DELETE'):f.request(),f.dependencies);
 expect(r.status).toBe(503);expect(await r.json()).toEqual({error:'sanctuary_meta_unavailable'});
 expect(console.warn).toHaveBeenCalledExactlyOnceWith('sanctuary_meta_failure',{action:stage==='rpc_delete'?'delete':'refresh',stage,sqlState:stage.startsWith('rpc_')?'42501':null});
 expect(JSON.stringify(vi.mocked(console.warn).mock.calls)).not.toMatch(/private-secret-body|10000000|aaaaaa|stack|Bearer/);
});

...
owner of final counts and fit. The large night view leads Off / Subtle / Brighter
choices with actual light counts. Customise lighting retains the full existing
editor, including individual ceiling lights and LED strips.

The mobile journey owns one JourneyNightPresentation timeline across mounted
views; child previews inherit it without clearing the palette when replaced.
Desktop views retain their local owner. Reduced motion settles immediately.
useJourneyPosition remembers each view's scroll position for return visits and
focuses its heading without scrolling; a new design clears those positions.
Mobile review places the estimate summary before Enquire / Share and Edit,
followed by the always-open breakdown. PublishedPriceDisplay and ReviewPriceDisplay
support separate summary/detail presentation around a stable journey-owned action slot.
Loading, unavailable and tailored-price responses never remount Share or Enquire;
desktop keeps the combined presentation. A stable polite live region announces
mobile price/status changes, with the review actions outside that region. Existing
pricing qualifications and individual allowance details remain visible.
MobileSideLooks leads mobile page 3 with the actual model and previous/next chevrons, horizontal swipe and keyboard navigation. mobileSidePresets builds complete configurations from actual front/left/right openings, validates them through sideTreatment and omits incompatible blind layouts. Only a deliberate preset choice replaces side settings. useSideLookComparison retains the latest custom side combination as a comparison bookmark while browsing, switching tabs and returning from Review; the carousel and Return to your combination restore only sides, never older roof or lighting choices. Its geometry-scoped bookmark clears on a different roof/opening geometry or new design. The active design remains durable through the existing draft store; the comparison bookmark is session UI state. Presets use established blind and timber defaults and never change roof, size or lighting. The geometry currently has no rear-screen opening, so copy names front and sides explicitly. The model remains at a steady overview while browsing. Customise sides keeps a smaller model pinned above the controls. MobileSideChoices offers Left/Front/Right multi-selection and all five treatments directly beneath it; individual-opening checkboxes and existing screen/blind finishes remain in one disclosure. Same-kind customisations and unselected openings survive grouped edits. The duplicate plan picker is retired. The edit model supports orbit but does not change editing targets on a mesh tap. Choosing an individual finish also selects that opening for treatment; changing the grouped targets clears the individual highlight. The one-sided timber preset can mirror left/right and follows the existing 90 x 39 mm edge-out, 100 mm clear-gap defaults. Side overview framing leaves space beside the product and chevrons retain large transparent touch areas without large opaque plates. Sides and Lighting share page 3 as tabs, without Apply or confirmation subpages. Starting a new design clears side selection/errors and returns to the Sides tab.
...
Size, in that order with Attached/Freestanding first. MobileDesignJourney owns gable Parallel/Extending inside optional roof details,
without reserving empty space on other shapes. Size uses the simple architectural footprint;
page 4 is Review, with the daylight model available through Explore your design.
...
Four-page mobile refinement (19 September 2026): mobileJourneyState orders Roof, Size, Extras, Review. Finished remains an optional model subview; saved Finished progress migrates to Review. Stored shape progress migrates to Roof; retired Explore progress migrates to Extras. Existing design storage and immutable share contracts are unchanged. MobileRoofChoice combines shape/material rows with nine matched references; six solid/combination variants are in public/images/configurator/roof-*-v1.webp. The actual model owns customised dimensions and ridge orientation. Main choice controls/visual/next action fit at 360–430px by750px; advanced options, long price detail and enquiry fields may scroll. MobileDesignFinish pairs white Edit design with Share design above a full-width black Enquire action, omitting the secondary Copy link button while preserving mounted form state, price details, staff return, cancellation and retry behavior.
Ground/Elevated and gable orientation are explicitly exempt from above-fold placement (owner clarification19September). Orientation is disclosed under roof details; Ground/Elevated remains below attachment choices with safe scrolling on shorter screens.

Mobile review editing: MobileDesignEdit owns a concise four-section chooser and current-selection summaries. MobileDesignJourney owns the transient edit-menu and return-to-review navigation; every editor and the model can return directly without replaying steps. All sections returns to the chooser, including advanced connection options. MobileDesignFinish stays mounted while hidden so inline enquiry fields survive edits. This adds no persisted design field or desktop navigation change.
...
Mobile 3D presentation (September 2026): the enhanced renderer is enabled at widths up to720px. Desktop retains its default renderer; development render=studio remains available for comparison. StudioTreatment scopes material changes. StudioQuality uses demand rendering, lowers pixel density during orbit, restores capped density after release, and bakes directional shadows after edits. Sustained slow orbit reduces background foliage/detail while retaining the product. No new remote assets or continuous animation are required. Roof profiles and all priced product geometry remain solver-owned. Steel uses a30-degree normal crease threshold and matte finish; fine aliasing can remain at shallow angles and low pixel density.

Earlier choices retain clean product views. MobileDesignPortrait and optional Explore request the richer review setting: reference paving, recessed glazing, wall finish and full-size furniture. The illustration is labelled, unpriced and excluded from saved design intent. Paving follows attached terrace geometry. Foreground foliage fades when it obstructs the product. Review portraits fit the whole product; Explore fits the complete product with a size-aware opening angle and allows user zoom/orbit. Contact cues and fabric/stone/glazing detail are procedural.

studioFurnitureLayout ranks full-size furniture arrangements by useful seating, mixed uses and composition rather than taking the first template that fits. studioFurnitureCatalog owns setting envelopes: L lounge, generous social lounge, compact sofa groups, six-place dining, four-place dining with a bench, bistro and shallow bench settings. Long, shallow spaces can show lounge and dining side by side; narrow, deep spaces can arrange them in sequence. A 6 x 3 m footprint normally shows a compact sofa/chair/table group plus four dining places. A 2400 mm sofa supports the larger social and shallow sofa settings. Very shallow wide footprints use a four-place group of full-size benches and a square side table, or two groups where width permits. Casual bench places rank below proper lounge/dining places, with an additional preference for comfortable full settings rather than maximising headcount alone. Furniture is never scaled down; genuinely tight footprints remain clear.

Owner decision: mobile freestanding designs use Ground only. Choosing Freestanding resets Elevated to Ground and disables Elevated in both Size and advanced details. Attached re-enables it. The mobile journey reconciles resumed/shared freestanding elevated designs to Ground. The proposed raised freestanding setting was removed; desktop rules and product geometry are unchanged.

Mobile extras clarity: preset browsing includes a Left / Front / Right treatment summary derived from actual openings and selected panels/blinds. Mixed custom faces remain labelled Mixed rather than being represented as a complete preset. Lighting presets share one high-contrast segmented row with geometry-derived fitting counts, followed by the evening model and selected rafter/downlight/strip counts. Detailed controls remain available; switching a custom strip/ceiling layout retains the replacement warning. MobileExtrasEstimate displays only the current server-priced design total including GST, clears during recalculation, and handles unavailable/custom pricing without inventing accessory allowances. It adds no requests or pricing rules. Studio night fill preserves visible structure around the lights; desktop baseline illumination is unchanged. Review portraits use a taller responsive frame and a more frontal camera angle, retain whole-product fitting and remain static after capture. Enquire/Share stay visible on short mobile screens. Focused browser coverage includes delayed/failed pricing, actual face summaries, fitting counts, editing and above-fold controls.

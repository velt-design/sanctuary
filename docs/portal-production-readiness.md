...
- [x] `npm run test:jobs` passes the last pushed JOB-01/JOB-02 TypeScript, static migration, and repository-security checks at 5 files/86 tests.
- [x] `npm run test:worker`, `npm run typecheck:worker`, `npm run build:worker`, and the built CLI smoke pass locally; worker coverage is 9 files/76 tests.
...
- [ ] JOB-03 `npm run test:email-provider`, updated `npm run test:jobs`/`npm run test:worker`, adapter/webhook/repository tests, typecheck/lint/build, and seven-migration real-PGMQ matrix pass at the checkpoint commit.
...
- [ ] All seven current JOB-01/JOB-02/JOB-03 migrations have passed the isolated database contract and normal deployment review before any producer, active worker, or rollout is enabled.
...
3. Preserve JOB-02's green six-migration and worker-container CI signal, complete JOB-03's local and seven-migration checkpoint evidence, then continue JOB-04 through JOB-08 as repository work only; require shared-environment deployment review before enabling any producer or active worker rollout.

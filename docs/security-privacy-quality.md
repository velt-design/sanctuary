...
The durable foundation defines one logged PGMQ queue, a durable job ledger, a private frozen-payload table, append-only events, effect checkpoints, worker heartbeats, and service-role-only security-definer RPCs. JOB-02 adds a dark-by-default Node worker with strict response validation, safe structured logs, cached health output, bounded execution, and an RPC-only Supabase adapter. JOB-03 adds the Node-only `@sp/email-provider` contract, durable email-effect coordinator, private append-only provider receipts, and signed provider-acceptance reconciliation. PR-AI-007 adds one deterministic, effect-free synthetic handler and service-only producer; it does not enable deployment, OpenClaw, a model/provider, external communications, customer/project mutation, or a commercial handler. JOB-04 through JOB-08 remain pending.
...
PR-AI-007 links each eligible task to exactly one durable `ai_synthetic_v1` job. Its private worker payload has exactly four fields: contract version, task ID, PostgreSQL-derived input snapshot hash, and fixed fixture key. Enqueue is service-role-only, replay is exact, linked task-only cancellation is rejected to prevent task/job divergence, and task state follows the durable job. A successful job is accepted only when its exact bounded result matches the frozen task fixture; the same database transaction then records one zero-unit, zero-cost `synthetic.mock` usage row and one deterministic passing evaluation before the task becomes evaluated. Invalid or mismatched completion rolls back atomically. The three linkage/evidence tables are append-only and staff reads remain parent-task RLS scoped. The handler has no effect checkpoint, external network, provider, model, OpenClaw, customer, project, communication, or business-write capability.
...
- Every pull request runs Trivy against the repository for high/critical secret and configuration findings. The background-worker gate also scans the built container for high/critical fixed operating-system vulnerabilities and embedded secrets; JavaScript dependency findings remain owned by the blocking npm audits.
...
- The September 2026 dependency repair requires Next.js `^16.3.4` across the workspace and both apps, Sharp `0.35.4` in every direct declaration and the root override, and a patched `baseline-browser-mapping` resolution (current lock: `2.11.21`). Next `16.3.3` is the fixed floor for the [Windows](https://github.com/vercel/next.js/security/advisories/GHSA-p293-qw3h-jr36) and [AVIF](https://github.com/vercel/next.js/security/advisories/GHSA-2xp9-vwfh-vxw4) advisories; [Next 16.3.4](https://github.com/vercel/next.js/releases/tag/v16.3.4) restores AVIF optimization with patched Sharp. [Sharp 0.35.4](https://github.com/lovell/sharp/security/advisories/GHSA-rgj7-g3m4-5g8c) fixes the bundled image-library vulnerabilities. Keep Sharp's Linux addon/libvips packages in the production graph and retain the booklet lazy-loader/tracing checks. Validate actual native image processing, local Next AVIF-input/JPEG-output paths, both app builds and both audit gates; Windows proof does not establish a deployed Linux function's behavior. No hosting, runtime credentials or production rollout is authorized by a dependency PR.
...
Mobile configured-design Review adds closed `design_enquiry_open`, `design_share_open`, `design_share` (native-share or clipboard success), and `design_share_link_ready` (manual-copy recovery) events through the same consent/source/staff gate. Native cancellation records only the user's open intent, never completion. Payloads retain only the existing closed funnel metadata; no design URL, dimensions, contact fields or free text. `contact_start/success/error` still describe actual valid submission attempts/outcomes, not form opening. Test submissions and analytics verification must intercept enquiry/vendor traffic rather than create real leads or conversions.
The configured-design tracker stays mounted above the responsive layout switch, so rotating between mobile and desktop does not restart the funnel or duplicate review events.

...
- The shared staff header adds the same global Projects/Contacts search to Dashboard, Project and Contact list/detail/create routes, and the other adopted operational routes named in `docs/ui-foundation.md`. Global search is an authenticated server read and remains separate from route-local filtering: `View all matching projects` hands `q` to the existing Projects filter, while `View all matching contacts` initializes the existing Contacts filter. Search-result navigation uses the shared non-blocking progress owner and clears its query after route commit. The Projects-index instant view releases when canonical route children settle, when the observed pathname diverges, or when the shared transition targets a different pathname; Project A therefore cannot mask a searched Project B. Search results never bypass the normal Project/Contact route, permissions, cache, or local-first owners.

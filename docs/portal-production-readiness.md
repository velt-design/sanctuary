...
| Docs and routing | Green | The 2026-07-20 JOB-02 docs pass completed `npm run docs:guard`, `npm run text:mojibake`, `npm run docs:impact`, `npm run docs:navigation`, and `npm run docs:readiness`. Guard, mojibake, and impact were clean; navigation remained advisory with the pre-existing plan-doc cues reported separately, while the durable-job canonical docs have routing cues. | Keep this tracker and owner docs current; refresh readiness when process signals change or the tracker ages past 7 days. |
...
| Durable background jobs | Yellow | JOB-01 owns the durable ledger, private payload, logged PGMQ queue, effect history, and lease-fenced RPC foundation. JOB-02 adds the sixth runtime-projection migration and a dark-by-default Node 22 worker with RPC-only access, strict wire parsing, lease/visibility heartbeats, bounded concurrency and retry, safe shutdown, metrics, health, and one-shot/drain/reconcile modes. Local evidence passes `npm run test:jobs` at 5 files/86 tests and `npm run test:worker` at 9 files/76 tests, plus full typecheck/lint, worker build/CLI smoke, architecture/security, and dead-code gates. | Background Jobs [run 29713940507](https://github.com/velt-design/sanctuary/actions/runs/29713940507) passes the six-migration PostgreSQL 18/PGMQ 1.10.0 and Supabase PostgreSQL 17/PGMQ 1.5.1 matrix plus worker typecheck, tests, lint, production build, built CLI, strict service-role guard, and non-root container build. The lane remains Yellow because JOB-03 through JOB-08 are pending. No migration was applied to a shared environment, no worker or producer was deployed, and no commercial handler is enabled. |
...
| JOB-03 through JOB-08 | Yellow | Pending. Provider reconciliation, workflow producer/handler migration, staff/admin job experience, and controlled rollout have no completion evidence yet. Existing quote, invoice, job-pack, automation, and email owners remain authoritative. |
...
- [x] `npm run test:jobs` passes the JOB-01/JOB-02 TypeScript, static migration, and repository-security checks at 5 files/86 tests.
...
- [ ] All current JOB-01/JOB-02 migrations have passed the isolated database contract and normal deployment review before any producer, active worker, or rollout is enabled.
...
3. Preserve JOB-02's green six-migration and worker-container CI signal while implementing JOB-03 through JOB-08 as repository work only; require shared-environment deployment review before enabling any producer or active worker rollout.
...
| Durable background jobs | `apps/worker`, `packages/jobs`, the six JOB-01/JOB-02 migrations and tests, isolated database harness, and Background Jobs CI workflow. | Docs tracker updates and domain lanes that do not yet migrate a producer or handler. | Existing quote/invoice/job-pack/email/automation behavior until a named later JOB explicitly owns that integration. |

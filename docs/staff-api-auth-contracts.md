...
- `GET /api/integrations/praxis/v1/context` and `GET /api/integrations/praxis/v1/health` are server-to-server, default-dark Praxis reporting routes. They require a dedicated bearer token plus exact source-key, connection-ID, and environment headers; configured values must also match the database-owned reporting identity. The adapter connects only as the dedicated reporting LOGIN, proves its read-only default, exact direct/transitive role membership, absence of forbidden object and security-definer capability, then executes bounded read-only transactions with statement and lock timeouts. Health performs a real bounded projection probe. Context accepts only the closed 12-resource enum, a 1-100 limit, optional project/change filters, and a source-bound signed keyset cursor. Responses are versioned, recursively sanitized where nested source JSON is approved, `private, no-store`, and contain stable record version/freshness evidence; diagnostics contain no body or business values. Mutation methods are not exported. The routes do not use staff cookies, Supabase service role, browser credentials, or any business write path.

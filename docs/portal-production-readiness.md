...
| Durable background jobs | Yellow | JOB-01 defines `@sp/jobs`, one logged `portal_background_jobs` PGMQ queue, a durable ledger, private immutable payloads, lease-fenced service-role RPCs, append-only state events, effect checkpoints, recovery/repair, and an isolated database harness. `npm run test:jobs` passed locally with 3 files and 25 tests. | `npm run test:jobs:db` was attempted but stopped at Docker readiness with `spawnSync docker ENOENT`; no SQL executed. Require a successful Background Jobs CI database contract before treating the migrations as executable; do not deploy or roll out a worker from this signal. JOB-02 through JOB-08 remain pending. |
...
| Item | Status | Evidence and remaining work |
| --------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| JOB-01 | Yellow | Foundation is present in this change: `@sp/jobs` owns kinds and transition policy; four forward migrations own the logged PGMQ queue, ledger, immutable private payloads, state/effect history, random lease tokens, service-role RPC lifecycle, reconciliation, and safe inspection. `npm run test:jobs` passed 3 files and 25 tests. The isolated harness uses a test-only minimal bootstrap plus only the four JOB-01 migrations because historical ordered migrations are not independently bootstrappable. Its local attempt stopped before container startup with `spawnSync docker ENOENT`, so no SQL ran; CI success and deployment are still unproven. |
...
- Recorded the JOB-01 durable-job foundation without claiming rollout: `@sp/jobs`, the logged minimal PGMQ queue, durable/private/state/effect/lease RPC boundary, static/unit checks, test-only database bootstrap, Docker harness, and dedicated CI workflow are present. `npm run test:jobs` passed with 3 files and 25 tests. Docker, host `psql`, and the Supabase CLI are unavailable locally; `npm run test:jobs:db` stopped at Docker readiness with `spawnSync docker ENOENT`, so no container started and no SQL ran. The full historical migration chain is not independently bootstrappable, the harness intentionally covers only minimal prerequisites plus the four JOB-01 migrations, and JOB-02 through JOB-08 remain pending.

...
`npm run portal:search-readiness` safely probes `portal_search_v1()` and the `portal_search_bigrams()` optimization helper with the anonymous role. A ready database must report that anonymous execution is denied for both; a `PGRST202` response fails immediately with the exact missing migration path. The preflight never uses the service-role key, returns search data, or treats anonymous execution as success.

`npm run test:portal:search-performance` is the focused authenticated global-search gate and starts with that migration preflight. Its deterministic delayed-response case proves a cached repeat renders without a second request and previous results remain visible while a different query refreshes. Its live case samples five real no-store API reads, requires API wall-clock p75 plus the 100 ms debounce to stay at or below 400 ms, measures the first rendered result/empty state at or below 400 ms, then repeats the same normalized query and requires an in-memory cached result within 75 ms with no second request. It writes `artifacts/portal-search-performance.json` with raw API/server timings and rendered-result measurements. Apply `20260722_000001_portal_search_v1.sql` followed by `20260722_000002_portal_search_bigram_indexes.sql` before running it; a missing RPC or index migration is a failed prerequisite, not a skipped test.

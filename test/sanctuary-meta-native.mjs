...
const sql = (body, db) => docker(args(db), body);
...
try {
 docker(['run','-d','--rm','--name',container,'-e','POSTGRES_HOST_AUTH_METHOD=trust','postgres:17-alpine']);
 ...
 sql('update private.sanctuary_meta_control set enabled=false',revoke);

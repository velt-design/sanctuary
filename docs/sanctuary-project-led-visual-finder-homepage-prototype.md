...
## 2026-09-16 homepage visualiser and proof batch (working agreement)

Owner source: Codex task `Rate Sanctuary Pergolas website`, 2026-09-16; Jordan agreed to the proposed homepage batch and instructed "Agreed, begin". Worktree: `C:/Dev/sanctuary-homepage-20260916`, branch `codex/homepage-visualiser-proof-20260916`, based on main `2b6fda9` (released contact designer). Related configurator UI task was idle at discovery; other checkouts are preserved.

Scope: clarify the first card with the agreed 3D/plan/conditional estimate/no-contact-details copy, use "Start designing", show an authentic released-designer preview, and add two or three governed completed projects beneath the choices. Preserve the hero, separate Custom/Commercial pathways, pricing, enquiry and consent contracts. Initial authority covered local implementation and verification only. Jordan approved the rendered homepage ("Great looks good") and then authorized publication ("go ahead") in this task on 2026-09-16. Commit, PR, required CI, merge and homepage production deployment are now authorized for this approved batch. Real test enquiries/emails, pricing changes and additional tracking remain outside this release.

Acceptance and status:
- Copy, first-card action and real preview: verified locally. Preview is a 35 KB WebP captured from the live released 3D view on 2026-09-16, cropped to omit prices. Mobile preview is visible inside the first card; other mobile choices retain their text treatment. Browser evidence: `homepage-desktop.jpg` and `homepage-mobile.jpg`.
- Compact proof before any selection: verified locally. Reuses the existing evidence renderer and approved Dairy Flat / Tindalls Bay records. A selection replaces this general proof with the existing relevant evidence; no duplicate project section or invented direction on general project links.
- Existing entry, editing/returning and enquiry handoff: verified through the local enquiry page. Manual Chrome check changed width to 5.4 m, switched to Plan, opened Review and followed the enquiry with original homepage attribution. Edit/Review/Enquire preserved the design and synthetic name (`enquiry-roundtrip.txt`). The released live model and estimate were also observed. Local production pricing is unavailable without production credentials; no production enquiry, email or staff receipt was tested. Existing enquiry/payload/recovery contracts pass in the full marketing unit suite. End-to-end production receipt remains explicitly unverified, outside this local homepage release batch.
- Responsive, keyboard/focus, reduced motion, no-JavaScript access, loading and required checks: verified locally. All 22 homepage browser cases have passing evidence across `changed-journey-browser.txt` (7), `hero-browser.txt` (6), `integration-performance.txt` (7), and `rechecked-browser.txt` (2 repaired failures). The latter supersedes the failed fade/landing assertions. Production-performance mode passed LCP <= 2500 ms, CLS <= 0.1 and first-result response <= 500 ms on the local unthrottled browser; this is not field CWV. The 12-size journey matrix covers 320-1440 px. All 856 marketing tests pass, production build and marketing TypeScript pass; focused lint, documentation, text and architecture guards are clean.
- Measurement review: verified source inspection; commercial outcomes remain unmeasured. `ProjectFinder` emits consent-gated direction selection, `configuratorOverlay.ts` retains source context, `ContactEnquiryForm` emits contact start/success/error and `lead_submitted`, and the platform has downstream lifecycle delivery. There are no dedicated stage events in the current configurator-prototype owner, so do not claim a measured launch/edit/review/abandonment funnel or live delivery merely from source. No new tracking was added.

Review dimensions selected before implementation: correctness, journey/clarity, visual/accessibility, wider-system fit, maintainability, reliability, privacy, loading cost, and verification/handover. Security/backend mutation work is excluded because there is no backend or permission change. Evidence lives in `artifacts/homepage-visualiser-2026-09-16/`; paths above are relative to it.

Local batch assessment (reasoned scores, not measured business results): correctness 8/10 (all agreed homepage changes rendered, conditional estimate wording); journey/clarity 8/10 (direct overlay and edit/enquiry return checked; no production receipt); visual/accessibility 8/10 (desktop/mobile inspection, full responsive/keyboard/reduced-motion matrix; physical phone and assistive-technology checks not repeated); wider-system fit 9/10 (shared evidence/data owners, preserved destinations and attribution); maintainability 8/10 (small route-only extension and separate preview CSS, no dependencies); reliability/privacy 8/10 (existing recovery tests and consent checks pass, local WebGL-paused fallback was observed and Plan remained usable; this batch makes no viewer change); loading cost 8/10 (lazy 35 KB asset, existing project images, local performance gates pass); verification/handover 8/10 (full marketing tests, build and all homepage browser cases covered, explicit production limits). The existing 1,200-line homepage browser spec is a warning-size owner; extraction is deferred because this batch updates existing cases rather than creating a parallel suite. Its next safe extraction is the shared cinematic timing/geometry helpers.

The homepage choice/evidence batch passed owner visual review at `http://localhost:3027/`; publication is now authorized and in progress. Original section 29's all-text mobile first-layer rule is superseded only for the first card's real visualiser preview; other mobile choices and the hero remain unchanged. The mobile opening now aligns beneath the header and scrolls naturally rather than squeezing the preview and all choices into one screen. Browser expectations also now follow the released overlay and sample the existing fade in a single browser frame instead of racing a removed element.

Next authorized action is commit/PR, required CI, merge and verification of the live homepage. Main was refreshed at release start and remains `2b6fda9`; concurrent PR #141 changes only separate design-enquiry owners. For the next measurement batch, first verify existing events reach the analytics destination, then add only the missing consent-gated design-stage/error events. Use lead creation date cohorts and separate configured enquiries from other residential enquiries (excluding commercial/staff/test traffic); compare qualification, quote acceptance among issued quotes, and recorded clarification time. Allow equal follow-up time and report raw counts and source mix. Do not infer uplift from tiny or differently aged cohorts. Live receipt testing needs an explicitly controlled recipient/environment; no automatic monitor or test communication was created.

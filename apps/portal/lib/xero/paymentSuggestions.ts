...
export function suggestDepositMatches(invoice: MatchInvoice, entries: Pick<ProjectPaymentEntrySummary, 'id'>[], receipts: ReceiptCandidate[]): PaymentSuggestion[] {
  return receipts.map(receipt => {
    ...
    if (amount !== null && amount > invoice.totalIncGstCents) blockers.push('Receipt exceeds this invoice. It may cover more than one payment or project.');
    ...
    if (amount === invoice.totalIncGstCents) reasons.push('Receipt equals the requested deposit.');
    else if (amount !== null && amount > 0 && amount < invoice.totalIncGstCents) reasons.push('Receipt is part of the requested deposit; any verified deposit counts as a customer win.');
    ...
    return { receipt, amountCents: amount, assessment: blockers.length ? 'blocked' : 'review', reasons, blockers,
      depositRemainingIfApprovedCents: blockers.length || amount === null ? null : invoice.totalIncGstCents - amount,

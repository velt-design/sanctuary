...
- UI foundation proof currently covers Projects Index, Project Detail, and the full Contacts workflow. Projects Index composes the shared operational header, search/filter controls, stage badges, loading and truthful empty/error/stale states without changing `useProjectsIndexData`, prefetch, archive scope, or optimistic mutations. Project Detail composes the shared detail header, stage tracker, controls, alert, and navigation-loss guard without changing lazy tab boundaries or `useProjectDetailsDraft` queue semantics.
  - The shared staff header adds the same global Projects/Contacts search to Dashboard, Project and Contact list/detail/create routes, and the other adopted operational routes named in `docs/ui-foundation.md`. Global search is an authenticated server read and remains separate from route-local filtering: `View all matching projects` hands `q` to the existing Projects filter, while `View all matching contacts` initializes the existing Contacts filter. The browser waits 100 ms before an uncached query, stores successful results in the authenticated user's in-memory QueryClient for five minutes, serves fresh repeated queries immediately, and keeps the previous successful group visible while a different query refreshes. The single RLS-respecting search operation reads generated normalized documents for queries of three or more characters and generated literal bigram arrays for the two-character minimum; GIN indexes cover both without changing result fields or ranking. Project/contact mutation owners mark search entries stale so later discovery cannot present known-obsolete data. Search-result navigation uses the shared non-blocking progress owner and clears its query after route commit. The Projects-index instant view releases when canonical route children settle, when the observed pathname diverges, or when the shared transition targets a different pathname; Project A therefore cannot mask a searched Project B. Search results never bypass the normal Project/Contact route, permissions, cache, or local-first owners.

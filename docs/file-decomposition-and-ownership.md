...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | `CalculatorGridClient.tsx` is now below the critical threshold and owns final configuration/workspace composition only. Estimate/project loading and duplicate-draft orchestration live in `useCalculatorWorkspaceSession.ts`; result-derived presentation lives in `useCalculatorResultPresentation.ts`; the outer workspace shell lives in `CalculatorWorkspaceView.tsx`; module sell-price attribution lives in `calculatorModulePricing.ts`; deterministic module validation lives in `calculatorValidation.ts`; and job/module plus blinds/flashings mutation state lives in focused controllers. The retired calculator house-footprint editor controller is absent; historical drawing compatibility remains with the drawing adapters. Pure calculator input defaults, normalization, browser-draft persistence/session state, module navigation modelling/presentation, configuration-section definitions/presentation/field options, structure/site/workflow field-schema generation, issue model/focus navigation, infill lifecycle/actions/interactions/cost coordination/presentation, save orchestration/dialogs, blinds/flashings editor markup, materials-debug behavior, preview presentation, pricing presentation/splitting, and quote-status helpers also live outside the client. | Do not split solely for size; if future work expands the client, extract configuration-schema or workspace-prop assembly before adding inline behavior. | `npx vitest run apps/portal/app/staff/calculator`, `npm run test:portal:projects`, `npm run typecheck` |
| Costing engine | Package-owned calculator costing and site aggregation | `calculate.ts` retains the historical module and multi-pergola orchestration boundary, while `bom.ts` still assembles all material families and their takeoff-derived install drivers. Empty add-on sites, standalone existing-pergola infills, and the versioned infill labour policy are extracted to focused owners. The large entrypoint test remains the broad public-contract regression suite. | Move site rollup aggregation and infill-baseline orchestration out before adding another site-level accessory family. Extract canonical infill material/takeoff projection from `bom.ts` before adding another infill-derived material or labour driver. | `npx vitest run packages/costing`, package typecheck |
...
| Quote and estimate tabs | Project detail quote and estimate workflows | `QuotesTab.tsx` is just above the critical threshold and composes named query/selection, lifecycle, PDF-preview, modal, list, detail, and pure-model owners. Draft editing, local-first create/persist, send-form orchestration, and prepared-delivery recovery state remain in the composition owner; frozen-intent read/retry and dialog presentation are extracted. | Extract `useQuoteDeliveryRecovery`, then draft editing plus local-first create/persist orchestration, before adding another inline quote workflow. | `npm run test:portal:projects`, `npm run test:portal:quotes`, local-first tests |
| Commercial quote/invoice server | Server-owned quote delivery, acceptance, and deposit invoice workflow | Historical quote and invoice modules still assemble persistence/detail reads, artifact preparation, public-token delivery, acceptance, invoice creation, provider checkpoints, and finalisation. Commercial intent, audit, acceptance, and route-delivery owners are extracted. | Move quote payload/attachment preparation out of `serverEmail.ts`, split draft command/detail reads out of `serverCore.ts`, and move invoice render/token/log finalisation out of `invoices/server.ts` without changing RPC or frozen-intent boundaries. | `npm run test:portal:quotes`, commercial/route tests, typecheck, portal build |
| Marketing enquiry route | Public enquiry intake and conversion workflow | Request validation, indicative estimate preparation, attachment staging, attribution and response shaping remain in the route. Canonical intake, request-bound correlation delivery and compatibility outbox/audit logging have named owners. | Extract email payload/estimate composition before adding new enquiry behavior; retain the single-claim private intent/receipt boundary and early intake replay return. JOB-07 remains separate. | Focused enquiry/email integration tests, repository typecheck, marketing build |
...
| Marketing product catalogue | Public product content, claims, media and route identity | The approved redesign deliberately consolidates ten formerly fragmented product sources into one typed, reviewable public-claim owner. It contains static governed content only; composition, project records and enquiry behavior remain separate. Shared project-image focal positions remain in `projectImageFraming.ts`. | If the catalogue expands, keep `products.ts` as the only public export and move byte-for-byte record groups into private category modules. Do not recreate route, image, SEO or technical-content maps. | `npx vitest run apps/marketing/data/products.test.ts`, focused product Playwright, `npm run typecheck`, `npm run build:marketing` |

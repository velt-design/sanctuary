...
| Item | Status | Evidence and remaining work |
| --- | --- | --- |
| W2-MEASURE-01 | Green | The data-free `/qa/projects-index-mutation-fixture` mounts the production Project Index and Project Details mutation owners and holds persistence open in Chromium. Current local evidence records 44 ms versus 792 ms for index feedback/background completion and 37 ms versus 817 ms for Details Done/background completion, with one request per journey, zero long tasks, and no blocking overlay. Paired rejection journeys prove terminal rollback while retaining visible/reviewable failure state. Portal Quality uploads this schema-v2 artifact beside the workbench artifact. |

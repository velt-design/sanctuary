...
function buildLegacyEstimateDrawingDraftFromSnapshot(
  ...
  return buildEstimateDrawingDraftFromSnapshot(
    snapshot,
  ) as LegacyEstimateDrawingDraft | null;
}
...
describe("buildHouseFirstWorkbenchProjectModel houseForms[] contract", () => {
  ...
  it("emits N forms when the draft authors additional houseAssembly.houseForms[] entries", () => {
    ...
    if (!baselineObjectFirst)
      throw new Error("Expected objectFirst baseline draft.");
...
describe("buildHouseFirstWorkbenchProjectModel", () => {
  it("classifies mono, gable, and box legacy fixtures into first-pass roof forms", () => {
    ...
    expect(
      buildHouseFirstWorkbenchProjectModel({ snapshot: monoFixture.snapshot })
        .houseForms[0]?.roof.form,
    ).toBe("hipped");
    ...
    // boundary.
    expect(
      buildHouseFirstWorkbenchProjectModel({ snapshot: gableFixture.snapshot })
        .houseForms[0]?.roof.form,
    ).toBe("hipped");
    expect(
      buildHouseFirstWorkbenchProjectModel({ snapshot: boxFixture.snapshot })
        .houseForms[0]?.roof.form,
    ).toBe("flat");
  });
  ...
  it("derives pergolas and shared house state from a local drawing draft", () => {
    ...
    const draft = buildLegacyEstimateDrawingDraftFromSnapshot(
      monoFixture.snapshot,
    );
    if (!draft) throw new Error("Expected drawing draft.");
    ...
    expect(projectModel.pergolas[0]?.attachment.resolution.status).toBe(
      "ambiguous",
    );
  ...
  it("lets the shared roof draft override legacy roof inference", () => {
    ...
    expect(projectModel.houseForms[0]?.roof.primaryFallDirection).toBe(
      "negative_x",
    );
    expect(projectModel.houseForms[0]?.roof.ridgeAxis).toBe("y");
    expect(projectModel.houseForms[0]?.roof.validation.status).toBe("valid");
    expect(
      projectModel.houseForms[0]?.roof.validation.approximationReasons,
    ).toEqual([]);
  ...
  it("derives shared soffit and fascia attachment zones from shared house context", () => {
    ...
    expect(
      soffitProject.pergolas[0]?.attachment.houseAttachmentZoneId,
    ).not.toBeNull();
    expect(
    ...
    const fasciaDraft = buildLegacyEstimateDrawingDraftFromSnapshot(
      monoFixture.snapshot,
    );
    if (!fasciaDraft) throw new Error("Expected drawing draft.");
    fasciaDraft.inputs.modules[0]!.houseAttachmentStrategy =
      "fascia_under_gutter";
    ...
    const fasciaZoneId =
      fasciaProject.pergolas[0]?.attachment.houseAttachmentZoneId ?? null;
    expect(fasciaZoneId).not.toBeNull();
    const fasciaZone =
      fasciaProject.houseForms[0]?.derivedEnvelope?.attachmentZones.find(
        (zone) => zone.id === fasciaZoneId,
      ) ?? null;
    expect(fasciaZone?.kind).toBe("fascia");
    expect(
      fasciaProject.houseForms[0]?.attachmentZones.some(
        (zone) => zone.id === fasciaZoneId && zone.kind === "fascia",
      ),
    ).toBe(true);
    expect(
      fasciaProject.houseForms[0]?.derivedEnvelope?.attachmentZones.some(
        (zone) =>
          zone.kind === "roof_edge" &&
          zone.hostEdgeId === fasciaZone?.hostEdgeId,
      ),
    ).toBe(true);
  });
  ...
  it("recomputes shared attachment zones when the roof state becomes blocked", () => {
    ...
    expect(
      projectModel.houseForms[0]?.attachmentZones.some(
        (zone) => zone.side === "rear" && zone.kind === "soffit",
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      projectModel.pergolas[0]?.attachment.houseAttachmentZoneId,
    ).toBeNull();
    expect(
      projectModel.warnings.some(
        (warning) => warning.code === "invalid_house_attachment_zone_overlay",
      ),
    ).toBe(true);
  });
  ...
  it("resolves shared attachment zones once for a shared house across multiple pergolas", () => {
    ...
    expect(
      projectModel.houseForms[0]?.attachmentZones.filter(
        (zone) => zone.kind === "soffit" && zone.side === "rear",
      ).length,
    ).toBeGreaterThan(0);
    ...
    expect(
      projectModel.houseForms[0]?.attachmentZones.some(
        (zone) => zone.id === sharedZoneId && zone.kind === "soffit",
      ),
    ).toBe(true);
  });
  ...
  it.skip("marks inferred mono roofs as approximate and records roof provenance", () => {
    ...
    expect(projectModel.houseForms[0]?.roof.validation.status).toBe(
      "approximate",
    );
    expect(
      projectModel.houseForms[0]?.roof.validation.approximationReasons,
    ).toEqual(["inferred_form"]);
    expect(projectModel.houseForms[0]?.roof.primaryFallDirection).toBe(
      "negative_y",
    );
    expect(projectModel.houseForms[0]?.roof.provenance).toMatchObject({
  ...
  it.skip("keeps coherent explicit mono fall directions valid and blocks incoherent drain-back directions", () => {
    ...
    expect(coherentProjectModel.houseForms[0]?.roof.validation.status).toBe(
      "valid",
    );
    ...
    expect(incoherentProjectModel.houseForms[0]?.roof.validation.status).toBe(
      "invalid",
    );
    expect(incoherentProjectModel.houseForms[0]?.roof.validation.code).toBe(
  ...
  it("derives form-aware roof capabilities from the shared house footprint", () => {
    ...
    for (const [form, controls] of Object.entries(expectedControls)) {
      ...
      expect(projectModel.houseForms[0]?.roof.capabilities.controls).toEqual(
        controls,
      );
      expect(
        projectModel.houseForms[0]?.roof.capabilities.footprintTopology,
      ).toBe("rectangular");
      expect(
  ...
  it.skip("updates roof validation and capabilities when the footprint topology becomes orthogonally supported", () => {
    ...
    expect(projectModel.houseForms[0]?.roof.provenance?.ridgeAxis).toBe(
      "default_fallback",
    );
    expect(projectModel.houseForms[0]?.roof.geometryKind).toBe(
  ...
  it("keeps orthogonal hipped footprints valid and exposes joined-hipped geometry", () => {
    ...
    expect(
      projectModel.houseForms[0]?.roof.capabilities.controls.ridgeAxis,
    ).toBe(true);
  });
  ...
  it("filters invalid saved open gable ends when the ridge orientation changes", () => {
    ...
    expect(
      projectModel.warnings.some(
        (warning) => warning.code === "invalid_house_first_roof_overlay",
      ),
    ).toBe(true);
    expect(projectModel.houseForms[0]?.roof.validation.status).toBe("valid");
    expect(
  ...
  it.skip("marks near-square gable footprints as approximate when the ridge axis is inferred", () => {
    ...
    expect(
      projectModel.houseForms[0]?.roof.validation.approximationReasons,
    ).toEqual(["inferred_ridge_axis", "ambiguous_ridge_axis"]);
    expect(projectModel.houseForms[0]?.roof.provenance?.ridgeAxis).toBe(
      "default_fallback",
    );
  });
  ...
  it.skip("auto-heals stale preset ridge axes and dependent roof state for gable and hipped roofs", () => {
    ...
    for (const form of ["hipped"] as const) {
      ...
      expect(projectModel.houseForms[0]?.roof.provenance?.ridgeAxis, form).toBe(
        "default_fallback",
      );
      expect(projectModel.houseForms[0]?.roof.validation.status, form).not.toBe(
        "invalid",
      );
  ...
  it("auto-heals stale or too-low preset gable and hipped roof pitch while preserving flat and mono pitch behavior", () => {
    ...
    for (const [form, primaryPitchDeg] of [
      ...
      expect(
        projectModel.houseForms[0]?.roof.form,
        `${form}/${primaryPitchDeg}`,
      ).toBe(form);
      expect(
        projectModel.houseForms[0]?.roof.primaryPitchDeg,
        `${form}/${primaryPitchDeg}`,
      ).toBe("5");
      expect(
        projectModel.houseForms[0]?.roof.validation.code,
        `${form}/${primaryPitchDeg}`,
      ).toBeNull();
    }

    const flatDraft = buildLegacyEstimateDrawingDraftFromSnapshot(
      monoFixture.snapshot,
    );
    ...
    expect(
      buildHouseFirstWorkbenchProjectModel({
        snapshot: monoFixture.snapshot,
        draft: flatDraft,
      }).houseForms[0]?.roof.primaryPitchDeg,
    ).toBe("0");

    const monoDraft = buildLegacyEstimateDrawingDraftFromSnapshot(
      monoFixture.snapshot,
    );
  ...
  it("surfaces only the outer open-end options for U-shaped bent gables", () => {
    ...
    expect(
      projectModel.houseForms[0]?.roof.terminalEnds.map(
        (end) => end.sourceEdgeId,
      ),
    ).toEqual(["footprint-edge-7", "footprint-edge-3"]);
  });
  ...
  it("treats every preset and live roof form as supported in the shared house model", () => {
    ...
    for (const preset of HOUSE_FOOTPRINT_PRESETS) {
      for (const form of HOUSE_ROOF_FORMS) {
        ...
        expect(
          projectModel.houseForms[0]?.footprint.preset,
          `${preset}/${form} footprint`,
        ).toBe(preset);
        expect(
          projectModel.houseForms[0]?.roof.form,
          `${preset}/${form} form`,
        ...
        expect(
          projectModel.houseForms[0]?.roof.geometryKind,
          `${preset}/${form} geometry`,
        ).not.toBeNull();
        expect(
          projectModel.houseForms[0]?.roof.capabilities.selectedFormSupported,
          `${preset}/${form} supported`,
        ).toBe(true);
        expect(
          projectModel.houseForms[0]?.roof.validation.code,
          `${preset}/${form} validation code`,
        ).toBeNull();
        expect(
          projectModel.houseForms[0]?.roof.validation.status,
          `${preset}/${form} validation status`,
        ).not.toBe("invalid");
      }
    }
  });
    ...
    for (const attachmentSide of ATTACHMENT_SIDES) {
      for (const preset of HOUSE_FOOTPRINT_PRESETS) {
        ...
        for (const form of ["hipped"] as const) {
          ...
          expect(
            projectModel.houseForms[0]?.footprint.preset,
            `${preset}/${attachmentSide}/${form} footprint`,
          ).toBe(preset);
          expect(
            projectModel.houseForms[0]?.footprint.attachmentSide,
            `${preset}/${attachmentSide}/${form} side`,
          ).toBe(attachmentSide);
          expect(
            projectModel.houseForms[0]?.roof.form,
            `${preset}/${attachmentSide}/${form} form`,
          ).toBe(form);
          expect(
            projectModel.houseForms[0]?.roof.geometryKind,
            `${preset}/${attachmentSide}/${form} geometry`,
          ).not.toBeNull();
          expect(
            projectModel.houseForms[0]?.roof.capabilities.selectedFormSupported,
          ...
          expect(
            projectModel.houseForms[0]?.roof.validation.code,
            `${preset}/${attachmentSide}/${form} code`,
          ).toBeNull();
          expect(
            projectModel.houseForms[0]?.roof.validation.status,
            `${preset}/${attachmentSide}/${form} status`,
          ).not.toBe("invalid");
        }
      }
    }
  ...
  it("accepts custom orthogonal mono outlines and blocks non-orthogonal ones", () => {
    ...
    expect(
      orthogonalProjectModel.houseForms[0]?.roof.capabilities.footprintTopology,
    ).toBe("orthogonal");
    expect(
      orthogonalProjectModel.houseForms[0]?.roof.validation.code,
    ).toBeNull();
    ...
    expect(
      polygonalProjectModel.houseForms[0]?.roof.capabilities.footprintTopology,
    ).toBe("polygonal");
    expect(polygonalProjectModel.houseForms[0]?.roof.validation.code).toBe(
  ...
  it("attaches preset decks to the rendered house edge when footprint offset and setback are set", () => {
    ...
    expect(projectModel.houseForms[0]?.decks[0]?.validation.status).toBe(
      "valid",
    );
  });
  ...
  it("builds stable derived wall graphs for straight, custom, and multi-segment same-side footprints", () => {
    const monoFixture = getSanctuaryGeometryWorkbenchFixture("mono-standard");
    const screenshotFixture = getSanctuaryGeometryWorkbenchFixture(
    ...
    expect(
      straightProject.houseForms[0]?.derivedWallGraph.walls.map((wall) => ({
        id: wall.id,
        label: wall.label,
    ...
    const customDraft = buildLegacyEstimateDrawingDraftFromSnapshot(
      monoFixture.snapshot,
    );
  ...
  it("marks side-only opening hosts ambiguous when the selected side has multiple derived wall segments", () => {
    ...
    if (!screenshotFixture)
      throw new Error("Missing gable-u-hipped-screenshot fixture.");
    const draft = buildLegacyEstimateDrawingDraftFromSnapshot(
      screenshotFixture.snapshot,
    );
    if (!draft) throw new Error("Expected drawing draft.");
    draft.houseFirst = {
      openings: [
  ...
  it("preserves supported opening kinds and defaults unknown kinds to window", () => {
    ...
    expect(
      projectModel.houseForms[0]?.openings.map((opening) => opening.kind),
    ).toEqual(["slider", "hinged_door", "window"]);
  ...
  it("enforces simple slider corner clearance while leaving window validation shared", () => {
    ...
    expect(projectModel.houseForms[0]?.openings[0]?.validation).toMatchObject({
      ...
      message:
        "Sliders and stackers need at least 0.3m clearance from each wall corner.",
    });
    expect(projectModel.houseForms[0]?.openings[1]?.validation.status).toBe(
      "valid",
    );
  });

...
The active package manifest is `v2.1`. Published `v1.7`, `v1.8`, `v1.9` and `v2.0` snapshots retain their stored rates and original package behavior when applied to this package base. A fresh draft copies the active editable rates but advances to `v2.1`; only publishing that draft activates the Version 4 Simple labour/overhead/customer-price policy. Cloning an older version preserves that version's semantics for rollback.
...
Version 2 adds a package-owned `Simple | Bespoke` classification. Simple is limited to one residential pitched-acrylic pergola/module, fascia/facade/soffit connection, deck brackets, normal access, easy ground, standard black finish, no infills, and at most 30m² single-storey or 20m² two-storey. Any failed condition, manual Bespoke selection, engineering, or full building consent resolves to Bespoke. Under manifest `v2.1`, Simple overhead is `$750 ex GST` for the first productive crew-day plus `$500 ex GST` per additional productive crew-day, pro-rated. Its core customer price receives a 10% policy uplift after the standard `1.25x` markup and before discount and GST. Bespoke retains `fixed_plus_variable` and no uplift. Published `v1.9` and `v2.0` retain the earlier `$2,000`/three-day Simple policy; earlier manifests retain their own legacy behavior.

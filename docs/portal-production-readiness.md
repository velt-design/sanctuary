...
| Browser smoke | Yellow | `npm run test:portal:browser` passed on 2026-05-04 while a normal portal dev server remained on port 3001: the no-auth fixture harness used port 3011 with isolated Next dev output, 6 fixture tests passed, and 1 auth-backed smoke stayed skipped by design. `PORTAL_BASE_URL=http://127.0.0.1:3001 npm run portal:fixture-env` still fails against the auth-gated normal server as intended. Authenticated smoke/performance remain blocked by valid staff test credentials. | Provision or reset a valid staff test account, then rerun auth-runtime, authenticated smoke, and performance. |
...
| Design workbench | Yellow | `npm run test:portal:workbench` on 2026-05-04 reached the Vitest segment but failed in `apps/portal/lib/drawings/commercialDesignPayload.test.ts`: expected `quantityTakeoff.flashings.totalLengthM` 1.5 and received 0. The focused test fails the same way. The independent no-auth fixture browser gate is green on the isolated fixture server. | Route the commercial payload assertion to the geometry/costing shadow-parity lane, then rerun the full workbench gate. Keep manual edit/save/reload and high-risk visual QA in release checks with authenticated staff data. |

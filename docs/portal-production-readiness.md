...
| Portal route bundle budgets | Yellow | The reusable analyser passes Schedule, Projects Index, Project Detail, Calculator, and Design Workbench from one fresh build and detects Turbopack lazy groups honestly. Projects Index is 697,496/200,319 raw/gzip initial and 2,714,108/622,030 lazy, with +5% rounded ceilings; shared route gzip grew by about 0.9 KiB. Project Detail's honest combined total still exceeds its obsolete zero-lazy cap. | Keep the honest per-boundary gates blocking. Reduce the existing Estimates drawing/workbench dependency in a separately authorised lane; do not hide lazy bytes or increase budgets automatically. |
| Portal performance evidence | Yellow | Slice 1 project opening remains locked at p75 44 ms feedback and 60 ms useful content. Slice 2 removes the blocking Dashboard-to-Projects server read/overlay and locks the journey to 100/500 ms, but its required five authenticated production runs are not yet recorded. | Run exactly five authenticated CI repetitions, record p50/p75/p95 with no skipped routes, then return this row to Green only if the overlay and long-task checks also pass. |
...
- [ ] Project list navigation meets the new 100/500 ms five-run p75 contract; project detail remains green.
...
- Moved the Projects index from a route-blocking server read to one staff-authenticated combined query, current-user cache bootstrap, explicit pending/cached/fresh/failure/access states, intent-only prefetch, and instant URL feedback without the global loading overlay. The route loading boundary now renders the actual Projects frame and truthful updating state instead of a generic loading screen, while filter ownership starts client-side without awaiting request search params. The fresh Projects Index bundle baseline is locked; five-run authenticated timing evidence remains pending.

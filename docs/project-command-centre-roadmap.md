...
Portal worktree `sanctuary-project-clarity`, branch `codex/project-email-speed-20260917`, HEAD565cea7c9b2b4e063e72877b25f33ad2acda00ca (base e8894d9). Full pre-push checks, focused loading/access/phase tests and Portal types passed. Protected deployment dpl_6AqbwRSWLjqrq5KaYQ41BPMNHBoH is Ready and assigned to stable review alias `https://sanctuary-portal-git-codex-pro-2efc62-jordans-projects-43df95bd.vercel.app`. All hosted Portal checks on565cea7 have passed. Unrelated tsconfig newline-only dirty file is untouched. Overview owner remains ProjectPage/tabs; defer extraction of history subcomponent to avoid unrelated churn in this small mount correction.

Velt worktree `velt-staff-correspondence`, branch `codex/staff-email-region-20260917`, PR380. Base ea0ba1d equals inspected live receiver dpl_GPVBNDixT4g7CnR8CzFWhSWNPAp8. Candidate f480038 preferredRegion was ineffective: hosted lambda still iad1 despite successful build. Corrected dd19d3e uses only the staff-summary per-function region rule in apps/web/vercel.json; no global region/auth/handler/schema changes.49 focused tests plus full pnpm check including60 browser tests passed before config correction; Corrected READY preview dpl_3VYAUr2Yxd5W4D8UoQhZwt14WoBc confirms actual staff-summary lambda syd1 and api/health iad1. Independent reviewer rechecked exact dd19d3e414d8761d260be5f870c8285e4ebc93e6 and found no material issue; all hosted checks now pass on exact dd19d3e in completed run35196526849. Goal is database proximity to the existing Sydney DB; benefit is not proven. Current official Vercel region docs support per-function configuration. No new subscription or raised limit. No production release authority for these revisions.

Next action: Owner approved Velt PR380 dd19d3e backend release and measurement in the current reply. Exact clean head/checks revalidated; merged as32ff63cad073fd5ca029673793edd92c103021fd. Production-target deployment dpl_6WaoZ6gX9hWaW7NeHaPEg8K6N7iW is now promoted. Staged/live health confirms merge32ff63c, actual staff-summary placement is syd1 and project production target matches. Real saved quote emails work: browser-tool first7901ms/repeat4220ms; warm mail phase663ms with matching124ms. Accepted project6799ms first return. Region benefit is observed in service timing; total display target still unproven. Preparing preview-only numeric DOM render timing because browser reload automation adds overhead. Rollback dpl_GPVBNDixT4g7CnR8CzFWhSWNPAp8. Run35196526849 and watch90297 completed successfully. The backend release approval does not authorize the Portal PR153 release. The protected Portal changes remain unmerged. Signed receiver bindings prevent using another preview origin for real mailbox proof; do not weaken bindings or ask Jordan to re-login. Goal remains active, target not met, previous turn progressed measured evidence and verified corrected hosted placement.

Latest continuation: backend PR380 approved and live; receipt in Velt `.codex-tmp/staff-region-release.json`. Portal ac059043ae5a350a469aa195440f324372b1b1c1 adds default-off NEXT_PUBLIC_PORTAL_EMAIL_TIMING hidden numeric render attributes.18 query tests and full pre-push checks pass. Protected candidate dpl_91eYQaJWtjqqLhzK486Brz1DykKY is Ready and assigned to the stable review alias. Direct reload timings confirm remaining wait is real. Portal Quality and Performance on run35197919196 remain pending; other listed gates pass. Public Portal remains unchanged. Next distinguish instance-cache misses from repeated unsuccessful provider verification before selecting shared persistence. Existing Next data caching is forbidden by the repository guard. ProjectSnapshotPageClient also waits for project summary before mounting ProjectPageFrame/Overview and email component; repeat measured about3.7s before email mount. Earlier reading must preserve cancellation, final access/customer checks and private query handling. No shared-cache schema or additional implementation selected or applied.
Current local batch (independent review correction1): reviewer found completed warm replies could outlive customer/access changes while the shell waited. Corrected provider hands off only still-pending reads; settled unclaimed results are dropped and consumer reads afresh. Tests now explicitly cover changed-customer and denied-access handoff.8 warm-read tests and Portal types pass. Candidate909d532 is superseded before assigning the protected review alias.
Current local batch: saved GET starts alongside the initial project summary through a single-use mounted-page handoff; no change to layout or live-read limits.32 lifecycle/matching tests pass, including early access denial, cross-project cancellation, 15second unclaimed expiry, hidden-page discard and no duplicate request. Portal types and architecture changed checks passed before the final denial-test/doc amendment. Aggregate matching diagnostics now distinguish verified reuse from provider failure codes. Hosted measurements and independent review remain pending for this batch; no claim of achieved speed.

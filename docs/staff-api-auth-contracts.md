...
- `GET /api/staff/v1/work-items/queue` is the staff-authenticated one-row-per-project Work Queue read model. It uses `requireStaffContext()` and the auth-bound client, pages `project_work_queue_v3()` in 1,000-row hosted-safe ranges, and proves whole-portfolio marker/state completeness with `staff_project_state_counts_v1()`. For a full-portfolio read, bounded active-state, non-archived commercial-candidate, and relevant confirmation inventories start concurrently with the durable queue and completeness proof; the server filters those candidates by the authoritative active-state IDs before composition. Project-scoped callers retain V2-ID validation and bounded ID chunks. The route returns bounded command metadata, then filters its authoritative result in memory by search, effective owner, detailed stage, and urgency before pagination; filters do not rerank or invent work. Dashboard consumes a limited preview of the same owner; personal reminders remain separate. Missing marker/state or queue schema (`PGRST200`, `PGRST202`, or `PGRST205`), incomplete/malformed portfolio counts, or any explicit 5,000-row safety ceiling returns `503 WORK_ITEMS_UNAVAILABLE`; clients render the named not-ready state and must not expose stale actions. The browser may begin this private/no-store request on explicit Work Queue navigation intent through the current verified user's memory-only query cache; its five-second freshness window exists only to hand the in-flight result to the route, after which normal stale refresh and row-version conflicts protect current authority. No response is persisted or service-worker cached, and all actions remain server-authoritative.

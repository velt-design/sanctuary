...
export const CONFIGURATOR_STORAGE_VERSION = 'sanctuary.pergola-config.v1' as const;

export type ConfiguratorStorage = Pick<Storage, 'getItem' | 'setItem'>;

export type StoredCustomerPergolaConfigurationEnvelopeV1 = {
  storageVersion: typeof CONFIGURATOR_STORAGE_VERSION;
  savedAt: string;
  document: CustomerPergolaConfigurationV1;
};

export type StoredConfigurationReadResult =
  | {
      status: 'current';

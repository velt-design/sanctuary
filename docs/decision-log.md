...
| 2026-06-20 | Workbench House Forms            | Active   | PR-SS-2 part 3 (in progress): empirically proved the convergence gap is NOT fixable by a local merged-vertex velocity rule — a ridge vertex's along-ridge SLIDE is globally determined (the plus's arm-merge must slide toward the cross centre, T's stem-junction must not; both are locally identical antiparallel ridge vertices). Tried normal-sum, walk-to-third-edge, and neighbour-average: each fixes some shapes and breaks others. The correct fix is the standard edge+split event model with N-way coincidence re-stitch, not the bespoke pairwise vertex-merge. **Interim hardening landed: a completeness invariant in `solve.ts` (every wavefront vertex must be consumed) now makes an unresolved convergence fail loudly with `unsupported_topology: incomplete_wavefront` at the source — previously the +/H skeleton returned a broken-but-eave-covered graph that only the roof translator's area guard caught.** The full convergence rework is the remaining part-3 work. |
| 2026-06-20 | Workbench House Forms            | Active   | PR-SS-2 part 3 (rewrite in progress): new equidistance/offset-line solver `offsetSolve.ts` (parallel module, NOT wired — `index.ts` still exports the kinematic `solve.ts`). The wavefront is a cyclic list of MOVING EDGES (offset lines); every node position is the exact intersection of two edge lines, eliminating the kinematic model's ridge-slide ambiguity by construction. **Solves today with exact integer output + full eave coverage: convex (square, rectangles incl. odd dims) and single-reflex shapes (asymmetric L).** Remaining gap: multi-reflex shapes (T/U/+) and perfectly-symmetric shapes hit a SIMULTANEITY — for a T the bar ridge forms at the same instant both reflexes split (bar-height geometry forces it), so several splits + a ridge collapse coincide and must be batched/ordered together (single-reflex needs split-before-collapse tie-break; T needs the ridge collapse to win). That simultaneous-event batching is the next increment; until then those shapes return a typed `unsupported_topology` (pinned in `offsetSolve.test.ts`), never wrong geometry. The kinematic solver remains the wired path and is untouched. |
| 2026-06-20 | Workbench House Forms            | Active   | PR-SS-2 part 3 (offset solver progress): added **ridge-collapse** (adjacent parallel opposite-normal edges meeting), **collinear merge**, and **coincident-node dedup** to `offsetSolve.ts`. Tie-break at equal time is SPLIT < COLLAPSE < RIDGE. Result: **every rectilinear shape now produces a complete, full-eave-coverage, integer-output skeleton — square, rect (incl. odd), L (sym+asym), T (sym+asym), U, +, H** — including the convergence/symmetric cases the kinematic solver never could. **Roof-level area-conservation (facets partition the footprint, checked through the translator) holds for square/rect/L/symmetric-T/U.** Open piece: asymmetric T / + / H — the reflex-meet junction must ride down a collinear "straight vertex" to connect to the main ridge; until that lands their facets overlap, so the solver stays UNWIRED (kinematic `solve.ts` still drives the translator). offsetSolve.test pins full-coverage + integer + no-duplicate-nodes for all shapes; suite 40/40 green, typecheck clean. |
| 2026-06-21 | Workbench House Forms            | Active   | Workbench crash fix: `composeFootprintFromComposition` (the union-polygon builder, called by `buildHouseFormFootprintPolygonMm` on every workbench load) passed RAW composition coords with sub-mm drag-resize float noise straight into its cell grid; rectangles meant to share an edge differed by a fraction of a mm, opening a thin sliver so the union traced as **2 boundary loops** and the function threw — taking the entire workbench down ("Something went wrong"). Fix: **integer-mm snap** the rectangle corners before the grid (mirrors PR-COMP-UNIFIED-3's downstream snap), and **degrade gracefully** to the largest-area loop instead of throwing on a genuinely disconnected/holed union. Verified end-to-end via Playwright: the Jess-Oratia project (House 1, composite of 3 rectangles) now LOADS and renders in 3D (it previously crashed). Note: its roof still shows "Invalid geometry" under the current kinematic/stitched solver — that is the PR-SS-4 wiring target, separate from this crash. Regression tests added in composeFootprintFromComposition.test.ts (float-sliver repro + graceful-disconnected). |
| 2026-06-20 | Workbench House Forms            | Active   | PR-SS-2 part 3 (convergence solved for real shapes): added the **slab collapse** (a one-axis loop — a band between parallel eaves with straight-vertex junctions — retires to a single ridge line, each junction connecting down to it) plus an **SV-flank guard** on ridge collapse. The offset solver, wired into the translator `buildSkeletonRoof` (test-only; no production consumer — `composeRoofFromComposition` still uses kinematic), now produces **area-conserving roofs (facets partition the footprint exactly) for rect / L / T / U / plus / symmetric-L / symmetric-T**, and on the real composition corpus for **01 single, 02 fused, 03 Graham-Oratia L, 05 T, 06 U, 08 Jess-Oratia H** — the convergence and symmetric cases the kinematic solver never solved. Remaining gaps (return typed errors, never wrong geometry): **07 synthetic-H** (crossbar meets bars off-centre — a multi-T-junction near-miss), **09 stepped-plus** (20-vertex / 8-reflex), **04 float-noise L** (quarantined; near-degenerate after integer snap). roofSkeleton.test updated — plus + symmetric-L/T now assert correct roofs, with an explicit "never silently wrong" test pinning the remaining gaps. Suites 50/50 green; geometry typecheck clean; regression matrix 9/9 unchanged. |

| 2026-06-21 | Workbench House Forms            | Active   | PR-SS-4 (skeleton wired into the orchestrator): `composeRoofFromComposition` now routes single hipped rectangles through `buildRectangularRoof` (honours ridge axis) and non-fused hipped composites through the **orthogonal straight-skeleton** (`buildSkeletonRoof`) as the PRIMARY solver, falling back to the legacy unified-wavefront → stitched path only when the skeleton returns a typed error (so it never emits overlapping facets). Stamps `roofGeometry: "composition_unified"`, `roofTopologySolver: "orthogonal_straight_skeleton"`. **Corpus fixtures 01 (single-rect) and 08 (Jess-Oratia H) unquarantined and green**; 02–06 stay green (03/05/06 via skeleton, 04 float-noise via wavefront fallback); 07 (synthetic-H crossbar) and 09 (20-vertex stepped plus) stay quarantined (skeleton falls back). Verified end-to-end in the live workbench (Playwright): the Jess-Oratia roof's prior `roof_..._outside_eave_or_spans_void` QA failure is GONE and the 3D renders a coherent hipped H. Remaining: the rail still shows "Invalid geometry" from a **separate pre-existing validation** `invalid_ridge_axis` (houseRoofValidation.ts) — a "preferred ridge orientation" nudge that is moot for skeleton composites (the skeleton uses natural geometry, not the stored ridgeAxis); suppressing it for composites is a follow-up. Geometry suites 114/114 (5 pre-existing legacy custom-polygon/DXF failures unrelated); typecheck clean. Also fixed a TS `never`-narrowing bug in offsetSolve's event selection (collect candidates + select, instead of a closure-mutated `let best`). |

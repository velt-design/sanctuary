...
For a server-synced saved estimate, the `Actual vs estimated` review records post-job materials, install, overhead, travel, extras, crew hours, notes, and completion state through `/api/staff/v1/estimates/[estimateId]/actual-costs`. Variance is calculated against the frozen saved estimate outputs; historical estimates are not repriced. Records live in `estimate_cost_actuals`, are staff/RLS protected, and retain the last staff updater. Materials, install, and overhead actuals are required before a review can be marked complete.

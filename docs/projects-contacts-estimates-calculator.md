...
Calculator configuration section order, Basic/Advanced visibility, and field-width presentation live outside `CalculatorGridClient` in the configuration-section model and form presenter. The existing sections, field order, dependencies, validation IDs, and focus targets are unchanged. The responsive field grid deliberately renders three columns at the 1600px and 1024px browser checks, and two columns at 1366px and 768px, with one column on narrower mobile widths. Specialist Flashings, Blinds, and Infills controls use the full section width; ordinary controls are visually flat inside one section surface. The narrow command bar keeps the browser-draft explanation visible above its wrapped actions.
...
The infill configurator is a guided `Opening` -> `Existing supports` -> `Results` workflow. Required opening measurements gate progression; missing existing supports do not, because the takeoff adds the required 50x50 members. New custom infills and presets request automatic material and joiner direction, while saved sheet/strip and vertical/horizontal preferences remain manual. The portal resolves automatic choices through the canonical takeoff before mapping the unchanged `CostInputsV1` payload. At `900px` and below the desktop infill rail is replaced by the inline selector rather than a slide-over summary. Results are production-first in semantic order: manufacturing status and blockers, resolved system, added supports, canonical pieces, purchases, cutting diagram, then collapsed cost and technical detail. At `1200px` and below the tables remain ahead of a compact diagram so the first cut rows are reachable without initially scrolling past the drawing.

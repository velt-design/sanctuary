...
- Model Space layer JSX lives in the app-local `ModulePlanLayerRenderers` module beside `ModuleViewsCard`. Plan SVG presentation prep lives behind `buildPlanSvgPresentationModel`: it prepares top-projection layer data, overlay projection, and diagnostics before React renders the SVG shell. Geometry-backed SVG presentation prep lives behind `buildPlanSvgGeometryPresentation`: it prepares projection-only mode flags, projected pergola geometry, semantic house context projections, selected-opening emphasis, and geometry fallback decisions without JSX. Plan SVG client/SVG bridge wiring lives behind `ModulePlanSvgBridge`, including footprint canvas, raw plan, and projection-backed deck drag point resolvers. `ModuleViewsCard` should not rebuild render-graph ownership, geometry projection arrays, SVG resolver bridges, or duplicate overlay source counts inline.

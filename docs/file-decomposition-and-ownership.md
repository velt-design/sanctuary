...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | Calculator UI, module drawing surfaces, estimate state, and presentation helpers are still concentrated in a few client files. | Continue splitting oversized calculator drawing presentation owners by moving plan layout and annotation preparation into narrower model-space, sheet, and detail presenters before adding new drawing surface branches. | `npm run test:portal:projects`, targeted calculator/component tests, `npm run typecheck` |

...
export function enquiryExperienceFixtures(): WebsiteAutoresponderPreviewFixture[] {
  return (Object.keys(ENQUIRY_EXPERIENCES) as EnquiryExperience[]).map(experience => {
    ...
    const customerBrief: CustomerBrief = {
      version: 1, audience, designStatus: experience === 'help' ? 'help' : design ? 'configured' : 'bespoke',
    ...
    return {
      ...
      variables: {
        ...
        message: experience === 'help' ? 'We would like to use our deck more but are not sure which roof would suit.' : experience === 'configured' ? 'We’d like to cover our outdoor dining area. Please check the connection to our house.'
        ...
        widthM: design ? 5.4 : 0, depthM: design ? 3.2 : 0, heightM: 0,
        style: design ? 'Pitched' : '', roof: design ? 'Acrylic' : 'Not selected', addons: [], blindsSelected: false,

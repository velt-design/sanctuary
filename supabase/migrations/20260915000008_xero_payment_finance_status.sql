-- Include automatic payment failures in the attention queue before pagination.
begin;
-- Bounded finance read model. This does not approve payments or export history.
create or replace function public.xero_finance_review_filtered(p_actor uuid,p_search text,p_offset integer,p_view text)
returns jsonb language plpgsql security definer set search_path=pg_catalog,pg_temp as $$
declare v_rows jsonb;
begin
  perform public.xero_require_payment_approver(p_actor);
  if p_search is null or length(p_search)>120 or p_offset is null or p_offset<0 or p_offset>10000 or p_view is null or p_view not in ('all','attention','current','history') then
  ...
  with base as (
    select i.id as "invoiceId",i.invoice_ref as "invoiceRef",i.project_id as "projectId",
      ...
      (select jsonb_build_object('state',s.state,'reason',s.reason,'checkedAt',s.checked_at)
        from private.xero_payment_sync_status s where s.invoice_id=i.id and s.tenant_id=t.tenant_id) as "paymentSync",
      (select r.body::jsonb #>> '{Invoices,0,Status}' from private.xero_invoice_requests r where r.transfer_id=t.id) as "transferTargetStatus",
    ...
    select base.*,case
      ...
      when "paymentSync"->>'state' in ('review','unavailable')
        or ("paymentSync"->>'checkedAt')::timestamptz<now()-interval '24 hours' then 0
      ...
      when observation->>'state' in ('draft','awaiting_approval') or ("xeroInvoiceId" is not null and observation is null) then 1
  ...
  select coalesce(jsonb_agg(to_jsonb(r)-'queuePriority' order by r."queuePriority",r."dueDate" nulls last,r."invoiceId"),'[]'::jsonb)
    into v_rows from (
      select * from ranked where p_view='all' or (p_view='attention' and "queuePriority"<=1) or (p_view='current' and "captured") or (p_view='history' and not "captured") order by "queuePriority","dueDate" nulls last,"invoiceId" limit 51 offset p_offset
    ) r;
  return jsonb_build_object('rows',v_rows,'checkedAt',clock_timestamp());
end;
$$;
revoke all on function public.xero_finance_review_filtered(uuid,text,integer,text) from public,anon,authenticated;
grant execute on function public.xero_finance_review_filtered(uuid,text,integer,text) to service_role;

-- Preserve existing readers while the Finance page adopts the filtered contract.
...
returns jsonb language sql security definer set search_path=pg_catalog,pg_temp as $$
  select public.xero_finance_review_filtered(p_actor,p_search,p_offset,'all');
$$;

...
describe.skipIf(!wireFile)('Portal sender wire proof', () => {
  it('exports exact signed bytes and accepts the request-bound response', async () => {
    const config = { origin: 'https://velt.example.test', secret: 'a'.repeat(64) };
    const projectId = '11111111-1111-4111-8111-111111111111';
    const actorId = '22222222-2222-4222-8222-222222222222';
    const requestId = '33333333-3333-4333-8333-333333333333';
    const now = Date.parse(correspondenceFixture.observedAt);
    const fetcher: typeof fetch = async (url, init) => {
      writeFileSync(wireFile!, JSON.stringify({ url: String(url), headers: Object.fromEntries(new Headers(init?.headers)), body: init?.body, now, expected: { projectId, actorId, requestId } }));

...
| 2026-05-13 | Plan Tool Chain | Active | `EdgeDragTool.onPointerDown` must early-fallthrough when `event.shape?.metadata?.openGableEndId` is set, so hip↔gable toggle clicks reach SelectTool instead of starting a resize session. The synthetic gable triangle overlaps the active house outline within `edgeHitToleranceMm`; without the priority check, the toggle never fires when the house is selected. |
...
### 2026-05-13 - Plan Tool Chain - Terminal-End Click Priority over Edge Drag

Area: Plan Tool Chain

Status: Active

Decision or mistake: clicking the synthetic gable triangle to re-close an opened Dutch-hip end did nothing when the house was the active selection. The toggle pipeline (synthetic triangle shape → `metadata.openGableEndId` → `selectionRouter` → `house_terminal_end_toggle` → `commitSharedHouseRoofDraft`) was fully wired and tested, but the click event never reached `SelectTool`. The cause was in the Plan tool chain composition: `EdgeDragTool` is the entry tool, and its `onPointerDown` accepts ANY click that lands within `edgeHitToleranceMm` (500mm) of the active outline's perimeter. The synthetic gable triangle is built from `[apex, eaveStart, eaveEnd]` with the eave corners pushed outward by the eave overhang -- so it overlaps the house outline's perimeter edge entirely. With the house selected, `getActiveOutline()` returned the house footprint, `findClosestPolygonEdge` found the perimeter edge within tolerance, and `EdgeDragTool` started a resize session, swallowing the click before the chain could fall through to `MoveTool` → `SelectTool`.

Why it mattered: the symptom (toggle silently fails ONLY when the house is selected) doesn't match any obvious failure path in the toggle code itself. Three rounds of code review of the toggle pipeline (router, dispatcher, callback, commit action) found no defect because there was no defect there -- the click was being intercepted earlier in the tool chain. This is the same class of bug as the `pointerCancel` -> `pointerUp` aliasing in milestone 14 (`2026-05-08 PlanViewport / Pointer Events`): the user-visible symptom is downstream, but the boundary breaking it is upstream at the input layer.

Current guardrail: `EdgeDragTool.onPointerDown` MUST early-fallthrough when `event.shape?.metadata?.openGableEndId` is a string -- BEFORE consulting `getActiveOutline()` and tolerance math. The terminal-end toggle target is a click target, not an edge-drag handle, even though its polygon overlaps the active house outline. Future tools added to the Plan chain (e.g. a future house Move support) MUST mirror this precedence rule for any shape whose classifier returns `kind === 'house_terminal_end_toggle'`. If the chain grows more "click-only" UI targets that overlap movable outlines (zone labels, edit handles outside the perimeter, etc.), promote this precedence rule into a shared `isClickOnlyShape(shape: GeometryTopProjectionShape)` helper consumed by every tool's `onPointerDown` so the rule lives in one place.

Promoted to: None

Related docs/tests: [apps/portal/components/drawings/viewports/PlanViewport/tools/EdgeDragTool.ts](../apps/portal/components/drawings/viewports/PlanViewport/tools/EdgeDragTool.ts), [apps/portal/components/drawings/viewports/PlanViewport/tools/EdgeDragTool.test.ts](../apps/portal/components/drawings/viewports/PlanViewport/tools/EdgeDragTool.test.ts) ("falls through immediately when the click shape is a terminal-end toggle target"), [apps/portal/components/drawings/viewports/PlanViewport/interactions/selectShape.ts](../apps/portal/components/drawings/viewports/PlanViewport/interactions/selectShape.ts), [apps/portal/components/drawings/viewports/selection/selectionRouter.ts](../apps/portal/components/drawings/viewports/selection/selectionRouter.ts) (`house_terminal_end_toggle` classification), [packages/geometry/src/topProjection.ts](../packages/geometry/src/topProjection.ts) (`enrichHouseRoofShapesWithTerminalEnds` -- emits synthetic triangle with `openGableEndId` + `isOpen` metadata).
...
- The deeper fix is to migrate `form: 'gable'` -> `form: 'hipped' + openGableEndIds: <all terminals>` at the workbench draft normalization boundary so every consumer reads coherent state. The current helper is a UI-side workaround; the deep fix retires the migration's special case entirely.

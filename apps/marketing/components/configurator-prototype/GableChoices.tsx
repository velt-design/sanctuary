...
export type PreviewRoofChoices = { attachmentIntent?: 'freestanding' | 'unsure'; family: 'mono' | 'gable' | 'box'; orientation: 'parallel' | 'away'; infills: boolean; finish?: RepresentativeRoofFinish; roofBattens?:RepresentativeRoofBattens; lighting?:PergolaLighting; blinds?: PreviewBlind[]; sidePanels?:SidePanel[] };
...
export function RoofTypeChoice({ value, onChange }: { value: PreviewRoofChoices; onChange: (next: PreviewRoofChoices) => void }) {
  return <fieldset className={guide.choices}><legend>Roof style</legend>
    ...
    <p className={guide.description} id="roof-style-description">{value.family === 'mono' ? (value.attachmentIntent==='freestanding'?'A single sloping roof.':'A single slope away from your home.') : value.family === 'gable' ? 'Two slopes meet at a raised ridge.' : 'A level frame conceals the roof slope.'}</p>
  </fieldset>;
}
  ...
  return <div className={styles.gableChoices}>
    <fieldset className={styles.choices}><legend>Ridge direction</legend>
      {(['parallel', 'away'] as const).map(orientation => <label key={orientation} data-selected={value.orientation === orientation}>
        ...
        {value.attachmentIntent==='freestanding'?(orientation==='parallel'?'Across the width':'Along the projection'):orientation === 'parallel' ? 'Parallel to house' : 'Away from house'}
      </label>)}
    </fieldset>
    <p className={styles.small}>{value.attachmentIntent==='freestanding'?'Two roof slopes, with a gutter on each side.':value.orientation === 'parallel' ? 'Along the house, with a gutter on each side.' : 'Connects into the Dutch-gable roof fascia.'}</p>

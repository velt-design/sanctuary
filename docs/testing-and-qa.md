...
The enquiry-identity extension runs through the same `test:praxis:db` and `test:praxis:db:fast` gates. Both apply the exact correlation prerequisite and shared `supabase/tests/praxis_enquiry_identity.sql` cases. Real PostgreSQL additionally uses two independent sessions to prove a concurrently committed receipt is invisible to the existing repeatable-read snapshot and visible to the next read. The Docker database binds only a random loopback port for that test. The native equivalent is `node scripts/test-praxis-enquiry-identity-pg.mjs` with `SANCTUARY_TEST_PG_BIN` set to an absolute PostgreSQL 17 bin directory; it accepts no target URL, creates its own cluster, and stops it on exit. Native startup is shared with the existing correlation race harness.

Focused HTTP/contract verification is `npx vitest run apps/portal/lib/praxis apps/portal/app/api/integrations/praxis test/praxis-context-migration.test.ts`. Required cases cover UTC microsecond edges, exact 31-day/100-reference bounds, canonical/reference deduplication, outside-window mappings, all receipt absence/outcome states, no personal/hash fields, 100/101 rows, whole-envelope 128 KiB bounds, identity drift, and bearer/source mismatch before a read. Native grant checks and actual LOGIN denials run with the default read-only setting disabled. PGlite is query/fixture evidence, not a substitute for those native security and concurrency proofs.

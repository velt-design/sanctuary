...
On 2026-09-09, parent-owned catalog receipts proved this core source remained unapplied on staging `tnsiprehuldksnuowubv` (12:53:52 UTC) and production `iytanftukulcnavossmd` (13:00:36 UTC): no reporting schema, group role or source identity existed, and the production receipt also confirmed no core ledger entry. PR #113 and the owning documentation recorded disposable tests only; no other persistent applied target was found. The preserved original Git blob is `99ae4f7e82f8cd150a1303c23b50c83aecda14a6`, with canonical-LF SHA-256 `f4b0b4e4d8ecd4f9ff83fdd378c52a2c6bacbb747e66fc801bcd604932a432fa`. The commissioned source correction changes only optional legacy project-field reads before installation; it rewrites no applied ledger. Recheck the exact target before applying the corrected source, and stop for a forward-compatible repair if evidence of an existing installation differs from these observations.

The project projection retains required `version` and `depositReceivedAt` keys as JSON null when their legacy source columns are absent or null, and preserves genuine existing values. It reads only those exact keys; it does not publish the full project row. No project columns, synthetic revision defaults, deposit dates or conversion-outbox migration are introduced. The other core schema, freshness, function and grant effects still require their own scoped rehearsal; this compatibility fix does not establish activation readiness.

A subsequent parent-owned staging rollback rehearsal of PR #121 source failed on PostgreSQL 17's prohibition against a non-superuser issuing `ALTER ROLE ... NOSUPERUSER`. The 2026-09-09 14:20:30 UTC postflight confirmed the core still absent and ledger unchanged. The separately commissioned pre-install correction creates a missing reader with all seven hardened flags, but asserts an existing reader's flags, absence of outgoing role memberships and absence of object ownership rather than repairing it. All later projection, freshness and grant SQL remains unchanged.

For a non-superuser installer, `createrole_self_grant` must be empty on first creation. PostgreSQL 17 adds exactly one unavoidable incoming membership: reader granted to the current creator by the bootstrap superuser, ADMIN true, INHERIT false, SET false. The migration verifies that exact initial shape; it neither revokes it nor grants the operator's capabilities to the reader. Compatible replay preserves all existing memberships, including an explicitly provisioned reader LOGIN. This ADMIN edge is trusted installer authority, not isolation from the installer. Pre/postflight must preserve every existing edge and permit only that exact new edge; reject every other new edge or SET/INHERIT capability. See [PostgreSQL role attributes](https://www.postgresql.org/docs/17/role-attributes.html) and [ALTER ROLE restrictions](https://www.postgresql.org/docs/17/sql-alterrole.html). Rehearse through the actual operator with exact source ownership and grants before hosted application; disposable proof does not replace that gate.

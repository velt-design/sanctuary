...
| `apps/worker/**`, `packages/jobs/**`, `scripts/test-background-jobs-db.mjs`, `supabase/tests/background_jobs*.sql`, `test/background-jobs-migration.test.ts`, `.github/workflows/background-jobs.yml` | `docs/supabase-schema-map.md`, `docs/target-architecture.md`, `docs/security-privacy-quality.md`, `docs/testing-and-qa.md` | Keep the queue message minimal, frozen input private, service-role RPC access explicit, every worker-owned payload read/lifecycle mutation lease-fenced, and the worker dark by default until a later checkpoint owns a producer/handler rollout. |

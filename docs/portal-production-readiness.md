...
This snapshot records the most recent known production-readiness state from the portal review and follow-up checks through 2026-05-30. Re-run the listed commands before treating any item as current after new work lands. If a row is older than the current work, treat it as a last known signal, not live truth, until the relevant command or manual check is repeated.

| Area | Status | Last Known Signal | Next Action |
| --- | --- | --- | --- |
| Portal tests | Yellow | `npm run portal:doctor:quick:log` failed on 2026-05-30 during portal Vitest. The original focused rerun `npx vitest run apps/portal/lib/estimates/drawingEdits.test.ts` shows one failing assertion in `writes object-first drafts without dual-writing compatibility state`: the test still expects `deckDraft.objectFirst.decks[0].label`, while the received deck draft no longer includes `label`. A post-cleanup broad quick-doctor attempt was stopped after the logged Vitest worker hung; before it hung, the log had already reported non-doc failures in `apps/portal/lib/supabaseClient.boundaries.test.ts`, `apps/portal/components/drawings/rail/objectWorkbenchImportGuards.test.ts`, `apps/portal/lib/estimates/drawingEdits.test.ts`, `DesignWorkbenchFixtureClient.test.tsx`, the staff design-workbench page test, `DesignWorkbenchEstimateClient.test.tsx`, and the QA design-workbench fixture page test. | Fix or intentionally update the failing portal code/test expectations in the owning lanes, then rerun `npm run portal:doctor:quick:log`. |
...
| Design workbench | Green | Focused object-first helper coverage passed on 2026-05-21: `npx vitest run apps/portal/lib/drawings/state/objectFirstWorkbenchAdapter.test.ts` ran 13 tests for multi-form id generation, add/remove behavior, source-form cloning, and no-op safety. The broader last-known signal remains `npm run test:portal:workbench` on 2026-05-04 with 58 Vitest files and 589 tests, followed by the no-auth browser fixture pass. | Keep manual edit/save/reload, pricing-source rollout, and high-risk visual QA in release checks with authenticated staff data; rerun the full workbench and browser gates before treating this as release-complete. |
...
- Attempted `npm run test:portal:workbench`; it failed before the browser segment in `apps/portal/lib/drawings/commercialDesignPayload.test.ts` because `quantityTakeoff.flashings.totalLengthM` was 0 instead of 1.5. The focused commercial payload test fails the same way, so the blocker is tracked separately from the fixture browser gate.
...
- Stabilized the Fixture And QA Gates registry with fixture-only metadata for parity-critical baked workbench fixtures, a shared `listParityCriticalSanctuaryGeometryWorkbenchFixtures()` helper, geometry-ready solved-model assertions, metadata-backed commercial parity checks, representative saved estimate snapshot parity coverage, and supported-fixture takeoff smoke coverage. `npx vitest run apps/portal/lib/drawings/sanctuaryWorkbenchFixtures.test.ts apps/portal/lib/drawings/commercialDesignPayload.test.ts packages/geometry/src/takeoff.test.ts` passed with 3 files and 17 tests. `npm run test:portal:workbench` passed its Vitest segment with 57 files and 583 tests, then the no-auth browser segment was blocked by an existing normal portal Next dev server on port 3001; no pricing behavior changed.

...
Owner approved a goal loop targeting saved email visibility in1–2seconds without changing the UI. New performance changes require their own release approval; prior release authorization is not reused. Work branch codex/project-email-speed-20260917 starts from current main. Prior release evidence edits are retained; unrelated tsconfig newline change remains untouched.

| Requirement | Status / evidence |
| --- | --- |
| Baseline saved vs first-time reads | In progress. Real production accepted project reload to visible Read message:9843ms; quote project navigation:7087ms. These are browser-tool wall-clock observations including automation overhead, not precise network-phase measurements. Both had saved email evidence. First-time uncached sample still required; do not erase production snapshots to manufacture it. |
| Minimal project/customer checks | Implemented locally;25 focused identity/route tests pass. Both auth-bound project checks remain, with final staff-role and customer-identity revalidation. Query reads canonical project ID/contact ID/email only, omitting owner and work-model queries. Production schema confirms canonical id/contact_id with contact email on joined contact; preview timing still pending. |
| Reuse verified send identities | Implemented locally: bounded512-entry encrypted server-instance cache of verified RFC IDs,15minute expiry, project/recipient/provider-key binding, fresh canonical rows on every hit. Cold instances still fall back to provider; hosted benefit and whether additional cross-instance reuse is necessary remain unverified. Current uncached request can make8 external Resend identity reads with8second total bound even for saved mail. Keep canonical project/recipient/send visibility fresh; do not introduce plaintext shared private caches, cache authorization, or weaken24hour mail retention. Choose bounded encrypted reuse and test recipient/project/key changes, expiry, misses and failures. |
| Show saved data while refreshing | Existing15minute/24hour behavior retained; load path and visibility remount behavior still need full review. |
| Measurement and protected preview | Pending: phase timing, repeat samples, warm/cold cache separation, real preview, no increased mailbox limits or spending. |
| Security/correctness/release | Pending full cache implementation, focused regressions and final review. No sends, backfill, AI summaries or new production change in this goal. Ordinary-staff live check from previous release remains outstanding as explicitly deferred by owner. |

Previous publication step: inspect existing protected candidate dpl_BjTtaDCfBLKVCZUzMqie3FVouAmP (created successfully; do not duplicate), assign the existing protected review alias only after Ready, then measure warm/cold/first-time samples. Push88450 completed successfully including full pre-push checks. Draft PR153 is open. Implementation commit e9d7f4e1ad531c6ca5d611deb15425ab9ca4c1b1. Phase-header tests, Portal types and architecture checks pass. Receiver remains dpl_GPVBNDixT4g7CnR8CzFWhSWNPAp8. The existing candidate helper must use the new codex/project-email-speed-20260917 branch rather than its original clarity branch. Candidate receipt .codex-tmp/clarity/protected-candidate-e9d7f4e.json; PR https://github.com/velt-design/sanctuary/pull/153. Public production unchanged. Instrumented route and encrypted reuse are implemented;67 focused tests pass,2 paired-wire tests remain conditionally skipped without their external fixture. No performance release yet. Previous continuation is progress, not a blocked turn. The1–2second target is not achieved; do not call the narrow query change goal completion. Previous goal setup was planning; this continuation produced baseline evidence and tested implementation progress.

Performance continuation: e9d7f4e protected candidate is Ready and assigned to the stable review alias. Real accepted-project first visit 10933ms; repeat 7287ms (browser-tool wall clock, saved evidence, instance warmth not proven). The target remains unmet. Request logs show mail starts about2.6seconds after navigation because Overview gates it on the full snapshot. A local correction now mounts the independently authorized email query from the available summary, with separate Suspense from notes; access-ending state unmounts it.47 focused component/query/route tests pass. Optional numeric-only PORTAL_CORRESPONDENCE_TIMING_LOGS is enabled only in the next protected candidate to isolate remaining server delays. Next: publish this coherent correction and measure phases plus representative first-time/repeat reads. No production change. Overview owner remains ProjectPage/tabs; a history subcomponent extraction is deferred to avoid unrelated churn during the small mount correction.

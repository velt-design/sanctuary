...
function makeStraightHouseForm(
  overrides: Partial<HouseFormModel> = {},
): HouseFormModel {
  return {
    id: "house-form-2",
    label: "House 2",
...
describe("buildHouseFormReferenceGeometry", () => {
  it("builds the shared raw-house boundary from the form id and transform", () => {
    ...
    expect(result?.rawHouse).toEqual(
  ...
  it("preserves explicitly authored mono roof intent at the raw-house boundary", () => {
    const result = buildHouseFormRawGeometryInput(
      makeStraightHouseForm({
        id: "house-main",
  ...
  it.each(HOUSE_FORM_PRESET_REGRESSION_CASES)(
    ...
    (preset) => {
      ...
      expect(polygonMinX(second?.footprint ?? [])).toBeGreaterThanOrEqual(
        10000,
      );
      expect(polygonMinX(second?.footprint ?? [])).toBeGreaterThan(
        polygonMinX(primary?.footprint ?? []),
      );
    },
  );
});

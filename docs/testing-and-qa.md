...
PR-AI-004 database changes additionally require `npm run test:ai:db`. `scripts/test-ai-task-ledger-db.mjs` starts and removes its own disposable database, applies the test-only bootstrap, runs the exact task-ledger migration inside `BEGIN`/`ROLLBACK`, proves that no AI objects remain, reapplies the exact file, and executes the live RLS, grants, replay, changed-input, append-only, frozen-payload, and synthetic/no-effect/no-spend assertions. It reads no Supabase target credentials and must never run against a shared local, staging, or production database. The `AI Foundation` workflow runs the same harness on Supabase PostgreSQL 17 and upstream PostgreSQL 18. A missing Docker command or configured workflow is not a pass; wait for green executable evidence before deployment review.

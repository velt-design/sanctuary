...
Current website enquiry autoresponders keep the existing payload shape for preview compatibility, but new base pergola estimates are sent and stored as a single lower-only amount by setting equal low/high values. Their canonical one-pergola site snapshot explicitly requests Simple with no approval requirement, resolves the same active immutable published configuration as the public calculator, and stores its version provenance and resolved policy. Manifest `v2.3` retains the Simple 21% customer-price uplift and adds the Version 6 longer-rafter labour curve; the shared package calculation applies both to the calculator and autoresponder. Publishing the reviewed configuration switches portal, public calculator and autoresponder together, while older published manifests retain their original pricing semantics and labour curve. Missing or invalid publication omits the base estimate without blocking the enquiry; package defaults are not a fallback. Historical rows with unequal low/high values still preview as ranges. Short residential or commercial enquiries without enough dimensions for a reliable costing snapshot still receive the same confirmation, with the investment panel and estimate wording omitted. Optional blinds remain range-based, use the shared corrected `@sp/costing` baseline, and persist into the generated calculator draft with No cover unless staff later selects a flashing or pelmet.
...
- Marketing enquiry autoresponder budgets and auto-created estimate drafts share one canonical costing snapshot and one published configuration provenance. Saved calculator inputs must describe that snapshot (including the two-post standard assumption); do not recalculate separately for email and persistence or fall back to package defaults.

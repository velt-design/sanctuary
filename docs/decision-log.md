...
- The deeper fix is to migrate `form: 'gable'` -> `form: 'hipped' + openGableEndIds: <all terminals>` at the workbench draft normalization boundary so every consumer reads coherent state. The current helper is a UI-side workaround; the deep fix retires the migration's special case entirely. **Status as of 2026-05-14:** still open; with both rail + Plan callsites migrated through the helper, doing the deep migration becomes a smaller follow-up that also unblocks Milestone 13 session C (drop `'gable'` from the `HouseRoofForm` type union).

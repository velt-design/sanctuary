...
- `GET /api/staff/v1/projects/index` is the staff-authenticated Projects-list read model. It accepts bounded `archive`, `q`, `status` (with `stage` retained as a transport-compatible alias), `journey`, `state`, `page`, `pageSize`, and `sort` parameters; calls `staff_projects_index_v2()` through the request's auth-bound client; returns one exact-count project page plus its linked contact display rows, server-owned operational state, and query identity; and is always `private, no-store`. The V2 function fails closed when portfolio rollout completeness is not provable. Missing rollout schema returns `503 PROJECTS_INDEX_SCHEMA_NOT_READY`; `20260731000002_project_work_portfolio_rollout.sql` owns the current function and state-count contract.
...
- `GET /api/staff/v1/work-items/queue` is the staff-authenticated one-row-per-project Work Queue read model. It uses `requireStaffContext()` and the auth-bound client, pages `project_work_queue_v3()` in 1,000-row hosted-safe ranges, proves whole-portfolio marker/state completeness with `staff_project_state_counts_v1()`, composes durable work/recovery/state rows with canonical specialist candidates on the server, and returns bounded command metadata for normal staff actions. Dashboard consumes a limited preview of the same owner; personal reminders remain a separate route and table. Missing marker/state or queue schema (`PGRST200`, `PGRST202`, or `PGRST205`), incomplete/malformed portfolio counts, or reaching the explicit 5,000-row safety ceiling returns `503 WORK_ITEMS_UNAVAILABLE`; clients render the named not-ready state and must not expose stale actions.

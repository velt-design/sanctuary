...
export async function POST(req: Request) {
  ...
  const jobs = crewCtx.jobs.map((job) => (job.id === jobRow.id ? { ...job, mode: 'floating' as const } : job));
  ...
  await supabaseServer.from('scheduled_jobs').update({ mode: 'floating' } as any).eq('id', jobRow.id);

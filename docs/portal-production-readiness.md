...
| WB-01 | Yellow | The fixture journey measures selection/view-switch timing, Plan rendering avoids selection/hover render-model rebuilds, and workbench clients now memoize one solved base by draft/project identity. Selection, visibility, and viewport changes reuse that solved model; focused coverage proves identity reuse. The final current-head fixture performance run is pending. |

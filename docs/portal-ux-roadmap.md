...
2. **Project Page Command Centre**
   ...
   - Project shell Slices 1-2 are present in the current repository: the page is full width with project facts and stage in Overview; Commercial opens to a list-first Estimates surface before the authoritative embedded Calculator, followed by Quotes and Invoices; the Emails UI is retired while delivery/audit data and side effects remain unchanged. Staff-facing tabs are Overview, Commercial, and conditional Job Packs; compatibility keys remain `activity`, `estimates`, `quotes`, `invoices`, and `job-packs`.
   - The Project Work V2 new-project foundation, Work Queue, Dashboard preview, guarded legacy review, and authenticated staging command smoke are complete. Portal release merge `c9e73651` and snapshot-cache hotfix merge `809f2c5e` are deployed; the three exact Project Work files were applied individually to production, and authenticated read-only cutover checks passed without migrating a pre-cutover project or changing shared business rows.
   ...
   - Focused/component, production-owner fixture, responsive/accessibility, unchanged Project Detail budget, build/static, manual authenticated inspection, and automated authenticated read-only staging evidence passed on 2026-07-30. The Overview handoff was accepted under a narrow exception for aggregate Contacts/Calculator overruns reproduced at `060bea19`; a later isolated route-optimization build closed both without raising their ceilings.
...
3. Keep the unchanged Contacts/Calculator bundle ceilings and current green route-optimization evidence; retain the earlier Overview exception as historical truth rather than authority to raise budgets.
4. Bounded full-journey summary contracts for deposit, Schedule/Running Jobs readiness, and meaningful activity where evidence exists.
5. Activity category expansion and timeline clarity.
6. Dashboard/Project Work evolution without merging personal reminders.

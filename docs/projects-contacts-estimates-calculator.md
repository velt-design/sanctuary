...
The full preview also shows `Price by item`. Pergola rows use the saved names and exact customer sell prices; a single pergola absorbs shared site costs exactly as quote mapping does, while multi-pergola jobs receive the separate shared-site row. Every blind is listed at its authoritative `@sp/costing` price. Each module infill is listed beneath its pergola as `Included in pergola price`, because pooled infill materials remain part of pergola costing and must not be counted again as a quote line. Preserved legacy lighting appears when present. The item table never exposes internal true-cost columns.

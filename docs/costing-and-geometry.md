...
Version 2 adds a package-owned `Simple | Bespoke` classification. Simple is limited to one residential pitched-acrylic pergola/module, fascia/facade/soffit connection, deck brackets, normal access, easy ground, standard black finish, no infills, and at most 30m² single-storey or 20m² two-storey. Any failed condition, manual Bespoke selection, engineering, or full building consent resolves to Bespoke. From manifest `v2.1`, Simple overhead is `$750 ex GST` for the first productive crew-day plus `$500 ex GST` per additional productive crew-day, pro-rated. Manifest `v2.2` raises the Simple core customer-price uplift from 10% to 21% after the standard `1.25x` markup and before discount and GST; this is exactly 10% above the Version 4 customer price. Bespoke retains `fixed_plus_variable` and no uplift. Published `v2.1` retains the 10% uplift, while `v1.9` and `v2.0` retain the earlier `$2,000`/three-day Simple policy.

The staff calculator's `roof_material: none` mode is an open pergola frame, not a new roof form. `@sp/costing` normalizes it to the standard pitched structure at `0°`, fixes rafters, the front beam, and any attached ledger to `150x50`, and leaves the post profile unchanged. `rafter_spacing_mm` is a positive target maximum: it defaults to `500`, has no upper cap, and the engine evenly distributes rafters so the resolved spacing does not exceed it. Roofing, roof foam, gutters, downpipes, automatic or entered flashings, and their covering/drainage labour are zero. Open pergolas can use the existing Simple eligibility rules and progressive overhead, with a `10%` customer-price uplift instead of the current acrylic Simple uplift. Failed eligibility or approval requirements still resolve through Bespoke pricing. The public Simple cover calculator remains acrylic-only.

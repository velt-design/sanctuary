...
The active package manifest is `v2.0`. Published `v1.7`, `v1.8` and `v1.9` snapshots retain their stored rates and original package behavior when applied to this package base. A fresh draft copies the active editable rates but advances to `v2.0`; only publishing that draft activates total-purchase-cost stock selection. Cloning an older version preserves that version's semantics for rollback.
...
Version 2 adds a package-owned `Simple | Bespoke` classification. Simple is limited to one residential pitched-acrylic pergola/module, fascia/facade/soffit connection, deck brackets, normal access, easy ground, standard black finish, no infills, and at most 30m² single-storey or 20m² two-storey. Any failed condition, manual Bespoke selection, engineering, or full building consent resolves to Bespoke. Simple overhead is `$2,000 ex GST` for the first three crew-days plus `$500 ex GST` per additional crew-day, pro-rated; Bespoke retains `fixed_plus_variable`. Published v1.8 remains on the earlier pitched-acrylic overhead rule; v1.9 and later use the Version 2 commercial policy.

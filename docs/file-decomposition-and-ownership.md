...
| Quote and estimate tabs | Project detail quote and estimate workflows | `QuotesTab.tsx` is below the critical threshold and composes named query/selection, lifecycle, PDF-preview, modal, list, detail, and pure-model owners. Draft editing, local-first create/persist, and send-form orchestration remain in the composition owner. | If `QuotesTab.tsx` grows, extract draft editing plus local-first create/persist orchestration before adding another inline quote workflow. | `npm run test:portal:projects`, `npm run test:portal:quotes`, local-first tests |

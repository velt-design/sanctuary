...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | Calculator workspace shell, estimate state, and infill action/cost coordination are still concentrated in the main client, though pure calculator input defaults, normalization, browser-draft persistence/session state, module navigation modelling and presentation, configuration-section definitions/presentation/field options, structure/site/workflow field-schema generation, issue labels/section ownership/model construction/focus navigation, infill draft/selection/modal/delete-undo state, save confirmation/persistence/outcome orchestration, blinds/flashings custom editor markup, materials-debug request/lifecycle/export behavior, warnings/BOM/debug/labour/structure preview presentation, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation and styles, save-readiness, quote-status/save-preflight helpers, the guided infill dialog, selected-infill header, Opening/Supports/Results stage presenters, visual infill shape-template picker/model, support-confirmation model, infill UI/payload plus summary view-model helpers, infill overview/list presenters, save dialog presenters, flashings view-model helpers, and blinds view-model helpers now live outside `CalculatorGridClient.tsx`. | Move infill option-cost coordination into a controller and the remaining infill workspace/dialog JSX into a focused presenter. | `npx vitest run apps/portal/app/staff/calculator`, `npm run test:portal:projects`, `npm run typecheck` |

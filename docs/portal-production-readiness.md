...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, project picker, browser-draft persistence/session state, module navigation, configuration-section presentation, issue labels/section ownership/model construction, infill draft/selection/modal/delete-undo state, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation/styles, and the infill production Results presenter now live outside `CalculatorGridClient.tsx`. The client no longer owns draft hydration, local persistence effects, module action policy, section ordering, responsive field layout, issue model construction, infill draft cleanup, selection reconciliation, modal flags, delete-undo expiry, preview-resize effects, pricing-summary markup, price-impact ranking/styles, or Results-stage structure, but it still owns field construction, deeper save orchestration, issue focus timing, and infill telemetry/action/cost coordination. | Move field-schema generation and field-builder logic into a pure tested owner; run `npm run files:changed` before handoff. |

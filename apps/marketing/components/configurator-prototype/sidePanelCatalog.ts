...
export function parseSidePanels(value:unknown):SidePanel[]|null {
  ...
  for(const p of value){
    ...
    seen.add(p.opening);result.push({opening:p.opening,kind:p.kind,profile:p.profile,edge:p.edge,gap:p.customGap?p.gap:faceWidth(p.profile,p.edge),customGap:p.customGap,frame:p.frame,battens:p.battens,...(p.direction?{direction:p.direction}:{})});
  }return result;
}

...
`playwright/portal.calculator-infills.spec.ts` is the authenticated infill accuracy lane. It creates scratch-draft infills only, verifies the exact `2.4m x 2.1m` vertical-sheet pieces and purchases at desktop size, verifies the kerf-safe `3m x 1m` horizontal-strip purchase plan at `1024px`, and asserts that copied CSV contains the same canonical records shown in the two result tables. It does not save an estimate or create a quote.

...
- `packages/email-provider`: Node-only direct-`fetch` email provider boundary for canonical Resend request bytes, stable provider identity, typed outcomes, timeouts/abort handling, and Svix-signed webhook-envelope verification, imported as `@sp/email-provider`. The local correspondence candidate also has a bounded sent-email identity reader: one fixed-origin GET resolves a persisted Resend API ID to its RFC Message-ID only after exact ID and recipient-set validation. It returns no email body, retries nothing and does not persist or send. Project authorization, capped send-log selection and reply association remain application responsibilities. The local Portal integration reads authorized quote/invoice sends and durable enquiry audit/outbox evidence, computes conservative message-specific links and strips raw lineage before browser delivery. Real historical coverage and rollout remain unverified.

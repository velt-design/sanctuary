...
export function parseSidePanels(value:unknown):SidePanel[]|null {
  ...
  for(const p of value){
    ...
    if(p.species!==undefined&&(p.kind==='aluminium'||!['cedar','thermopine'].includes(p.species)))return null;
    seen.add(p.opening);result.push({...(p.species?{species:p.species}:{}),opening:p.opening,kind:p.kind,profile:p.profile,edge:p.edge,gap:p.customGap?p.gap:defaultSideGap(p.profile,p.edge),customGap:p.customGap,frame:p.frame,battens:p.battens,...(p.direction?{direction:p.direction}:{})});
  }return result;
}
export function describeSidePanels(panels:SidePanel[]=[]){return panels.map(p=>{
  ...
  const slats=`${p.kind==='aluminium'?'':(p.species==='thermopine'?'ThermoPine':'cedar')+' '}${p.profile.replace('x',' × ')} mm ${p.edge?'on edge':'flat'}, ${p.gap} mm clear gap`;

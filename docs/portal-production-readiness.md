...
This snapshot records the most recent known production-readiness state from the portal review and follow-up checks, with project-opening, Projects/Contacts navigation, and calculator browser coverage refreshed on 2026-07-19. The current repository ref at this update is `005be1a7`; this documentation pass re-ran only the dependency audit and source-level checks named in the affected rows, not the full portal gate.
...
| ----------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Portal tests | Yellow | Portal Quality run `29687042640` passed repository typecheck/lint, 296 portal Vitest files with 1,663 tests passing plus 11 intentional skips, the 61-page production build, all bundle budgets, security audit, fixture browser/performance, and authenticated smoke. That run predates `005be1a7`, and the current dependency audit now fails, so it is not current-head proof. | Resolve the audit blocker, then rerun the broad portal gate at the current ref before returning this row to Green. |
...
| Production security audit | Red | At `005be1a7` on 2026-07-28, `npm audit --omit=dev` fails with two high-severity PostCSS advisories (`GHSA-6g55-p6wh-862q` and `GHSA-r28c-9q8g-f849`). The repo resolves Next.js 16.2.11 and overrides PostCSS to 8.5.10; the earlier zero-vulnerability result on Next.js 16.2.10 is superseded. Portal Quality runs `npm run audit:security` as a blocking pull-request gate. | Resolve the dependency chain without accepting the audit command's breaking `--force` downgrade, then rerun `npm run audit:security` and the blocking Portal Quality gate. |
...
| Security and data boundaries | Yellow | The last-known browser/service-role/root-compat guards, portal quote/invoice tests, and marketing tests passed, but the current `npm run audit:security` fails as recorded above. JOB-03 adds a second explicit background-job service-role owner: the server-only provider webhook repository, which may call only the verified-acceptance reconciliation RPC after bounded streaming raw-body verification. Direct receipt/payload/job/PGMQ access remains revoked, the worker's local acceptance RPC remains lease-fenced, and provider/customer content stays out of logs and receipts. | Resolve and rerun the audit, then repeat the boundary sweep; manual staff workflow and public-token browser QA still require valid credentials and compatible data. |
| Portal build | Yellow | The last recorded fresh isolated Stage 2 `next build` passed on Next.js 16.2.10 with `Compiled successfully`, TypeScript completed, and 66 pages generated. The repo now resolves Next.js 16.2.11 at `005be1a7`, so that result is historical rather than evidence for the installed framework version. | Rerun the isolated portal build on Next.js 16.2.11 after dependency, quote, invoice, PDF, job-pack, or Next config changes before returning this row to Green. |
| Typecheck | Green | `npm run typecheck` passed on 2026-07-20 across marketing, portal, the standalone Node worker, `@sp/costing`, `@sp/geometry`, `@sp/jobs`, `@sp/quote-format`, and `@sp/theme`. | Keep typecheck in quick doctor and CI, including the dedicated Background Jobs workflow for changes to the durable-job foundation. |
...
| Docs and routing | Yellow | At `005be1a7` on 2026-07-28, `npm run docs:guard` and `npm run text:mojibake` are clean. `npm run docs:navigation` reports 31 dense documents without a simple cue. `npm run docs:impact` is advisory-clean as a command but currently maps the unrelated in-progress quote/PDF/email worktree changes to `docs/quotes-invoices-job-packs.md`, which has not changed in this documentation lane. | Keep the quote owner-doc reminder with the owning quote/PDF/email lane, and treat the 31 navigation findings as deferred structure work rather than expanding this truth-restoration slice. |

...
Important tables include `contacts`, `projects`, the V2 `project_work_model_versions` / `project_operational_states` / `project_work_items` / event / confirmation / receipt tables, the retained legacy-evidence table `project_task_checks`, `estimates`, `costing_configuration_versions`, `costing_configuration_publication`, `costing_configuration_audit_events`, `quote_versions`, `quote_send_logs`, `site_visit_events`, `schedule_items`, `deposit_invoices`, and `job_pack_generations`.
...
- `POST /api/staff/v1/projects` owns project creation. The browser submits one stable project command ID plus either an existing contact ID or one stable new-contact command. The server checks normalized exact email/phone duplicate candidates, then calls the transactional `project_create_v2` RPC, which creates the project, assigns the active Enquiry to Ellen, records its V2 model marker and Active state, and seeds the first-email obligation. The same owner policy applies to website intake. Matching stable-ID replays return the committed identity; a different payload returns `PROJECT_CREATION_COMMAND_CONFLICT`. Missing V2 schema fails closed with no legacy fallback. `20260731000002_project_work_portfolio_rollout.sql` brings every pre-cutover project through the same V2 boundary from its stored stage.
...
- The persistent portal content boundary renders the real header, actions, filters, and truthful "Updating projects" list frame synchronously on an ordinary Projects-index click, while keeping the sidebar available; the matching route loading boundary uses the same frame for direct route transitions. The page entrypoint does not await request search params or data; its client controller reads the current filters and date, then dismisses the pending frame after mounting. One authenticated React Query request to `GET /api/staff/v1/projects/index` loads one bounded server-filtered/sorted active/archived/all page plus only the linked contact display rows needed by that page through `staff_projects_index_v3()`. The response includes server-owned journey, detailed stage, `ACTIVE|WAITING|CLOSED|ARCHIVED` state, Project Owner, Waiting/Closed context, and one server-ranked next-action summary for only the returned page. On narrow screens the search remains visible while secondary filters collapse behind one disclosure.
- Projects-index reads have five states: `pending`, `cached`, `fresh`, `refresh-failed`, and `unavailable`. A response is usable only when archive, search, stage, journey, state, sort, page, and page-size identity match the current request, so one scope/filter cannot borrow another scope's rows. Known same-request rows remain visible during background refresh and network/server failure, while `401`/`403` hides them. An empty state is truthful only after a fresh successful response.
...
- Links to the Projects index update the canonical `/staff/projects` URL immediately, bypass the blocking route overlay, and preload route/data only on hover, focus, touch, or pointer-down. Project opening reads the freshest matching combined Projects-index response directly for its immediate project/contact shell, with the established canonical list keys retained as a compatibility fallback; a current-user global-search result is the final summary-only fallback when the project was opened from search. The exact project route and authenticated snapshot query preload only from destination intent. Mutations patch or invalidate the combined, canonical, and search caches through their existing owners.
...
- Legacy project-task/action UI, browser controllers, and HTTP routes are retired. Historical task/follow-up/check/action rows remain database evidence only and are not rendered or executed. The shared V2 visibility policy still fails closed as `Legacy work needs review` if stale prohibited data crosses the server boundary; browser code never selects a replacement, and Call/Site Visit work remains hidden.
- Overview renders one V2 `ProjectWorkSection` from the command-centre/snapshot boundary. There is no second Tasks card, sidebar, or legacy project-task presentation. Blocked work is an exception rather than an enabled primary action. `projectWorkCache.ts` is the only cache patch/invalidation module: `patchProjectCommandCentreCache` owns complete command-centre response patches, the projection helper owns command-centre/snapshot/summary fan-out, and the shared invalidator refreshes project, Work Queue, and Dashboard reads. Controls pause whenever the owning read is refreshing, stale, refresh-failed, rollout-incomplete, or disagrees with the other authoritative read.
...
- When the pipeline is still at Sent, the Overview commercial card exposes **Record deposit received** only after the first whole invoice for the current accepted lifecycle version has been marked paid in Invoices. One locked database command verifies that paid invoice and its date, then conditionally projects the Sent-to-Deposit stage and `deposit_paid_date`; staff cannot type a date that disagrees with the ledger. PostgreSQL records the first transition in immutable `projects.deposit_received_at`, and replay returns the same invoice-backed evidence. It is not modelled as a generic V2 work-item completion.
...
The one-way compatibility projection writes the highest-ranked open item to `projects.next_action*` and `follow_up_date` only for remaining compatibility consumers and clears those fields for Waiting, Closed, or archived projects. Projects Index uses `staff_projects_index_v3` for journey/stage/state/owner filtering and page-scoped authoritative action enrichment, while Dashboard uses `staff_project_state_counts_v1`; neither derives lifecycle truth in the browser. The compatibility projection excludes blocked work and specialist actions and is never a reverse import.
...
- Calculator inputs freeze the requested `Simple | Bespoke` classification and `Neither | Engineering required | Full building consent` approval requirement. The package freezes the resolved classification/reasons and any approval sell allowance in outputs. Ineligible Simple inputs and either approval option automatically step to Bespoke. Approval lines already include markup, receive GST only, never receive quote discounts, and pass to quotes from the frozen output rather than being recalculated.
- Staff can select `Open / no roof covering` for a standard pitched pergola frame. This mode fixes pitch at `0°`; defaults rafters, front beam, and attached ledger to `150x50`; allows each profile to be changed independently across the available `50mm`-wide sizes; retains the chosen post profile; and accepts a positive manually typed target rafter spacing with a `500mm` default and no upper cap. It removes roofing, gutter, downpipe, flashing, foam, and roof-covering labour from the quote. Changing from Open to Acrylic, Timber, or Mixed applies fresh standard pitched roof defaults: Auto pitch (currently `5°`), the primary flashing, standard gutter settings, and no Open-injected ledger/rafter/front-beam overrides. Dimensions, connections, post count/profile, colours, infills, and other job context remain unchanged. Eligible jobs use the active versioned Simple policy; approvals or failed Simple conditions fall back to Bespoke.
...
The calculator workspace keeps workflow context and save readiness in a persistent command bar. At `/staff/calculator`, staff can search active projects by name, quote reference, or address; selecting one opens that project's calculator workflow and active draft while the scratch draft remains in local/session storage under its original key. Warm navigation renders the shared Calculator workspace frame immediately and preloads its exact route chunk from intent; calculator inputs, drafts, costing results, and save authority remain with the mounted specialist owners. The project-page wrapper fixes the Calculator to its current project, passes the existing host/cache identity and a typed design-navigation model into the workspace, removes the project picker, and renders the Calculator as an embedded section beneath the measured sticky masthead. Embedded mode folds design selection, editing/module context, browser-draft state, freshness, and Save into one compact command bar; it does not repeat the project name or Calculator heading. Historical, invalid, loading, and failed design states retain a minimal navigation row because the full Calculator command bar is not mounted there. The standalone Calculator retains its heading and project picker.

The mounted command bar uses one source, reading, visual, and focus sequence: project/design/draft/module identity, readiness, then one Save. Responsive wrapping does not reorder that sequence. Readiness presentation derives a root-cause count, blocked-check count, and review count from the existing Quote Status rows without changing `hasStatusBlockers` or Save preflight. Input-caused Engine rows remain visible but say they are blocked by input issues; Updating and recalculation pending are waits for a current result, while an independent Engine error remains its own cause. The primary label uses singular/plural causal language such as `1 input issue blocks Save`, and Quote Status retains the complete check list plus its separate blocked-check count.
...
Commercial > Estimates is list-first and shows every version with Create estimate, Create add-on estimate, Open/Edit in calculator, Rename, and Duplicate actions. The original contract uses the base commercial scope (`commercial_scope_id = null`); each add-on uses one stable UUID across its estimate revisions and later quote family. The newest editable draft is resolved independently per scope, so work such as blinds, an infill, or an extra post can remain in the existing project without revising or superseding the original accepted quote. A new base estimate still starts with one pergola, while a new add-on starts with zero pergolas and an explicit Add pergola action. Add-on drafts may stay at zero or contain one or more pergolas; zero-pergola normalization, browser-draft restoration, saving, current costing snapshots, and quote handoff must not synthesize a fake module or pergola line. Add-on rows are explicitly badged, while opening, duplicating, revising, saving, deep links, and browser history retain that scope. Each estimate may have a searchable staff-only `internal_name`; the name is the primary list identity while the immutable version label remains visible beneath it, and existing unnamed rows fall back to that version label. Create may collect the optional name before opening the Calculator, rename remains available even when pricing is historically locked, and Duplicate seeds `Copy of <name/version>` without mutating its source. Entering the Calculator from the list pushes a history entry and switches Project Detail into a focused workspace: the two-row project header and Commercial subtabs are replaced by one compact Back to estimates, project-name, and estimate-version bar, while the Calculator uses the remaining viewport. Browser Back or the compact return control restores the normal project header and Estimates list. `estimateId` opens an estimate, `newDesign=1` opens a deliberate blank design, and `fromEstimateId` loads a copy as a new unsaved revision. Historical records remain locked sources: opening one shows the explicit locked-source state, while Duplicate or Start revision enters the existing `fromEstimateId` flow without mutating the source. Saving a blank design or revision creates a new estimate, then replaces `newDesign`/`fromEstimateId` with the saved `estimateId` without replaying the mutation. Estimate internal names are portal metadata only and never enter quote line mapping, customer PDFs, or emails. Per-design internal notes remain stored for compatibility but no longer have a project-page editing surface.
...
Multi-module calculator drafts use one canonical module navigator. Modules remain in stored array order but are grouped under every pergola, including an empty pergola left by Move, and their computed identity is `Pergola N · Module N` using the module's local ordinal inside that pergola. Staff can replace each default pergola label with a job-specific name; that name follows the pergola into the saved estimate, price breakdown, and quote line description. Rows show style, dimensions, active state, and validation-error count. Add module and Add pergola create fresh defaults; an empty estimate shows its zero-pergola state and Add pergola remains available. Attached modules start with a facade connection and an editable post suggestion that keeps spacing at or below four metres along the front beam, while freestanding templates suggest the same layout on both support beams. Length and connection changes keep following the suggestion only while the current count matches the prior suggestion, so staff-entered counts are never enforced or overwritten. Duplicate deep-copies the active module and regenerates flashing/infill IDs; Move changes only `pergolaId`; Remove updates the browser draft immediately, except removing the final pergola asks for confirmation and then preserves a valid zero-pergola draft. Infill deletion follows the same immediate draft pattern and offers a short Undo action. No removal updates the estimate until Save, where the existing save confirmation and validation preflight remain the single modal decision point. Common starting templates replace the active module immediately after staff choose a template and press Apply to active module; the deliberate select-and-apply sequence supplies intent without another modal. The pergola name and site-level allowances remain unchanged. The navigator is a sticky rail at calculator widths of 1080px and above and an accessible modal launcher below that width. These actions do not reorder modules or change estimate versioning or automatic persistence semantics.

Additional aluminium is estimate-level, not module-level. Staff select full-bar profile, stock length, quantity, and finish once for the job; it remains available when the estimate has no pergola and prices as a separate materials-only item. Historical module-owned rows are lifted to the estimate level on load using their original module finish so saved pricing is preserved.
...
Calculator configuration section order, disclosure behavior, and field-width presentation live outside `CalculatorGridClient` in the configuration-section model and form presenter. There is no global Basic/Advanced mode. Flashings and Structural overrides are always available as closed native disclosures; Issue Jump opens a containing disclosure before focusing its invalid field. The legacy House Footprint configuration section is absent, while historical footprint values remain readable for old estimate drawings. The two-column configuration workspace keeps module navigation as one structural owner and groups templates plus the complete configuration form in one main-column stack; new controls must join the appropriate stack or declare their grid ownership rather than relying on auto-placement. Templates and specialist Flashings content respond to their own available container width before they can widen or be clipped by the configuration pane. Consecutive ordinary sections share quiet sheets with subtle dividers, while Lighting, Blinds, and Infills remain separate configuration cards. Lighting follows the active pergola, appears when that pergola has an acrylic-roof module, and shows quantity, dimmer, derived driver count, an itemized GST-inclusive price, and the package-owned capacity rule. A configured lighting row remains visible and blocks Save/quote if acrylic eligibility is later removed. Labels and input-like toggles use a consistent control rhythm; routine field helper subtext is not rendered, while validation errors remain attached to their fields and visible. Automatic Roof pitch and Downpipes inputs add one presentation-only cue sourced from the selected authoritative result: current output says what Auto uses, retained output says what the last valid result used, and waiting output makes no numeric claim. Raw blank pitch and downpipe `0` are not rewritten; explicit values get no Auto cue; errors temporarily replace the cue in `aria-describedby`. Drainage controls and issue routing use the full `Downpipe joins` and `Downpipe elbows` labels without changing `downpipeJoinCount` or `downpipeElbowCount`. The responsive field grid deliberately renders three columns at the 1600px and 1024px browser checks, and two columns at 1366px and 768px, with one column on narrower mobile widths. Specialist Flashings, Lighting, Blinds, and Infills controls use the full section width. The presentation-only Orientation diagram is omitted. The narrow command bar keeps Save prominent while retaining project/module identity, browser-draft explanation, and readiness.

Calculator money labels distinguish internal costing from customer-facing values. The pricing preview's primary figure is the complete customer total produced by summing the same cent-accurate sell lines used at quote handoff: each pergola, shared site costs, valid blinds, and structured per-pergola lighting. The saved `quoteDiscountPct` applies only to pergola and shared-site sell lines; blinds and lighting remain at list price. Exactly one persistent customer summary is visible for each layout: the Result Inspector header in the split workspace and the early result card in the stacked workspace. Both inc-GST and ex-GST values are explicitly labelled as rounded to the nearest whole dollar. The Pricing panel does not repeat that total; it carries the undiscounted exact-cent context and discount scope, unpriced-item qualification, permission-gated internal-cost disclosure, and exact-cent `Price by item` table with a matching footer total. Invalid blinds or lighting appear as unpriced, are excluded from the partial displayed total, and remain Save/quote blockers. Each valid lighting row is a separate quoteable pergola accessory; old snapshots without structured lighting retain their historical total as one compatibility line. Blind base/fabric pricing receives its shared `1.15x` core uplift before GST; the fixed `$900 inc GST` motor and width-priced roll-cover options (`No cover`, `$44/m inc GST` flashing, `$145/m inc GST` pelmet) are not uplifted. The calculator labels the vertical dimension `Blind drop` to distinguish it from the roll cover. Saved snapshots missing `rollCover` normalize to No cover without a schema or database migration.

The desktop preview rail is a tabbed Result Inspector. Its header keeps the current or last-valid customer price, freshness, input-issue count, and quote readiness visible while Pricing, Materials, Labour, Workings, and Issues route the existing output owners. Pricing contains pricing context, exact-cent price-by-item, permitted actual-cost review, and admin cost-impact surfaces without repeating the rounded lead total. Materials consumes the whole-job `@sp/costing` trusted breakdown: all BOM rows remain grouped by user-facing purpose and show quantity, unit, scope ownership, profile, assumptions, and supported stock/sheet `Why this quantity?` disclosures. Labour is available to every staff user and preserves every work-stage activity, quantity, unit, minute, crew-hour, non-neutral loading, owner, and calculated explanation; only admins see labour dollar costs. Each Materials/Labour group is a native disclosure keyed by its stable package group ID; the first starts open, the rest start closed, and user choices survive current/retained rerenders while the panel remains mounted. Material summaries show line count; Labour summaries show activity count and group crew hours. Routine explanations use plain purchasing/crew language, while unchanged package IDs are nested under collapsed `Technical source`. Staff see quantities, total crew time, and explanations but never hourly rates or internal dollar totals; admins retain cost visibility and a separate closed diagnostics disclosure. Both breakdowns explicitly label retained last-valid results and fail closed for old outputs without the contract. Workings follows the selected module and presents its rafter answer, written calculation, read-only Plan/Section drawings, and structure outputs without a detail-mode gate. The Calculator no longer exposes house move, rotate, preset, outline, or footprint-edit controls in its plan. Issues contains the cause-aware Quote Status and warnings. Jump closes the Issues dialog, completes any module switch, opens a containing disclosure when required, then uses the nearest real vertical scroll owner to reveal the complete field and inline error below visible chrome before moving focus to the invalid control; queued routing is cancellable and does not move the Inspector rail. The Workspace controls the active tab. A genuine tab change resets the independent desktop result rail to its start, while same-tab activation does nothing; ordinary tab changes in the stacked layout do not move the page. Tabs use the standard keyboard arrow, Home, and End behavior; inactive panels remain mounted so switching tabs does not discard disclosure, drawing, debug, or actual-cost state. The tab strip becomes horizontally scrollable in a narrow preview rail, while the established stacked-layout page scrollbar remains authoritative below `1080px`. Calculations, permissions, save behavior, quote mapping, and source-of-truth boundaries are unchanged.
...
The full preview also shows `Price by item`. Pergola rows use the saved names and exact customer sell prices; a single pergola absorbs shared site costs exactly as quote mapping does, while multi-pergola jobs receive the separate shared-site row. Each physical module is an indented explanatory child whose customer-price allocation is weighted by that module's package-produced direct cost; the shared deterministic cent allocator assigns any remainder so module rows reconcile exactly to the pergola sell cents. These allocations do not create independently quoteable lines. Every blind and structured pergola-lighting row is listed at its authoritative `@sp/costing` price. A ready package-owned v2 infill attribution adds each configured infill beneath its owning module and shows the incremental amount already contained in that module allocation. Infill children are never added to the customer total or quote as separate lines. Admins may expand each infill's incremental materials, labour, overhead, and total true cost ex GST; staff receive no internal-cost markup. Historical or blocked results without ready v2 attribution retain `Included in pergola price` without an invented infill amount. Preserved legacy lighting appears only when structured lighting is absent.

Manifest `v2.4` freezes `pricing_policy.customer_price_multiplier: 1.3` and zero classification uplift for both Simple and Bespoke. Manifest `v2.5` keeps that shared multiplier and raises the once-per-site operational startup from `$500` to `$1,000 ex GST`; published `v2.4` configurations retain the `$500` startup. Manifest `v2.6` retains those semantics and adds a `60`-minute allowance once per infill job plus `30` minutes for each genuinely sloping or triangular infill opening; published `v2.5` and earlier configurations retain their prior infill labour. Older outputs without the multiplier continue to use `1.25`; their frozen Simple uplift remains authoritative. Calculator price rows, dashboard summaries, marketing pricing and quote handoff all use the frozen multiplier and uplift through `@sp/costing`. Site days derive from productive installation actions for both classifications; Bespoke productive actions are multiplied by `1.2`, while mobilisation/demobilisation remain charged once and are not lengthened.
...
- `patchProjectCommandCentreCache` in `projectWorkCache.ts` is the sole complete command-centre response patch owner used by header-owner commands; V2 work commands use the projection patch owner.
...
- Multiple accepted rows in one family select the newest accepted-lifecycle version deterministically and emit an integrity warning. A later declined or superseded version that was previously accepted tombstones that family instead of reviving an older acceptance.

`GET /api/staff/v1/projects/[projectId]/command-centre` owns the bounded auth-bound commercial, owner, and Project Work read. It returns either the current V2 projection or a non-actionable retired-model integrity response for an unexpectedly unmarked project; it exposes no legacy command contract. For V2, one central projection ranks safe recovery, critical/overdue/today work, a stage-eligible specialist action, future work, then triage. The specialist adapter covers quote-delivery recovery, Contacted/Site Visit routing, durable draft-quote continuation, and Quoting-only creation from a valid estimate. `qk.projects.commandCentre(host, projectId)` owns its user-scoped browser cache. The query is immediately stale and refetches when Overview remounts, so returning from a specialist tab refreshes state without adding cache logic to the critical `QuotesTab.tsx` or Calculator workspace. A `401`, `403`, or `404` hides the command centre and project snapshot and clears protected project-domain query families for the current host.

The project header shows one Project Owner and remains the only owner-management entry point. The Overview command grid places the one Project Work region beside the design/commercial region. The owner is required from lead through Delivery. Ellen owns every active New/Contacted project; after a manual move into Proposal an admin explicitly assigns its Proposal owner, and before leaving Proposal that owner hands over and assigns Dave for Confirmed/Delivery. The approved roster is Ellen, Jordan, JP, Joe, Bruce, and Dave. These are ownership changes only: stages remain manual. Project Details treats next action as Overview-managed and excludes it from local-first API payloads. V2 work items are the only actionable project-work model; retained legacy stage rows are not rendered or executed. Dashboard Project Exceptions is a V2 project-integrity read. `portal_dashboard_tasks` powers the separate personal **My Tasks** reminders and never represents a project's operational obligation.

A committed V2 `CLOSE` command with one of the structured lost outcomes records `marketing.project_lost` after the state transition succeeds. `COMPLETE` is not a lost outcome. Selecting a structured `Lost` outcome is sufficient to close: the route supplies a neutral outcome-derived cancellation explanation for remaining work when staff omit free text, and an additional note remains optional. The audit event contains only the bounded outcome code for lifecycle analytics; free-text cancellation reasons remain outside marketing delivery. Consent, outbox, GA4 mapping, and retention rules are owned by `docs/security-privacy-quality.md` and `docs/automation-email-audit.md`.
...
- Automation events, V2 Project Work, retained legacy task/follow-up evidence, email outbox, and audit behavior: `docs/automation-email-audit.md`.

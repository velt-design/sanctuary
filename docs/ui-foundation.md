...
Within Project Work, `ProjectWorkControls` owns V2 manual/state/correction controls, the stage-gated direct Site Visit link, and the separate manual completion fact; `useProjectWorkCommandController` owns V2 stable command/feedback orchestration; and `useLegacyProjectWorkCommandController` owns the equivalent legacy stable command and cache orchestration. `ProjectPrimaryActionCard` is only the nested legacy shell, with conflict/fallback in `LegacyProjectWorkConflict` and audit disclosure in `LegacyProjectWorkHistory`. `projectWorkVisibilityPolicy` is the shared fail-closed Call/Site Visit work-row filter across both models and recent events; it does not suppress the separately approved direct control. All cache writes remain in `projectWorkCache`: `patchProjectCommandCentreCache` is the sole complete command-centre response patch owner, while V2 projection fan-out and shared invalidation retain their named owners there. None of these owners adds shared tokens, restyles another route, infers project truth, or moves specialist workflow ownership.

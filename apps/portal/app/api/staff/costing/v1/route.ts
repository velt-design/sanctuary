...
function parseFlashings(raw: unknown): CostInputsV1['flashings'] | { error: string } | undefined {
  if (raw === undefined) return undefined;
  if (!raw || typeof raw !== 'object') return { error: 'flashings must be an object' };
  ...
  if (defaultOverridesRaw !== undefined) {
    if (!Array.isArray(defaultOverridesRaw)) return { error: 'flashings.default_overrides must be an array' };
    for (let i = 0; i < defaultOverridesRaw.length; i += 1) {
      const item = defaultOverridesRaw[i];
      if (!item || typeof item !== 'object') return { error: `flashings.default_overrides[${i}] must be an object` };
      const key = typeof (item as any).key === 'string' ? (item as any).key.trim() : '';
      if (!key) return { error: `flashings.default_overrides[${i}].key must be a non-empty string` };

      const bandRaw = (item as any).band;
      if (bandRaw !== undefined && !isOneOf(FLASHING_BANDS_OR_NONE, bandRaw)) {
        return { error: `Invalid flashings.default_overrides[${i}].band` };
      }
  ...
  if (extrasRaw !== undefined) {
    if (!Array.isArray(extrasRaw)) return { error: 'flashings.extras must be an array' };
    for (let i = 0; i < extrasRaw.length; i += 1) {
      const item = extrasRaw[i];
      if (!item || typeof item !== 'object') return { error: `flashings.extras[${i}] must be an object` };

      const bandRaw = (item as any).band;
      if (bandRaw !== undefined && !isOneOf(FLASHING_BANDS, bandRaw)) {
        return { error: `Invalid flashings.extras[${i}].band` };
      }

      const length_m = toNumber((item as any).length_m);
      if (!Number.isFinite(length_m) || length_m < 0) {
        return { error: `flashings.extras[${i}].length_m must be a number >= 0` };
      }
...
export async function POST(req: Request) {
  ...
  const parsedFlashings = parseFlashings(body.flashings);
  if (parsedFlashings && 'error' in parsedFlashings) return badRequest(parsedFlashings.error);

...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | Estimate/project state, house-footprint interaction wiring, validation construction, and several custom input mutation controllers are still concentrated in the main client, though the outer calculator workspace shell and preview/configuration presenters now live in `CalculatorWorkspaceView.tsx`. Pure calculator input defaults, normalization, browser-draft persistence/session state, module navigation modelling and presentation, configuration-section definitions/presentation/field options, structure/site/workflow field-schema generation, issue labels/section ownership/model construction/focus navigation, infill draft/selection/modal/delete-undo state, infill item mutations/stage policy, infill clipboard/hotkeys/warning focus/telemetry, infill baseline/variant cost coordination, infill dialog/stage/rail/empty-state presentation, save confirmation/persistence/outcome orchestration, blinds/flashings custom editor markup, materials-debug request/lifecycle/export behavior, warnings/BOM/debug/labour/structure preview presentation, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation and styles, save-readiness, quote-status/save-preflight helpers, guided infill stages, infill UI/payload/summary view models, and save dialog presenters also live outside `CalculatorGridClient.tsx`. | Move house-footprint editor gesture/lifecycle state into a focused controller, then extract calculator validation construction and custom blinds/flashings mutation controllers. | `npx vitest run apps/portal/app/staff/calculator`, `npm run test:portal:projects`, `npm run typecheck` |

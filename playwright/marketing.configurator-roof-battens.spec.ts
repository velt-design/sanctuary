...
for(const width of [390,1440])test('roof battens profiles, materials, views and saved enquiry at '+width,async({page})=>{
  ...
  for(const name of ['Gable','Box perimeter','Pitched']){
    await page.getByRole('radio',{name,exact:true}).check();expect(await page.locator('[data-roof-batten]').count()).toBeGreaterThan(10);
  ...
  await page.reload();await page.getByRole('button',{name:'Design your pergola',exact:false}).click();await expect(gap).toHaveValue('27');
  ...
  await page.getByRole('radio',{name:'Solid',exact:true}).check();await expect(c).toHaveCount(0);

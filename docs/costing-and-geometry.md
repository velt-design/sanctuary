...
`GeometryQuantityTakeoff` includes package-owned physical buckets for primary/secondary dimensions, roof planes, members, beams, gutters, roof cladding, joiners, flashings from `Assembly3D.roofFlashings`, and rafter layout facts such as bay count and average spacing. Low-level `QuantityHook` values remain compatibility data. Custom calculator flashing rows, downpipes, sheet rounding, BOM, labour, markup, and pricing policy remain outside geometry.

...
- Run `npm run architecture:changed:strict` only for architecture/tooling PRs or explicit strict verification; routine handoffs should use the advisory aggregate. Strict mode starts with `worktree:changed:strict`, so dirty-tree runs must declare `WORKTREE_OWNER_PATTERNS`; it also includes the new-unused-file dead-code gate, is CI-visible as non-blocking Portal Quality advisory output, and remains outside `npm run lint`.
...
| Worktree ownership | `docs/parallel-work-guardrails.md` | Run `npm run worktree:status` before editing in parallel or dirty-tree work; `architecture:changed` includes `worktree:changed` at handoff. Use `WORKTREE_OWNER_PATTERNS=... npm run worktree:changed:strict` when a dirty lane needs strict verification. |

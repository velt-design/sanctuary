...
- Praxis Context Intelligence PR11, Sanctuary reporting boundary: implemented in the repository as a default-dark, 12-resource, versioned read projection behind an exact source binding and dedicated low-privilege reader. The migration, environment LOGIN/identity, credentials, Velt connection, live reads, model use, and deployment remain separate and unapplied.

...
- 🟡 **Per-object solve**: the geometry pipeline still iterates per pergola module (Stream 2B.1 pending). Object-first pergolas without a persisted module now use a runtime-only solve source, and the rail can add new freestanding object-first pergolas through that path, but the solve spine still adapts each pergola through a temporary `CalculatorModuleInputs` shape until the full per-object rewrite lands.

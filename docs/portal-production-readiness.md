...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, project picker, browser-draft persistence/session state, module navigation, configuration-section presentation, structure/site/workflow field-schema generation, issue labels/section ownership/model construction/focus navigation, infill draft/selection/modal/delete-undo state, save confirmation/persistence/outcome orchestration, blinds/flashings custom editor markup, materials-debug request/lifecycle/export behavior, warnings/BOM/debug/labour/structure preview presentation, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation/styles, and the infill production Results presenter now live outside `CalculatorGridClient.tsx`. The client no longer owns draft hydration, local persistence effects, module action policy, section ordering, responsive field layout, field option/schema construction, issue model/focus timing, infill draft cleanup, selection reconciliation, modal flags, delete-undo expiry, save dialog state/persistence callbacks, blinds/flashings rendering, materials-debug effects, preview-detail cards, preview-resize effects, pricing-summary markup, price-impact ranking/styles, or Results-stage structure, but it still owns the workspace shell and infill telemetry/action/cost coordination. | Move infill option-cost coordination and the remaining infill workspace/dialog JSX into focused owners; run `npm run files:changed` before handoff. |

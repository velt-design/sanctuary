...
export default function SideGapControl({value,onChange,roof=false}:{value:number;onChange:(gap:number)=>void;roof?:boolean}){
  ...
  return <div className={styles.dimension}>
    <div className={slider.dimensionHeading}><label htmlFor={roof?"roof-gap-range":"side-gap-range"}>Clear gap</label><label className={slider.dimensionValue}><input aria-label={roof?"Roof batten gap in millimetres":"Clear gap in millimetres"} inputMode="decimal" value={draft??String(value)} onFocus={e=>e.currentTarget.select()} onChange={e=>{setDraft(e.target.value);setNotice('');}} onBlur={commit} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();}}/><span>mm</span></label></div>
    <div className={slider.rangeControl} style={{'--range-progress':`${(value-min)/(max-min)*100}%`} as CSSProperties}>
      <input className={slider.range} id={roof?"roof-gap-range":"side-gap-range"} aria-label={roof?"Roof batten clear gap":"Side slat clear gap"} aria-valuetext={value+' mm'} type="range" min={min} max={max} step={1} value={value} onChange={e=>{setDraft(null);setNotice('');onChange(Number(e.target.value));}}/>

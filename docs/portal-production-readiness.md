...
| Browser smoke | Yellow | The fixture-safe workbench performance journey passed, and drawing smoke passed with 5 tests plus 1 intentional scenario skip after aligning its diagnostic assertion with the established `geometry-canvas` contract. Authenticated calculator customer-pricing, module switching, local restoration, stale/live state, two-column 1366px fit, and no-clipping checks passed at 1600px, 1366px, 1024px, and 768px using browser-draft-only mutations. The canonical provisioned suite remains blocked because the stored staging record still has one module while the repository fixture generator correctly defines three across two pergolas. | Refresh the local/staging scenario with service-role provisioning, then rerun the canonical calculator, authenticated smoke, and authenticated performance gates. |

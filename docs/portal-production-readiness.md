...
| --- | --- | --- | --- |
| Portal tests | Green | `npm run portal:doctor:quick` passed locally on 2026-07-19 with 275 files/1,577 tests plus 11 intentional skips. The focused Projects/Contacts suite passed with 58 files/286 tests, and the shell suite passed with 21 files/61 tests. | Keep the service-role owner explicit and rerun quick doctor after shared auth, persistence, or API changes. |
...
| Portal performance evidence | Yellow | Slice 1 project opening and Slice 2 Dashboard-to-Projects remain green at the locked 100/500 ms ceiling. Slice 3 Dashboard-to-Contacts implementation and local gates are ready, but its required five authenticated production repetitions have not yet completed. | Run Portal Quality exactly five times, record p50/p75/p95, apply the downward-only ratchet, then return this row to Green if all repetitions have no blocking overlay or ordinary long task. |

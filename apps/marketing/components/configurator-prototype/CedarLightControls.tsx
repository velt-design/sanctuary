...
export default function CedarLightControls(){
 ...
 const settings=(section?:string)=>{
  ...
  return <div className={css.tools} role="group" aria-label={section?section.replace('section-','Section ')+' lighting':'All ceiling sections'}>
 ...
 <p><strong>{w.value.cedarCount} ceiling downlights total</strong>{slopes===2?' · Includes both slopes':''}</p>
 <p>Centred between rafters within each ceiling section. Acrylic stays clear. Only grids that fit {w.value.cedarIndividual?'this section':'every section'} are offered.</p>
 </div>;
}

...
| Calculator Brain configuration | Yellow | Repository implementation adds package-owned typed configuration/validation/diff/impact, admin history/draft/clone/publish UI and APIs, immutable database versions with atomic audited publication, fail-closed staff reads, exact calculation provenance, and estimate version linkage. Focused costing/estimate/admin/control-centre coverage passed 31 files and 301 tests; the full portal Vitest suite, typecheck, lint/docs/package guards, and static migration contract are green. Existing live override rows were inspected read-only and use supported scalar action shapes. | Keep Yellow until `20260723_000001_costing_configuration_versions.sql` passes executable PostgreSQL/Supabase smoke and an authenticated admin browser pass proves clone, invalid-field feedback, diff/impact, stale conflict, publish, calculator use, estimate provenance, and rollback. No shared database migration or publication was performed in this repository pass. |

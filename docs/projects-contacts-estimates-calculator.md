...
- The Projects route loading boundary renders the real header, actions, filters, and truthful "Updating projects" list frame immediately. The page entrypoint does not await request search params or data; its client controller reads the current filters and date. One authenticated React Query request to `GET /api/staff/v1/projects/index` loads the selected active/archived/all project scope and complete contact list concurrently through the existing paginated server loader.
...
Calculator infills keep draft-string and field validation in the portal, but every complete infill delegates geometry, source/orientation resolution, joiner/support cuts, and stock purchasing to `calculateInfillsTakeoffV1()` from `@sp/costing`. The cut-list result has two groups: `Pieces to cut` for individual finished panels and linear cuts, and `Materials to purchase` for physically allocated stock. CSV exports those same canonical records, including source IDs and allocated stock. A critical takeoff error makes the infill incomplete, hides exportable rows, and participates in Quote Status/save blocking.

The infill configurator is a guided `Opening` -> `Existing supports` -> `Results` workflow. Required opening measurements gate progression; missing existing supports do not, because the takeoff adds the required 50x50 members. New custom infills and presets request automatic material and joiner direction, while saved sheet/strip and vertical/horizontal preferences remain manual. The portal resolves automatic choices through the canonical takeoff before mapping the unchanged `CostInputsV1` payload. At `900px` and below the desktop infill rail is replaced by the inline selector rather than a slide-over summary. Results show manufacturing status, the resolved system, added supports, canonical pieces, and purchases; cost comparison is secondary and requested only when its disclosure is opened.

...
| ---------- | -------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-08 | Portal Function Data Locality    | Promoted | Read the live `x-vercel-id` execution hop and confirm the Supabase project's primary region before setting Portal function geography. Place server functions near the authoritative data source and remeasure authenticated routes; do not infer database location from the user's edge or Supabase's Cloudflare edge. |
| 2026-08-08 | Projects Index Live Data         | Promoted | Measure from navigation intent through live rows. Share one fresh current-user memory query between preload and mount, reuse V3 active state only as a validated subset of the exact page, retain the independent completeness proof, and run remaining bounded reads concurrently. Keep the endpoint private/no-store and server-authoritative. |
| 2026-08-08 | Work Queue Portfolio Reads       | Promoted | Do not enumerate a full portfolio and then enrich 100 IDs at a time sequentially. Run independent bounded state/project/confirmation inventories with the durable queue and completeness proof, filter through authoritative active IDs, fail closed on every ceiling, and overlap the private/no-store current-user memory query with explicit navigation intent. Preserve a hydration-stable shell and server-authoritative actions. |
| 2026-08-08 | Calculator Live Data Bootstrap   | Promoted | Return the active draft detail with estimate metadata, seed only the current user's memory cache, and do not calculate saved-design defaults before hydration. Reuse the authorized client and join the published configuration pointer/version in one database read without weakening hash/provenance validation; keep every response private/no-store. |
| 2026-08-08 | Project Detail Bootstrap         | Promoted | Build the authenticated Command Centre once inside the complete private/no-store snapshot, derive owner and Project Work from that result, and seed only the current user's short-lived memory cache. Keep the direct route as refresh/fallback and never persist or service-worker cache either response. |
| 2026-08-08 | Dashboard Live Data Isolation    | Promoted | Do not make independent Dashboard values wait for a portfolio-wide authoritative aggregate. Measure named server steps, keep the expensive Work Queue server-authoritative and `private, no-store`, but request it through its own bounded current-user memory query so only that stable card remains inline-loading. |
| 2026-08-08 | Portal Browser Cleanup Quarantine | Promoted | Gate every fresh authenticated document on separate legacy-unscoped cleanup before owner data mounts. Persist a non-sensitive departing-owner cleanup marker synchronously before logout, owner change, or access-loss purge; retry it after reload and clear it only after owner-scoped plus legacy cleanup both succeed. If no reload-safe marker can be written, keep the current document hard-locked rather than navigating. |
| 2026-08-06 | Portal Immediate Offline Shell   | Promoted | Accept useful UI only when a route's exact final structure is visible and remains visible while values settle. Preload small presentation frames after live role verification; keep specialist code intent-loaded. The versioned worker caches only safe static build/public assets, never documents, HTML, RSC/Flight, APIs, or protected responses. Soft offline navigation applies only while the app remains open; hard refresh/new-tab offline startup is a separate security-reviewed slice. |
| 2026-08-06 | Portal Offline Data Boundary     | Promoted | Keep server query responses memory-only and mount owner data only after live role verification. A cached role may render the data-free shell and cached theme, but never hydrate customer/commercial data. Persist only explicit per-user drafts/queues, clear them on logout, owner change, or access loss, and use a new document at identity/access boundaries so Next Router/RSC prefetch memory cannot cross owners. |
...
| 2026-07-18 | Portal Browser Persistence       | Superseded | Superseded by the 2026-08-06 offline data boundary: durable query-response caching is retired, while explicit drafts and queues remain per-user.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
### 2026-08-06 - Portal Offline Data Boundary - Persist Drafts, Not Server Responses

Date: 2026-08-06
Area: Portal authentication, browser persistence, and offline shell
Status: Promoted
Decision or mistake: Persisting selected React Query editor responses put quote, estimate, customer, and commercial server data in IndexedDB even though the offline shell needs only local code, public assets, presentation preferences, and explicit staff drafts. A cached role was also capable of looking like verified access before a live role read completed.
Why it mattered: Fast paint does not require a durable copy of sensitive server responses. On a shared device, durable response caches increase disclosure and stale-authority risk, while cached authorization can mount owner data after access has changed.
Current guardrail: React Query is memory-only and mounts owner data only after the current session and role verify live. A cached role may shape the data-free shell and a cached owner theme may apply as presentation, but neither can hydrate customer/commercial data. Persist only explicit per-user drafts and queued mutations. On logout, owner change, or access loss, clear that owner's local-first/session/theme state and retired query-cache keys independently; never restore access after partial cleanup.
Promoted to: `docs/local-first-sync.md`; `docs/portal-production-readiness.md`
Related docs/tests: `apps/portal/app/providers.tsx`; `apps/portal/components/auth/PortalAuthProvider.test.tsx`; `apps/portal/lib/portalBrowserDataBoundary.test.ts`; `apps/portal/lib/react-query/persistence.test.ts`; `apps/portal/lib/offline/portalServiceWorkerContract.test.ts`

### 2026-08-06 - Portal Immediate Offline Shell - Accept Final Structure, Not Loading Theatre

Date: 2026-08-06
Area: Portal routing, performance acceptance, and offline presentation
Status: Promoted
Decision or mistake: Route timing previously accepted headings, generic pending shells, or short-lived loading states as useful content. That could report a fast journey while staff still crossed several loading screens or waited seconds for the real page geometry. Common portal destinations now render their exact data-free final structure immediately and update only live value slots.
Why it mattered: A fast shell is primarily a code, routing, and presentation boundary; it does not require storing customer records locally. Stable route geometry removes perceived delay and layout churn while keeping authorization and sensitive facts live. It also makes an honest offline subset possible without treating stale data as authority.
Current guardrail: Route performance must assert destination-specific controls, regions, tabs, or table columns and observe that accepted structure continuously until background settlement. Include a Dashboard revisit after a realistic idle period. Preload only the small core frame code after live role verification and keep heavy specialist modules behind route/intent boundaries. The release-versioned worker may cache only allowlisted static code/assets with credentials omitted; never cache documents, HTML, RSC/Flight, APIs, server query responses, or authority-bearing content. Soft offline navigation is supported only while the app remains open, and it must unmount the previous protected route content. Offline hard refresh/new-tab startup requires a separate data-free document-bootstrap design and security review. Email, quotes, invoices, payments, permissions, and destructive actions remain server-authoritative.
Promoted to: `docs/testing-and-qa.md`; `docs/ui-foundation.md`; `docs/security-privacy-quality.md`; `docs/portal-route-catalog.md`; `docs/portal-production-readiness.md`
Related docs/tests: `playwright/portal.performance.spec.ts`; `playwright/support/portalPerformance.ts`; `apps/portal/components/page-state/PortalRouteTransition.test.tsx`; `apps/portal/lib/offline/portalServiceWorkerContract.test.ts`; `apps/portal/lib/offline/portalCoreShellPreload.test.ts`; `apps/portal/components/auth/PortalAuthProvider.test.tsx`

### 2026-08-08 - Portal Browser Cleanup Quarantine - Prove Cleanup Before Mount

Date: 2026-08-08
Area: Portal authentication and browser persistence boundaries
Status: Promoted
Decision or mistake: Departing-owner cleanup was best effort, while a fresh document with no previous React owner could skip cleanup of legacy unscoped contacts/projects and let the old bootstrap migrate them into the newly signed-in account. A purge failure could also be followed by a hard navigation with no durable record of which owner still required cleanup.
Why it mattered: Owner-scoped keys prevent ordinary cross-user reads, but pre-owner stores and interrupted cleanup are device-level hazards. Logging an error is not a reload-safe security boundary, and a server-authenticated snapshot must not mount browser data before the device boundary is proven clean.
Current guardrail: Start every fresh authenticated document in presentation-only loading state. Before owner data mounts, clear legacy unscoped stores and finish any recorded departing-owner purge. Before logout, owner change, or access-loss cleanup begins, synchronously persist a minimal non-sensitive quarantine marker in local storage with a same-origin cookie fallback; clear it only after owner-scoped and legacy cleanup both succeed. If neither marker store is writable, keep the current document hard-locked and do not navigate.
Promoted to: `docs/security-privacy-quality.md`
Related docs/tests: `apps/portal/components/auth/PortalAuthProvider.tsx`; `apps/portal/components/auth/PortalAuthProvider.test.tsx`; `apps/portal/lib/portalCleanupQuarantine.ts`; `apps/portal/lib/portalCleanupQuarantine.test.ts`; `apps/portal/lib/portalBrowserDataBoundary.ts`; `apps/portal/lib/portalBrowserDataBoundary.test.ts`

### 2026-08-08 - Dashboard Live Data Isolation - One Slow Aggregate Must Not Block Independent Values

Date: 2026-08-08
Area: Dashboard live-data loading and Project Work
Status: Promoted
Decision or mistake: The single Dashboard API awaited six concurrent owners, including a portfolio-wide authoritative Work Queue calculation. Browser evidence showed that queue work consumed 9,641.7 ms of a 10,321.5 ms response even though the snapshot, state counts, activity, estimates and personal tasks were independent and mostly completed below 850 ms.
Why it mattered: Stable UI structure alone could not make the useful values feel live, and changing project/customer storage would not remove the blocking dependency. One expensive but legitimate aggregate withheld every unrelated Dashboard value and made the entire page appear database-bound.
Current guardrail: Measure named server steps before changing data structure. Keep portfolio Project Work server-authoritative and sensitive responses `private, no-store`, but load the bounded Dashboard queue through its own authenticated current-user memory query and leave only that existing card inline-loading. Core Dashboard freshness, failure handling and cache invalidation remain independent; Project Work mutations invalidate both reads. Do not use service-worker API caching or persist the response locally.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/testing-and-qa.md`
Related docs/tests: `apps/portal/lib/dashboard/getDashboardData.ts`; `apps/portal/app/dashboard/useDashboardData.ts`; `apps/portal/app/api/staff/v1/work-items/queue/route.ts`; `playwright/support/portalPerformance.ts`

### 2026-08-08 - Project Detail Bootstrap - Build Authoritative Work Once

Date: 2026-08-08
Area: Project Detail live-data loading and Overview Command Centre
Status: Promoted
Decision or mistake: Cold Project Detail independently requested the complete snapshot and Command Centre. The snapshot then rebuilt authoritative Project Work through a helper that itself rebuilt Command Centre, creating duplicate browser work and a sequential server read chain. Named timing showed a 5,606.1 ms snapshot, including 3,442.6 ms in work projection, beside a separate 4,601.7 ms Command Centre request.
Why it mattered: The duplicate reads returned overlapping sensitive facts and delayed the same Overview values. A new database shape or persisted local customer cache would not remove this duplicated composition path.
Current guardrail: The complete snapshot is the one cold-detail bootstrap owner. Build Command Centre once with the authenticated viewer, embed it in the private no-store snapshot, and derive owner and Project Work from that response. After project/design selection, load selected estimate detail, domain facts, and the base work projection concurrently, then apply pure specialist-action composition to the loaded projection. Seed only the current user's in-memory query cache for a short freshness window; retain the direct Command Centre route as refresh/fallback, mutation invalidation, and row-version/server authority. Never persist or service-worker cache either response.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/testing-and-qa.md`
Related docs/tests: `apps/portal/lib/projects/getProjectPageSnapshot.ts`; `apps/portal/lib/projects/commandCentre/getProjectCommandCentre.ts`; `apps/portal/components/projects/ProjectPage/tabs/OverviewTab.tsx`; `playwright/portal.performance.spec.ts`

### 2026-08-08 - Calculator Live Data Bootstrap - Hydrate Once, Then Calculate Once

Date: 2026-08-08
Area: Calculator live-data loading, estimate bootstrap and costing configuration
Status: Promoted
Decision or mistake: Opening a saved design fetched estimate metadata, fetched the selected detail afterward, priced transient default inputs, and then priced the saved inputs. The costing route also resolved the publication pointer and immutable version through two sequential database requests. The corrected browser baseline was 4,021 ms with four requests; the package calculation itself later measured only 9 ms.
Why it mattered: Repeated auth/network/database work, not cost-engine computation or customer-data storage shape, dominated the wait. Persisting sensitive responses or moving costing configuration into the browser would have weakened the established security and authority boundaries without removing the waterfall safely.
Current guardrail: Return the active editable draft detail with estimate metadata from one auth-bound private/no-store read and seed only its exact current-user memory key. Do not issue saved-design costing until that detail hydrates. Reuse the route's authorized Supabase client, load the publication pointer and joined immutable version in one read, and retain ID/hash/provenance validation plus server-authoritative calculation. Blank designs may calculate immediately; no response is service-worker or durably browser cached.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/costing-and-geometry.md`; `docs/testing-and-qa.md`
Related docs/tests: `apps/portal/app/api/projects/[projectId]/estimates/route.ts`; `apps/portal/lib/queries/projectEstimates.ts`; `apps/portal/app/staff/calculator/useCalculatorCostingRequest.ts`; `apps/portal/app/api/staff/costing/v1/job/route.ts`; `apps/portal/lib/costing/configurationResolver.ts`; `playwright/portal.performance.spec.ts`

### 2026-08-08 - Work Queue Portfolio Reads - Bound Concurrency, Not Authority

Date: 2026-08-08
Area: Work Queue live-data loading and whole-portfolio Project Work composition
Status: Promoted
Decision or mistake: The full Work Queue first enumerated project IDs, then enriched state, project/commercial facts, and confirmations in roughly twelve sequential 100-ID requests per owner. The authenticated route took 5,851 ms server time even though the underlying bounded reads were independent; changing the project/customer schema would not remove that application waterfall.
Why it mattered: The instant shell was already visible, but useful rows still settled near seven seconds. Increasing an unbounded query or persisting the private response would trade correctness and security for speed without addressing the measured sequencing defect.
Current guardrail: For the whole portfolio, start bounded active-state, non-archived commercial-candidate, relevant-confirmation, durable queue, and strict completeness reads concurrently. Filter candidates through authoritative active-state IDs before server composition, and fail closed if any inventory reaches its 5,000-row ceiling or completeness cannot be proven. Keep project-scoped reads on V2-validated bounded ID chunks. Begin the private/no-store query only on explicit Work Queue navigation intent in the verified current user's memory cache, give that handoff only a five-second freshness window, keep hydration on the exact pending structure until handoff, and leave commands, permissions, cache invalidation, row-version conflicts, and destructive effects server-authoritative. Never service-worker cache or durably persist the response.
Promoted to: `docs/project-work-items-and-follow-up.md`; `docs/staff-api-auth-contracts.md`; `docs/supabase-schema-map.md`; `docs/testing-and-qa.md`
Related docs/tests: `apps/portal/lib/projects/workItems/teamQueue.ts`; `apps/portal/lib/list/listLimits.ts`; `apps/portal/lib/queries/projectWorkQueue.ts`; `apps/portal/app/staff/projects/work-queue/WorkQueueClient.tsx`; `playwright/portal.performance.spec.ts`; `scripts/summarize-portal-performance.mjs`

### 2026-08-08 - Projects Index Live Data - Reuse Proven Scope Once

Date: 2026-08-08
Area: Projects Index live-data loading and page-scoped Project Work enrichment
Status: Promoted
Decision or mistake: The Projects journey originally began after hover preload, hiding part of the user's wait. Once measured from intent, route mount could force a second identical Projects request, and each server request repeated V2-marker and active-state reads even though the fail-closed V3 index had already proven the exact page's effective states.
Why it mattered: The duplicate request produced a seven-second sample, while the repeated server checks contributed about 480 ms of avoidable sequencing in the normal baseline. Persisting private project data or redesigning the project/customer schema would not remove either application-owned duplication.
Current guardrail: Measure Projects from navigation intent through its explicit data-ready marker. Share one five-minute current-user memory query between preload and mount; do not force-refetch a fresh handoff. Reuse V3 active state only through an explicitly validated subset of the exact requested page, reject broader or unscoped inputs, retain the independent portfolio-completeness proof, and begin the remaining bounded reads concurrently. Keep the endpoint `private, no-store`, never service-worker or durably cache its response, and retain server-authoritative ranking and mutation invalidation.
Promoted to: `docs/projects-contacts-estimates-calculator.md`; `docs/project-work-items-and-follow-up.md`; `docs/staff-api-auth-contracts.md`; `docs/testing-and-qa.md`
Related docs/tests: `apps/portal/lib/projects/serverProjectsIndex.ts`; `apps/portal/lib/projects/workItems/teamQueue.ts`; `apps/portal/app/staff/projects/useProjectsIndexData.ts`; `playwright/portal.performance.spec.ts`; `playwright/portal.performance.budgets.json`

### 2026-08-08 - Portal Function Data Locality - Confirm Both Ends Before Moving

Date: 2026-08-08
Area: Portal production function region and Supabase data locality
Status: Promoted
Decision or mistake: After application waterfalls were removed, every auth-bound request retained a roughly 0.36-0.42 second access-verification floor plus variable database time. A live production response entered Vercel through Sydney but its `x-vercel-id` recorded function execution in Washington (`iad1`), and the Portal has no checked-in region override. Supabase requests reached the Melbourne Cloudflare edge, but that edge location does not identify the database's primary region.
Why it mattered: Physical distance between a server function and its authoritative database is paid on every auth and data round trip. Moving the function based only on user location or an API edge could instead make a correctly co-located database path slower.
Current guardrail: Read the live Vercel execution hop, confirm the Supabase project's actual primary region in its dashboard, and place Portal functions near that data source. Make geography a separate deployment slice and rerun the authenticated production suite before and after. Do not weaken live access checks, persist private responses, or infer database location from Cloudflare termination.
Promoted to: `docs/portal-production-readiness.md`; `docs/testing-and-qa.md`
Related docs/tests: `playwright/portal.performance.spec.ts`; `playwright/support/portalPerformance.ts`; Vercel `x-vercel-id`; Supabase project region setting

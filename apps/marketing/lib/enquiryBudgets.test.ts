...
describe('buildEnquiryBudgets', () => {
  it('returns a lower-only base amount and keeps blinds ranged', () => {
    ...
    // 19999 * 1.20 = 23998.8 -> rounds to 24000 ($250 residential step).
    expect(budgets.baseRange).toEqual({ lowIncGst: 24000, highIncGst: 24000 });
    // Blinds round-trip the quote multiplier (6250 / 1.20 * 1.20 = 6250),
  ...
  it('keeps the base-only budget basis concise', () => {
    ...
    // 41234.56 * 1.20 = 49481.472 -> rounds to 49500 ($500 commercial step).
    expect(budgets.baseRange).toEqual({ lowIncGst: 49500, highIncGst: 49500 });
    expect(budgets.blindsRange).toBeNull();
    expect(budgets.budgetBasis).toBe('website ballpark: base uses 1.20x true cost lower-only, fascia assumption');
  });

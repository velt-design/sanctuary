...
| Security and data boundaries | Green | The boundary sweep passes `npm run audit:security`, browser/service-role/root-compat guards, portal quote/invoice tests, and marketing tests. JOB-03 adds a second explicit background-job service-role owner: the server-only provider webhook repository, which may call only the verified-acceptance reconciliation RPC after bounded streaming raw-body verification. Direct receipt/payload/job/PGMQ access remains revoked, the worker's local acceptance RPC remains lease-fenced, and provider/customer content stays out of logs and receipts. | Manual staff workflow and public-token browser QA still require valid credentials and compatible data. |
| Portal build | Green | A fresh isolated Stage 2 `next build` passed on Next.js 16.2.10 with `Compiled successfully`, TypeScript completed, and 66 pages generated. The normal preflight correctly reported the user's pre-existing port-3001 portal dev server rather than terminating it. | Keep build in portal CI and re-run after quote, invoice, PDF, job-pack, dependency, or Next config changes. |
...
| Docs and routing | Green | The 2026-07-20 JOB-03 docs pass completed `npm run docs:guard`, `npm run text:mojibake`, `npm run docs:impact`, `npm run docs:navigation`, and `npm run docs:readiness`. Guard, mojibake, and impact were clean; navigation remained advisory only for 10 pre-existing plan/vision docs, and the newly dense email owner doc now has a routing cue. | Keep this tracker and owner docs current; refresh readiness when process signals change or the tracker ages past 7 days. |

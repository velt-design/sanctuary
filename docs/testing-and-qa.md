...
The real PostgreSQL harness also calls `scripts/test-praxis-context-reporting-operator.mjs` in a separate disposable database. It installs and replays the complete migration through an actual nonsuperuser LOGIN with CREATEROLE/CREATEDB/REPLICATION/BYPASSRLS, installer-owned commercial objects and separately owned auth/storage objects with representative grants. It checks exact creator ADMIN-only membership, unchanged replay rows/function bodies/owners/ACLs, a provisioned reader LOGIN, all seven unsafe flags, outgoing membership, object ownership, and SET/INHERIT/both `createrole_self_grant` rejection with no rollback residue. Fixtures prove these selected capabilities and ownership boundaries; they do not reproduce the complete hosted catalogue or establish activation readiness.

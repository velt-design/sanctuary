...
| Portal route bundle budgets | Yellow | The reusable analyser passes Schedule, Projects Index, Project Detail, Calculator, and Design Workbench from one fresh build and detects Turbopack lazy groups honestly. Projects Index is 687.3/197.8 KiB raw/gzip initial and 2,651.9/606.2 KiB lazy under its locked ceilings; shared route gzip grew by about 2.4 KiB, within the Slice 2 5 KiB allowance. Project Detail's honest combined total still exceeds its obsolete zero-lazy cap. | Keep the honest per-boundary gates blocking. Reduce the existing Estimates drawing/workbench dependency in a separately authorised lane; do not hide lazy bytes or increase budgets automatically. |
| Portal performance evidence | Green | Slice 1 project opening remains locked at p75 44 ms feedback and 60 ms useful content. Slice 2 Portal Quality run `29675363201` completed exactly five authenticated production repetitions: Dashboard-to-Projects was 43/44/44 ms feedback and 75/76/99 ms useful content p50/p75/p95, with no blocking overlay or observed long task. | Keep the 100/500 ms regression ceiling locked; future optimisation may ratchet it downward but must never increase it automatically. |

...
This snapshot records the most recent known production-readiness state from the portal review and follow-up checks. The current repository ref is `fab5c35b`; the uncommitted 2026-07-28 commercial-trust slice below has current local test, typecheck, lint, isolated build, security, architecture, deterministic browser-fixture, rendered-PDF, executable PostgreSQL, and staging quote-smoke evidence. Its migrations are staged on the positively identified staging database only; real provider/public-token journeys and production deployment have not been executed.
...
| Portal route bundle budgets | Green | The reusable analyser passes Schedule, Projects Index, Contacts Index, Project Detail, Calculator, and Design Workbench from the Stage 1 isolated production build and detects Turbopack lazy groups honestly. Project Detail is 662.8/190.5 KiB raw/gzip initial plus 1,771.0/371.5 KiB lazy, with a 1,526.9/308.7 KiB largest lazy entry, below its unchanged cap. Design Workbench is 1,586.9/386.6 KiB initial plus 942.8/247.1 KiB lazy under a redistributed, not increased, cap. | Keep every per-boundary gate blocking and ratchet limits down from fresh evidence; never hide nested lazy bytes or increase the combined allowances automatically. |
| Current portal UI baseline | Green | The checked-in portal UI, shared catalogue, responsive shell, status contracts, focus-managed modal/drawer, current production patterns, reduced-motion and contrast rules, focused component coverage, and Projects Index/Detail browser proof are implemented. Full portal tests, an isolated fresh production build, all six existing route bundle ceilings, and the dedicated 1440/1280/1024/768/390 plus 200% zoom production Playwright re-audit are green. | Treat the current rendered portal as canonical. Re-run the relevant route evidence when an explicitly approved UI change touches that surface; there is no pending portal-wide Foundation rollout. |
| Portal performance evidence | Green | Portal Quality run `29687042640` completed exactly five authenticated production repetitions. Cold Project Detail useful-content p50/p75/p95 is 1,664/1,666/1,680 ms against the unchanged 2,699 ms guard derived from pre-Slice run `29681955081`; all five project runs had no overlay or long task. This remains last-known evidence, not a run against `005be1a7`. | Keep every ceiling locked and retain separate summary, useful-content, and background-settled markers so later work cannot relabel incomplete data as fast. |
...
| Project command centre | Yellow | Stages 1 and 2 are present in the current repository. Canonical owner/action/control/selection/audit records, deterministic selector/conflicts, transactional staff commands, Auckland due rules, project header/Overview controls, dashboard exceptions, audit history, extracted automation persistence, and the server-owned Schedule compatibility projection are implemented without touching quote/estimate/workbench/costing owners. Legacy Dashboard/Details writers are retired. Local evidence passes 335/335 project tests, 18/18 fixture Playwright checks, full typecheck/lint, isolated 66-page build, unchanged six-route bundle ceilings, and architecture/docs/dead-code guards. | Keep Stage 2 Yellow until `20260720_000008_project_command_centre_stage2.sql` and `20260721_000001_project_command_single_owner.sql` pass an executable PostgreSQL/Supabase smoke and authenticated real-project Playwright passes with provisioned staff credentials plus dedicated safe mutation and conflict projects. Stage 3 lead-to-quote workstreams are next after those gates. |
...
| Schedule workflow | Green | `npm run test:portal:schedule` passed with 43 files and 329 tests, covering Schedule V2 APIs/readiness, atomic Gantt adjustment, strict affected-job preview/re-preview/confirmation, cross-instance save ownership, malformed-response rejection, optimistic rollback/reconciliation and cache authority, Board/Gantt keyboard/responsive behavior, Site Visits, command boundaries, and legacy fallback isolation. Network/408/unexpected 5xx ambiguity reconciles authoritatively; HTTP 501 is the explicit pre-commit schema/RPC-unavailable exception. | Keep live readiness and manual Board/Gantt/Site Visit mutation persistence checks in release QA with staff credentials and a migrated database; add a guarded RPC or per-crew revision before claiming protection from near-simultaneous staff edits. |
...
| Calculator Brain configuration | Yellow | Repository implementation adds package-owned typed configuration/validation/diff/impact, admin history/draft/clone/publish UI and APIs beneath Pricebook, immutable database versions with atomic audited publication, fail-closed staff reads, exact calculation provenance, and estimate version linkage. Focused costing/estimate/admin/control-centre coverage passed 31 files and 301 tests; the full portal Vitest suite, typecheck, lint/docs/package guards, and static migration contract are green. `20260723_000001_costing_configuration_versions.sql` has also been applied to the linked Supabase project and verified for its tables, RLS, publish RPC, and estimate provenance column. An authenticated read-only Chromium smoke now proves the `/pricebook` entrypoint, control-centre status/history UI, no browser runtime errors, and `private, no-store` read APIs. Existing live override rows use supported scalar action shapes. | Keep Yellow until authenticated browser journeys prove clone, invalid-field feedback, diff/impact, stale conflict, publish, calculator use, estimate provenance, and rollback. Those mutation journeys must use an approved non-production scenario or explicit publication authority. No costing configuration has been published yet, so existing legacy-effective calculator behavior remains active. |
...
| PROJECT-01 | Green | Recorded warm opening is 38 ms feedback / 49 ms useful-content p75 with intent-only prefetch. A cold direct link promotes a small authenticated project/contact summary while the complete snapshot continues in the background. Exactly five recorded runs measured cold useful content at 1,664/1,666/1,680 ms p50/p75/p95, below the fixed 2,699 ms guard derived from the 2,454 ms pre-Slice baseline; every project run had no overlay or long task. This is last-known evidence, not a run at `005be1a7`. |
...
- [ ] Project page Overview tasks + notes: open project lands on the internal `activity` key labelled Overview, tasks render and respond to manual completion, notes compose / edit-own / delete-own succeed, non-author staff sees no edit/delete affordance, admin can edit/delete any note, soft-deleted notes hide on reload.

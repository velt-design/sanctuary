...
describe("objectFirstWorkbenchModel contracts", () => {
  it("expresses a house assembly as multiple independently movable house forms", () => {
    ...
    expect(assembly.derivedEnvelope?.mergedFormIds).toEqual([
      "form-a",
      "form-b",
    ]);
    ...
    expect(assembly.derivedEnvelope?.wallGraph.walls[0]?.edgeIds).toEqual([
      "edge-wall-1",
      "edge-eave-1",
    ]);
    expect(assembly.derivedEnvelope?.roofZones[0]?.edgeIds).toEqual([
      "edge-ridge-1",
      "edge-eave-1",
    ]);
  ...
  it("keeps derived edges as the assembly-level bridge for hosted objects", () => {
    ...
    expect(assembly.derivedEnvelope?.attachmentZones[0]?.hostEdgeId).toBe(
      "edge-eave-1",
    );
  });
...
describe("normalizeObjectFirstHouseFormDraft — gable→hipped migration (slice 2)", () => {
  ...
  it("preserves stored openGableEndIds when migrating (union semantics match the geometry compat layer)", () => {
    ...
    expect(draft.roofIntent.openGableEndIds).toContain(
      "house-gable-end-x-legacy",
    );
...
describe("normalizeObjectFirstHouseFormDraft — roof intent authorship", () => {
  function makeMonoDraft(overrides: { roofIntentAuthored?: boolean } = {}) {
    return {
      id: "house-main",
      label: "Main",
      transform: { offsetXM: 0, offsetYM: 0, rotationQuarterTurns: 0 as const },
      ...
      ...(overrides.roofIntentAuthored === true
  ...
  it("keeps persisted unauthored mono raw state during generic normalization", () => {
    const draft = normalizeObjectFirstHouseFormDraft(makeMonoDraft());
    expect(draft).not.toBeNull();
    if (!draft) return;
    expect(draft.roofIntent.form).toBe("mono");
    expect(draft.roofIntent.material).toBe("corrugated_iron");
    expect(draft.roofIntent.primaryPitchDeg).toBe("5");
    expect(draft.roofIntentAuthored).toBeUndefined();
  });

  it("resolves unauthored mono roof intent to the canonical hipped house default at the read boundary", () => {
    const normalized = normalizeObjectFirstHouseFormDraft(makeMonoDraft());
    expect(normalized).not.toBeNull();
    if (!normalized) return;
    const resolution = resolveHouseRoofIntentForAuthorship({
      roofIntent: normalized.roofIntent,
      roofIntentAuthored: normalized.roofIntentAuthored,
    });
    ...
    expect(resolution.roofIntent.form).toBe("hipped");
  });

  it("preserves authored mono roof intent as an explicit user choice", () => {
    const draft = normalizeObjectFirstHouseFormDraft(
      makeMonoDraft({ roofIntentAuthored: true }),
    );
    expect(draft).not.toBeNull();
    if (!draft) return;
    expect(draft.roofIntent.form).toBe("mono");
    expect(draft.roofIntentAuthored).toBe(true);
  });
});
...
describe("normalizePergolaAttachment", () => {
  ...
  it("returns null when spatialKind is missing or invalid", () => {
    ...
    expect(
      normalizePergolaAttachment({ spatialKind: "bogus" as never }),
    ).toBeNull();
  });

...
async function discoverFirstProjectDetailRoute(browser: Browser, authenticatedPage: Page): Promise<string> {
  const discoveryContext = await browser.newContext({ storageState: await authenticatedPage.context().storageState() });
  try {
    const discoveryPage = await discoveryContext.newPage();
    await discoveryPage.goto('/staff/projects');
    const href = await (await firstProjectOpenLink(discoveryPage)).getAttribute('href');
    expect(href, 'The authenticated performance account needs an active project for the cold detail journey.').toBeTruthy();
    const projectId = new URL(String(href), discoveryPage.url()).pathname.split('/').filter(Boolean).at(-1);
...
async function measureColdProjectDetail(page: Page, route: string) {
  ...
  expect(feedbackMs).toBeLessThanOrEqual(budget.shellVisibleMsMax);
  expect(usefulContentMs).toBeLessThanOrEqual(budget.contentVisibleMsMax);
}

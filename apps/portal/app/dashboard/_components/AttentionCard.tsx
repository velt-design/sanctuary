...
import { dashboardHref, projectsHref } from '@/lib/dashboard/links';
...
export default function AttentionCard({ items }: { items: AttentionItem[] }) {
  ...
  return (
    <section className={styles.section} aria-label="Attention">
        ...
        {total === 0 ? (
          <div className={dash.attentionAllClear}>
            ...
            <div className={dash.attentionActions}>
              ...
              <Link className={dash.attentionActionLink} href={projectsHref({})}>
                Open projects
              </Link>
            </div>
          </div>

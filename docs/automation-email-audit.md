...
Forward marketing attribution starts with project audit events. New enquiries store campaign fields, Google click identifiers, landing URL, and referrer only after marketing consent; landing/referrer query strings and fragments are removed. The first-party GA client ID requires analytics consent. The server independently enforces both category gates and stores the submission consent snapshot in `enquiry_requests.raw_payload.attribution`. Lifecycle events are `marketing.lead_submitted`, confirmed-only `marketing.site_visit_booked`, `marketing.quote_accepted`, `marketing.deposit_received`, and structured `marketing.project_lost`.

The browser sends the initial `generate_lead` through GTM. The four downstream event types enter `marketing_conversion_deliveries` from an `audit_events` trigger and are delivered by the marketing cron as GA4 `qualify_lead`, `quote_accepted`, `close_convert_lead`, or `close_unconvert_lead`. The first three downstream events and `generate_lead` are GA4 key events; the loss event is diagnostic only. Each portal event is written only after its authoritative business transition, preserves that transition's database occurrence time, and permits idempotent crash repair only inside the shared 72-hour window. The claim/complete RPCs are service-role-only and use bounded leases, attempts, and retry delays. Exhausted rows are terminally failed before another claim, and the sender claims one row immediately before each outbound request so queued rows do not spend their leases waiting behind network calls. The sender uses the originating visitor's consented GA client ID and projects an explicit analytics-safe payload: source, value/currency where authoritative, confirmed visit status, or a closed loss code. It does not send portal IDs, contact data, notes, campaign names, or provider response bodies. Missing consent/identity and events outside the 72-hour GA4 window are terminally skipped. Delivery is at-least-once because GA4 can accept an event before the completion checkpoint is persisted and does not generically deduplicate non-purchase Measurement Protocol events. This is not a direct Google Ads API or enhanced-conversions integration.

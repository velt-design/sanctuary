...
- The Projects list starts its independent project and contact reads together. Its route-level pending shell can render while those reads settle, and internal navigation uses the canonical `/staff/projects` route rather than the legacy `/projects` compatibility path.

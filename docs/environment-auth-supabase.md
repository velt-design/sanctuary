...
As of 2026-07-20, this workstation had no `docker`, `psql`, or Supabase CLI command available. `npm run test:jobs:db` was attempted and stopped at the Docker readiness check with `spawnSync docker ENOENT`; no container started and no SQL executed. `npm run test:jobs` passed the TypeScript contract and static migration/security assertions, but that does not prove the migrations execute against a real PGMQ database or that rollout is ready. The dedicated Background Jobs workflow runs the harness against upstream PostgreSQL 18/PGMQ 1.10.0 and Supabase PostgreSQL 17/PGMQ 1.5.1; only successful real-database matrix results count as execution evidence.

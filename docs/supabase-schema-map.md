...
Reader creation and compatible replay enforce the hardened group posture without privileged `ALTER ROLE` repairs. The necessary PostgreSQL 17 creator ADMIN-only membership and exact pre/postflight membership rules are owned by `docs/environment-auth-supabase.md`; the reader itself must have no outgoing memberships or object ownership.

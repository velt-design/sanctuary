...
- Deck dragging in projection-backed plans must invert the same screen-axis transform used to draw the top projection, so pointer movement is screen-native. Live snap candidates come from projected wall-edge context lines when available, with the committed top-projection house body as fallback; object/geometry frames are only commit targets. Snapped releases map the rendered preview through matching frame coordinates back into canonical object plan metres. Floating releases persist the released preview as an absolute floating rect and must not be blocked just because no wall snap was chosen; late top-projection rebuilds are diagnostics, not user-facing failures.

...
import { consentScopes } from '@/lib/xero/oauthScopes';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const cfg = config(); const state = nonce(); const scopes = consentScopes();
    ...
    response.cookies.set('__Host-xero-state', seal({ state, userId: session.user.id, expires: Date.now()+600000, scopes }, cfg.key),

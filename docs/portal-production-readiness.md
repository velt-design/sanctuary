...
| Browser smoke | Yellow | `npm run portal:calculator-ui` passed 8/8 authenticated checks on 2026-07-18: command-bar/current-result state and internal-cost/customer-price wording at 1600px, stored-versus-Live save review without persistence, no horizontal clipping at 1366px, responsive review plus page scrolling at 1024px and 768px, stale-result save blocking, and project selection into the active draft. Route coverage remains catalog-owned and shared evidence remains owned by `playwright/support/portalBrowserEvidence.ts`. | Extend seeded coverage and debug exports to Design List, Running Jobs, schedule, and admin routes only after each domain has a safe write contract. |
...
| File decomposition | Yellow | Calculator input normalization, save readiness, result freshness, command bar, and project picker already live outside `CalculatorGridClient.tsx`; the commercial-clarity slice adds dedicated pricing-comparison, save-outcome policy, and save-outcome dialog owners instead of adding those responsibilities inline. Broader strict enforcement remains advisory while the client still owns draft/session orchestration and the main form schema. | Continue with draft/session orchestration as the next safe calculator extraction; run `npm run files:changed` before handoff. |

...
The active package manifest is `v2.5`. It adds the two governed powdercoat assumptions used by Additional aluminium and raises the operational startup to `$1,000 ex GST` for newly published `v2.5` configurations. Compatible published `v2.4` and earlier controls hydrate only the new material identities from the immutable `v2.5` base after their original content hash is verified; all stored rates and original commercial semantics, including the `$500` startup, remain unchanged. A fresh draft copies the active editable rates and advances to `v2.5`. Cloning an older version preserves that version's commercial semantics for rollback.

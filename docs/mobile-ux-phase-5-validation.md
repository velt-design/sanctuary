...
## 2026-09-16 bounded configured-enquiry delivery and measurement goal

Owner source: Codex task `Rate Sanctuary Pergolas website`, Jordan accepted the bounded goal on 2026-09-16 and delegated test identity/recipient selection ("you choose"). Homepage PR #142 is complete and live; this is a separate delivery/measurement batch. Worktree: `C:/Dev/sanctuary-enquiry-measurement-20260916`; branch `codex/enquiry-measurement-20260916`; starting production/main revision `84c7e9f`. The related configurator UI task was idle at discovery.

Acceptance record:
- Pending: one labelled controlled production enquiry, design/source intact, reconciled to accepted record and staff receipt. Choose an established internal test recipient; inspect actual configured destinations before submission. No customer data, no attachment unless needed for this goal, no destructive cleanup. Retain the labelled test for evidence and exclude it from commercial analysis.
- Pending: prove existing consented events reach the intended analytics destination; dataLayer/source inspection alone is insufficient.
- Pending: implement and verify only missing essential designer start/edit/review/submit and submission-error events, with consent-negative coverage and no entered values or contact details in analytics.
- Pending: concise evidence report and funnel guidance, with limitations and raw-count/cohort cautions.

Authority covers scoped investigation, implementation and verification, plus the single controlled production test with operator-selected internal identity/recipient. It does not authorize broader CRM work, pricing changes, homepage redesign, extra test communications or conversion-uplift claims. Any necessary production code/tag publication will be made concrete and reviewed against this batch before release; prior homepage publication approval is not reused as blanket authority.

Relevant quality dimensions: correctness (single accepted enquiry and reconciled source/design), journey/clarity (success/error states), system fit and maintainability (reuse existing intake/consent/event owners), reliability (deduplication and failure semantics), privacy (category gates, allowlisted metadata), operating cost (bounded event frequency), and verification/handover (destination evidence and explicit limits). Visual redesign and geometry accuracy are excluded because this goal does not change their behavior.

Reuse PDR-04 in `sanctuary-mobile-production-closure-and-direct-manipulation-plan.md` for reconciliation, scoped to configured enquiries without attachments. Current findings: GTM is the sole Google loader; the configured form already emits contact start/success/error and lead_submitted; designer-stage emitters are absent in the current prototype owner. An existing signed-in Analytics browser tab is available; destination access and event delivery remain unverified. Next: inspect that property and the existing internal test/delivery configuration before changing event code.

Discovery update: Sanctuary GA4 property `277310638` is accessible in the signed-in browser, including DebugView (no active debug device yet). Current GTM container `GTM-W438QM7H` has zero workspace changes. Its existing Generate Lead tag maps `lead_submitted` to `generate_lead` at measurement ID `G-KGLF83X6JW`; destination receipt and parameter forwarding still need verification. The existing internal preview recipient has been identified privately, and current autoresponder source uses the normal Sanctuary info inbox as BCC. No enquiry, tag change or production code change has occurred.

Implementation update: `DesignFunnelTracker` adds only `design_start`, first `design_edit` per opening, and `design_review` on review entry. It observes actual restored selections, excludes staff/nonpublic routes, bounds edit frequency, keeps raw selections only in local comparison memory, and does not replay denied interactions after consent changes. Existing contact events now include a configured-design boolean and the validated submission ID on success. No intake, pricing, email or visual behavior changed. All 867 marketing tests and marketing TypeScript pass. Both intercepted browser journeys pass (analytics on/off), including a failed attempt followed by one success and exact accepted-ID/event matching. No live enquiry has been submitted.

Operational discovery: GTM's lead tag maps to `generate_lead`; the homepage's other dataLayer-only events have no matching tag in the current container, so do not claim those events reach GA4. This goal uses direct consent-gated GA events for the essential designer funnel and the existing contact events; broader homepage event routing remains outside the essential funnel scope. GA4 Realtime is accessible and shows page_view/scroll/session_start. Exact controlled event receipt remains pending. Tag Assistant cannot establish its popup connection under the existing `Cross-Origin-Opener-Policy: same-origin` header; preserve that protection and use realtime event/parameter reconciliation instead. This replaces the debug-window mechanism, not the requirement for destination evidence.

Release-readiness review: the implemented slice meets 8/10 for correctness and journey (867 tests, consent-on/off browser error/retry reconciliation), wider-system fit/maintainability (one small tracker owner; unchanged intake and conversion owner), reliability/privacy (first-edit cap, denied-history suppression, staff exclusion, closed metadata, analytics exceptions cannot block design), and local verification (marketing build/typecheck/lint/docs/architecture clean). These are evidence-based release-readiness assessments, not completion of the goal: production event receipt and the one controlled lead remain pending. No visual layout was changed. The large ContactEnquiryForm receives only two metadata additions; extraction is deferred to avoid moving submission/idempotency logic during this measurement batch. Its next safe extraction remains the existing submission-event helper with direct contract tests.

External reporting configuration: GA4 event-scoped dimension `Configured design` was created and verified in property `277310638`, mapped to the closed `configured_design` parameter. Existing Event Category/Event Label definitions were preserved. This enables later configured/nonconfigured report filtering without storing the high-cardinality submission ID as a custom dimension. Production event receipt remains pending.

Routing defect and repair: the initial candidate queued gtag API events but did not send them to GA4. A local production-build browser diagnostic using the real Google runtime captured only page_view/scroll responses. The repaired `lib/googleAnalyticsEvent.ts` explicitly supplies the verified `send_to` stream (`G-KGLF83X6JW`, overrideable with NEXT_PUBLIC_GA_MEASUREMENT_ID); GTM remains the sole loader. Both designer and existing contact API events use this shared routing helper. A repeat diagnostic captured Google HTTP 204 responses for design_start/design_edit/design_review, then contact_start/contact_error with the local enquiry request blocked. No lead success was fabricated or production enquiry sent. Evidence: `local-delivery-diagnostic.json`, `local-delivery-routing.json`, `local-error-delivery.json` under the current artifact directory. HTTP acceptance is distinct from GA4 report receipt; the latter remains a production completion gate.

The initial CI on 64012ee passed all checks, but is superseded by the routing repair. Focused production-browser consent/error/retry cases pass after the repair. A relative import repair resolved the repository test alias mismatch; all nine routing/tracker/form-recovery cases pass, with other marketing evidence unaffected. Fresh final release checks are required for the repaired revision.

PR #144 is the ongoing release/receipt record. GA4 Realtime recorded all three designer stages from deployed preview dad7ebf, including configured_design=true. All mandatory CI passed on that revision. Automated review then identified a regional-policy initialization race and missing public overlay entry routes; merge protection correctly held the release. The tracker now waits for unresolved regional policy, preserves denied-history suppression, and covers current public entry pages with closed project/product route templates. All 872 marketing tests, the production build and all three focused browser journeys pass, including delayed regional consent through the simple-pergola overlay. Docs guard and focused lint pass. Fresh final CI is required for this correction. No production enquiry has been sent; a read-only query confirmed zero rows with the unique test marker.

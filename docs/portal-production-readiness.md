...
| Item | Status | Evidence and remaining work |
| --- | --- | --- |
| W2-MEASURE-01 | Green | The data-free `/qa/projects-index-mutation-fixture` mounts the production Project Index, Project Details, Contact Details, and manual-task mutation owners and holds persistence open in Chromium. Current full local evidence records 45/794 ms for index feedback/background completion, 35/816 ms for Project Details Done, 39/808 ms for Contact Details Done, and 39/803 ms for a task toggle. Each measured journey used one request with zero long tasks and no blocking overlay. Paired rejection journeys prove precise rollback plus visible Review/Retry state. Portal Quality uploads this schema-v2 artifact beside the workbench artifact. |
...
| W2-CONTACT-DETAIL-01 | Green | Done exits edit mode immediately and Contact Detail persists full normalized drafts through the authenticated user's ordered local-first queue. Working copies survive navigation/reload/offline; transient failures retain desired data and retry, while terminal failures restore confirmed caches and keep the rejected draft reviewable/retryable. Fixture Chromium measured 39 ms feedback against an intentionally delayed 808 ms background completion, with no overlay or long task; the paired 403 journey proved rollback plus retained review state. |

...
- `apps/portal/lib/quotes/pricing.ts` owns the shared `1.25x`, discount, rounded-ex-GST-then-GST customer-pricing sequence used by quote mapping and the calculator preview. `quoteDiscountPct` applies to pergola and shared-site sell lines; separately customer-priced blind and lighting lines remain at list price. Discount scope is stated in the calculator and recorded in affected quote-line descriptions.

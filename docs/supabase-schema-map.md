...
- The full staff Work Queue and Dashboard preview call `project_work_queue_v3()` through the auth-bound server repository and compose canonical specialist candidates in `apps/portal/lib/projects/workItems/teamQueue.ts`. The repository reads 1,000-row hosted-safe ranges while whole-portfolio active states, non-archived commercial candidates, relevant confirmation facts, and `staff_project_state_counts_v1()` completeness proof run as independent bounded reads. Candidate rows are filtered by authoritative active-state IDs before composition. Every inventory fails closed at the explicit 5,000-row ceiling. Project-scoped callers keep V2-ID validation plus bounded ID chunks; Projects and Dashboard state counts continue to use the strict project/state RPCs, not browser-derived lifecycle truth.

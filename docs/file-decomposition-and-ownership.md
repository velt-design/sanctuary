...
| --- | --- | --- | --- | --- |
| Calculator | Portal calculator and estimate workflow | Calculator workspace shell, estimate/project state, and house-footprint interaction wiring are still concentrated in the main client, though pure calculator input defaults, normalization, browser-draft persistence/session state, module navigation modelling and presentation, configuration-section definitions/presentation/field options, structure/site/workflow field-schema generation, issue labels/section ownership/model construction/focus navigation, infill draft/selection/modal/delete-undo state, infill item mutations/stage policy, infill clipboard/hotkeys/warning focus/telemetry, infill baseline/variant cost coordination, infill dialog/stage/rail/empty-state presentation, save confirmation/persistence/outcome orchestration, blinds/flashings custom editor markup, materials-debug request/lifecycle/export behavior, warnings/BOM/debug/labour/structure preview presentation, pricing-summary presentation, preview-split sizing and persistence, price-impact ranking/presentation and styles, save-readiness, quote-status/save-preflight helpers, the guided infill stages, visual infill shape-template picker/model, support-confirmation model, infill UI/payload plus summary view-model helpers, infill overview/list presenters, save dialog presenters, flashings view-model helpers, and blinds view-model helpers now live outside `CalculatorGridClient.tsx`. | Extract the outer calculator workspace shell and preview/configuration presenters, then move house-footprint editor interaction state into a focused controller. | `npx vitest run apps/portal/app/staff/calculator`, `npm run test:portal:projects`, `npm run typecheck` |

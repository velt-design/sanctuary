...
Calculator configuration section order, Basic/Advanced visibility, and field-width presentation live outside `CalculatorGridClient` in the configuration-section model and form presenter. Field dependencies, validation IDs, and focus targets are unchanged. The two-column configuration workspace keeps module navigation as one structural owner and groups templates plus the complete configuration form in one main-column stack; new controls must join the appropriate stack or declare their grid ownership rather than relying on auto-placement. Consecutive ordinary sections share quiet sheets with subtle dividers, while Blinds and Infills remain separate configuration cards. Labels and input-like toggles use a consistent control rhythm; routine field helper subtext is not rendered, while validation errors remain attached to their fields and visible. The responsive field grid deliberately renders three columns at the 1600px and 1024px browser checks, and two columns at 1366px and 768px, with one column on narrower mobile widths. Specialist Flashings, Blinds, and Infills controls use the full section width. The presentation-only Orientation diagram is omitted. The narrow command bar keeps Save prominent while retaining project/module identity, browser-draft explanation, freshness, and Basic/Advanced controls.

Calculator money labels distinguish internal costing from customer-facing values. The pricing preview's primary figure is the complete customer total produced by summing the same cent-accurate sell lines used at quote handoff: each pergola, shared site costs, valid blinds, and preserved legacy lighting. The saved `quoteDiscountPct` applies only to pergola and shared-site sell lines; blinds and lighting remain at list price. The preview shows the complete undiscounted total when a discount is active and states that scope. Full and compact customer-price displays add thousands separators and round the inc-GST and ex-GST headline figures to the nearest whole dollar, while `Price by item` shows exact cents and a matching footer total. Invalid blinds appear as unpriced, are excluded from the partial displayed total, and remain Save/quote blockers. Blind base/fabric pricing receives its shared `1.15x` core uplift before GST; the fixed `$900 inc GST` motor and width-priced roll-cover options (`No cover`, `$44/m inc GST` flashing, `$145/m inc GST` pelmet) are not uplifted. The calculator labels the vertical dimension `Blind drop` to distinguish it from the roll cover. Saved snapshots missing `rollCover` normalize to No cover without a schema or database migration.

The desktop preview rail is a tabbed Result Inspector. A compact overview keeps the current or last-valid customer price, freshness, input-issue count, and quote readiness visible while Pricing, Materials, Labour, Workings, and Issues route the existing output owners. Pricing contains the full price, price-by-item, permitted actual-cost review, and admin cost-impact surfaces. Materials consumes the whole-job `@sp/costing` trusted breakdown: all BOM rows are grouped by user-facing purpose and show quantity, unit, scope ownership, profile, assumptions, and supported stock/sheet `Why this quantity?` disclosures. Staff see quantities and explanations but not internal dollar costs; admins retain cost visibility and the separate Advanced debug trace. Labour retains its existing admin and Advanced gates, then groups every install action by work stage with quantity, unit, minutes, crew hours, non-neutral loadings, ownership, internal cost, and package-owned explanation. Both breakdowns explicitly label retained last-valid results and fail closed for old outputs without the contract. Workings follows the selected module through Plan/Section drawings and Advanced structure outputs. Issues contains Quote Status and warnings. Tabs use the standard keyboard arrow, Home, and End behavior; inactive panels remain mounted so switching tabs does not discard local disclosure, drawing, debug, or actual-cost state. The tab strip becomes horizontally scrollable in a narrow preview rail, while the established stacked-layout page scrollbar remains authoritative below `1120px`. Calculations, permissions, save behavior, quote mapping, and source-of-truth boundaries are unchanged.

Workings includes the first trusted calculation explanation: common-rafter cut length. The written chain shows the selected result's normalized span, per-plane deductions, effective projected run, engine-selected pitch, sloped length, angle-cut allowance, final cut, assumptions, source, and nearest-millimetre display rule from `DerivedV1.rafter_cut_length_explanation`. The Section annotations use the same plane values. Pitched, gable, low-gable, and hip common rafters are supported; asymmetric gables retain separate house/outer results. Invalid runs, old snapshots without the contract, and hip-corner modules fail closed. Retained last-valid workings state that their calculated inputs may differ from current edits. Input-fallback drawings label their local slope as schematic and do not present it as a cut length.
...
Below the 1120px split-workspace breakpoint, a compact customer-price card appears between the module launcher and configuration form so the current or last-valid inc-GST and ex-GST figures are available before configuration. Admins receive an `Internal costing` disclosure inside the full pricing card; it starts collapsed on every visit. Staff receive no internal-cost markup in the calculator. The same role gate removes staff access to True cost change, Actual vs estimated, BOM dollar amounts, material/labour cost diagnostics, infill cost comparisons, and Save-dialog cost figures while retaining customer prices, BOM quantities, warnings, and Preserve/Reprice decisions. This is a presentation permission, not a server-payload secrecy boundary. Untouched preview sizing is 480px when the measured workspace is at least 1280px and 440px from 1120px to 1279px; only an explicit drag or keyboard resize writes the v2 preference. Stacked layouts add scroll clearance for the sticky command bar, pricing card, configuration sections, and validation focus targets. Context uses compact density without changing its values, field order, or Basic/Advanced visibility.

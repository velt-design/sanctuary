...
`npm run test:portal:performance` writes a schema-version-2 journey artifact. It measures cold Dashboard, Projects, Contacts, and Schedule; warm Dashboard to Projects, Projects to project, browser back, and project tab navigation; and Schedule/Calculator interactions. Each journey separates visible feedback, useful content, and background-settled time, and records same-origin requests/transfer, long tasks, and blocking overlays. Dashboard-to-Projects feedback ends when `/staff/projects` reaches the URL, useful content requires the heading, filters, and truthful list region, and background completion requires a fresh combined project/contact index. Project-opening background completion still requires both the fresh snapshot and active tab workflow. Portal Performance CI builds once and runs all five authenticated repetitions against `next start`; development compilation time must never be recorded as product latency. CI rejects missing journeys and publishes p50/p75/p95. Product targets stay visible separately from regression ceilings so noisy baselines cannot redefine the product goal.

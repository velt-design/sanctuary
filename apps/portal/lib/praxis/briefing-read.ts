...
import {BRIEFING_LIMIT,BRIEFING_SCHEMA,briefingRecordSchemas,briefingSchema,type BriefingDomain,type BriefingSnapshot} from './briefing-contract';
...
export async function readPraxisBriefing(config:ConnectorConfig,requestId:string,dependencies?:PraxisServerDependencies):Promise<BriefingSnapshot> {
  const read=await withPraxisReadTransaction(config,async sql=>{
    ...
    const quotes=views!.quotes?await sql<Row[]>`select id,project_id,parent_id,recorded_at,jsonb_build_object('quoteRef',payload->'quoteRef') as payload,omission_count from praxis_reporting.quotes_v1 where project_id is distinct from ${TEST_PROJECT}::uuid order by id limit 1001`:null;
    const enquiries=views!.enquiries?await sql<Row[]>`select id,project_id,parent_id,recorded_at,jsonb_build_object('contactId',payload->'contactId','enquiryType',payload->'enquiryType','createdAt',payload->'createdAt') as payload,omission_count from praxis_reporting.enquiry_requests_v1 where id<>${TEST_ENQUIRY}::uuid and project_id is distinct from ${TEST_PROJECT}::uuid order by id limit 1001`:null;
    const quoteVersions=views!.versions?await sql<Row[]>`select id,project_id,parent_id,recorded_at,jsonb_build_object('versionNumber',payload->'versionNumber','status',payload->'status','sentAt',payload->'sentAt','acceptedAt',payload->'acceptedAt','supersededAt',payload->'supersededAt','expiresAt',payload->'expiresAt','totalIncGstCents',payload->'totalIncGstCents') as payload,omission_count from praxis_reporting.quote_versions_v1 where project_id is distinct from ${TEST_PROJECT}::uuid order by id limit 1001`:null;
    const manualWork=views!.manual?await sql<Row[]>`select work_item_id as id,project_id,payload,omission_count from praxis_reporting.workload_v1 where project_id<>${TEST_PROJECT}::uuid and work_item_id is not null and payload->>'manual'='true' and payload->>'retired'='false' order by work_item_id limit 1001`:null;
    ...
    const installation=views!.specialist?await sql<Row[]>`select record_id as id,project_id,payload,omission_count from praxis_reporting.specialist_workload_v1 where domain='installation' and project_id<>${TEST_PROJECT}::uuid order by record_id limit 1001`:null;
    const design=views!.specialist?await sql<Row[]>`select record_id as id,project_id,payload,omission_count from praxis_reporting.specialist_workload_v1 where domain='design' and project_id<>${TEST_PROJECT}::uuid order by record_id limit 1001`:null;
  ...
  if(Buffer.byteLength(JSON.stringify(result),'utf8')>262144)throw new PraxisConnectorError(503,'PROJECTION_NOT_READY','Complete briefing exceeds the payload limit.',true);
  return result;
}

...
function Blind({opening,blind}:{opening:BlindOpening;blind:PreviewBlind}) {
  const parts=useMemo(()=>buildRepresentativeBlind(opening,blind),[opening,blind]);
  return <group name={'ziptrak-'+opening.id}>{parts.map(part=><Part key={part.id} data={part} blind={blind}/>)}</group>;
}
function OpeningTarget({opening,active,onSelect}:{opening:BlindOpening;active:boolean;onSelect:()=>void}) {
  ...
  useEffect(()=>()=>geometry.dispose(),[geometry]);
  return <mesh geometry={geometry} onClick={event=>{if(event.delta<6){event.stopPropagation();onSelect();}}}><meshBasicMaterial side={DoubleSide} color={active?'#a7bc87':'#afbab0'} transparent opacity={active?.24:.07} depthWrite={false}/></mesh>;
}
export default function PreviewBlinds({workspace}:{workspace:NonNullable<ReturnType<typeof usePreviewBlinds>>}) {
  return <group>{workspace.openings.map(o=>{const blind=workspace.blinds.find(b=>b.opening===o.id);return <group key={o.id}>{blind&&<Blind opening={o} blind={blind}/>} {workspace.editing&&<OpeningTarget opening={o} active={o.id===workspace.selected} onSelect={()=>workspace.select(o.id)}/>}</group>;})}</group>;
}

...
- `estimates.commercial_scope_id` and `quotes.commercial_scope_id` are null for the original contract and hold one stable UUID for each independent project add-on. The base family has one partial unique quote row per project; each non-null project/scope pair has one quote family and its own reference/version chain.

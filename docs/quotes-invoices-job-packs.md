...
- Paid value is job-level payment-ledger truth, not a sum scoped to the latest quote version. `project_payment_entries` records actual payments, adjustments and equal/opposite reversals; `project_payment_allocations` explains which quote stage currently consumes a positive entry. Paid invoices from superseded versions are backfilled as unallocated job credit and can be manually reassigned without mutating their invoice or quote evidence.
- All staff can read job total, actual paid, whole open-invoice value and the remaining amount available to invoice. Only administrators receive the payment-entry detail and controls. Current schedule rows show allocated paid value; the complete invoice history continues to label older quote versions as historical.
- Admin invoice creation is atomic and serialised against job payment changes. The modal supports next stage, full remaining, custom fixed or percentage-of-remaining, and a two-to-ten-way split that persists all planned instalments but creates only the first invoice. Due date, label/reference, save-only or create-and-send, and the resolved before/after balance are explicit. Amounts above the remaining job balance fail unless the administrator supplies an override reason, which is stored on the invoice and in the transaction audit event.
